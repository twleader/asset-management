 ...
 *
 * <p>{@code movingAverage} 是 instance method 且不觸及 {@code businessServicesClient}，
 * 故以 {@code new GdpTwseBffController(null)} 取得實例即可（不需要起 Spring context 或 mock WebClient）。
 */
class GdpTwseBffControllerMaTest {

    private final GdpTwseBffController controller = new GdpTwseBffController(null);
    ...
    void ma5定義() {
        ...
        List<Object> ma5 = controller.movingAverage(
    ...
    void 資料不足視窗() {
        List<Object> ma5 = controller.movingAverage(closes("100.00", "100.01"), 5);
