...
public class PublicQuoteMarketDataExceptionAdvice {
    ...
    public ResponseEntity<ProblemDetail> invalidRequest(PublicQuoteRequestException ignored) {
        return problem(HttpStatus.BAD_REQUEST, "報價日期參數不合法");
    }

    @ExceptionHandler(PublicQuoteRawTimeoutException.class)
    public ResponseEntity<ProblemDetail> rawTimeout(PublicQuoteRawTimeoutException ignored) {
        return problem(HttpStatus.GATEWAY_TIMEOUT, "原始報價暫時逾時");
    }

    @ExceptionHandler(PublicQuoteRawUnavailableException.class)
    public ResponseEntity<ProblemDetail> rawUnavailable(PublicQuoteRawUnavailableException ignored) {
