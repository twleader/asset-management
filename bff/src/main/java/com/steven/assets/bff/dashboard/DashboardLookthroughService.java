...
/** 台股權重正規化、美股真實權重的共用穿透演算法。 */
@Slf4j
@Service
@RequiredArgsConstructor
public class DashboardLookthroughService {
    private final WebClient businessServicesClient;
    private static final ParameterizedTypeReference<Map<String, Object>> MAP =
            new ParameterizedTypeReference<>() {};

    public Mono<TwStockLookthroughDto> taiwan(Map<String, Object> detail, List<Map<String, Object>> merged) {
        return taiwan(detail, merged, false);
    }

    public Mono<UsStockLookthroughDto> unitedStates(Map<String, Object> detail, List<Map<String, Object>> merged) {
        return unitedStates(detail, merged, false);
    }

    public Mono<TwStockLookthroughDto> legacyTaiwan(Map<String, Object> detail, List<Map<String, Object>> merged) {
        return taiwan(detail, merged, true);
    }

    public Mono<UsStockLookthroughDto> legacyUnitedStates(Map<String, Object> detail, List<Map<String, Object>> merged) {
        return unitedStates(detail, merged, true);
    }

    private Mono<TwStockLookthroughDto> taiwan(
            Map<String, Object> detail, List<Map<String, Object>> merged, boolean allowUpstreamFallback) {
        ...
        Mono<List<EtfHoldingsFetched>> etfMono = twEtfs.isEmpty()
                ...
                : Flux.fromIterable(twEtfs)
                        .flatMap(row -> fetchEtfHoldings(row, "台股", allowUpstreamFallback), 4)
                        .collectList();
    ...
    private Mono<UsStockLookthroughDto> unitedStates(
            Map<String, Object> detail, List<Map<String, Object>> merged, boolean allowUpstreamFallback) {
        ...
        Mono<List<EtfHoldingsFetched>> fetchMono = usRows.isEmpty()
                ...
                : Flux.fromIterable(usRows)
                        .flatMap(row -> fetchEtfHoldings(row, "美股", allowUpstreamFallback), 4)
                        .collectList();
    ...
    private Mono<EtfHoldingsFetched> fetchEtfHoldings(Map<String, Object> etfRow, String market, boolean allowUpstreamFallback) {
        ...
        return businessServicesClient.get()
                ...
                .retrieve()
                .bodyToMono(MAP)
                .switchIfEmpty(allowUpstreamFallback ? Mono.empty() : Mono.error(
                        new ResponseStatusException(HttpStatus.BAD_GATEWAY, "ETF 成分股資料來源回應不完整")))
                .onErrorResume(e -> {
                    if (!allowUpstreamFallback) return Mono.error(e);
