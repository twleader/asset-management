...
/** BFF-local sanitizer: never reads raw request/response bodies. */
@Component
public class ApiErrorLogDiagnosticRenderer {
    ...
    /** Compatibility constructor for isolated WebFlux tests. */
    public ApiErrorLogDiagnosticRenderer(String value){this(value,List::of);}

    @org.springframework.beans.factory.annotation.Autowired
    public ApiErrorLogDiagnosticRenderer(@Value("${api-error-log.runtime-secrets:}") String configuredSecrets,
            @Value("${API_ERROR_LOG_INGEST_TOKEN:}") String ingestToken,
            @Value("${GOOGLE_CLIENT_SECRET:}") String googleClientSecret){
        this(configuredSecrets,()->Arrays.asList(ingestToken,googleClientSecret));
    }
    ...
    public String render(Throwable error){
        StringBuilder out=new StringBuilder();
        for(Throwable current=error;current!=null;current=current.getCause()) { if(!out.isEmpty())out.append("Caused by: "); out.append(current.getClass().getName()); String msg=sanitize(current.getMessage()); if(!msg.isBlank())out.append(": ").append(msg); out.append('\n'); for(StackTraceElement frame:current.getStackTrace())out.append("\tat ").append(frame).append('\n'); }
        return out.toString();
    }
