...
public class SchedulePublicBffController {
    ...
    private static final List<ScheduledJobDto> JOBS = List.of(
            ...
            new ScheduledJobDto(BUSINESS, "資產匯出", "每日匯出排程檢查",
                    "每分鐘檢查各使用者的每日自動匯出設定，命中執行時間即產出 Excel；輸出含 Google Drive 同步（若已啟用）",
                    "每分鐘", "0 * * * * *", TPE),
            new ScheduledJobDto(BUSINESS, "交易日曆", "交易日曆每日匯出排程檢查",
                    "每分鐘檢查各使用者的交易日曆自動匯出設定，命中執行時間即以其格式（JSON／Excel）產出當前年度交易日曆（Requirement 37）；輸出含 Google Drive 同步（若已啟用）",
                    "每分鐘", "0 * * * * *", TPE),
            new ScheduledJobDto(BUSINESS, "已實現損益匯出", "每日匯出排程檢查",
                    "每分鐘檢查各使用者的已實現損益自動匯出設定，命中執行時間即產出 Excel 到指定目錄（Requirement 39）；輸出含 Google Drive 同步（若已啟用）",
                    "每分鐘", "0 * * * * *", TPE),
            new ScheduledJobDto(BUSINESS, "油價金價匯出", "每日匯出排程檢查",
                    "每分鐘檢查各使用者的油價金價自動匯出設定，命中執行時間即產出 Excel 到指定目錄（Requirement 41）；輸出含 Google Drive 同步（若已啟用）",
                    "每分鐘", "0 * * * * *", TPE),
            new ScheduledJobDto(BUSINESS, "台幣兌美元匯出", "每日匯出排程檢查",
                    "每分鐘檢查各使用者的台幣兌美元匯率自動匯出設定，命中執行時間即產出 Excel 到指定目錄（Requirement 42）；輸出含 Google Drive 同步（若已啟用）",
                    "每分鐘", "0 * * * * *", TPE),
            new ScheduledJobDto(BUSINESS, "大盤指數匯出", "每日匯出排程檢查",
                    "每分鐘檢查各使用者的大盤指數日線自動匯出設定，命中執行時間即產出所選指數的開高低收 Excel 到指定目錄（Requirement 45）；輸出含 Google Drive 同步（若已啟用）",
                    "每分鐘", "0 * * * * *", TPE),
            ...
            new ScheduledJobDto(EXTERNAL, "財經新聞", "財經新聞抓取",
                    "抓取財經新聞＋公開資訊快照；執行時間改由 DB 驅動，可於「爬蟲資訊查詢」頁增減多個時間點（Requirement 38）。"
                            + "每輪輸出公開資訊 JSON 至本機設定資料夾，並於已啟用時同步上傳一份副本至 Google Drive"
