<template>
  <div v-loading="loading">
      ...
      <el-table :data="types" size="small" stripe>
        ...
        <el-table-column label="方向" width="100" align="center">
          <template #default="{ row }">
            <el-tag :type="row.payable ? 'danger' : 'success'" size="small">
              {{ row.payable ? '待付（負）' : '待收（正）' }}
            </el-tag>
          </template>
        </el-table-column>
    ...
    <!-- Dialog -->
    <el-dialog v-model="dialogVisible" :title="editing ? '編輯在途款項類型' : '新增在途款項類型'" width="440px">
      <el-form :model="form" :rules="rules" ref="formRef" label-width="110px">
        <el-form-item label="代碼" prop="code">
          <el-input v-model="form.code" :disabled="!!editing" placeholder="如 信用卡待付款" />
        </el-form-item>
        <el-form-item label="顯示名稱" prop="displayName">
          <el-input v-model="form.displayName" placeholder="如 信用卡待付款" />
        </el-form-item>
        <el-form-item label="方向">
          <el-radio-group v-model="form.payable">
            <el-radio :value="true">待付（金額為負）</el-radio>
            <el-radio :value="false">待收（金額為正）</el-radio>
          </el-radio-group>
        </el-form-item>
...
const form = reactive({ code: '', displayName: '', payable: true, sortOrder: 0 })
const rules = {
...
function openDialog(item = null) {
  ...
  if (item) {
    ...
    form.payable     = item.payable ?? true
