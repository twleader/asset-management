<template>
  <div v-loading="loading">
    <div class="page-header">
      <h2>🏦 銀行設定</h2>
      <el-button type="primary" :icon="Plus" @click="openDialog()">新增銀行</el-button>
    </div>

    <el-card>
      <el-table :data="banks" size="small" stripe>
    ...
    <!-- Dialog -->
    <el-dialog v-model="dialogVisible" :title="editing ? '編輯銀行' : '新增銀行'" width="480px">
      <el-form :model="form" :rules="rules" ref="formRef" label-width="110px">
        ...
        <el-form-item label="顯示名稱" prop="displayName">
          <el-input v-model="form.displayName" placeholder="如 台北富邦銀行" />
        </el-form-item>
        <el-form-item label="Excel 關鍵字">
          <el-input v-model="form.keywords" placeholder="逗號分隔，如 富邦,北富,Fubon" />
          <div class="form-hint">匯入 Excel 時，欄位內容含有任一關鍵字即視為此銀行</div>
        </el-form-item>
      </el-form>
      <template #footer>
...
const editing = ref(null)   // null = 新增；object = 編輯中的銀行
const formRef = ref()
...
function openDialog(bank = null) {
  editing.value = bank
  if (bank) {
    form.code = bank.code
    form.displayName = bank.displayName
    form.keywords = bank.keywords || ''
  } else {
    form.code = ''
    form.displayName = ''
    form.keywords = ''
