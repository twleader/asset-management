...
test('exchange-rate schedule supports add/remove, validates times payload, and keeps legacy fallback narrow', () => {
  for (const token of ['scheduleTimes', 'addScheduleTime', 'removeScheduleTime', 'scheduleDialog.times.length === 1', 'times.push({ runHour:', 'seenTimes.has(key)', 'draft.enabled && !times.some(time => time.enabled)']) assert.ok(view.includes(token))
