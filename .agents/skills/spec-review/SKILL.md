...
**本檔只管流程（誰來審、餵什麼、結果怎麼處理）。審查判準（維度、架構鐵則檢查表、報告格式）住在 [`.claude/agents/spec-auditor.md`](../../../.claude/agents/spec-auditor.md)，不要在這裡重複一份——兩份判準必然漂移。**

> 本 runtime 若不支援具名 subagent，改為：另起一支**唯讀** subagent，把 `.claude/agents/spec-auditor.md` 的**全文**當成它的指示貼進去。判準的唯一來源仍是那個檔，不要另寫一份。
> 同理，PostToolUse／PreToolUse 閘門是 Claude Code 專屬；其他 runtime 沒有那兩支，Step 4 仍請執行（`spec-review-pass.sh` 是純 bash，跨 runtime 可用），以免切回 Claude Code 時被擋。
