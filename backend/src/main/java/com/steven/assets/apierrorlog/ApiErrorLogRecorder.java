...
public class ApiErrorLogRecorder {
    ...
    public void record(String source, String operationKey, String apiName, String messageHeader, String stackTrace, Instant occurredAt) {
        try {
            ApiErrorLogOperationCatalog.require(source, operationKey, apiName);
            writes.executeWithoutResult(status -> repository.save(new ApiErrorLog(source, operationKey, apiName, renderer.sanitize(messageHeader), renderer.sanitize(stackTrace), occurredAt)));
    ...
    public void record(Throwable throwable, String source, String operationKey, String apiName, Instant occurredAt) {
        record(source, operationKey, apiName, renderer.message(throwable), renderer.render(throwable), occurredAt);
    }
}
