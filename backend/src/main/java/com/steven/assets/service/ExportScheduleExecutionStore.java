...
@Service
public class ExportScheduleExecutionStore implements ExportExecutionPort<ExportScheduleSetting> {
    private final ExportScheduleSettingRepository settingRepo;
    private final ExportScheduleFreshRead freshRead;
    public ExportScheduleExecutionStore(ExportScheduleSettingRepository settingRepo, ExportScheduleFreshRead freshRead) {
        this.settingRepo = settingRepo; this.freshRead = freshRead;
    }
    private ExportScheduleSetting lockOwner(Long owner) {
        return settingRepo.findLockedByOwnerUserId(owner).map(freshRead::refresh).orElse(null);
    }
    @Transactional(propagation = Propagation.REQUIRES_NEW)
    public <T> T update(Long owner, Function<ExportScheduleSetting, T> change) {
        ExportScheduleSetting s = lockOwner(owner);
        if (s == null) { s = ExportScheduleSetting.builder().ownerUserId(owner).build(); settingRepo.saveAndFlush(s); }
        s.getTimes().size();
    ...
    public List<CapturedExport> due(Long id, Long owner, LocalDate date, LocalTime now) {
        ...
        if (s.getTimes().isEmpty()) {
            ExportScheduleTime legacy = ExportScheduleTime.builder().runHour(s.getRunHour()).runMinute(s.getRunMinute())
        ...
        for (ExportScheduleTime t : s.getTimes()) {
    ...
    public void complete(CapturedExport input, LocalDateTime completedAt, String localStatus, String driveStatus) {
        ...
        ExportScheduleSetting s = lockOwner(input.ownerId());
        if (s == null || !Objects.equals(s.getId(), input.scheduleId())) return;
        ExportScheduleTime child = null;
        ...
        if (CapturedExport.notOlder(completedAt, s.getLastRunAt())) {
            ...
            ExportScheduleTime representative = s.getTimes().stream().filter(t -> Boolean.TRUE.equals(t.getEnabled()))
                .min(Comparator.comparing(ExportScheduleTime::getRunHour).thenComparing(ExportScheduleTime::getRunMinute)
                    .thenComparing(ExportScheduleTime::getId)).orElseGet(() -> s.getTimes().stream()
                .min(Comparator.comparing(ExportScheduleTime::getRunHour).thenComparing(ExportScheduleTime::getRunMinute)
                    .thenComparing(ExportScheduleTime::getId)).orElse(null));
