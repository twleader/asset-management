...
/**
 * 油價金價每日排程自動匯出設定端點（Requirement 41 / Task 202）。
 *
 * <p>per-user（owner-scoped）：由 BFF 帶 {@code X-User-*} → {@code CurrentUserContext} → {@code ownerFilter}，
 * 每個使用者只存取自己的排程設定。非 admin-only。
 *
 * <p>路徑選擇：掛獨立前綴 {@code /api/commodity-export}，不掛在 {@code /api/market-data/commodity} 之下——
...
public class CommodityExportController {
    ...
    private final CommodityExportScheduleService service;

    @GetMapping("/schedule")
    public CommodityExportDto.SettingResponse getSchedule() {
        return service.getForCurrentUser();
    }

    @PutMapping("/schedule")
    public CommodityExportDto.SettingResponse updateSchedule(@RequestBody CommodityExportDto.SettingRequest req) {
        return service.updateForCurrentUser(req);
    }

    @PostMapping("/run-now")
    public CommodityExportDto.RunNowResponse runNow() {
