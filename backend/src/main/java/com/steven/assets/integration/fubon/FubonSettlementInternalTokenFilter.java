...
/**
 * Exact-path, constant-time service-token boundary for the settlement sync endpoint
 * (Requirement 129 / Task 394).
 *
 * <p><b>Deliberately independent from {@link FubonInternalTokenFilter}, {@link FubonTradeInternalTokenFilter}
 * and {@link FubonBankBalanceInternalTokenFilter}.</b> All three existing filters hard-code their own exact
 * path in {@code shouldNotFilter} and return {@code true} (i.e. "do not protect this request") for
 * every other path, including this endpoint's -- reusing any of them would leave
 * {@code /internal/brokers/fubon/settlement-sync} completely unauthenticated. This filter mirrors
 * their structure exactly; only the {@link #PATH} and the {@link FubonSettlementOutcome}
 * / {@link FubonSettlementOutcomeCounters} response shape differ.
 */
@Component
public class FubonSettlementInternalTokenFilter extends OncePerRequestFilter {
    static final String PATH = "/internal/brokers/fubon/settlement-sync";

    private final Optional<FubonConfigState> configState;
    private final Optional<FubonSettlementOutcomeCounters> counters;
    private final ObjectMapper objectMapper;

    @Autowired
    public FubonSettlementInternalTokenFilter(
            Optional<FubonConfigState> configState,
            Optional<FubonSettlementOutcomeCounters> counters,
    ...
    FubonSettlementInternalTokenFilter(
    ...
    protected void doFilterInternal(
        ...
        if (config.state() == FubonConfigState.State.DISABLED) {
            writeUnavailable(request, response, FubonSettlementOutcome.DISABLED, "DISABLED");
            return;
        }
        if (config.state() != FubonConfigState.State.READY || config.token() == null) {
            writeUnavailable(request, response, FubonSettlementOutcome.MISCONFIGURED, "MISCONFIGURED");
            return;
        }
    ...
    private void writeUnavailable(
            HttpServletRequest request,
            HttpServletResponse response,
            FubonSettlementOutcome outcome,
            String reason)
            throws IOException {
        FubonSettlementOutcomeCounters outcomeCounters = counters.orElseThrow();
        outcomeCounters.increment(outcome);
        response.setStatus(HttpServletResponse.SC_SERVICE_UNAVAILABLE);
        response.setContentType(MediaType.APPLICATION_JSON_VALUE);
        response.setCharacterEncoding(StandardCharsets.UTF_8.name());
        objectMapper.writeValue(response.getOutputStream(), new FubonSettlementSyncService.SyncResult(
                outcome, FubonAccountingSyncRequest.dryRun(request), reason, null, null));
    }
