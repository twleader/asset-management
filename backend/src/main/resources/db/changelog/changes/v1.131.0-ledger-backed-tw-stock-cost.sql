...
--changeset steven:v1.131.0-ledger-backed-tw-stock-cost
--comment: Requirement 154: only provable all-buy Taiwan TWD latest-holding cost correction
WITH latest AS (
  ...
  SELECT sh.id holding_id, sh.snapshot_id,
         round(sum(greatest(t.amount, t.shares * t.price + coalesce(t.fee, 0) + coalesce(t.transaction_tax, 0))), 2) cost
  ...
  UPDATE stock_holding sh SET investment_cost = c.cost
  FROM candidate c WHERE sh.id = c.holding_id AND sh.investment_cost IS DISTINCT FROM c.cost
  RETURNING sh.snapshot_id
)
UPDATE asset_snapshot s SET total_stock_cost = round(coalesce((
  SELECT sum(CASE WHEN h.currency = 'USD' THEN h.investment_cost * coalesce(h.transaction_exchange_rate, s.usd_exchange_rate)
                  ELSE h.investment_cost END) FROM stock_holding h WHERE h.snapshot_id = s.id), 0), 0)
WHERE s.id IN (SELECT DISTINCT snapshot_id FROM updated);
