...
--changeset steven:v1.55.0-crawler-schedule
--comment Requirement 37（Task 184）：公開資訊爬蟲（NewsPoller）執行時間設定，由「爬蟲資訊查詢」頁維護。一列一時間點（全域設定、無 owner），NewsPoller 每分鐘讀已啟用列比對當前 HH:mm 觸發。Seed 預設 08:00/12:00/18:00 等同原寫死行為。
...
INSERT INTO crawler_schedule (crawler_key, run_hour, run_minute, enabled) VALUES
    ('news-poller', 8, 0, TRUE),
    ('news-poller', 12, 0, TRUE),
