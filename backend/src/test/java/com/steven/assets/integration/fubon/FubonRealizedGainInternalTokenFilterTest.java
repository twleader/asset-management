...
/**
 * Independent token boundary for the realized-gain sync endpoint (Requirement 130 / Task 395).
 *
 * <p>Deliberately its own test class, not folded into {@link FubonInternalBoundaryTest},
 * {@link FubonTradeInternalTokenFilterTest}, {@link FubonBankBalanceInternalTokenFilterTest} or
 * {@link FubonSettlementInternalTokenFilterTest}, because it exercises a different filter class
 * with its own {@link FubonRealizedGainOutcome} response shape. Those four carry the regression
 * that the *existing* filters still do not protect this new path; this class carries the mirror
 * regression that the new filter does not protect any of the four existing paths.
 */
class FubonRealizedGainInternalTokenFilterTest {
    ...
    void disabledEndpointReturnsTyped503AndPreservesRequestedDryRunFalse() throws Exception {
        ...
        FubonRealizedGainInternalTokenFilter filter = new FubonRealizedGainInternalTokenFilter(
                config, new FubonRealizedGainOutcomeCounters(), mapper);
    ...
    /**
     * The new realized-gain-sync filter must not protect any of the four *existing* paths either
     * -- each filter is scoped to exactly one exact path, they never overlap.
     */
    @Test
    void filterDoesNotProtectTheExistingInventoryTradeBankBalanceOrSettlementSyncPaths() throws Exception {
        FubonConfigState config = mock(FubonConfigState.class);
        FubonRealizedGainInternalTokenFilter filter = new FubonRealizedGainInternalTokenFilter(
                config, new FubonRealizedGainOutcomeCounters(), mapper);
        ...
        for (String path : new String[] {
                ...
                "/internal/brokers/fubon/settlement-sync",
        }) {
    ...
    void matrixVariantCannotReachMvcWithoutTheExactPathBoundary() throws Exception {
        ...
        MockHttpServletRequest request = new MockHttpServletRequest(
                "POST", "/internal/brokers;v=1/fubon/realized-gain-sync");
        MockHttpServletResponse response = new MockHttpServletResponse();
        FilterChain chain = mock(FilterChain.class);
    ...
    private MockHttpServletRequest request() {
        return new MockHttpServletRequest("POST", FubonRealizedGainInternalTokenFilter.PATH);
    }
}
