...
- `PriceQueryService`: 對 `external-materials-service` 寫入 Redis 的 live 行情做唯讀。`getLive` 保留原始 live 語意（Redis first、miss 才 fallback 最近完成日），供警示／技術指標與交易雷達規則引擎；`getDisplayPrice`／`getAllDisplayPrices` 則是公開報價與估值的防線：以可注入 `Clock` 與市場日曆解析 `DisplaySession(phase,targetTradingDate)`，開盤中回當日 Redis（`LIVE`）；盤後／盤前／非交易日的台股只回 target date 且 `close_source` 命中信任清單的列（盤後 `VERIFIED_CLOSE`，其餘 `PREVIOUS_CLOSE`），否則回空價 placeholder（`CLOSE_PENDING`），不得向前跳過缺檔拿更舊收盤冒充。`StockPriceService` 的 prices／price／live-assets、Trading Radar 使用者可見價與歷史圖最新完成格一律走此 gate
...
- `WatchStockBffRoutes`（WatchStockView 專屬，`/api/bff/watch-stock/**`）：純 Spring Cloud Gateway passthrough route，rewrite `/api/bff/watch-stock(/**)` → `/api/watch-stocks(/**)` 轉至 business-services。觀察清單已改為「`stock_alert` 衍生 view」，**衍生與 enrichment 一律在 business-services（`WatchStockController` / `WatchStockService`）完成，BFF 僅轉發不重算**：
  - `GET /api/bff/watch-stock` → `GET /api/watch-stocks`：business-services 由 `stock_alert` 群組去重衍生清單，對每筆 (stockCode, market) 補上 live 報價、技術指標、最近觸發資訊
    - 使用者可見報價改走 `PriceQueryService.getDisplayPrice`，`WatchStockDto.Response` 傳遞 `quoteStatus`；`CLOSE_PENDING` 時價格顯示「收盤價待補」、不 fallback 昨收，技術指標內部計算仍可用 raw `getLive`
...
- `SnapshotFormBffController`（SnapshotFormView 專屬）：把表單頁的多步協調邏輯（價格批次查 + backfill fallback + 配息率補抓 + 名稱補齊 + 匯率智慧 fallback）集中於此
  ...
  - `POST /api/bff/snapshot-form/prices?date=YYYY-MM-DD`：批次取得每筆股票的歷史收盤價 + 漲跌 + 名稱 + 配息率（DB 缺資料時自動 backfill 重試）。`enrichBatch` per-market 基準日規則：basedate==該市場今日→使用 display quote；若收盤後為 `CLOSE_PENDING`，必須回 `price=null` 並傳遞 status，前端清除先前的盤中價並顯示「收盤價待補」，不得回退昨收；否則（frozen）改用 `hist`（`prices-on-date`）已算好的「當日漲跌」（該收盤日 vs 前一交易日）。前端 `SnapshotFormView` `.price-up=#dc2626`（漲紅）/`.price-down=#16a34a`（跌綠），台股慣例
  - `GET /api/bff/snapshot-form/realtime`：2 分鐘輪詢用，先 trigger 後端刷新行情再回傳 display `stockPrices` + marketStatus；每輪都必須套用 null/status，不得因為 price 為 null 就保留上一輪舊價
  ...
  - `GET /api/bff/snapshot-form/snapshots`：只供表單查找前一版快照；唯讀轉送既有 business `GET /api/snapshots`，不另建查詢實作
  - `POST /api/bff/snapshot-form`：新增快照；透過 `businessServicesClient` 原樣轉送 body 至既有 business `POST /api/snapshots`
  - `PUT /api/bff/snapshot-form/{id}`：更新快照；透過同一 client 原樣轉送 body 至既有 business `PUT /api/snapshots/{id}`
...
> **Task 409 current-state override:** the preceding Task 153／158／236 prose about a bare `List<IntradayTick>` response, request-time cold-start／`refreshOne`, and frontend history calculation is retained only as superseded historical context. The exact BFF → business → external path now carries one `IntradaySession` object. Tick buckets are cache-only on every request; only a valid Taiwan individual stock's **current-session reference-cache miss regardless of tick completeness** may bounded-self-heal exact TWSE MIS `d/y` reference evidence. That exception calls neither TickRefresher, FinMind nor Yahoo, and writes neither tick/history/DB nor publish. The business resolver supplies provenance and comparison/change; frontend and `AlertChartRenderer` render that result only.
...
- **共享 / 跨頁 passthrough routes**（非單一頁面專屬，由 Spring Cloud Gateway 直接轉發至 business-services，服務跨頁共用的 store CRUD、下拉 lookups、SSE 與基金主檔；皆為刻意的共享資源，不另立 `/api/bff/{page}/**`）：
  ...
  - `SnapshotBffRoutes`：`/api/snapshots/**` → business-services 為暫留 legacy passthrough，前端不得呼叫。SnapshotFormView 的建立／更新一律走 `SnapshotFormBffController` 自有 `POST /api/bff/snapshot-form` 與 `PUT /api/bff/snapshot-form/{id}`；表單的前一版列表也走其 `GET /api/bff/snapshot-form/snapshots`。共享 Pinia store 僅以 `SnapshotListBffController` 的 `GET /api/bff/snapshot-list` 刷新 App layout 的最新快照 ID，不能再繞回 `/api/snapshots`。三條 BFF 皆藉 `businessServicesClient` 自動傳遞目前登入者 tenant headers 至既有 business `/api/snapshots`，成功回應原樣回傳；非 2xx 不得 `onErrorReturn`，交 `BusinessErrorAdvice` 原樣 relay status 和 ProblemDetail，使既有日期唯一鍵 400 可在表單顯示。此項不新增 business endpoint、DB schema、公開 9090 route 或券商 I/O。
...
│   ├── ClosePersister  # 盤後收盤 cron；台股官方逐檔對帳、美英既有校正，寫 stock_price_history
│   ├── MarketClock     # isTwMarketOpen / isUsMarketOpen（平日 + 時段 + 假日；委派 MarketCalendar 判假日）
│   ├── MarketCalendar  # 交易日 / 國定假日判定（台股→TWSE primary／完整 DGPA provisional／unknown，再 union operator closure；美/英→NYSE/LSE 純函式）
...
├── client/
│   ├── PriceFetchClient
│   ├── TwOfficialCloseClient # TWSE MI_INDEX（指定日）＋ TPEx 全市場收盤；逐來源日期守門
│   ├── DividendFetchClient
...
- 若外部 API 未回傳，直接採用聚合值
- 聚合 key 的 `tradingDate` 與 JSON 中的 `tradingDate` 同源；Requirement 89／Task 350 起由 immutable observation 在 producer 形成，無來源日期的 provider 呼叫 `TradingDateResolver.resolve(code,market,freshnessInstant)`，同一 date 同時餵給聚合 key 與 payload，writer 不再重算，確保跨日正確分桶
- **大盤 `0000` 自 Task 263 起不走這條聚合**：`PriceCacheWriter.write` 新增三參數 overload `write(result, markClosed, aggregateHighLow)`，`aggregateHighLow=false` 時跳過 `hlTracker.observe` 直接採用 `result` 帶的 high／low；既有兩參數版委派為 `true`，個股路徑逐行不變。理由有二——(a) 上述「外部 API 不給 dayrange」的前提對 Yahoo 5 分 K 不成立，它本來就給整日 high／low 陣列；(b) 聚合是 max／min 的單向累積，一旦誤入極值就**無法**被後續正確值修正，而 `TaiexIndexPoller` 恰好每個交易日開盤都會誤入一次昨日點位（見「大盤新鮮度與盤中即時判斷」節）

> **`market:status` Redis key 從未落地（Task 313 訂正 Task 47.3 記載）**：原規劃由 `external-materials-service` 的 `MarketClock` 每分鐘寫入獨立 key、TTL 90s，供 `business-services` 讀取；`git log -S` 全歷史掃過所有 `.java` 檔案零命中，`MarketClock` 本身亦無 `RedisTemplate` 依賴、無 `@Scheduled`，此 key 從未被實作。實際行為：`business-services` 的 `StockPriceService.getMarketStatus`（`MarketDataController` `GET /api/market-data/market-status`）呼叫 `MarketDataService.isMarketOpenNow(market)`（`MarketZones` 時區/時段 + 交易日曆含國定假日）於每次 API 呼叫時即時運算，不讀寫 Redis；與 `external-materials-service` 的 `MarketClock`（供該服務內部多個排程／查詢類別——`PricePoller`／`TaiexIndexPoller`／`EtfNavPoller`／`IntradayTickRefresher`／`TwRadarRefreshService`／`TradingDateResolver`——做開盤狀態判斷，另透過 `InternalPriceController` 的 `/internal/health` 對內網揭露 `twMarketOpen`/`usMarketOpen`）是完全獨立的兩套實作，並非同一份邏輯的兩個入口。**不補實作的理由**：前端多處各自低頻 polling（`DashboardView`／`TradingCalendarView` 均 60 秒，`SnapshotFormView` 120 秒），皆經各自 BFF proxy 至同一 `/api/market-data/market-status`，符合「同義欄位、同一 business service API」規範，加總頻率仍低。運算以記憶體內時區比對為主；台股假日表另有 `MarketDataService.twHolidayCurrentYearCache`（10 分鐘 TTL）記憶體快取，僅該快取到期時才對 `external-materials-service` 的 `/internal/tw-holidays` 發一次網路請求（美股／英股假日判斷為純函式，無此問題）——此頻率遠低於任何前端輪詢週期，且比原規劃的 90 秒 Redis TTL 更粗略／更省，效能仍遠不需要 Redis 快取層。「開盤中」語意已透過 `closed` 欄位 denormalized 進 `price:{market}:{code}`（`PricePoller` 依 `MarketClock` 布林值決定是否更新該欄位），另開集中 key 儲存同一事實會違反本文件「相同的資料只能存一份」的正規化原則。

**抓價來源**：TWSE mis API（台股 live）、NASDAQ info API（美股 live）；台股正式收盤由 TWSE `MI_INDEX` 指定交易日檔（上市）＋ TPEx 全市場日收盤檔（上櫃）對帳，FinMind `TaiwanStockPrice` 只作缺檔 fallback。
...
`PriceCacheWriter.write` 收到 client 驗證過的正值 observation 才進寫入流程（不以 `source` 括號決定 latest 是否可寫，因 client 已保證不送 `o`／`y` 衍生值）；Requirement 89／Task 350 起 latest 是否真正 mutation 再由 Redis 日期時間 tuple 原子守門，舊 observation 可被 `REJECTED_STALE`。`source` actual-trade gate 只控制 day-H/L 與 tick eligibility。
...
- 盤中沒成交的 5 秒 tick + 之前已有 cache → 保留前一筆真實 intraday 價（cache 仍是 `TWSE`，updatedAt 也維持上輪來源／取得時間，誠實表達「沒新資訊」）。
- 首輪 polling 即 `z='-'`（盤剛開、剛清過 cache、TTL 過期）+ Redis 空 → `PriceQueryService.getLive` fallback 至 `stock_price_history` 顯示昨收，前端有明確值。
- 設計意圖：**Redis 內的值反映「最近一次真實成交」；Requirement 89／Task 350 起，updatedAt 優先反映 provider 可驗的 quote timestamp，來源無 timestamp 才反映 response／DB row 被本機取得的時間**。它在 observation 形成時只捕捉一次，不是 Redis SET 完成時間；盤中可能「看起來價格沒變」，但那是真實狀態（無新成交），不是 bug。

`source` 欄不再出現 `"TWSE(前收)"`、也不會出現實驗階段曾用過的 `"TWSE(開盤)"`；走勢圖今日格的 `source.contains("(")` 守門條件仍適用於 `(history)` 等非「今日真實成交」來源（保留，避免未來其他來源被誤拼入今日格）。

**持股抓價清單的 owner 涵蓋範圍（Task 257，推翻 Task 249 的「`collectHeldStockCodes` 不得修改」）：** `StockSourceQuery` 有三條收集路徑，Task 257 只動中間這條：
...
```text
PricePoller.scheduledTwIntradayUpdate（每 2 分鐘）跳過 2885
  （log 逐輪為「更新台股即時價格 (18 檔)」；唯一涵蓋 2885 的是開機一次性的
    warmCacheOnStartup「背景補抓 cold cache：台股 34 檔」＝ collectAllStockCodes）
  ...
  → 13:32 ClosePersister.dumpTwCloseFromRedis 依 price:index:台股（34 檔，含 2885）
      ...
      （07-29 列與 07-28 完全相同，依 C/O/H/L/volume 順序為 63.5 / 64.6 / 64.7 / 62.6 / 36045128，
        即 close 63.5、open 64.6、high 64.7、low 62.6——實際 07-29 收盤為 62.2。**`volume` 為修復前的值，現已不可查證**：該列落在 `repairRange` 的區間內，已被 FinMind 權威值覆寫為 36615128；
  ...
  → TTL 到期後 price:台股:2885 消失
  → 前端 $63.50 ▲$0.00 (0.00%)：price 取自 stock_price_history 的假收盤，
    priceChange／changePercent 因 Redis 無 entry 而為 0（實際 07-29 收盤 62.2）
```

> `stock_price_history` 07-29 的台股批次為連續 id 157154–157187 共 34 列，與 13:32 dump 的「34 檔」一致，可確認該日今日列全部出自 Redis dump 而非其他路徑。`2885` 在 `stock` 主檔內，故 `collectAllStockCodes`（34 檔）涵蓋它——這也是 Redis 曾有該 key 的來源；缺的是**持續刷新**與**收盤校正**。
...
`codes` 來自 Redis SET `price:index:{market}`，而該 SET 是由 `PriceCacheWriter` 在**任何**寫入時加入的——包含開機一次性的 `PricePoller.warmCacheOnStartup`，它用的是涵蓋 `stock` 主檔的 `collectAllStockCodes`（實測台股 34 檔），比盤中 cron 的 `collectHeldStockCodes`（Task 257 修正後 19 檔）大得多。**差集那 15 檔**（已不在任何 owner 最新快照、也不在 `stock_alert`，但仍在 `stock` 主檔的「曾持有／曾觀察」標的）於是每次重啟就被寫進 Redis 一次、之後再也不被 cron 刷新，卻仍被 13:32 的 dump 當成當日收盤寫進 DB。

**這不是一次性殘留，而是一個自我延續的迴路：**

```text
DB 錯誤列（某檔某日的假收盤）
  → PriceCacheWriter.syncClosedFromDb 讀 DB 最近收盤寫回 Redis（休市刷新／warmCacheOnStartup 走此路）
  → 13:32 dumpRedisToDb 讀 Redis 寫回 DB，冠上「今日」日期
  → 新的錯誤列成為下一輪 syncClosedFromDb 讀到的「最近收盤」
  → 每個交易日多一列，且值永遠不變
```

實測腐化量（2026 年，判準為「與該檔前一個有資料的交易日 OHLCV 全等」）：

| 市場 | 2026 年腐化列 | 最早 | 最晚 |
|---|---|---|---|
| 台股 | 122 | 2026-06-25 | 2026-07-29 |
| 美股 | 3 | 2026-07-10 | 2026-07-29 |
| 英股 | 3 | 2026-07-17 | 2026-07-29 |

受害代號高度集中且固定：`006205 00642 00646 00695B 00929 1301 1616 2002 2409 2412 2606 2882 7556 9933`（＋ Task 257 的 `2885`、美股 `AAPL`／`MSFT`、英股 `IB01`）。典型樣態是 O/H/L 為 `null`、`volume` 為 `0`、收盤固定不動——因為 `syncClosedFromDb` 寫的 payload 只有 `price` 沒有 OHLC，dump 的 `bd(r,"openPrice")` 取到 null、`volume` 缺漏補 0。例 `2002` 中鋼 2026-07-16～07-29 的 **10 個交易日中有 8 列**被記成 `19.1000`（`07-22`／`07-23` 那兩日原本**完全沒有列**，修復時才由 upsert 以 INSERT 補上；故 10 ＝ 腐化列 8 ＋ 缺列 2，「8」是腐化列數、不是區間交易日數。下表的 122／3／3 亦只計腐化列，不含缺列）。權威來源逐日對照：`07-16` 18.80、`07-17` 18.65、`07-20` 18.55、`07-21` 18.80、`07-22` 18.85、`07-23` 19.10、`07-24` 19.15、`07-27` 19.25、`07-28` 19.00、`07-29` 18.95——僅 `07-16`／`07-21` 同為 18.80，其餘每日不同。

**守門條件（兩條，皆必要）：**

1. payload 的 `tradingDate` 必須等於本次 dump 的目標交易日。擋掉上述迴路（陳舊值的 `tradingDate` 是舊日期或 DB 的 `maxTradingDate`）。
   **英股曾有一條規則 1 擋不住的路徑（Task 258 歷史成因）**：舊 `writeVerifiedClose` 以台北牆鐘猜英股日期，17:00 London 已是台北隔日，可能產生未來日期；當時由規則2的12分鐘新鮮度擋住。**Requirement 89／Task 350 已推翻此舊行為**：verified observation 的唯一date來自已驗證Yahoo bar，不再由writer猜日期；本段保留說明為何 Task258 兩條dump守門仍不可刪。
...
> **條件 2 之所以用「距 now 的間隔」而非「與市場當地收盤時刻比較」**：Requirement 89／Task 350 起，observation 保留 `Instant`；payload `updatedAt` 統一轉台北牆鐘，因此三市場格式/時基一致。Intraday tick 則由同一 Instant 另轉市場時區，不可把台北 LocalDateTime直接塞給美/英。dump 比距 now 時兩端同時基。
>
> **刻意不用 payload 的 `closed` 當 Task 258 dump 守門條件**：美／英股 dump 要取的仍是收盤前最後一輪 `closed=false` 的盤中 cron 值。台股在 Task 258 當時同理，但 Task 290 已停止 13:32 台股 Redis→DB dump；該 payload 只保留為暫定行情，正式收盤改走官方日檔。

守不過即**跳過該檔不寫任何列**，不得回填昨收／開盤價（Requirement 7 禁止回寫充數）。Task 290 起本守門的實際 dump 消費者只剩美股 16:02 ET 與英股 16:32 LON；台股 13:32 不再落 DB。美股另有 18:00 ET FinMind、英股另有 17:00 LON Yahoo 校正，但兩者的實證強度不同：美股曾實測整批空資料，英股樣本亦不完整，故仍保留 Task 258 守門與手動歷史修復能力。

> **對 `selfHealMissedClose` 的連帶影響：** 美／英股的 Redis dump self-heal 仍受上述 12 分鐘守門，重啟太晚時可能整批擋下，缺列需靠各自校正或 `/internal/repair/history`。台股自 Task 290 起不走 Redis dump self-heal：14:05 後啟動一律重跑 TWSE／TPEx 官方全集合對帳，16:00 後再對仍缺檔者跑 FinMind。判斷單位由舊的 `hasAnyHistoryFor` 全市場布林改成 `expected − exactDateRows` 逐檔集合，已有部分列不得短路整輪補救。
...
else start = maxDate.plusDays(1);                    // ← 阻擋一：起點永遠是 maxDate+1
...
if (store.existsHistory(stockCode, "台股", bar.tradingDate())) continue;   // ← 阻擋二：skip-if-exists
```

阻擋一使區間中段永遠碰不到（腐化標的的 `maxDate` 就是最近交易日，故 `start` 落在明天、`!start.isBefore(end)` 直接 `return 0`）；阻擋二使即使硬塞區間也會跳過每個已有列的日期。`StockSourceQuery.upsertHistory` 本身是真 upsert（先 `SELECT id`，存在則 `UPDATE`），但這兩道守門讓 backfill 路徑從不對既有日期呼叫它。故修復必須是**新的路徑**，不是既有路徑加參數。
...
- 台股假日委派既有 `MarketDataFetchService.getTwHolidays(year)`（TWSE holidaySchedule primary；TWSE 年度表尚不可用時採完整 DGPA 辦公日曆暫行 base；兩者皆空才保守視為 unknown／由非 known legacy consumer 退化，最後 read-time union `tw_market_closure`）。
...
| GET | `/internal/tw-holidays` | 台股假日表（TWSE primary；尚不可用時 DGPA 完整辦公日曆 provisional；最後與 `tw_market_closure` read-time union） | `MarketDataService` |
| GET | `/internal/stock-name` | 股票名稱查詢 | `MarketDataService` |
| GET | `/internal/intraday-5m` | 5 分 K 當日走勢 | `HistoricalDataService` |
| GET | `/internal/intraday-ticks` | Task 409 當日 `IntradaySession` object；ticks cache-only，server 提供 provenance／comparison／change。只有台股個股今日交易日的 reference-cache miss（不依 tick 完整度）可 bounded self-heal exact TWSE MIS `d/y` evidence；零 tick refresh、FinMind/Yahoo、tick/DB write 或 publish | `MarketDataService` |
...
**Sequence（盤後收盤）：**
```
[台股 13:32] Redis last tick 保持 closed=false，只是暫定行情，不寫 DB

[台股 14:05／15:35／17:35] → TwOfficialCloseClient
   ├→ TWSE MI_INDEX?date=YYYYMMDD&type=ALLBUT0999（上市；回應 date 必須完全相等）
   └→ TPEx tpex_mainboard_daily_close_quotes（上櫃；民國 Date 轉 ISO 後必須完全相等）
      → 與 collectHeldStockCodes 台股預期集合交集
      → [DB stock_price_history UPSERT] + Redis closed=true + SSE

[台股 16:00] → FinMind（只查上述流程仍缺當日列的代號；來源 date 必須完全相等）
   → [DB UPSERT] + Redis closed=true + SSE

[美股 16:02 ET／英股 16:32 LON] → Redis last tick
   → ClosePersister.dump*CloseFromRedis → [DB stock_price_history UPSERT]

[美股 18:00 ET FinMind／英股 17:00 LON Yahoo] → ClosePersister verify
...
> 為什麼官方／FinMind／Yahoo 校正後也要覆寫 Redis：盤中最後一輪 cron 通常落在 13:28 / 15:58（集合競價開始前 2 分鐘），
...
**台股官方收盤對帳（Task 290）：** `TwOfficialCloseClient` 每輪只做兩個 bounded curl（`--fail --silent --show-error --max-time 25`，短 UA），而不是逐檔請求：TWSE `MI_INDEX` 以 `date=YYYYMMDD&type=ALLBUT0999` 指定目標日，於 `tables[]` 以欄名定位「證券代號／證券名稱／成交股數／開盤價／最高價／最低價／收盤價」；TPEx OpenAPI 讀 `Date／SecuritiesCompanyCode／CompanyName／TradingShares／Open／High／Low／Close`。TWSE 根節點 `date` 與 TPEx 每列民國年月日轉換結果都必須等於目標日；`--`、空字串、無法解析或 `close <= 0` 的列略過。兩來源各自 fail-closed、互不連坐，合併後再與 `collectHeldStockCodes` 台股集合取交集，避免把全市場上千檔全寫入 DB。

`ClosePersister.reconcileTwOfficialClose(LocalDate targetDate)` 先把 `collectHeldStockCodes` 台股集合顯式移除 `0000`，再把剩餘個股當 expected；大盤不進 coverage、missing 或 FinMind 個股 fallback。每次都 upsert 所有命中的官方列，不採 skip-if-exists，因此可覆寫 13:32 舊版曾誤寫的盤中值，也可安全重跑。命中列以 `StockSourceQuery.upsertVerifiedHistory(..., closeSource)` 同時寫入價格與出處；舊 `upsertHistory` 繼續服務歷史回補，INSERT 不填來源，UPDATE 覆寫價格時必須同步 `close_source=NULL`，避免新值沿用舊 provenance。結果以不可變 `TwCloseReconciliation(expected, verified, missingCodes)` 表示並完整記錄缺檔。16:00 排程以 `verifyTwCloseWithFinMind(today)` 算 `expected − hasTrustedTwClose(code,targetDate)`，只對缺檔逐檔查詢；17:35 官方批次仍可覆寫。coverage 也必須用 `hasTrustedTwClose`，不能以單純 row 存在代替。

排程入口只在 today 為交易日時傳 today；啟動 `selfHealMissedClose` 的 candidate 為「today 是交易日且 now < 14:05 時用 yesterday，否則用 today」，再以 `MarketCalendar.isTwTradingDay` 向前有界尋找 target。當 `now` 已晚於 `targetDate 14:05` 時，不論今天是否週末／假日，都呼叫 `reconcileTwOfficialClose(targetDate)`；已晚於 `targetDate 16:00` 時再呼叫 `verifyTwCloseWithFinMind(targetDate)`。因此週一盤前與週六部署都會修復週五，跨日不會被「today 非交易日」或 `hasAnyHistoryFor` 短路，且不會把週一盤前錯選為尚未完成的週一。目標日解析與兩個帶日期 helper 均可直接單元測試；查無有界交易日才 fail-closed 並記 log。

`stock_price_history.close_source` 是可稽核的收盤出處，不等於 Redis 的暫定 `source`。Liquibase `v1.89.0-stock-price-close-source.sql` 新增 nullable `VARCHAR(64)`，並同步 `db/schema.sql` 與 `StockPriceHistory` entity；既有列保持 null，migration 不回推出處。台股信任清單固定為 `TWSE_MI_INDEX`、`TPEX_DAILY_CLOSE`、`FINMIND_TW_CLOSE`；美／英現行收盤寫入路徑也須留下自己的非空出處，否則 display gate 視為待確認。普通 `upsertHistory` 的 UPDATE 明確把 `close_source` 清為 null；這不是宣稱普通歷史來源不可信，而是防止覆寫 OHLCV 後仍錯誤繼承另一筆價格的 verified provenance。

`PriceCacheWriter.writeVerifiedClose` 在 Task 290 時改收 `(PriceResult result, LocalDate tradingDate)`，曾以第二參數作日期權威；**Requirement 89／Task 350 再推翻此過渡介面**：verified timed observation 自帶唯一date/Instant，writer只收observation。若為相容暫留舊overload，第二參數不再是權威，只能做相等檢查，不等即FAILED且零Redis。payload仍以已驗證observation的source/date標`closed=true`、`quoteStatus=VERIFIED_CLOSE`；盤中live與DB sync狀態不變。

**「今日列」獨佔規則（Task 84／290）：** `stock_price_history` 中市場時區「當日」row 只能由上述 `ClosePersister` 路徑寫入。台股為 TWSE／TPEx 官方對帳與 FinMind 缺檔 fallback，**13:32 Redis last tick 不在寫入者之列**；美／英股仍含 16:02 ET／16:32 LON dump 與各自校正。`HistoricalBackfillService.backfillTwStock` / `backfillUsStock` / `backfillUkStock` / `repairRange` 即使被任意路徑觸發（`startupBackfill` 條件 stale、`SnapshotFormBffController.triggerBackfillThenRefetch`、`/api/market-data/history/backfill-stock` 手動觸發、`/internal/backfill/all`、`/internal/repair/history` 手動維運觸發），在 for-loop 中遇到 `bar.tradingDate().equals(LocalDate.now(該市場時區))` 必須 `continue`（`repairRange` 用 `MarketClock.zoneOf(market)` 統一分流；三支 backfill 各自寫死 `TW_ZONE`／`US_ZONE`／`LON_ZONE`，語意等價）。原因：外部歷史 API 在盤中也會回一根「今日 partial bar」，若直接 upsert 就會充當收盤。今日列由 ClosePersister 在收盤後（含 self-heal）建立，使 backfill 與當日 close 職責不重疊。
...
| POST | `/internal/close/verify-tw` | 手動觸發 FinMind 補齊**當日台股仍缺列**的收盤價（同 16:00 fallback；不重查官方已對帳完成的代號），覆寫 `stock_price_history` 並回寫 Redis，回 `{verified:n}`。見「Sequence（盤後收盤）」 |
...
| GET | `/internal/intraday-ticks?code=&market=&date=` | Task 409「當日」`IntradaySession` object。ticks 只讀 cache；business server resolver 產生 comparison/change 和 raw reference provenance。台股個股今日交易日 reference-cache miss（不依 tick 完整度）才可 bounded exact TWSE MIS `d/y` evidence self-heal；不呼叫 TickRefresher/FinMind/Yahoo、不寫 tick/history/DB 或 publish |
...
| Method | Path | 說明 |
|--------|------|------|
| GET | `/internal/tw-holidays?year=` | 台股假日表（TWSE primary；尚不可用時 DGPA 完整辦公日曆 provisional；**已 union 颱風假 / 臨時休市**），回 `{date: reason}`。台股所有 consumer 的唯一入口，見「交易日 / 國定假日判定（單一事實來源）」與 Requirement 7 颱風假偵測的「單一注入點」 |
...
#### external-materials-service 原始報價快取 API（`PublicQuoteController`，由 BFF 封裝後經 api-gateway 對 host 可達，Requirement 66／108）
...
| Method | Path | 說明 |
|--------|------|------|
| GET | `/api/quotes?market=&start=&end=` | 對 host 的公開回應由 BFF 保留原始 Redis row 的 19 個 top-level 欄位與既有 `marketData` 四頁籤市場投影，並於其後追加同一 snapshot 的 `quoteDetail`、batch-ready `bidLevels`／`askLevels` 與 `dividendHistory`；`market` 選填（`台股`／`美股`／`英股`其一），省略時回三市場全部（含 `price:台股:0000` 大盤——與個股共用同一 key schema，見上方「Redis key schema」）。`start/end` 同時選填時控制共同走勢窗口，兩者省略為近一年。查無快取（Redis 為空）仍回**空陣列**，非錯誤 |
| GET | `/api/quotes/one?code=&market=&start=&end=` | 對 host 的公開回應由 BFF 保留原始 Redis 19 欄與既有 `marketData`，並於其後追加同一 snapshot 的 `quoteDetail`、batch-ready `bidLevels`／`askLevels` 與 `dividendHistory`；`code`／`market` 皆必填（缺一或日期參數不合法回 400）。價格 Redis 命中回 200；價格 cache miss 維持 **204 No Content**。matching ETF NAV cache miss／錯誤不改 status，只讓 `premiumDiscountPct=null`。原始 quote 本身全程**不** fallback 查 DB、**不**觸發抓取；其下所有市場資料依下一節各自唯讀／fail-soft 規則取得 |

回應 JSON 的既有 top-level `LatestQuote` record 19 欄依 raw record 宣告順序為：`stockCode`／`stockName`／`market`／`price`／`previousClose`／`priceChange`／`changePercent`／`buyPrice`／`sellPrice`／`openPrice`／`highPrice`／`lowPrice`／`volume`／`tradingDate`／`updatedAt`／`closed`／`source`／`quoteStatus`／`premiumDiscountPct`。前 18 欄語意與型別比照 `backend` 端 `PriceQueryService.LivePrice`；最後一欄是 nullable `BigDecimal` 百分點，只取 matching `price:etfnav:*` payload 的同名來源欄位，正數為溢價、負數為折價，不由 quote price 與 NAV 重算。即使值為 `null`，property 仍固定出現在 list／one JSON。現在固定序列為 raw 19 欄、`marketData`、`quoteDetail`、`bidLevels`、`askLevels`、`dividendHistory`：前 20 個 property 是既有相容 prefix，後四個是同一 immutable market child 的直接 projection，絕不把原欄包裝、刪除、改名或改以 display-price fallback。原始解析維持手動 `JsonNode` helper，不依賴 Jackson record 自動反序列化。
...
**不需要應用層身份驗證**（唯讀公開市場報價；外層由 Tailscale identity/TLS、loopback bind 與 Nginx allowlist 保護，理由見 Requirement 66，與 `TreasuryYieldController` 的 token／ADMIN 雙重驗證要求不同）。既有 browser／business 即時報價路徑繼續走 `PriceQueryService` 直讀 Redis，不改道；**唯一例外**是 Requirement 108 的 BFF public aggregation，在 container network 內讀本 controller 的 raw 19 欄後封裝成同 path 的 host response。本端點不因而成為個人資產、估值或交易資料來源。

##### Requirement 108：`/api/quotes*` 的市場資料封裝（不新增第十條 allowlist）

`api-gateway` 保留原本兩個 exact path／method 與 Tailscale handler，但 upstream 從 `external-materials-service` 改為 BFF 的 `PublicQuoteMarketDataController`。controller 只委派 `PublicQuoteMarketDataService`；service 注入兩顆專用、container-only、沒有 `TenantIdentity`／`X-User-*` filter 的 WebClient：(1) `publicQuoteRawClient` 只向 `external-materials-service:8080/api/quotes{,/one}` 讀既有 raw list／one；(2) `publicMarketDataBusinessClient` 只向明列的 business market-data read endpoint 取四頁籤資料。不得由 BFF 直讀 Redis、DB、Yahoo、configured-admin 或用 `PriceQueryService` display result 偷換資料來源。原始 list 的 order、market filter、空 array 和 one 的 204 都由 raw source 決定。

```text
api-gateway:9090 exact GET /api/quotes{,/one}
  └─ PublicQuoteMarketDataController → PublicQuoteMarketDataService
       ...
       └─ publicMarketDataBusinessClient → business-services
            ...
            ├─ /api/market-data/quote-detail                                  # Redis/DB pure-read persisted FUBON_BOOKS/YAHOO_TW snapshot
            ...
            └─ /internal/public-market-data/intraday-ticks-readonly
                  └─ external-materials /internal/intraday-ticks-readonly      # read-only tick outcome
```

每一個成功 raw row 固定得到下列 market-only object（所有列出的 property 都是 required；nullable 僅依明示處）：

```text
marketData
  chart: ChartMarketData
    status: AVAILABLE | NO_DATA | UNAVAILABLE
    message: nullable sanitized string
    requestedStart: LocalDate
    requestedEnd: LocalDate
    series: ChartSeriesDto             // 永不為 null；失敗／無資料用 canonical empty
    intraday: IntradayMarketData
      status: AVAILABLE | NO_DATA | UNAVAILABLE
      message: nullable sanitized string
      tradingDate: nullable LocalDate
      ticks: List<IntradayTick>        // 永不為 null；非 AVAILABLE 固定 []
  quoteDetail: QuoteDetailDto.Response // 台股十秒 persisted 五檔（FUBON_BOOKS primary／YAHOO_TW fallback）；其他市場 typed unsupported
...
`ChartMarketData.status` 只描述 history＋indicator 的共用 chart aggregation。兩者都成功但沒有資料為 `NO_DATA`；兩者都不可用為 `UNAVAILABLE`；若價格或指標其中一側成功，保留現有 partial alignment，為 `AVAILABLE` 並回實際 `ChartSeriesDto`。`NO_DATA`／`UNAVAILABLE` 的 `series` 是 canonical empty（所有 flat／frame list `[]`，numeric／latest null，daily／weekly 使用既有 empty frame），`message` 只可為一般化 client-safe 文案。popup 的 `/api/bff/stock-analysis/chart-series` 仍直接回 `ChartSeriesDto`，但與 public service 共用同一個 history＋indicator fetch／align service，controller 不互相呼叫、不得演化出第二套 MA/KD 計算。

`start/end` 必須同時存在或同時缺省；缺省由 BFF `Clock` 的 Asia/Taipei today 形成 `[today.minusYears(1), today]`，明確 range 最長十年，format／順序／future 違規回 400。`IntradayMarketData` 只取 raw quote 的 ISO `tradingDate`：可解析且落在該 validated range 才呼叫 readonly bridge；否則不呼叫下游，回 `NO_DATA,tradingDate=null,ticks=[]`。target 有效時，bridge 的 `readStatus=DATA`（非空且資料合法）為 `AVAILABLE`，`EMPTY` 為 `NO_DATA`，`UNAVAILABLE`／`MALFORMED` 或 BFF→business transport／timeout／schema 問題為 `UNAVAILABLE`；非 AVAILABLE 均以 `ticks=[]` 和消毒 message 回覆。不得用既有 `/api/market-data/intraday-ticks` 或 `/internal/intraday-ticks`：Task 409 的 non-public session route 雖 tick cache-only，仍可能在 current-session reference-cache miss bounded 讀 exact TWSE MIS evidence；9090 public aggregation 必須保持零 request-time provider I/O，故只能使用 exact readonly tick bridge。

`GET /internal/public-market-data/dividends-readonly-result?code=&market=&years=` 是新、只在 container network 使用的 backend exact endpoint：它驗證現有 code／market 和 `years=1..10`，只回 `DividendHistoryService.findFromDbReadOnly` 的完整 `DividendHistoryResult`。它刻意不位於 `/api/market-data/**`，避免既有 `MarketDataBffRoutes` 將它 proxy 給已登入 frontend；不得新增 BFF route/controller，frontend Nginx 也不得 proxy `/internal/**`。既有 `/api/market-data/dividends-readonly` 的 bare `List<DividendRow>` 完全不變；public service 固定傳 `years=10`。
...
##### Requirement 110：批次直接 market-data projection

`PublicQuoteMarketDataService` 取得四個既有 child 後，在同一個 immutable `DetailedLatestQuote` 同時放入 nested `marketData` 與 direct projection，絕不再發 I/O：`quoteDetail` 與 `marketData.quoteDetail` 是同一個 normalized `QuoteDetail` 值；`dividendHistory` 與 `marketData.dividends` 是同一個 normalized `DividendHistory` 值。top-level `bidLevels`／`askLevels` 只從 available approved `FUBON_BOOKS` 或 `YAHOO_TW` `quoteDetail.levels` side-by-side 投影，各項為 `{price: BigDecimal, size: Long}`，最多五筆、買方嚴格由高到低、賣方嚴格由低到高；正價與正 lots 才可保留，任一 side 投影排序錯誤即安全回空 array。不得把 generic `buyPrice`／`sellPrice` 或未知／跨來源資料當成五檔。`DividendHistory` 保留 readonly DB event `rows`，另以 immutable BFF mapping 加 `cashYieldPct`（cashDividend／previousClose × 100，scale 6、HALF_UP）和 `annualSummaries`；summary 每年加總 cash／stock dividend，並以該年 anchor date（除息日優先、無除息日則除權日）由新到舊第一筆 `previousClose != null` event 作候選分母，只有該候選值 `> 0` 才計算，否則為 null，不得跳用較舊正值。rows／summary 空、source unavailable 或 non-Taiwan book 都是 typed empty／fallback，不影響 raw quote；這四個 direct property 不含個人資產資料。
...
##### Requirement 113：global 年度交易日曆 public read

```text
GET /api/public/trading-calendar?year=YYYY
  -> api-gateway exact GET -> BFF PublicTradingCalendarController
    -> Asia/Taipei single-year window validation (invalid: no outbound)
    -> explicit no-tenant WebClient + cleared caller identity
      -> business GET /internal/public-market-data/trading-calendar?year=YYYY
        -> PublicTradingCalendarService (typed defensive snapshot)
          -> selected-year MarketDataService.getTwHolidays/getUsHolidays/getUkHolidays once each
          -> separate current StockPriceService.getMarketStatus read
```

這是 global market-data route，不做 configured-admin bootstrap，也不帶或回任何 tenant／資產／交易資料。BFF 要求恰好一個四位 `year`，以 Asia/Taipei Clock 只接受 `currentYear-1..currentYear+1`，失敗回 sanitized 400 且零 downstream；business 也驗證 exact single year。內部 endpoint 不放在 `/api/market-data/**`，防止 `MarketDataBffRoutes` 或 frontend generic browser route 直接接住。建立所選年份的 `days`／`holidays` 時，business 對 TW／US／UK holiday authority 各讀一次、defensive-copy 後生成完整日期遞增 365/366 `TradingCalendarDay[]`；`marketStatus` 是獨立既有當下 session read，不能用「全 request 僅三次 authority call」誤判其內部行為。不得逐日 authority／外呼或觸發 price refresh／calendar export／schedule／notification／broker／trade／DB／Redis write。靜態 market definitions 固定為 TW `Asia/Taipei`／`09:00–13:30`／DST false、US `America/New_York`／`09:30–16:00`／DST true、UK `Europe/London`／`08:00–16:30`／DST true；當下開盤狀態一律以 typed `marketStatus` 為準。

`PublicTradingCalendarResponse` 是完全 typed 的 yearly snapshot：Asia/Taipei `generatedAt`／timezone／year window，固定 TW／US／UK market definitions，per-market availability、trading-day counts、sorted holiday arrays、365/366 `days` 與 typed `marketStatus`。TW availability 依現有 TWSE-priority/DGPA-provisional authority chain 的 read result 判斷，source 欄描述該 chain（而非捏造單一 upstream）；US／UK 使用既有 NYSE／LSE rules。已知 market 的 weekday/non-holiday flag 為 boolean；該 authority 不可用時 flag 與 count 都是 null，不能以 `false`／`0` 偽裝休市。這條 read 可保留既有 calendar authority 的 request-time read/fetch 以保持日曆新鮮度，但不得啟動 external sync job 或任何 persistent mutation。

##### Requirement 109／114：台股五檔 canonical snapshot（富邦十秒 primary、Yahoo producer fallback）

`TwLiveQuoteDispatcher` 仍是 Requirement 106 的唯一已授權台股 LIVE producer。它使用同一個 40 檔 round-robin 富邦 normalized batch：actual-price observation 依既有流程優先保存；optional `orderBook` 只在完整且同來源時交給獨立、single-flight、無 queue 的五檔 writer。worker 永遠不發第二個富邦 request；Requirement 114 只允許對該輪缺完整富邦 book 的代號委派受限 Yahoo worker，且任何 DB／Redis／Yahoo 卡住或 worker reject 只保留上一份 book，不能影響價格 chain 或下一輪 price refresh。

```text
Fubon raw quote (same 10-second call)
  └─ Python adapter: optional orderBook(lastUpdated, bid/ask x5, summary)
       ├─ complete valid Fubon book → FUBON_BOOKS typed snapshot
       │    └─ TwFubonOrderBookRoundWriter (separate single-flight)
       └─ missing / invalid Fubon book for selected code
            └─ TwYahooOrderBookFallbackRoundWriter (separate single-flight; bounded Yahoo calls)
                 └─ complete valid YAHOO_TW typed snapshot
                      ├─ IntradayOrderBookSnapshotStore: atomic PostgreSQL canonical comparator/revision first
                      └─ QuoteDetailCache: price:quote-detail:{market}:{code}, canonical-revision atomic fence, TTL 24h

GET /internal/quote-detail
  └─ QuoteDetailReadService: Redis candidate → PostgreSQL revision validation → matching cache or DB canonical → typed unavailable
```

`stock_intraday_order_book` is one header per `(stock_code, market)` and `stock_intraday_order_book_level` has exactly level 1..5 rows for that header. Header identity, market=`台股`, trading/source/fetch times, source/status, actual price/previous close and positive `canonical_revision` are NOT NULL; source is exactly `FUBON_BOOKS` or `YAHOO_TW`, and every level bid/ask side is a paired positive `(price,lots)`—never half-null, duplicate, partial or cross-source. Header and levels are one transaction. **PostgreSQL alone, using a conditional UPSERT or row-locked transaction rather than JVM read-compare-write,** makes the source-aware decision: a same-source candidate only applies when its `source_updated_at` is strictly newer; a valid Fubon candidate is primary and applies over Yahoo; a Yahoo candidate applies over Fubon only when its source timestamp is strictly newer. A new header starts at revision 1 and every accepted update increments it in that same transaction; equal/older values do not alter header, levels, stock name, fetch time, revision or generic quote state. `PersistResult` contains only the DB-written/read canonical snapshot plus its revision. The dedicated Redis envelope carries the exact decimal-string canonical revision and source-time verification token; Lua only accepts a strictly newer DB revision, never independently applies source priority. Thus an older delayed writer cannot overwrite a newer canonical cache value. Only an APPLIED row or the canonical row read after a stale conflict may enter that Redis writer. A legacy envelope without a revision is a cache miss. The reader validates the Redis envelope revision against the current DB header before returning it; a mismatch or DB failure cannot yield stale cache, and a mismatch reads the full DB canonical snapshot without repair. The writer has no generic quote index, tick, day-H/L or SSE side effect. Redis failure leaves the DB canonical row intact; a later round may repair only from that row.

The exact internal reader never repairs cache or triggers upstream IO. `market != 台股` and `code == 0000` return typed unsupported before Redis/DB/HTTP. It reads a Redis candidate only to compare its revision with the current DB header, then returns either that matching candidate or the full DB canonical snapshot; DB revision/snapshot failure returns typed unavailable rather than freshness-unknown cache. Every supported-but-unavailable cache/DB or proxy failure returns `source=null`, `marketStatus=UNKNOWN`, empty levels and a generic message. An available canonical snapshot exposes only `FUBON_BOOKS` or `YAHOO_TW`; existing `MarketDataService`, `StockAnalysisBffRoutes` and public BFF keep their typed proxy shape and never select a vendor. This preserves the public raw 19-field contract and all personal-data boundaries.

> **Requirement 114／Task 379 current-state override:** The previous FUBON-only and “no Yahoo fallback” wording in Requirement 108／109／110 is superseded for the producer flow and the source eligibility of the same immutable direct bidLevels／askLevels projection. In every ten-second Fubon batch, a selected Taiwan code with no valid complete Fubon book is handed to an independent, bounded Yahoo fallback worker. A stable maximum-40 candidate set uses an 8-code fair cursor, so at most 16 probes fit the four-concurrency/two-second/nine-second budget and every candidate reaches a probe within five accepted worker rounds. A valid complete book for either source has five positive paired bid/ask levels, bid descending, ask ascending, no duplicates, nonblank name, OPEN, positive price/previousClose, and valid current-Taiwan-date nonfuture source time. It is persisted as `YAHOO_TW`, never relabelled as Fubon and never composed with another source. Each Yahoo probe has a typed outcome: only HTTP 404 is `STRUCTURAL_MISS` and can serially advance `.TW` to `.TWO`; a valid 2xx matching snapshot is `FOUND` even when its exchange reports `TWO`; timeout, transport, non-404 HTTP, over-2-MiB body, and parse/validation failure are `TRANSIENT_OR_INVALID` and stop the code for that round. PostgreSQL applies source priority atomically and increments a canonical revision; Redis fences only on that DB revision, preventing cross-writer cache rollback. The query rule remains zero-vendor-I/O: readers use a Redis candidate plus PostgreSQL revision validation, returning cache only on a revision match or the full DB canonical snapshot otherwise. An available response may expose exactly `FUBON_BOOKS` or `YAHOO_TW`; unavailable / unsupported remains source null and typed empty.
...
#### business-services Internal API（容器內運維與受限資料 bridge；不提供 frontend route）

> business-services 的 `/internal` **現有 controller mapping** 包含 `UserAdminController` 的 `/internal/users`、純讀分鐘級的 `/internal/backtest/rules` 與 Treasury proxy/refresh；新增前必須再次以 controller mapping 實查更新本表，不得沿用歷史盤點。現有端點的授權拓撲不能以一個 blanket `AdminGateInterceptor` 描述：Treasury business proxy/refresh 由 business AdminGate 保護，external Treasury 查詢則另由 shared token＋ADMIN role filter 保護。**本次 public-read BFF bridge 不掛 AdminGate 的精確 mappings 為：Requirement 108 的 `GET /internal/public-market-data/dividends-readonly-result`／`GET /internal/public-market-data/intraday-ticks-readonly`、Requirement 111 的 owner-scoped `GET /internal/public-trading-radar/current/list`／`stock`、Requirement 112 的 owner-scoped `GET /internal/public-transaction-history/current`，以及 Requirement 113 的 no-tenant `GET /internal/public-market-data/trading-calendar`；它們與既有 `/internal/backtest/rules` 分別受各自的精確 controller／BFF route boundary 約束。**上述 owner-scoped radar／transaction bridges 只由 configured-admin bootstrap 後、帶顯式 tenant header 的 named client 呼叫；quote bridges 和 calendar 只由 named no-tenant client 呼叫。沒有 inbound controller／route／proxy 可把它們交給 frontend，且不得用 `/internal/public-market-data/**`、`/internal/public-trading-radar/**` 或 `/internal/**` 建立寬鬆 bypass。business 8080 不映射 host，故這些例外以精確 mapping、BFF route denial 與 container network 為邊界；若日後對外映射 8080，必須先重新設計明確的服務間保護再開放。Requirement 61／Task 278 的估值回補目前仍是**規劃端點、尚無 controller mapping**；實作該任務時才可新增路徑，且必須同時把它納入 `AdminGateInterceptor.addPathPatterns`，不得把規劃描述當成已存在的安全保護。
...
| GET | `/internal/public-market-data/dividends-readonly-result?code=&market=&years=` | Requirement 108／Task 372 的 exact public-market bridge。只讓 `PublicQuoteMarketDataService` 的 no-tenant outbound client 在 asset-net 內取完整 `DividendHistoryResult`；驗證 `years=1..10`、只呼叫 `findFromDbReadOnly`，不 cold-sync／寫入。刻意不掛 AdminGate，但沒有 BFF inbound route、frontend proxy 或 9090 path。 |
| GET | `/internal/public-market-data/intraday-ticks-readonly?code=&market=&date=` | Requirement 108／Task 372 的 exact public-market bridge。只讓同一具名 no-tenant outbound client 在 asset-net 內 proxy external safe tick outcome；date 必填、不得重用 cold-start path、不得寫入。刻意不掛 AdminGate，但沒有 BFF inbound route、frontend proxy 或 9090 path。 |
| GET | `/internal/public-trading-radar/current/list` | Requirement 111／Task 376 的 exact owner-scoped list bridge。只讓 public trading-radar BFF 在 configured-admin bootstrap 後帶明確 tenant header 呼叫；只投影一次 `TradingRadarService.getCurrent()` 的首頁 typed row，不得 refresh、export、snapshot write 或被 `/api/bff/trading-radar/**` browser rewrite 到達。 |
| GET | `/internal/public-trading-radar/current/stock?stockCode=&market=` | Requirement 111／Task 376 的 exact owner-scoped detail bridge。只讓同一 named BFF client 呼叫；驗證 selector 後從單次 current response exact `(stockCode,market)` 回完整 `StockDecision`，合法無 match 是 sanitized 404，沒有 frontend proxy 或 9090 direct path。 |
| GET | `/internal/public-transaction-history/current?year=&start=&end=` | Requirement 112／Task 377 的 exact owner-scoped read bridge。只讓 public transaction BFF 在 configured-admin bootstrap 後帶明確 tenant header 呼叫；backend 重驗 ALL/year/range mutual exclusion，以 owner filter 只讀 `asset_transaction`、stable date/id order，絕不 export、sync 或寫入。沒有 frontend proxy 或 9090 direct path。 |
| GET | `/internal/public-market-data/trading-calendar?year=` | Requirement 113／Task 378 的 exact global calendar bridge。只讓 public calendar BFF 的 no-tenant client 呼叫；selected-year holiday authorities 各讀一次並 defensive-copy 生成 typed complete year，market status 是獨立 current read，未知 source fail-soft，絕不 refresh、export、排程或寫入。沒有 frontend proxy 或 9090 direct path。 |
...
| 條件 | `price`／`openPrice`／`highPrice`／`lowPrice`／`tradingDate` | `closed` |
|---|---|---|
| 完成日 K 最新一筆 **未到**台北當日，且 `getLive("0000","台股")` 有值且其 `tradingDate` **等於**台北當日 | 取自 Redis 即時點位 | `false` |
| 其餘（Redis 無值／`tradingDate` 非今日／完成日 K 已含今日） | 取自完成日 K 最新一筆（Task 263 當時行為；盤後已由 Task 290 覆寫） | `true` |

判定條件與 `computeAllForTaiex()` 併入今日 live 的條件**語意等價（對任何輸入判定結果逐次相同）**，否則同列的股價與指標會再次落在不同日期。**不是文字照抄**——兩邊的區域變數形狀不同（`toIndexResponse` 已持有降冪的 `recent` 清單、`computeAllForTaiex` 另有自己的 `desc`），且 Task 263 明訂不得改動 `TechnicalIndicatorService` 的任何一行；稽核時比對判定結果而非字面。`previousClose` 兩種情形共用同一條規則——`twse_index_daily_history` 中 `trading_date` **嚴格早於當列 `tradingDate`** 的最後一筆 `close_point`，**不讀 Redis payload 的 `previousClose`**（與「股市大盤查詢」頁當日卡的昨收同源，見該節）。`priceChange`／`changePercent` 一律由 `price` 與 `previousClose` 現算（scale 4／先除 scale 6 再 ×100 取 scale 4，HALF_UP，與個股路徑同式）。`buyPrice`／`sellPrice`／`volume` 恆為 null。**走勢圖股價側不受影響**：`HistoricalDataService.getStockHistory` 對 `0000` 明訂「完全不併 live」（Task 261）維持不變

> **Task 290 display 覆寫：** `0000` 排除於個股 `getAllDisplayPrices` universe，仍由 `toIndexResponse` 專用路徑處理。交易日盤中且 Redis 為台北當日才回上述即時點位並標 `quoteStatus=LIVE`；收盤後只在 `twse_index_daily_history` 有精確當日完成日 K 時回 `VERIFIED_CLOSE`，否則回 null price／`CLOSE_PENDING`，不得退昨日；盤前或非交易日才回最近完成日／`PREVIOUS_CLOSE`。`closed` 僅為相容欄位，不能代替 `quoteStatus`。`previousClose` 與技術指標的既有規則不變
...
- **交易日 / 國定假日判定（單一事實來源）**：`business-services` 側以 `MarketDataService.isTradingDay(market, date)`（dispatch 至 `isTwTradingDay/isUsTradingDay/isUkTradingDay`）與 `isMarketOpenNow(market)`（`MarketZones` 時段 + `isTradingDay` 假日）為唯一入口。`StockAlertService.evaluate`（live 觸發閘門）/ `computeTriggeredAt`（交易時段判斷）、`AlertNotificationDispatcher.withinSendWindow / lastTradingDate`、`StockPriceService.getMarketStatus`（→ `isXxxMarketOpen`）全部委派之，不再各自只判週末。`external-materials-service` 側對應為 `MarketCalendar` + `MarketClock`（抓價 / 收盤排程閘門）。台股年度 base 權威順序 = TWSE holidaySchedule primary → TWSE 尚不可用時的完整 DGPA 辦公日曆 provisional → unknown；business-services 經 `/internal/tw-holidays` proxy，ext-materials 以 source-aware cache 統一管理，再 read-time union operator closure。美 / 英假日為 NYSE / LSE 法定規則純函式，因兩服務無共用 module 而各持一份（交叉註解鎖定，修改須同步）。Requirement 7 / 16 / 23 / 37
- **颱風假 / 台股臨時休市偵測（Requirement 7）**：颱風等臨時停班停課由地方政府當日 / 前一晚公布，**不在** TWSE 年度 holidaySchedule 中；證交所休市與否，法規上取決於「臺北市政府是否宣布停止上班」。故在既有國定假日機制外，另建一條「當日偵測 → 台股假日 override」資料流：
  ...
  - **單一注入點**：`MarketDataFetchService.getTwHolidays(year)` 於回傳 source-aware 年度 base（TWSE 或 DGPA provisional）前，read-time union `TwTyphoonClosureService.closuresForYear(year)`（DB → in-memory 快取，不污染年度 authority cache）。因 `MarketCalendar.isTwHoliday` 與 `/internal/tw-holidays` 皆匯集於此方法，颱風休市對台股所有下游（market-status、`MarketClock`/`PricePoller`/`ClosePersister` 抓價收盤閘門、警示、備份、08:45 分析、交易日曆）與國定假日同一 cascade 自動一體生效，**前端零改動**（`TradingCalendarView` 既有 `day.twHoliday` 渲染 + market-status 開盤中→休市）。DGPA 辦公日曆 fallback 與 DGPA 天災停班公告是兩種不同資料：前者只建立年度 provisional base，後者只提供 operator closure union。
  ...
  - **傳播（business 端快取設計）**：`business-services` `MarketDataService.getTwHolidays` 只對**當年度**保留短 TTL（10 分鐘）快取 `twHolidayCurrentYearCache`。理由：颱風假可能於**任何時刻**（早盤排程、開機 self-heal、手動 `detect`）由 ext 寫入，短 TTL 保證同日任何偵測於 ≤10 分鐘內傳播至 market-status / 分析 / 警示 / 備份 / 交易日曆；`fetchTwHolidaysFromExt` 逾時 / 失敗回空表時不覆蓋前一次成功值。**非當年度不再由 business 永久快取**，每次本機 proxy 都交給 external-materials 的 source-aware per-year cache：TWSE entry 可永久、DGPA provisional 最多 6 小時後重試 TWSE，避免 business 的舊 future-year map 永久遮蔽升級。DGPA 天災公告失敗 / 查無臺北市狀態時只代表沒有新增 operator closure，不覆寫 TWSE／DGPA 年度 base。內部觸發 `POST /internal/tw-closure/detect` 供手動 / 驗證。
  - **資料層一體休市（偵測落後補清；Task 161）**：偵測有時序落差——颱風假可能盤中甚至收盤後才公告 / 偵測到（如部署晚於當日盤中）。在偵測寫入 `tw_market_closure` 前 `isTwTradingDay` 仍回 true；**Task 290 前的舊版** `ClosePersister.dumpTwCloseFromRedis`（13:32）可能把 Redis 昨收平盤 dump 進 `stock_price_history` 當日列，Task 290 後則可能已有官方／FinMind 在晚偵測前寫入的部分當日列；盤中 polling 亦可能把昨收 tick 塞進 Redis `price:ticks:台股:*:date` bucket → 「當日」走勢圖顯示假資料、`InternalPriceController.intraday-ticks` 預設日期經 `findMaxTradingDate` 落回該休市日、且假日列污染日線 / 均線。故 `TwTyphoonClosureService` 於**偵測命中**（`detectAndPersistToday` upsert 後）與 **`ApplicationReadyEvent` 開機 self-heal**（`selfHealClosureMarketData` 逐一掃本年度 `closuresForYear`）兩路徑呼叫 `purgeClosureMarketData(date)`：`StockSourceQuery.deleteTwHistoryOn(date)` 刪台股當日 `stock_price_history` + `IntradayTickStore.purgeTwTicksOn(date)` 刪台股當日 Redis 分時 bucket。**嚴格限市場字串 `台股`**——英股 / 美股同日照常交易，其 bucket 與日線不得清；**不動即時價 live cache** `price:{market}:{code}`（休市日 last price = 昨收，本就是正確現價，且比照 price cache 跨夜耐久性不回寫充數）。清除後 `findMaxTradingDate` 回休市前一交易日，「當日」分時端點自然回退（如 7/10 颱風假 → 顯示 7/9），與交易日曆一體休市一致。持久性：真休市日外部日線源本無該日 bar、`backfillTwStock` 亦 skip 今日列，故清除後不被回補重灌。redeploy（image rebuild + container recreate）即觸發開機 self-heal，無需手動 SQL / redis-cli。
...
> **Schema 基準線與 DB 層唯一鍵（重要澄清）：** 本專案的 **DB 初始化**基準線由 `db/init/01_dump.sql`（完整 `pg_dump` 快照，掛載進 `docker-entrypoint-initdb.d`）提供，**非** Liquibase 的 `v1.0.0-initial-schema` changeset；Liquibase 僅在此基準線之上做**增量**變更（dump 已含 `databasechangelog` 歷史，過往 changeset 視為 already-ran）。**這是「初始化來源」，不是「查證標準」——查證標準是下方的 `db/schema.sql`。** 因此下列 Hibernate 早期建立、已固化進 dump 的 DB 層約束**不會出現在 Liquibase changelog**，但每個環境（運行中＋全新以 dump 初始化）皆已存在，`ddl-auto: none` 亦不會重建：
>
> 📌 **查證標準：`db/schema.sql`——DB schema 的唯一標準。** 表存在與否、欄位、型別、位數、nullable、預設值、CHECK、索引一律以它為準。
>
> ✅ **`db/schema.sql` 是 DB schema 的唯一標準，其與運行中 DB 的同步由機械閘門逐次查核（Requirement 103／Task 367；定性見 Requirement 104／Task 368）。** 本檔由 `pg_dump --schema-only` 直接自運行中的 `asset-postgres` 產生。過去它靠人工重產、長期落後（Task 245 當時僅 55 張 `CREATE TABLE`，Task 367 動工前為 74 張、運行中 DB 已是 85 張），**Task 367 起**改由 `scripts/tests/schema-sql-drift-test.sh` 逐位元比對「去除專案檔頭後的檔案內容 vs 此刻重跑 `pg_dump` 的輸出」，並由 `scripts/spec-check.sh` 的 **B10** 在每次 spec 機械檢查時執行。因此欄位型別、位數、nullable、預設值與索引定義都直接引用本檔即可。**發現它與運行中 DB 不一致，就是這個檔過期——依它檔頭「重新產生」段的指令重產本檔並納入本次變更**，不要改用別的來源當基準。**同理不要引用 `db/changelog/**` 描述現況**——那裡有永不執行的 changeset（下方 `v1.0.0` 的 `NUMERIC(20,4)` 即為前例）。
>
> **B10 的兩項已知性質，引用前要知道：**（a）它是 **lagging** 檢查——`spec-check.sh` 只在實作**之前**被呼叫，漂移卻產生於實作之後，所以它攔到的是「上一輪沒重產的漂移」，不是「這一輪即將產生的」；剛動過 `db/changelog/**` 的當下在本檔查不到某張表，代表**本檔過期需要重產**；（b）**嚴重度分流**——本次變更若碰到 `db/schema.sql` 或 `backend/src/main/resources/db/changelog/**` 則漂移記為 BLOCK，否則記為 CHECK（全機數十個 worktree 共用一套 `asset-postgres`，一律 BLOCK 會讓與 schema 無關的任務頻繁被中斷、最後全體繞過閘門；且本檔是 4600+ 行的產物檔，逼多條分支同時重產會製造大量 merge 衝突），`asset-postgres` 未運行時一律降為 CHECK「無法查證」。兩種級別的處置相同：重產本檔。
>
> - `stock_price_history`：`UNIQUE (stock_code, market, trading_date)`（Hibernate 名 `ukgoyp…`）＋ `INDEX idx_sph_code_date (stock_code, trading_date)`；**以 `db/schema.sql` 為準**：OHLC 皆 `NUMERIC(15,4)`（`open/high/low` nullable、`close` NOT NULL，見 v1.14.0）、`volume BIGINT`（nullable）＋ `CHECK ck_sph_close_price_positive (close_price > 0)`（v1.85.0，Task 279 起）。Task 290 以 `v1.89.0-stock-price-close-source.sql` 再加 nullable `close_source VARCHAR(64)`；null 表示無法證明產生路徑，不得被盤後 display gate 當成已驗證。
>   ⚠ `v1.0.0-initial-schema.sql` 寫的是 `NUMERIC(20,4)` ＋ `volume NOT NULL`，但該 changeset 在 dump 中已標記 already-ran、**永不執行**，故 20,4 從未套用到任何環境——位數與 nullable 一律以唯一標準 `db/schema.sql` 為準，勿照抄 v1.0.0。
...
`StockPrice` Entity 與 `stock_price` 表已廢除，盤中即時行情改存 Redis（schema 見上方 External Materials Service Architecture）。`PriceQueryService.getLive` 從 Redis 取值並組裝成 live DTO，供內部盤中判斷；公開報價改由 `getDisplayPrice`／`getAllDisplayPrices` 依市場階段解析：

| 市場階段 | 價格來源 | `quoteStatus` | 缺資料行為 |
|---|---|---|---|
| 交易日開盤中 | 當日 Redis | `LIVE` | 沿用最近完成日 fallback，但不得標成今日收盤 |
| 交易日收盤後 | `stock_price_history` 精確當日且 `close_source` 通過市場信任條件的列 | `VERIFIED_CLOSE` | 回同代號 placeholder：`price=null`、`CLOSE_PENDING` |
| 交易日盤前／非交易日 | 台股：市場日曆解析的最近目標交易日且 `close_source` 可信；其他市場沿既有最近完成日 | `PREVIOUS_CLOSE` | 台股 exact target 缺列／來源不可信則回該 target 的 `CLOSE_PENDING`，不得跳到更舊日 |

`StockPriceService.getAllPrices/getPrice/getLiveAssets`、`WatchStockService`、Trading Radar response 的使用者可見價格與 `SnapshotFormBffController` 最新完成日價格一律走 display 路徑。雷達規則組裝仍可讀 raw live，但個股 DTO 的 `price/changePercent/quoteStatus` 必須由 display quote 覆寫；大盤 `MarketSummary` 也新增 nullable `quoteStatus`，規則輸入可留 raw，response 可見 `price/changePercent` 則走 TAIEX exact target gate。舊 Redis radar snapshot 是 JsonNode 讀取、不反序列化 record，缺新欄仍可匯出；部署後 content hash 改變而寫第一筆新 schema 快照是預期行為。前端 SSE 套同一單調狀態守門。個股批次 universe 是「當前 owner 最新快照持股 key ∪ Redis index key」，並排除走專用 TAIEX 路徑的 `0000`；不能只枚舉 Redis index，否則正是缺快取的持股會從 API 整列消失，連 `CLOSE_PENDING` 都無法顯示。`HistoricalDataService.getPricesOnDate(stocks,date)` 在 requested date 落於 `DisplaySession.targetTradingDate <= date <= marketToday` 時逐一呼叫 `getDisplayPrice`（含週末 latest snapshot 被 `SnapshotDateRollScheduler` 釘成週六／週日，以及交易日盤前 today），`SnapshotPriceDto` 傳 `quoteStatus/source` 且 pending 仍回同 key／null price；早於 target 才走 `findClosestPrice`。因此 `SnapshotEnricher.fetchSnapshotPriceRows` 的 Dashboard summary、切換快照、look-through、SnapshotForm bootstrap 及 live-assets 失敗降級都不能從 closest-history 把未驗證價滲回最新完成日。`getLiveAssets` 遇 `CLOSE_PENDING` 時該 holding 的 `currentPrice` 留 null、`liveValue` 保留快照既存 `currentValue`，並把 status 傳到 BFF／前端，不得拿 Redis last tick 或昨收重算。

`HistoricalDataService.getStockHistory` 對台股最新 display target date 套同一 provenance gate：開盤中才可合成當日 raw live；盤後、盤前或非交易日若 exact target row 缺少可信來源，從日線結果移除該 row 且不合成 Redis。如此 Dashboard／觀察清單／交易雷達與七頁共用的 StockAnalysisDialog 不會出現同日不同價。舊於 target 的歷史 row 不因 migration 後 `close_source=null` 被整批刪除；守門只作用於最新應完成交易日。

Dashboard 每分鐘 realtime 回應必須同時套用 `stockPrices`、`marketStatus`、`liveAssets`，使 13:30 前已開啟的頁面也會進入 pending。Dashboard 與 TradingRadar SSE 的狀態轉移保持單調：現有 `CLOSE_PENDING` 或 `VERIFIED_CLOSE` 都只有在 payload **顯式**帶 `quoteStatus=VERIFIED_CLOSE` 且 `tradingDate` 為該市場今日時才可覆寫；`closed=true` 不是證據，尤其 `syncClosedFromDb` 的 `PREVIOUS_CLOSE + closed=true` 必須被拒絕，不能升級 pending、也不能降級 verified。SnapshotForm 每次 realtime 收到 pending 都要清掉既有暫定價，WatchStock 與 TradingRadar 則直接顯示待補文案。前端將接受狀態／pending 判斷抽為 `utils/displayQuote.js`，使用 Node 內建 `node:test` 驗證，無需新增測試 dependency。這層只保護「對使用者宣稱的現價／收盤估值」；警示、技術指標與交易雷達規則輸入仍可使用 `getLive`。
...
>   - **台股**：`basedate == LocalDate.now(Asia/Taipei)` 且 display quote 為 `LIVE` 或 `VERIFIED_CLOSE` → 顯示 `stockPrices` 價格＋漲跌%；`CLOSE_PENDING` → 顯示「收盤價待補」。
>   - **美股／英股**：同樣以各自市場時區今日與 `quoteStatus` 判斷；EST/EDT、BST/GMT 由 JVM `ZoneId` 自動處理。
...
> - **per-market 基準日判斷一律由 BFF 完成**（`SnapshotEnricher.mergePerMarketPrices`）；前端只額外執行 API 契約要求的 `quoteStatus`／`tradingDate` 顯示守門，不自行推導官方收盤。live vs frozen 仍由 `basedate == 該市場當地今日`（前端 `shouldApplyLive` / BFF `isCurrentBasedate`）決定，**不得以 `priceChange` 是否為 null 判定**。前端 `getRealtimePrice()` 對今日列只接受 `LIVE` 的當日盤中價或**顯式** `VERIFIED_CLOSE` 的當日收盤；`closed=true` 本身、`PREVIOUS_CLOSE`、`CLOSE_PENDING` 與缺 status 的 SSE 一律拒絕。`getPriceCell()` 對 pending 顯示固定文字，不再 fallback 顯示快照盤中價；選歷史快照則維持凍結收盤。
...
>
> KPI「資產總計」一致性（**per-market 基準日閘門**，見 Task 121／290）：Dashboard `liveLatest` 對台股 / 美股 / 英股各自判斷 `basedate == 該市場當地今日`（`isBaselineToday`），今日市場用 `customTableData × overlayLivePrice` 的 display value、非今日市場維持快照凍結收盤；`totalAssets` **僅在三市場皆為今日時**才採用 `liveAssets.liveTotalAssets`，否則一律 per-market 加總。`CLOSE_PENDING` 的 holding 由 backend 把 `liveValue` 保留為快照既存 `currentValue`，故不會用盤中尾值重算；前端不得以市場開盤狀態另選資料源，只按 `quoteStatus` 接受或拒絕報價。
>
...
- `treasury_yield_batch` + `treasury_yield_daily` — **Requirement 58／Task 275**
  - batch 保存 `curve_date/provider/source_url/available_at/availability_basis/fetched_at/complete/content_hash`；daily 以 `(batch_id,tenor)` PK 保存 `yield_percent` 與 `source_url`（Yahoo 四個 ticker 各自保留 URL，官方可共用 curve URL）。batch＋四 tenor 同一 transaction 原子落地，resolver 防禦性確認四個 distinct tenor 齊全。合法零利率可存，null/<0/>100 不存。batch-level complete 與 immutable content hash 確保一次 decision 的四 tenor 同 provider、同 revision
  - 美國公債殖利率四個天期 `M3/Y5/Y10/Y30`；美國財政部 Daily Treasury Par Yield Curve Rates 年度 CSV/XML 為 primary。Yahoo `^IRX/^FVX/^TNX/^TYX` 僅為整批 fallback；partial official 永不進規則，完整 fallback 可用，complete official 到達後同日優先 official
  ...
  - Yahoo fallback 代碼另開 `TREASURY_YAHOO`，不污染 `US_INDEX_YAHOO`；官方歷史初次重建 available-at 採 curve date 翌日 00:00 New York，proxy 採 curve date 18:00 New York，修正版採實際 fetched-at，全部明列 basis
  - 任一 yield null／負值／非有限一律略過，不以前值回填；curve completeness 明示，decision-time resolver 只選當時 available 的單一 complete batch
...
**規則**（`external-materials-service` 的 `TradingDateResolver.resolve(code,market,observationInstant)`；Requirement 89／Task 350 起由 producer 在 immutable observation 形成時呼叫，writer 不再自行決定）：
...
if isLiveSession(observationInstant):
    trading_date = observationInstant.atZone(MarketClock.zoneOf(market)).toLocalDate()
else:
    trading_date = max(stock_price_history.trading_date for this code+market)
                    fallback observationInstant.atZone(zoneOf(market)).toLocalDate()
```

`isLiveSession` 的市場交易日與時間窗也必須以同一 `observationInstant` 換算，不可在 resolver 內另讀牆鐘而跨過午夜／開收盤邊界。若 provider 本身給可信日期（MIS、Yahoo LSE、TAIEX 5分K、verified close、DB row），依 Requirement 89 producer矩陣直接採來源date，不再呼叫resolver猜測。
...
> **Task 357 起：除息日與除權日拆為兩個獨立欄位。** FinMind 的 `CashExDividendTradingDate`（除息）與 `StockExDividendTradingDate`（除權）在此之前被**六處**抓取路徑壓成單一 `ex_dividend_date`（其中三處是這兩個原始欄位互相 fallback；另三處——TWSE/TPEx 官方日曆與 `TaiwanStockDividendResult` 的兩個解析點——本來就只有單一日期欄，依型別字串判準路由卻沒有把判讀結果帶進落地值），只要一檔同時配息又配股（或純配股走上述任一單欄位來源）、除權日即在入庫當下被丟棄；純配股事件的「除息日」欄顯示的實際上是除權日（`2881` 是混合配息配股標的，其現金股利與股票股利各自入列為獨立事件；純配股的那一列子事件現金股利為 0 卻有除息日——不是該標的整年零現金股利）。Task 357 新增 `ex_rights_date`、停止壓合，並定義 `anchorDate = min(除息日, 除權日)` 供區間過濾與年度推導使用；落地值仍必須是各自的真實日期，不得以 anchorDate 回填。詳見 `spec/tasks/t357_dividend_four_dates.md` 357.2a–f。
>
...
**投影去重與事件同一性（Task 345）**：事件同一性以「除息日＋現金股利＋股票股利」為準（relaxed identity；金額比對一律 **null 視 0**，與 `uk_dividend_event` 索引的 `COALESCE(…,0)` 口徑一致）；`event_key`（其 canonical 定義含發放日，維持不動）、兩個發放日與 `source` 都是公告 metadata，只能更新既有列、不得分裂新列或取消同日同額事件。落地在三處：(1) `JdbcDividendCurrentStateRepository.findMatchingIds` 六段依序匹配——**先三段限 `ACTIVE`（key→全值→relaxed），再同序三段不限狀態**（revive 語意保留；ACTIVE 整塊優先，避免 `upsertHistoricalEvent` 的 `AND event_status='ACTIVE'` guard 選中 CANCELLED 列後靜默 no-op），UPDATE 分支改「先讀後寫」：Java 端算出發放日不倒退（incoming 非 null 才覆寫）的最終值後執行；(2) `DividendCurrentStateProjectionService` cancel pass 的權威身分集合加入 `date-amount:` token（金額 null 視 0），同除息日同金額的未來列不因 key／發放日差異被取消；(3) `projectOne` 進場先跑 `collapseDuplicateActiveEvents`——經 port 新增的 `findActiveEventDetails`／`applyMergedEnrichment` 原子操作，把同 relaxed identity 的多列 ACTIVE 收斂成單列（keeper 偏好：發放日非空→`yield_pct` 非空→id 最小；metadata 與 enrichment 欄位 keeper 優先、缺值由重複列補），其餘轉 CANCELLED。**唯一索引 `uk_dividend_event`（v1.99.0，不含 `event_status`）是所有 UPDATE 的硬約束**：寫入前須先刪除與寫入後 tuple 在**索引 9 欄 COALESCE 口徑**下全等的 CANCELLED 冗餘列（`yield_pct`／`previous_close`／`fill_days`／`source` 不在索引內、不參與比對；索引口徑不全等的 CANCELLED 列一律保留 finalized 還原語意），否則 duplicate key 會讓 `REQUIRES_NEW` 投影整體 rollback、自癒永久失效。收斂不依賴 snapshot 存在，`DividendHistoryService.projectFailSoft` 每次讀取都會觸發，故既有髒資料（如 00881 的 2026-01-20／2025-01-17 各兩列 ACTIVE）於下次開啟頁面自癒，不寫 data migration。同日不同金額仍視為兩事件：未來事件由既有 cancel pass 收斂，已發生事件並存（已知限制）。重複列的下游影響不只年度小計：`findAdjustmentEvents` 的還原權息（BacktestService／TradingRadarService／HistoricalBondYieldBetaEvidenceAdapter）會把同筆配息扣兩次。
...
- `AlertChartRenderer.renderIntradayPng(code, market)`（Task 159／409）：警示 email 第二張圖「當日分時走勢」，**單一 pane**（無 KD 副圖），只 render 與 `StockAnalysisDialog` 相同的 server `IntradaySession`：
  - 分時與交易日取 `HistoricalDataService.fetchIntradaySession(code, market, null)`；此 route 的 tick bucket 是 cache-only，僅合法台股個股今日的 reference-cache miss（不依 tick 完整度）可 bounded 取得 exact TWSE MIS `d/y` evidence，零 TickRefresher／FinMind／Yahoo／tick write。
  - **比較基準線、漲跌與色彩**：直接讀 session 的 server-ready `comparisonPrice/comparisonKind/lastPrice/change/changePercent`；renderer 不查第二份日線、不自行計算。無比較 tuple 時不畫基準線、使用中性顏色；任何來源／日期／tuple 不完整由 session fail closed。
...
Task 290 後 47 個帶 `zone` 的 `@Scheduled` annotation（Spring 的 `zone` 覆蓋 JVM 預設；官方對帳同一方法的兩個 annotation 分開計數。英股 `0 32 16` zone=Europe/London 已用運行中日誌實證於 `15:32:00Z` 觸發＝倫敦 16:32 BST）、3 個 `fixedDelay` 排程、`MarketZones`／`MarketClock`、`AlertNotificationDispatcher.withinSendWindow()`、`BackupService` 的 `DISPLAY_ZONE`、所有 `Instant.now()`（絕對時間軸）、10 個 `timestamptz` 欄位、以及**全部前端顯示程式碼**（後端改吐台北牆鐘後自動正確；前端不存在任何 +8 補償 hack，不得為此新增，否則雙重補償）。
...
- **帳號選擇器（Requirement 122）**：`.oauth2Login(o -> o.authorizationRequestResolver(...))` 注入自訂 `ServerOAuth2AuthorizationRequestResolver`（`DefaultServerOAuth2AuthorizationRequestResolver` 包一層 `authorizationRequestCustomizer`），對每次產生的 `OAuth2AuthorizationRequest` 加入 `additionalParameters().put("prompt", "select_account")`，使 Google 每次都顯示帳號選擇畫面而非沿用瀏覽器既有 session 自動選帳號；只新增這一個 additional parameter，不動 `redirect_uri`／`scope`／`client_id` 既有邏輯。
...
### Task 351／354：三年查詢窗口、雙年度匯出與 DGPA 暫行台股日曆（現行契約）
...
`holidays.tw/us/uk` 的空 map 是 unavailable 訊號，不是「該年度零假日」。TW authority 的第一順位仍是 `MarketDataFetchService.fetchTwHolidaysFromTwse(year)` 所查 TWSE 官方歷年報表 `https://www.twse.com.tw/holidaySchedule/holidaySchedule?response=json&date={year}`。只有 HTTP 成功、`stat=ok`、頂層 `date` 以要求的西元年開頭、`title` 含對應 ROC 年，且 `data` 非空、每筆日期皆屬要求年份，才解析為完整 TWSE base；錯年、空列或格式錯誤都視為 TWSE 尚不可用。資料列仍排除「開始交易日／最後交易日」，其餘開休市列依既有規則轉成 `yyyy-MM-dd → 名稱`。

#### DGPA 暫行 authority（Task 354）

TWSE 回空時才呼叫新增的 `DgpaCalendarAuthority` port；外層 `DgpaCalendarClient` 實作先讀 data.gov.tw 官方資料集 metadata `https://data.gov.tw/api/v2/rest/dataset/14718`，以 `year - 1911` 選出該民國年的一般 `CSV` resource。名稱只接受精確 base「{ROC}年中華民國政府行政機關辦公日曆表」及其單一括號 revision，明確排除 `_Google行事曆專用`；候選編碼只接受 `UTF-8`／`BIG5`，但不得先按 encoding 過濾。revision ranking 不把 `resourceQualityCheckTime` 誤當版本時間：任何括號 revision 勝過原版；只有一筆 revision 直接選；多筆 revision 必須全部能解析 `{ROC}MMdd更新` 且 requested ROC year 相符，才按 revision date 取最新，否則 fail closed。QC time 僅用來處理相同 description／同 revision 的重複 resource，仍 tie 就拒絕。以 metadata 宣告的 charset 建立 `CharsetDecoder`，malformed／unmappable 都設 `CodingErrorAction.REPORT`，未知或空白 encoding fail closed。metadata 所給下載 URI 必須再次做 allowlist：scheme=`https`、host=`www.dgpa.gov.tw`、port=`-1|443`、path=`/FileConversion`、無 user-info／fragment、query 恰一個 `filename` 且值為 `.csv`；duplicate filename、alternate port 與未重新驗證的跨 host redirect 都拒絕。metadata／CSV 各自使用 15 秒 request timeout，任一步 HTTP 非 200、strict decode 或 parse 失敗都回 unavailable。

CSV parser 以選定 charset strict decode，移除首欄可選 BOM，沿用抽出的共用 quoted-field parser，不把逗號內文字切壞。schema 必須精確為 `西元日期,星期,是否放假,備註`；資料列數恰等於 `Year.of(year).length()`，日期為 `yyyyMMdd`、全部屬 requested year、唯一且完整覆蓋 1/1～12/31，中文星期 `日一二三四五六` 與實際日期一致，`是否放假` 僅允許 `0` 或 `2`。任一列、任一欄或全年 coverage 不符即拒絕整份，禁止用部分 CSV 補成全年 authority。114 年 metadata 的較新 `114年...辦公日曆表(1141020更新)` 為 BIG5，必須勝過舊 UTF-8 原版並正確包含 2025-09-29、2025-10-24、2025-12-25 三筆修正放假日。

推導台股 base 時，先把 DGPA `是否放假=2` 且為週一至週五的日期加入休市 map（名稱採非空 `備註`，空白則用「行政院人事行政總處放假日」）。再找全年按日期排序後第一筆 `備註` 含「春節」的 row，向前掃描並只計 `是否放假=0` 的兩個工作日，跳過週末與 `2`；兩日加上「農曆春節前市場無交易」。找不到錨點或不足兩日就拒絕整份。故 2026 的額外休市為 2/12、2/13，與 TWSE 官方「市場無交易」資料相符；2027 的額外休市為 2/2、2/3。除此之外，DGPA `0` 的平日就是台股交易日；不另列人工日期清單。

`MarketDataFetchService` 的年度 cache 改存 immutable `{holidays, source, expiresAt}`。`source=TWSE` 可沿用既有年度永久 cache；`source=DGPA_PROVISIONAL` 最多 6 小時，到期後同一條 shared loader 先重試 TWSE、TWSE 仍空才刷新 DGPA。TWSE 一旦成功，當次讀取立即覆蓋 provisional entry，後續不再呼叫 DGPA。若 provisional 到期時兩個網路來源同時暫時失敗，保留最後一份已通過全年驗證的 DGPA map，將下次 authority retry 設為最多 10 分鐘後；若從未成功則回空且不快取空值。

loader 固定採以 observed immutable entry 為 expected value 的 source-aware optimistic CAS，來源順位為 `TWSE > DGPA_PROVISIONAL`。發布 candidate 時必須在 CAS loop 內重讀 incumbent 並比較順位：(1) incumbent 已為 TWSE，任何 candidate 都採 incumbent；(2) candidate 為 TWSE、incumbent 為 DGPA 時，以該 incumbent 為新 expected value 持續 `replace`，失敗便重讀再比，直到裝入 TWSE 或發現另一個 TWSE；(3) candidate 為 DGPA 時只可 `putIfAbsent` 到空 slot，或在 observed incumbent 仍是同一筆 DGPA 時做同順位 `replace`，一旦 incumbent 變成 TWSE 就丟棄 DGPA；(4) 首次 `putIfAbsent` 若由 DGPA 先勝出，較晚取得的 TWSE 仍須進入第 (2) 步升級，不能因首次 CAS 失敗而遺失。禁止全域鎖，也禁止同年度序列化，讓不同 year 與同年兩個 in-flight refresh 都可並行，且無論完成順序，已取得的有效 TWSE 最終一定永久勝出。backend `MarketDataService` 只保留當年度 10 分鐘 cache；所有非當年度查詢都直接 proxy 此 shared loader，不再維護 `twHolidayCache`，否則 2026 首次讀到的 2027 DGPA map 會讓 BFF／匯出永久短路，external 的升級完全無法穿透。競態以 latch/barrier deterministic tests 固定重現 expired entry 與首次空 slot 的雙向完成順序：既驗 TWSE 先安裝／DGPA 後到，也驗 DGPA 先安裝／TWSE 後到；不以壓力測試碰運氣。

external-materials 的既有 `GET /internal/tw-holidays` 繼續使用 `MarketDataFetchService.getTwHolidaysKnown(year).orElse(Map.of())`，不新增另一支 business API。`getTwHolidaysKnown` 只有在 operator closure calendar 已知，且 shared loader 取得 TWSE 或完整 DGPA base 時才回 read-time union；兩個 base 都空但 closure-only 非空仍回 unknown，closure calendar 未知也回 unknown。如此 backend `MarketDataService.getTwHolidays`、BFF 查詢與匯出 preflight 共用同一份完整性訊號；2027 在 TWSE `data=[]` 期間可由 DGPA 暫行 base 成為 AVAILABLE，TWSE 日後公布則自動升級。
...
`exportYearPairToDir(anchorYear, subpath)` 只組合既有單年度 primitive，不複製 JSON／Excel builder。入口先在任何年度 authority read 與逐年 `try/catch` 之外呼叫既有 `requireValidSubpath(subpath)`；非法 `..` 跳脫維持由 `GlobalExceptionHandler` 映成 400，零查詢、零檔案。合法路徑才固定對兩年各執行一次窄 `try/catch`：每年建立一次 immutable `CalendarAuthoritySnapshot(tw, us, uk)`（名稱可等價），台股空 map 直接轉該年 null paths、`totalDays=0`、含年份失敗 `localStatus`；非空則呼叫接收 snapshot 的 private 單年度 overload。既有 public `exportToDir(year, subpath)` 自己建立一次 snapshot 後也進同一 overload。故 pair 不做重複 preflight，且每年度每市場精確查一次。

`buildDays(year, snapshot)` 的三市交易旗標都直接使用 `非週末 && !snapshot.<market>.containsKey(date)`，不得逐日再回呼 `MarketDataService.isXxxTradingDay`；`buildJson(year, days, snapshot)` 的 holidays 也 defensive copy 同一 maps，Excel 只吃同一 `days`。這讓每一年度的 JSON 與 Excel 成為同一 authority version 的原子快照：即使第 2 次 mock 呼叫會失敗或 external 在產檔途中由 DGPA 升級 TWSE，該年度也不會出現前後半混版或 `tw=true` 同時帶 `twHoliday` 的矛盾。某年度建立 snapshot／產檔擲 `RuntimeException` 時仍只失敗該年並繼續另一年；故合法 batch 永遠含兩筆逐年結果並以 200 回報，不會出現第一年檔已落地卻因第二年例外只回整批 5xx，也不會吞掉既有路徑安全 400。
...
TaiexIndexPoller（週一～五 09:00–13:30 Asia/Taipei，每 2 分鐘，MarketClock.isTwMarketOpen() 守門）
  → MacroDataFetchClient.fetchIndexIntradayDay("TWSE")  ← Task 263 新增，與 fetchIndexIntraday 同 URL、同 curlGetWithRetry
  → 回傳 DayQuote{date, open, high, low, latestClose}：date 為該批點位所屬台北日期，
    open=當日首格開盤、high=各格最高之最大、low=各格最低之最小、latestClose=最後一筆非 null 收盤
  → date != 台北當日 或 latestClose == null → 整輪不寫（保留 Redis 上一輪真實值，比照個股 z='-' 慣例）
  → previousClose = twse_index_daily_history 中 trading_date 嚴格早於 date 的最後一筆 close_point
  → PriceCacheWriter.write(PriceResult{code=0000, market=台股, source 含括號如 "TWSE指數(5m)",
                                       openPrice/highPrice/lowPrice 取自 DayQuote, ...},
                           markClosed=false, aggregateHighLow=false)
       → price:台股:0000（TTL 24h，schema 與個股相同）；source 含 "(" 故不進 IntradayTickStore
       → aggregateHighLow=false 故不碰 price:dayhl:台股:0000:{date}
```

**Task 263 為何要驗證日期（原 Task 228 的缺漏）：** `fetchIndexIntraday` 打的是 `interval=5m&range=5d`，回傳橫跨最近五個交易日，內部以 `byDate.lastKey()` 取最新交易日——**Yahoo 尚未產生今日第一根 5 分格時，「最新交易日」就是昨日**。cron 第一輪落在 09:00:00 整、今日 09:00 那格要到 09:05 才收，故每個交易日開盤都會發生一次。實測 2026-07-31 的 `price:台股:0000` 因此帶著 `lowPrice=39933.3008`（7/30 的盤中點位，經 `IntradayHighLowTracker` 併進當日 bucket），而 Yahoo 當日真實低點為 41610.41；聚合只取 min，錯值不會被後續正確值修正，會持續整個交易日。
...
> |---|---|---|
> | **美債殖利率** | 「需新資料源」 | **資料已確認可得**：美國財政部 Daily Treasury Par Yield Curve Rates 為 t275 正式 primary source；先前 Yahoo `^IRX`／`^FVX`／`^TNX`／`^TYX` 各 2514 筆的盤點保留作 fallback 可行性證據。由 Requirement 58（t275）落地為 `treasury_yield_daily` 並先作 `BOND` candidate，只有 t308 樣本外通過才正式啟用。**台灣央行政策利率仍不在範圍。** |
...
**六檔標的的極端態觸發率為 0.00%**（00719B／00865B／00697B／00695B／2412／00751B）——「極端超賣不殺低」對它們十年來從未生效。V13 candidate 改以標的自身近 60 日日報酬標準差 `σ` 正規化：`normalizedBias = bias ÷ (σ × 100)`（`bias` 是百分比、`σ` 是比例，量綱必須對齊），門檻改以 `σ` 倍數表示。`σ` 由 `RadarInputAssembler` 對 adjusted completed rows 單一計算、須有 calibration-derived floor，樣本不足時回退固定門檻並降低 confidence。**原始 `ma60BiasPercent` 照常輸出不變**；V12 的 2/98 percentile 保留揭露，但 normalized path 啟用時不得再參與 action。t274 只建立 candidate，t308 holdout/walk-forward 通過後才正式啟用並一次升 V13。
...
> **⛔ 本節的待辦設計已由 Task 291 取代。** 公式同源與 split-only 成交量因子的問題診斷仍有效；「先量測再決定是否納入」已被使用者最新要求改為指定類別全部納入。
...
> **⛔ 本節已由下方 Task 291 設計取代。** 「中長線至兩年」、依個人資金安排仲裁，以及短線通知 DB 擴充均不再適用。
...
兩軌衝突時**一律並列呈現、不自動仲裁**：差異只表示約一週與一至六個月的市場訊號不同，不讀取、推導或要求使用者的資金安排。既有的單軌 API 欄位語意明確定義為「中期軌」並保留，避免通知／匯出／快照三個既有消費端靜默改變語意。
...
```text
TradingRadarView
  → GET /api/bff/trading-radar（BFF gateway passthrough，不建第二份 DTO）
    → TradingRadarService
      ├─ holdings ∪ watchlist（owner-scoped，只有台股）
      ├─ RadarInputAssembler
      │   ├─ DistributionAdjustedPriceService（價格：配息＋分割；volume：split-only）
      │   ├─ TechnicalIndicatorService.computeFromSeries（MA5/20/60/240、KD/J、MACD、RSI、BIAS、W%R）
      │   └─ 最新完成日 volume ÷ 前 20 個正 volume 日的中位數
      ├─ TradingRadarMarketContextService（新增，純讀 global market data）
      │   ├─ twse_index_daily_history：完成日量／值比
      │   ├─ us_index_daily_history：IXIC／SOX 最新共同完成日
      │   ├─ news_headline：72 小時內 TW／US 公開資訊
      │   └─ MarketDataService.isTwTradingDayKnown：既有 external-materials 假日日曆 proxy／快取（Optional fail-closed）
      ├─ TradingRadarRuleEngine（同一輸入各跑 short／medium Accumulator）
      └─ TradingRadarSnapshotStore（完整 Response JSON round-trip）
```

`TradingRadarMarketContextService` 是全域市場資料唯一組裝者，不依賴 `CurrentUserContext`；它的 production／historical API 都接收顯式 `decisionInstant`，內部不得呼叫 `LocalDate.now()`。`MarketDataService` 新增 `Optional<Boolean> isTwTradingDayKnown(LocalDate)`：週末直接 known-false；平日只有年度假日 map 非空才回 known true／false，proxy 失敗或空 map 回 empty，且不得改變既有 `isTradingDay()` 的其他呼叫端語意。context service 只准呼叫這個新方法；遇 empty 立即 fail closed，不得自行以平日或歷史缺列猜測。它不得自行建立 WebClient，也不得注入 `MarketAnalysisService`、crawler 或 LLM client。新聞只做資料選取與 DTO 映射，不做 sentiment。`TradingRadarService` 仍負責 owner-scoped universe，兩種責任不得交換。

#### 規則引擎資料模型

```text
MarketInput
  price, intradayChangePercent, Indicators,
  ma60Confirmation, ma240Confirmation,
  marketVolumeRatio, marketTurnoverRatio,
  completedMarketChangePercent,
  nasdaqChangePercent, soxChangePercent, usTechCompositePercent

StockInput
  held, price, changePercent, completedChangePercent,
  Indicators(MA20/60/240,K,D), weeklyMa, ExtendedIndicators,
  previousK/D, MA20/60/240 confirmations,
  instrumentType, marketRegime, marketStale,
  fxPercentile, ma60BiasPercent, ma240BiasPercent,
  week52Position, kdBandWidthPercent,
  etfPremiumPct/Percentile, volumeRatio

StockResult
  score/action/reasons/risks              # medium, backward-compatible
  shortScore/shortAction/shortReasons/shortRisks
  horizonConflict, counterTrend, kdHeat, timingState,
  kdDeadCross, longTermBroken, profitTakingConfirmed
```

`ExtendedIndicators` 不在引擎內重算；DTO 與 engine 可共用語意相同但 package-owned 的 record，轉換只能發生一次。所有 contribution 皆由純函式回傳 `Double`／`null`，`Accumulator` 是唯一缺值重分配點。短期／中期權重數字只寫在 Requirement 43 Task 291 的權重表與引擎具名常數，design 不複寫，避免第三份數字漂移。

#### 量能與跨市場日期對齊

個股 `volumeRatio` 的分母排除訊號日，使用先前 20 個正值的中位數；最少 10 筆。`DistributionAdjustedPriceService` 將事件值定義為 `shareGrowth=postShares/preShares`，同時保有價格累積因子與 share-only 累積因子。逐列的 `historicalShareScale=cumulativeShareGrowthAtRow/finalShareGrowth`，`adjust(source, priceScale, historicalShareScale)` 以 `volume=round(source.volume/historicalShareScale)` 還原。1:4 分割前的 historical scale 為 0.25，故 volume ×4；股票股利 1 元為 ×1.1；現金股利的 shareGrowth 恆為 1。不得用含現金股利的價格 scale 改 Long volume。

大盤量能一律綁 `decisionInstant` 前已完成的最新日，不得與 Redis 盤中價格拼接。`marketVolumeRatio` 與 `marketTurnoverRatio` 各自缺值；規則層只平均可用者。美股科技先以 `16:00 America/New_York` 判斷 session 是否已完成，再以 IXIC／SOX 的日期交集找共同日，不能各取自己的 latest 再假裝同日；共同日之外還需要各指數嚴格前一筆才能算一日報酬。超過同一 decision instant 的台北日期 5 個日曆日則整組 unavailable。歷史回測固定用 signal date 的 `14:00 Asia/Taipei`，即使 DB 已有未來列也不能讀取。

匯率與跨市場 context 共用同一 as-of 原則：目標日由 `MarketDataService.isTwTradingDayKnown()` 找出 decision instant 前已完成 17:00 的最近台股交易日；任一候選日回 empty 時整組 fail closed。只接受該日精確列；`buyRate/sellRate` 缺值、非正或相等皆無效，五年有效樣本少於 600 回 null，不得用最近值補。DTO 顯示 `fxAsOfDate`，讓使用者能分辨「真的有完成日資料」與「缺值」。

#### 動作與相容性

兩軌分數都代表目前位置的 opportunity score，不是預測值。分數映射後依序套：資料完整性 → 追高／續跌買進閘門 → 低接 trial-buy → 多重轉弱獲利了結 → 極端超賣不殺低。一般 BUY／ADD 與 trial-buy 均要求 `completedChangePercent>=0`；高分但仍下跌只能 HOLD／WATCH。獲利了結至少要在極端高檔且 `kdDeadCross`／`OSC<0`／`下跌爆量` 三項中兩項成立；極端超賣保護最後套用且沒有 long-term-broken 例外。

既有欄位與通知只代表 medium：`StockDecision.score/action/reasons/risks`、`trading_radar_notification_setting.last_action`、`TradingRadarNotificationTransition` 全部維持同一語意。短期欄位只進 API、Vue、snapshot、Excel/JSON；不新增 changeset。通知 dialog 的文案改成「中期建議（1–6 月）」以使邊界可見。

匯出仍由 `TradingRadarExportService` 建唯一 `ExportDoc`：大盤表追加完成日量比與美股科技欄；個股表追加 short score/action、分歧、volume ratio；新公開資訊表逐列輸出 region/time/source/title/url。Excel 與 JSON renderer 不各自拼欄，舊 snapshot 的新增值為 null 時留白。

### 個股基本面與產業發展（Requirement 43／46，Task 292，`TW_RULES_V11`）

Task 292 將原本尚未實作的 t266／t267 合併到雙軌雷達，並增加全市場產業營收：

```text
news_headline (= public_info_<date> export source)
  -> PublicInfoEvidenceResolver (exact code / full stock name / industry-name match; evidence only)

missing structured field
  -> provider chain: TWSE/TPEx -> Yahoo -> WantGoo -> FinMind
  -> StockFundamentalFetchClient (external; six official financial schemas + bounded fallbacks)
  -> StockFundamentalPoller (crawler_schedule=fundamental; warmup/manual/scheduled share one run)
       |-- target individual rows -> valuation / financial quarter / monthly revenue
       `-- all-market official monthly rows -> industry_monthly_revenue
  -> PostgreSQL append-only observations (single provider + all source URLs + available + observed instant)
  -> FundamentalAnalysisService.resolve(code, market, decisionInstant)
       |-- source availability cut-off and no-future filtering
       |-- cumulative reports -> standalone quarters -> EPS growth / approximate ROE
       |-- revenue growth / PE own-history percentile
       `-- stock industry -> all-market industry YoY
  -> TradingRadarRuleEngine FundamentalInput (V11 short + medium accumulators)
```

business 層不新增對外 HTTP client，也不直接讀 `public_info_*` 檔案；`news_headline` 就是該檔的上游同源資料，
避免檔案日期、匯出失敗或路徑設定讓同義資料分岔。`PublicInfoEvidenceResolver` 只選相關證據，不做情緒或數值
解析。`FundamentalAnalysisService` 是唯一數值計算者，同時供 production 與 backtest 使用；不得在
`TradingRadarService`、`BacktestService`、BFF 或 Vue 重算。官方財報與月營收的 availability 來自出表日，
當日 18:00 Asia/Taipei 才視為已完成；估值日在 14:00 完成。fallback 若沒有發布時間，只能用第一次成功觀測
時間作保守 availability 並標 `OBSERVED`，不得回填成更早日期。這些時點一律對顯式 `decisionInstant` 比較，
不讀 system today。

provider resolver 對每個欄位固定使用 `PUBLIC_INFO(證據) -> EXCHANGE -> YAHOO -> WANTGOO -> FINMIND`；
`PUBLIC_INFO`／`WANTGOO` 只有新聞而無可信結構化數值時不產生數值候選，繼續往後。每筆候選帶 provider rank，
resolver 以「同期間、同衍生因子、較高順位優先」處理，低順位不得洗掉高順位非 null；同一衍生值所需欄位必須來自
同 provider family 與同期間，合併端點的全部 URL 存入 `source_urls`、availability 取最晚者，避免混出不存在的財報。
每次修正值以新 observation append 並帶自己的 `observed_at`，
同值重跑 no-op；as-of 同時限制 available 與 observed，日後修正版不會倒灌歷史。估值另以 nullable
`pe_loss_flag` 區分來源證實虧損與欄位缺漏。全市場產業彙總只准使用同輪官方交易所月營收，fallback 僅補
使用者持有／觀察個股，不得以目標清單充當全市場。

官方損益表是同會計年度累計值；內部先以 `standalone(Q1)=cumulative(Q1)`、
`standalone(Qn)=cumulative(Qn)-cumulative(Qn-1)` 轉為單季，才能組最近四季與前四季。
缺前季時該單季是 null，不得把累計數直接當單季。`revenue_yoy_pct` 與產業彙總為快照
denormalization，保留是為了 as-of 回放；不存衍生 ROE。

ETF 辨識集中在 `FundamentalAnalysisService`：`etf_nav_history` 存在 -> 台股 `00` 開頭；不新增硬編分類欄。
ETF 回 `fundamentalApplicable=false`，五個新 contribution 全為 null。個股回傳
`coverage 0..4`、四項值與資料日／逐因子 provider/sourceUrls、產業名與彙總月份，以及個股／產業公開資訊證據；前端與匯出
只呈現這份 DTO，不從新聞關鍵字推導產業方向。

V11 將基本面與產業作為五個 optional contribution 加入原同一 `Accumulator`；精確權重只存於
...
**跨市場改為 applicability 旗標，而非市場別分支。** 引擎原本只有「有資料 vs 沒資料」兩態，把「不適用」硬塞進「沒資料」。新增一個 `boolean` component 後分三段：不適用 → 沉默；適用但缺資料 → 維持既有的正當提醒；適用且有資料 → 加減分不動。**相容建構式一律填 `true`**，讓漏改的後果是多一則正當提醒而非靜默吞掉真提醒。不採「service 端過濾 risks 字串」（脆弱、破壞引擎純函數契約與回測可重現性），也不採「傳市場別讓引擎自行分支」（Requirement 64 明文「不得為美股另建規則類別或複製判定邏輯」）。同語彙先例見 `TradingRadarEvidenceConfidenceResolver` 的 `liquidityApplicable`——其 javadoc（英文）明確要求該值是 market adapter 提供的能力宣告（`a capability decision supplied by the market adapter`），且 `must never be inferred from a nullable ratio`。

**量價文案改為依實際可用 ratio 內插。** 原文案寫死「成交量或成交金額」「量能／成交金額同步放大」，在美股（結構性無成交金額）與台股週轉率缺樣本（`ratio()` 要求 ≥10 筆正值）時都會新造第二則假訊息。改由純函數依 `marketActivity` 實際由哪幾個 ratio 構成內插標籤。**這會改變台股在週轉率缺樣本日的既有可見文案**，屬修正而非退步。

**版號跳到 `TW_RULES_V14`，13 號永久保留給 candidate。** `RuleParameters.V13_VERSION = "TW_RULES_V13"` 是離線 calibration／candidate 參數集的標籤（`evaluateCandidate()` 明文「candidate evaluation 必須使用 TW_RULES_V13 參數」），與 production 的 `RULE_VERSION` 是**兩個獨立命名空間**；Task 336 已預先記載「若日後真要升版必須跳到 `TW_RULES_V14`」。**不新增 `V14_VERSION` 常數、不改 `RuleParameters.v12Default()`**——其 `ruleVersion` 在 production 的全部消費端都是 `V13_VERSION.equals(...)` 的二值判別式，本次未改動任何個股參數值。`spec/requirements.md` 中「V13 candidate 發布時 production 升為 `TW_RULES_V13`」那條承諾已同步改為「升為當時的下一個 production 版號」，不再寫死。

**三條不升版先例本次全部不成立**：Task 249 要求「同一份輸入前後產生完全相同的輸出」（本次 `reasons`／`risks` 立即改變）；Task 281 要求「不進 `StockInput`／`MarketInput`」（本次的本質就是灌進 `MarketInput`）；Task 336 要求「規則引擎輸出逐位不變、僅修正顯示值捨入缺陷」（本次會改變輸出）。

**回測側同批對齊，否則會製造新分岔。** `BacktestService` 組美股 `MarketInput` 的位置現在就已與 production 不一致（回測已填完成日漲跌幅、production 傳 `null`），只是被 `marketActivity == null` 短路而無可觀察後果；production 一補上量比，該分支就會在線上生效、回測不生效且無測試可抓——這正是本專案踩過的坑：`spec/tasks/t323_radar_us_market_volume_wiring.md` 的標題逐字就是「交易雷達美股大盤量能接線：`buildUsMarket()` 補上既有 IXIC 量能，**停止線上與回測分岔**」（對應 Requirement 64／65）；Task 299.5 亦明文規定「回測與 production 共用同一 assembler 輸出，**不得**在 `BacktestService` 另算一份」。故同批補上 `RadarInputAssembler` 既有的 `volumeRatio`（演算法與 `TradingRadarMarketContextService.ratio()` 逐項相同）。**代價：** 既有 V13 walk-forward／holdout 的美股 regime 序列改變，Task 308／310／314／316 已記錄的美股校準結果失效，未來 promote V13 時須重跑。

**上線當下可見的變化只有文案。** 落地時實測 IXIC 完成日漲跌 `-0.28%`、量比 `0.7915`，命中「完成日下跌但量能收斂」得 `+3`；但美股 regime 原已 100 分且引擎有上限截斷，故 `score` 仍 100、`regime` 仍 `RISK_ON`、美股個股買進閘門不變（`StockInput` 無大盤分數欄位，閘門只看 `marketRegime` tier 與 `marketStale`）。**這是當日資料的巧合，不得據此宣稱「本改動不影響分數」**——量價擺幅 `+8／−10／−3／+3` 在其他情境會實際翻動 regime 甚至關閉買進閘門。
...
PostgreSQL／Redis adapters
  → RadarObservationResolver（只決定 value／asOf／source／quality，不打分）
  → RadarInputAssembler／FundamentalAnalysisService／BondRateFactorService（純特徵）
  → TradingRadarRuleEngine（opportunity／downside／confidence／action gate）
  → TradingRadarDto／快照／Excel／Vue（只呈現，不重算）
```
...
**波動與債券。** `RadarInputAssembler` 以還原序列計算近 60 個日報酬的 ratio 樣本標準差；`biasRatio=ma60BiasPercent/100`，只有 raw sigma>0 才可套 calibration floor，normalized=`biasRatio/effectiveSigmaRatio`。t274 路徑啟用時取代 V12 2/98 action 路徑。Treasury 以 batch/daily 兩表保存 complete provider snapshot與 conservative available-at；同一次判斷四 tenor 不混源。台灣掛牌外幣債的 return beta 控制同期 USD/TWD；美國掛牌 USD 債不控制 USD/TWD。strict bond term UNKNOWN、樣本或跨窗穩定度不足皆回 missing，不猜 duration。

**公開資訊與事件。** `PublicInfoEvidenceResolver.resolve` 接收 market，台股只讀 TW/null，美股只讀 US/null；一般新聞只作 evidence。配息採 `stock_dividend_snapshot`＋event rows＋append-only fetch observation：resolver 取 decision instant 前最新完整、scope 足夠的單一 snapshot；新完整 snapshot 未含舊 future event即為取消，改期為舊取消＋新 active。`stock_dividend_history` 是 finalized current-state，另有 ACTIVE/CANCELLED。Upcoming scope 的時間窗固定為 `[decisionDate, decisionDate+45d]`，由 `MarketDividendUpcomingScopeClient` 先查台股 TWSE/TPEx 官方日曆或 Nasdaq；官方結果完整時不得再以其他 provider 覆寫。官方不可用時，才由 `DividendFetchClient.fetchProviderUpcomingScope` 以同一 bounded window 查 FinMind（台股）或 Yahoo（美股）並回傳 provider/source/as-of；provider 必須對該完整範圍明確回覆，不能把歷史請求的 HTTP 200、部分事件或 transport failure 靜默轉成「無事件」。partial／failure 一律保留 incomplete evidence，且不得遞迴呼叫 upcoming-scope router。所有 upcoming observations 與 finalized current-state 分開保存，取消與改期依 append-only event 規則處理。IXIC/SOX/SPX/DJI、index volume、法人與商品先轉 typed numeric `CandidateMarketFeature`，逐項有 conservative available-at；法人數值落 `twse_institutional_daily`，禁止從新聞字串反解析。與既有 regime/turnover/FX 同義者 rejected/disclosure-only。
      ...
      → TradingRadarRefreshService
          ...
          2. PriceQueryService.refreshTradingRadarPrices()（WebClient，30s timeout，同步等待）
             → POST /internal/refresh/tw-radar（external-materials-service）
                 ...
                 │    ├─ PricePoller.updatePrices(twCodes, "台股", false)  ← Task 350：兩個 global MIS batch waves，≈13s 網路上界
...
**但休市分支本身有一個必須守門的空窗**：`MarketClock.isTwMarketOpen()` 上界是 `13:30`，而今日收盤價要到盤後官方對帳才會進 `stock_price_history`；Task 290 後 pending window 是 13:30 至該檔最早 14:05 official close（缺檔可延續至更晚 fallback）。`syncClosedFromDb` 此時的 `findRecentClose` 仍可能取 T-1。Task 249 的頁面層 predicate 因此保留：`isTradingDay(今日)` 為真、已過 13:30、且該檔 DB 尚無今日列時跳過，同時產生誠實的 `SKIPPED_PENDING_CLOSE` outcome；盤前與非交易日不套這個 UI predicate。**Requirement 89／Task 350 再補 writer 層共用原子日期守門，明確推翻 Task 249 當時「不改 syncClosedFromDb」的範圍限制**：`refreshAll`／`warmCacheOnStartup` 即使沒有頁面 predicate，只要 Redis 已有今日 LIVE，DB 的 T-1 payload 也會被拒寫；Redis 缺值的盤前／非交易日仍可正常由最近 DB close 修復。UI predicate 與 writer guard 職責不同，兩者都保留。

> **Task 290 現況覆寫上段的「13:32／16:00」時間與來源敘述，但不移除 Task 249 守門：** 台股 13:32 不再落 DB，盤後 pending window 改為 13:30 起直到最早 14:05 官方對帳逐檔完成；缺檔可延續至 15:35／16:00 FinMind fallback／17:35。既有 predicate 本來就是「已過 13:30 且該檔 DB 尚無今日列」，沒有把 13:32 寫進程式，所以可直接涵蓋拉長後的逐檔 pending window；盤前 DB 的權威來源則改讀「最近一次官方對帳或缺檔 fallback」，不再假設一定來自 FinMind。

**逾時要分成硬邊界與健康態 SLO**。Requirement 89／Task 350 起，台股個股兩個 global MIS batch waves 每 request 5 秒、waves 間只 sleep 一次 3 秒，故 MIS 網路部分有約 13 秒硬上界；舊 virtual-thread-per-code 約 20 秒描述不再是 producer 契約。大盤與個股併行，大盤 `Future.get(12s)` 上限不變且不得被 executor close/await 抵銷。business→external 30 秒、frontend 45 秒、nginx 60 秒仍是由內而外的 caller/request 硬邊界；business 於30秒 fail-soft 回 TIMEOUT／FAILED。Redis Lua／name DB 寫入在健康 stack 預期使用剩餘預算，使 manual refresh 實測 `<30s`，但沒有新增全方法 monotonic deadline、取消令牌或 detached background task，故 Requirement 89 明確推翻「external 在任何 Redis／DB 故障下也必須自行於30秒完成」的舊承諾。`<30s` 是必測的健康態 SLO，不能只以13秒網路上界推論。
...
`priceRefresh` **刻意不回傳抓取檔數**：回補清單是全庫的（Redis 行情快取本就是跨租戶共用的市場資料），回檔數等於把「全庫台股標的數」洩漏給任一使用者；檔數只進 business／external 的 log。`radar` 部分與 GET 完全同形。Task 249 當時的「`TradingRadarDto.Response` 不新增欄位」只是在約束該 refresh 功能不要另造不同 schema；**Task 290 明確覆寫**，在 GET 與 refresh 共用的 `MarketSummary`／`StockDecision` 新增 nullable `quoteStatus`。Redis 舊快照以 JsonNode 讀取，缺欄自然為 null，不需 migration 或重寫；`TradingRadarExportService` 為手動欄序，須在「大盤總覽」與「個股決策」各插入 TEXT「行情狀態」欄，新快照寫 status、舊快照輸出空白／JSON null，並以 headers／formats／rows 欄數一致與 legacy 相容測試鎖住。
...
| R41 油價金價 | 全域 | 不需要 | `exportCommodityPrices(start, end)` |
| R42 台幣兌美元 | 全域 | 不需要 | `exportExchangeRates(currency, start, end)` |
| **R45 大盤指數日線** | **全域** | **不需要** | `indexDailyDoc(market, start, end)`；手動 `exportIndexDaily` 只作下載 wrapper |
...
第六～九欄是本匯出**唯一不直接取自 DB 欄位**的四欄，定義：

> 該日（含當日）往前 **N 個交易日**的 `close_point` 簡單移動平均，
> N ∈ {5（週線）, 20（月線）, 60（季線）, 240（年線）}——**交易日，非日曆週／月／季／年**。
> `BigDecimal` 精確加總後 `divide(N, 2, HALF_UP)`；視窗未滿一律 `null`。

**四欄走同一支 `indexMaAt(rows, i, window)`，只差視窗長度**——與 BFF 的 `movingAverage(closes, window)`
是同一個結構（那邊也是四條線共用一支）。**不得**為某一條均線另寫一份。
（刻意不叫 `maAt`：`TechnicalIndicatorService` 已有一支同名同形的 `maAt(asc, i, days)`，
語意不同——那支是股票路徑、`double` 累加、可能併入 Redis 今日即時點位——同名會讓
...
> **同理，backend 內部「直接注入 `TechnicalIndicatorService`」也不是本次的解**——
> 那兩支同專案、同 Spring context，技術上完全共用得了，擋住的一樣是 live 併入語意與覆蓋率，
> 不是建置結構。（真要收斂，見下方選項 2 與「已知分裂」段。）

被放棄的選項有三，都可行、都超出「加一條線一個欄」的範圍：
...
2. **新增一支 business 序列端點回 `{dates, closes, ma5, ma20, ma60, ma240}`、BFF 改為 relay**：
   ...
   它把 MA 計算搬出 BFF（而 CLAUDE.md 明文把「預先計算」派給 BFF），在 business 多出第三種同一份日線序列的
   表述，且要動一條使用者天天在看、目前正常運作的圖表路徑；換得的只有「少一份實作」，
   使用者看到的東西一模一樣。**這是本次借下的債；日後兩處若真的出現不一致，正解是回頭做這一條。**
3. **抽 shared module** 放共用 MA 工具——backend 與 bff 是兩個獨立 Maven 專案（`asset-management`／
   `asset-management-bff`，**無共同 aggregator／parent POM**——各自 parent 為 `spring-boot-starter-parent`
   ——且彼此無相依），新增共用模組是建置結構層級的改動。

**本次接受的做法是「同定義同精度」的人工約束**：同為 BigDecimal 精確加總（無中間捨入）＋
`divide(window, 2, HALF_UP)`（`window ∈ {5, 20, 60, 240}`），輸入同為兩張日線表的 `close_point`。
兩邊查詢區間本就不同（BFF 取近 10 年、匯出取 `[start−400d, end]`），成立的是**同一交易日的同一視窗
會取到同一組列**——因為 `close_point` 兩張表皆 NOT NULL，BFF 的 `if (d == null || c == null) continue;`
不會命中、不會少列。

> **同值保證的兩處界線**（不寫下來就會變成本專案典型的「過度絕對斷言」）：
> 1. **圖表 10 年視窗最左緣**：BFF 起點寫死 `today.minusYears(10)`，該處各自視窗未滿故為 `null`
>    （MA5 為最前 4 個交易日、MA240 為最前 239 個交易日）；匯出若回看得到更早的列則有值。
>    兩者定義相同，差別只在各自視窗的左緣。DB 因 `refresh?years=10` 逐年往回累積而超過 10 年後，
>    此差異會成為常態。
> 2. **交易雷達匯出也有欄叫「週線MA5」／「MA20」／「MA60」／「MA240」**（`TradingRadarExportService`
>    讀 `MarketSummary.weeklyMa`／`monthlyMa`／`quarterlyMa`／`annualMa`，走 `taiexSimpleMa` 的 `double`
>    路徑），走勢圖的同名線（`indicators/series` → `TechnicalIndicatorService.maAt`）亦然。
>    三者同名、同語意（台股大盤 5 交易日均）、同一張 `twse_index_daily_history`。**本次不收斂。**
>
>    **但差異的成因必須講對，否則會去修錯的地方：**
>    - **不是精度差。** 台股收盤為 `numeric(12,2)`，5 筆之和除以 5 的精確商恆為**三位小數且第三位為偶數**
...
>
>    要真正收斂成一份實作，必須先決定「MA 要不要併 live」這個**語意**問題（各消費端答案不同），
>    再統一 `TechnicalIndicatorService` 的算術路徑——那會動到交易雷達、觀察清單、走勢圖與其 golden，
>    是獨立任務，不在本次範圍。

**這個例外只適用本匯出，不寫進 `steering/structure.md` 3.2**（該條規範的是「BFF 不在不同頁重複呼叫不同
...
**400 天怎麼來的：由最長視窗 MA240 決定。** MA240 需要當日之前的 **239 個交易日**；
...
（`c -> c < 11 ? c : (c < 18 ? c + 1 : c + 15)`）對改動前的 golden 比對——但那要維護一組位移函式；
附加在最末可用 **identity 映射**比對，既有斷言的欄索引也一格都不用改。
JSON 那一份（R55 雙格式）欄名同為 `週線MA5`／`月線MA20`／`季線MA60`／`年線MA240`，
由同一份 header 清單產生，兩種格式不會分岔。

### 兩張來源表的分派

`indexDailyDoc(market, …)` 以 `market` 分派：`TWSE` → `TwseIndexDailyHistoryRepository`
...
- `backend/.../repository/IndexExportScheduleTimeRepository.java`
- `backend/src/main/resources/db/changelog/changes/v1.88.0-index-export-multi-time-market.sql`

**異動**
- `backend/.../model/IndexExportSchedule.java`：保留 owner 共用設定，移除單一時分／market／owner 層 last-run 欄位
- `backend/.../repository/IndexExportScheduleRepository.java`、`backend/.../service/IndexExportScheduleService.java`、`backend/.../controller/IndexExportController.java`、`backend/.../dto/IndexExportDto.java`：改為 `times[]`／`markets[]` 契約
- `backend/src/main/resources/db/changelog/changes/v1.66.0-index-export-schedule.sql`：既有歷史 migration 保留不改
- `db.changelog-master.yaml`：註冊 v1.88.0（保留既有 v1.66.0）
- `frontend/src/views/GdpTwseView.vue`：排程設定卡改為可新增多個時間列，每列用 `el-select multiple` 搭配 checkbox 選取多個指數，並保留既有手動匯出／資料夾選擇器／Drive／立即匯出；`frontend/src/api/index.js`、`GdpTwseBffController.java` 的既有 passthrough 路由保留不改
- `bff/.../schedulelist/SchedulePublicBffController.java`：保留既有「大盤指數匯出」項目，將說明同步為多時間點／多指數排程語意，不新增第二筆
...
- `GdpTwseBffController.java`：`index-daily` 回傳加 `ma5`（同一支 `movingAverage`，window=5）；
  該方法由 `private` 改 package-private 供測試釘定義 ＋ javadoc 的 MA 清單更新
- `bff/src/test/java/com/steven/assets/bff/gdptwse/`：新增一支單元測試，釘住 `movingAverage` 的視窗／null／精度
- `backend/.../service/export/DualFormatSingleTableExportTest.java`：新增「插欄零回歸（identity 映射）」
  「MA5 值」「回看視窗」三條測試，並把 `twseIndexHistRepo` 的 stub 改為可斷言查詢起點的形式
...
- `MacroHistoryController.java`：**僅 javadoc**——六欄→九欄。另 `ExcelExportService.java:534`
  「第六欄『週線MA5』是本分頁唯一的計算欄」須改為「第六～九欄是本分頁唯一不直接取自 DB 欄位的四欄」
  （標題錨點「週線MA5：唯一的計算欄」本身不變，見上方標題節的說明）
- `GdpTwseBffController.java` / `bff/src/test/.../GdpTwseBffControllerMaTest.java`：**不改**
  ——兩處對本節的交叉引用是錨點文字「週線MA5：唯一的計算欄」，標題未變則引用仍然有效
- `SchedulePublicBffController.java`：**僅文案**——「＋週線MA5」→「＋四條均線」
...
這條路徑即 Task 291 所採用的「擴充 `FullIndicators` 單一路徑」，因此把值接進 `StockInput` 時不得再長第二份輸出。代價是 `computeAll()` 的所有呼叫端（觀察清單、警示觸發落地、資產 Excel 匯出）也會多算這些值，即使部分呼叫端不使用——O(n) 且 n ≤ 241，刻意付這個代價換「只有一條取值路徑」，**不得**為此加 `boolean withExtended` 把路徑分岔。**不得改呼叫走勢圖那條 `indicatorSeries()`**（原始價基，會與同列的 `k`／`d` 矛盾）。既有視窗對 MACD／RSI 的單向遞迴已足夠收斂（序列長 241 時 EMA26 seed 殘留權重 `(1−2/27)^215 ≈ 6.5×10⁻⁸`、Wilder RSI10 `0.9^230 ≈ 3×10⁻¹¹`），故**個股維持 241／240、大盤維持 `computeAllForTaiex()` 既有的 240**，兩邊都不延長也不對齊——理由不是「改了會算錯」（240 vs 241 的 `k`／`d` 實測 bit-identical，`taiexSimpleMa(desc,240)` 只讀 `desc[0..239]`），而是那條路徑另有三個非雷達消費端（觀察清單 `0000` KD 欄、Requirement 44 通知門檻、走勢圖同源判準），動它超出範圍且數值上無收益。
...
`MarketCalendar.isTwTradingDayKnown(LocalDate)` 沿用同一 shared loader 的「TWSE primary → 完整 DGPA provisional → unknown」，再 union operator 維護的 `tw_market_closure`，作**保守專案代理**，不可冒稱銀行官方日曆。解析 session 所需的任一平日 authority 空／抓取失敗即為 unknown，所有來源 fail closed；FX known path 以每年份 30 秒節流重試 DB/TWSE/DGPA transient failure，成功後恢復，不改既有 `getTwHolidays()` 對空結果的 legacy cache/退化。current date 已知休市時，不開當日台銀、一般盤或新的兆豐夜盤；但由前一已知營業日 `16:30` 已開啟的兆豐夜盤仍跨越中間週末／休市日，直到次一已知營業日 `08:00`。代理 closed 即使銀行實際可交易仍可能造成保守 false-negative，禁止 weekday fallback、force-open 或 production bypass。
...
個股需求不建立新 producer。既有 `external-materials-service` 已有正確邊界：台股與美股各自以市場時區的 cron 每 2 分鐘執行，成功報價經同一個 `PriceCacheWriter` 寫 Redis；Requirement 89／Task 350 起，台股 client 對同一批代碼一次帶入每檔 `tse` 與 `otc` 候選 channel，取代逐檔依序查詢。本任務只補齊上櫃反回歸證據與排程清單文案，**不改 cron、不新增 `@Scheduled`、不把任何路徑改成 2 秒**。
...
PricePoller.scheduledTwIntradayUpdate
  ...
  → PriceFetchClient.fetchTwBatch(codes)
       每檔同時帶 tse_{code}.tw／otc_{code}.tw；每批最多 40 檔
       第一輪未解決者等待 3 秒後重試一次
  → 僅 requested code 唯一相符且 z 為可解析正成交價時回 PriceResult
...
持股資料模型中的上市與上櫃皆為 `market="台股"`，exchange discovery 留在 TWSE MIS client 的同批 `tse`／`otc` 候選，不新增 tse/otc schema 欄。`collectHeldStockCodes` 現況只在 `stock_alert` SQL 排除 `0000`，快照持股側仍可能帶入，因此 `scheduledTwIntradayUpdate` 必須在收集後、呼叫 `updatePrices` 前顯式移除並以快照 fixture 測試。錯誤交易所的空 placeholder 直接忽略；`z='-'`、code 不符、duplicate、非 200、解析失敗在一次有界重試後仍未解決者不呼叫 writer，因此保留上一輪 Redis 值。不得以昨收 `y`、開盤 `o` 或買賣價補成 live。
...
1. external：相容逐檔 `getStockPrice` 的可控 HTTP fixture可保留「`tse` miss後命中`otc`」回歸；新的兩分鐘producer另須證明同一批request同時帶`tse`／`otc`候選、錯誤交易所空placeholder不形成報價、合法OTC完整mapping、`z='-'`零寫入、快照持股`0000`不進MIS、TW／US `@Scheduled` cron均為每2分鐘。兩者不可混成producer仍逐檔序列查詢；同module以`{stat,tables:[{fields,data}]}` fixture驗證TPEx月OHLC、新「成交張數」與舊「成交股數（仟股）」兩alias join、民國年轉換、兩者×1,000、OHLC／volume失敗邊界，以及MIS必填`t/c`、末點缺`ts`仍保留的一分K聚合五分格與台北09:00–13:30；並負向驗證兩個Yahoo map均無TPEX。
...
|---|---|---|---|
| GET | `/api/quotes` | BFF（內部讀 external-materials raw Redis） | 全域市場資料，只讀；每列保留 19 欄並加 `marketData`，無個人資產 |
| GET | `/api/quotes/one` | BFF（內部讀 external-materials raw Redis） | 全域市場資料，只讀；保留 204 cache-miss 語意，無個人資產 |
...
## Requirement 89／Task 350：台股 latest cache 日期時間單調、MIS 批次重試與輪次可觀測性

### 問題分解與不變邊界

9090 的兩支 quote route 是 Redis reader，不是 fetch endpoint。高失敗率由兩個 producer-side 問題疊加：啟動 close self-heal 會在交易日 14:05 前選前一交易日並把其 `VERIFIED_CLOSE` 寫回同一個 latest key；每兩分鐘逐檔 MIS 又會在 `z='-'`／空回應時保留舊值。故 HTTP 200 仍可能帶回舊日 close 或超過嚴格五分鐘門檻的 LIVE。修正不改公開 API 的唯讀性，也不以 `y`／`o`／bid／ask 填補沒有真實成交的標的。

### latest cache 的兩道日期守門

close metadata 的 Lua 補齊另有不可自行選擇的數值契約：若同日 `VERIFIED_CLOSE` 借用 existing 正數 `previousClose`，script 必須以 `priceChange = incoming.price - previousClose`、`changePercent = priceChange * 100 / previousClose` 重算並覆寫兩欄；changePercent 最終為小數 6 位 `HALF_UP`，正負 midpoint 都遠離 0。previousClose/OHLC 只接受 finite JSON number 且大於 0；volume 只接受 finite JSON integer 且介於 0 與 9,007,199,254,740,991（含）之間。JSON numeric string 及其他型別、0/負 OHLC、負數/fractional/越界 volume 都不得從 existing 搬入。

第一道放在 `ClosePersister`：台股 historical repair 仍須完整抓來源與 upsert DB，但是否 publish latest cache 由純判斷決定：

```text
publishHistoricalCloseToLatest(targetDate, nowTw)
  targetDate >= nowTw.localDate                    → true
  today 非台股交易日                               → true
  nowTw.localTime < 09:00                          → true
  其餘（交易日 09:00 後且 targetDate < today）     → false
```

因此盤前可讓 Redis 指向最近完成日，週末／假日可修最近交易日，14:05 後的當日 official close 亦可覆蓋；但盤中重啟及 13:30–14:04 close pending 窗口不會再把 T-1／更舊資料寫回 latest。`selfHealTwClose(nowTw)` 只算一次此 decision，並同時傳給 official reconciliation 與 official coverage 不足時的 FinMind fallback；兩條分支都照常 upsert DB，但 decision=false 時都不得呼叫 cache writer。這可防 Redis 為空時第二道日期 guard 無法阻止 T-1 FinMind 寫入。DB repair 不受此判斷影響。

第二道是 `PriceCacheWriter` 內所有 latest-key 寫入共用的 classpath Lua resource `redis/price-cache-monotonic-write.lua`，禁止 Java 與 runtime 各抄一份。固定介面：KEYS[1]=latest、KEYS[2]=index；ARGV[1]=完整 incoming JSON、ARGV[2]=index member、ARGV[3]=TTL seconds、ARGV[4]=publish channel；tuple 只從 incoming JSON 解析，避免「比較一份date/time/status、SET另一份payload」。回傳 `{status,existingDate,existingUpdatedAt,existingQuoteStatus}` 供 outcome 與拒寫log使用，status 1=WRITTEN、0=REJECTED_STALE；existing status壞時第四格固定`""`，不得用Lua nil截斷multi-bulk，不得為log另GET。incoming status missing/null/非字串/trim後空白由script在任何mutation前回Redis error，Java映射FAILED；unknown非空字串合法rank0。existing JSON malformed或date壞可repair；existing JSON/date/time合法但status missing/null/非字串/空白同樣視legacy rank0，仍須走time比較。script 的比較分層：(1) existing date 合法且較新，無論 time/status 是否有效都拒絕；(2) date較舊接受；(3) 同日先守證據順位 `VERIFIED_CLOSE=3 > LIVE=2 > PREVIOUS_CLOSE=1 > 其他/legacy狀態=0`，incoming較低時即使取得時間較晚亦拒絕；順位相等或提高才比較正規化time，incoming time較舊永遠拒絕、相等／較新才接受；(4) existing time缺／壞仍不可讓可辨識高順位狀態被降級，只有順位不低才可修復；(5) date缺／壞才允許跨日修復。production writer只產生三個正式狀態。existing time相容`HH:mm`補秒與9位fraction、`HH:mm:ss`補9位fraction、1–9位fraction右補零；各欄須合法。

tuple 接受後、SET 前才可做同一 script 內的 close metadata 補齊，拒絕路徑不得先改 incoming 或 Redis。規則固定為：(a) incoming `stockName` 缺漏／null／空白時，可從可解析 existing 沿用非空字串名稱，名稱可跨日；(b) 其他數值只在 existing date 合法且等於 incoming date 時處理；(c) `VERIFIED_CLOSE` 缺 previousClose 時沿用 existing 正數 previousClose，再用 incoming price 重算 priceChange/changePercent；(d) `PREVIOUS_CLOSE` 的 previousClose 永遠採 DB incoming 值，但 incoming 缺少 OHLC/volume 時可逐欄沿用同日 existing 數值；(e) `LIVE` 不做這些沿用。malformed existing、壞 date 或跨日不能搬數值。補齊後重新 encode 的才是唯一 SET/PUBLISH payload，不能新增內部 marker、公開欄位或 ARGV。接受時同一script做SET EX/SADD/index EXPIRE/PUBLISH，Java對映`CacheWriteOutcome { WRITTEN, REJECTED_STALE, SKIPPED_INVALID_PRICE, FAILED }`。

REJECTED_STALE 的「零 mutation」精確限定為 latest key、index membership/TTL、publish 與 tick。`IntradayHighLowTracker` 的 `price:dayhl:{market}:{code}:{incomingTradingDate}` 是唯一例外：Requirement 14 原本就要求每筆有效 actual trade 貢獻該交易日高低，即使同日較舊 observation 延遲抵達也可能帶來真實極值；它只能更新 incoming 自己的 date bucket，不能讓 stale observation 改 latest payload或其他日期bucket。非正價在 day-H/L 與 Lua 前直接 SKIPPED，Redis零互動。這個例外須用測試明示，不能再用含糊的「拒絕零mutation」。

**Intraday tick ownership 與 eligibility 都只在 writer：**只有 live 入口 `write(result, markClosed=false)`、price 為正、既有 actual-trade gate `source != null && !source.contains("(")` 通過，而且 Lua outcome 為 WRITTEN，才 append 恰一次。`writeVerifiedClose`／`syncClosedFromDb` 即使 WRITTEN 也永遠零 tick；REJECTED_STALE、SKIPPED_INVALID_PRICE、FAILED 亦零 tick；`PricePoller` 絕不另 append。這保留 close payload 只更新 latest key、不污染當日成交 LIST 的既有語意。其餘 close self-heal、sync與同日延遲 observation race 邊界不變。

每個 observation 固定唯一 `tradingDate` 與 `Instant freshnessInstant`；`writeVerifiedClose` 不再另收第二份日期，若為遷移保留 overload 則不相等即FAILED且零Redis。writer以同一Instant格式化台北updatedAt；符合上述live actual-trade eligibility且WRITTEN時，才以同一Instant轉 `MarketClock.zoneOf(market)` append tick。沒有來源timestamp的live response在解析接受時Clock一次，再把同一Instant傳給 `TradingDateResolver.resolve(code,market,instant)`；resolver以它決定live/just-closed市場當地today，盤外仍查DB max date，不得再讀now。新payload固定9位奈秒，public shape不變。

| producer | tradingDate 權威 | freshnessInstant 權威 |
|---|---|---|
| TWSE MIS live | `d`，strict `yyyyMMdd` | strict `d+t` 以Asia/Taipei組成Instant；`tlong`是來源更新時間、不是z成交時間，只作選填格式/同日anomaly evidence，禁止要求等於`t` |
| NASDAQ live | 同一Instant傳入既有resolver（live/just-close取市場日，盤外DB fallback） | response完整解析接受時可注入Clock一次；現有payload無已驗證可信成交timestamp |
| Yahoo LSE live | `meta.regularMarketTime`轉Europe/London日期 | 同欄strict positive epoch seconds；缺漏即整筆fail，不能now修補 |
| TAIEX `0000` live | Yahoo最後一根5分K的市場日期，且等於DayQuote date | 最後一根K `timestamp` strict epoch seconds |
| TWSE／TPEx official、FinMind TW close | 已驗證來源列／target date | 來源無成交timestamp；row接受時Clock一次 |
| FinMind US／Yahoo UK verified close | US parser必讀最後row date且等於expected target；UK採已驗證bar date | row/bar接受時Clock一次；US date mismatch須DB/Redis零寫 |
| DB sync | single-query `DatedClose.date` | 同一row返回時Clock一次；此為取得時間而非新成交證據，payload固定`PREVIOUS_CLOSE`，同日不得降級覆蓋LIVE／VERIFIED_CLOSE |

timestamp單位、zone、格式或重複date不一致一律fail-closed。這張表是Task 350實作的完整producer契約，不允許writer自行猜日期。

`syncClosedFromDb` 以 single-query `DatedClose(date,close)` 消除兩查競速；previous close依該date，row返回時Clock.instant一次。official insert併發也不會產生T-1價+今日date。

### 台股批次 MIS 與一次重試

`PricePoller.updatePrices` 對台股分流至 batch；美股／英股保留現行 virtual-thread per-code 路徑。台股將 insertion-order codes 每 40 檔切一 chunk；每檔在同一個 `ex_ch` 放兩個候選，separator 使用 `%7C`：

```text
tse_0056.tw%7Cotc_0056.tw%7Ctse_00697B.tw%7Cotc_00697B.tw...
```

TWSE MIS 對錯誤交易所回空 code placeholder；parser 只接收 requested set 內唯一非空 code。HTTP 200 仍先驗 root success envelope：`rtcode="0000"`、`rtmessage="OK"`、`msgArray`為array；缺欄／非成功值是request failure，空msgArray則是合法success後逐code MISSING。

並行常數固定 `CHUNK_SIZE=40`、`MAX_CONCURRENT_CHUNKS=8`、`MAX_REQUESTED_CODES=320`。超過320不截斷：整批零HTTP、全部MISSING，`capacityRejectedCodes`帶全requested並WARN。合法輸入最多8 chunks，以最多8個固定worker同時執行；每wave coordinator global deadline與單request timeout都是5秒。deadline時已進HTTP send者計httpRequests+requestFailures，未進send／executor reject者只進capacityRejectedCodes，兩者codes都暫列MISSING。第一輪RESOLVED固定，其餘共同等一次3秒後按原序第二且最後wave；不得第三輪、無界executor/queue或用close/await拖過deadline。因此MIS網路硬上界為`5+3+5=13s`。等待、executor與deadline clock可注入，測試不真sleep；健康態manual refresh `<30s` 另做runtime SLO。

單次response分類固定：唯一matching row、positive真實z且strict d/t可組合法台北成交Instant為RESOLVED；唯一row的z為`-`/空/null為NO_TRADE；duplicate、非正/不可解析z、positive z缺/壞d/t為INVALID；無row為MISSING；request failure provisional MISSING。`tlong`不參與四類：存在但非strict epoch millis或轉台北calendar date不等d時累積`sourceTimeAnomalyCodes/schemaAnomalies`，仍以d/t解析合法row；實證`d=20260821,t=13:30:00,ot=14:30:00,tlong=1787293800000`即合法RESOLVED，因tlong對應ot而非t。第一輪只有RESOLVED terminal，其餘由第二輪覆蓋。

client summary 使用resolved表示來源有效真實z＋成交時間證據；另帶HTTP計數、schemaAnomalies、foreign、capacityRejected與sourceTimeAnomaly evidence sets。HTTP failure定義不變；四類partition=requested，tlong anomaly可與RESOLVED並存。PricePoller的cache outcome partition不變；INFO列所有evidence sets，tlong anomaly彙總可見但不逐檔WARN。

### 驗證邊界

單元測試以可控HTTP/Clock/deadline fixture驗分塊/capacity/waves/計數與分類；MIS必含`d/t`成交時間不同於`ot/tlong`更新時間仍RESOLVED、壞tlong只記anomaly、壞d/t才INVALID。writer三入口載入同一Lua並驗既定tuple/day-HL/tick矩陣；同日較晚PREVIOUS_CLOSE不得覆寫VERIFIED_CLOSE或LIVE，較舊時間亦不得因狀態升級而覆寫較新值。incoming status missing/null/number/empty皆FAILED零mutation；existing同四種legacy rank0仍比較time，舊time拒絕、新time修復。producer矩陣逐列驗date/Instant；美股FinMind parser另驗row date不等target時DB/Redis零寫。Docker以唯一隔離key載入正式Lua做repair/stale/清理，不能進public quote index；runtime只接受當時phase成功outcome。
...
| `GET /internal/market-data/taiex-index/stream` | SSE | typed index events（Requirement116） | 401/403/503 |
| `POST /internal/trades/read` | startDate/endDate | typed TradeBatch（Requirement120） | 400/401/403/503 |
| `POST /internal/market-data/etf-holdings` | ETF snapshot request（Requirement123） | typed ETF holdings（ETF session既有契約） | 400/401/403/503 |
| `POST /internal/bank-balance/read` | 無 selector | queryDate/observedAt/HMAC fingerprint/TWD/nonnegative balance | 401/403/503 |
| `POST /internal/settlement/read` | 無 selector，固定3d | identity-validated dates/signed amounts/coverage status | 401/403/503 |
| `POST /internal/realized-gains/read` | 無 selector | identity-validated Stock/Sell rows/sourceDate | 401/403/503 |
...
上述decimal限制是既有portfolio/quote價格profile；本輪account金額欄位限定重用nonnegative或signed parser（precision20/scale10），technical獨立signed profile（precision38/scale18），不放寬原價格規則。所有normalized wire仍為canonical字串，DB money在算完/捨入後另驗各column precision。
...
另提供 network-internal `POST /internal/brokers/fubon/inventory-sync?dryRun=true|false`，以backend exact-path internal-token filter保護，預設`dryRun=true`。dry-run可查帳務adapter並reconcile，但永遠不開寫入transaction；calendar不是明確true時不得呼叫quote或回可提交估值。dry-run要估值及`dryRun=false` commit都要求 shared known calendar（TWSE primary → 完整 DGPA provisional → unknown，再 union operator closure）為true，且每筆quote actual Instant的台北日期精確等於portfolio captured queryDate；13:35盤後五分鐘可接受同一交易日actual trade，禁止前一交易日值。commit另走和scheduler相同owner/snapshot/today gate。此路徑不加BFF/frontend/Nginx/host route。Schedule catalog新增一筆，總數56=22+34。
...
enabled mode 的 Fubon transport/HTTP/auth/schema/partial failure 不得觸發 TWSE MIS、Yahoo或 scrape fallback；該 symbol 保留上一筆 cache，並以`FubonQuoteOutcomeCounters`固定enum/LongAdder與每輪structured summary記reason。disabled mode 完全不建立/呼叫 Fubon client，但 existing provider 必須使用 Task 350 `PriceFetchClient.fetchTwBatch(codes)` 與共用 writer outcome，禁止退回逐檔 `getStockPrice`。切換只涵蓋 `market='台股'` 的 LIVE quote；美股、英股、FX、ETF iNAV/premium、official close reconciliation、歷史、基本面、新聞皆不變。

Fubon只是**盤中**LIVE producer。現行四個台股入口是：(a)`PricePoller.scheduledTwIntradayUpdate`、(b)`PricePoller.warmCacheOnStartup`、(c)`POST /internal/refresh → PricePoller.refreshAll`、(d)`POST /internal/refresh/tw-radar → TwRadarRefreshService.refresh → PricePoller.updatePrices`。四者的台股live分支都先委派同一個例如`refreshTwLiveThroughSelectedProvider(codes)`的窄入口；`updatePrices`即使保留既有public visibility，其`market='台股'`分支也只能轉呼該入口，tw-radar不得直接碰`PriceFetchClient`／MIS。該入口不能用現行calendar失敗時fail-open的`isTwMarketOpen()`授權，而須新增tri-state `isTwMarketOpenKnown()`：Asia/Taipei today只交給`MarketCalendar.isTwTradingDayKnown(today)`判日期；該 known calendar 共用 TWSE primary → 完整 DGPA provisional → unknown，再 union operator closure；authority true且`09:00 <= localTime < 13:30`回true，非交易日／盤外回false，empty或exception回empty。四入口與enabled/disabled provider都只在known true發selected-provider HTTP並進writer；false/unknown時Fubon/MIS live HTTP、Redis SET/index/TTL/PUBLISH/tick皆0，即使current missing亦同。盤外DB/history與official reconcile走既有非live流程。Task352 inventory估值不寫Redis，可於13:35在同一 shared known trading-day gate 下使用同一交易日actual trade，兩用途不得混淆。

### 為何採 per-code intraday 而不是 snapshot

目前納管約 35 codes、每 2 分鐘一輪。官方 intraday/snapshot 限額 300 requests/min，因此逐檔 intraday 在現況遠低於上限。provider dispatcher 對正常四入口每輪先限 100 codes，Fubon adapter 也只接受 1..100；Task 350 existing MIS client 的 320 hard cap 是直接呼叫時的防禦層，不放寬排程輪上限。`intraday/quote/{symbol}` 的raw payload提供既有 quote schema 需要的 `previousClose/openPrice/highPrice/lowPrice`、bids/asks、`total.tradeVolume`、`lastTrade.time`、`isTrial`、`lastUpdated`。TSE/OTC snapshot 雖只需兩 call，卻缺 bid/ask/previousClose且範例未證明 trial flag，會令現有 19 欄契約系統性退化，故本需求不用 snapshot。SDK 的 `query_symbol_snapshot` 官方另明示沒有即時更新，也禁止使用。
...
Java `FubonTwLiveQuoteProvider`將sanitized DTO轉為immutable observation；`tradingDate/providerUpdatedAt`來自同一actual trade event，另帶只可由同輪`MarketClock.isTwMarketOpenKnown()==Optional.of(true)`產生的`marketOpenAuthorized`，再委派唯一`PriceCacheWriter` provider-timestamp overload；legacy boolean clock、authority empty/throw都不能產生true。Task 350 的一般 Lua 繼續是 `write`、`writeVerifiedClose`、`syncClosedFromDb` 的權威；Fubon scoped Lua 與它共用同一 Java payload builder，並同樣守日期優先、狀態不降級與同日時間單調。scoped script第一步驗該flag，false/missing即回`MARKET_CLOSED`，且此授權判斷位於任何Redis operation（包含`TYPE/GET`）之前；因此current missing也不能盤外寫。授權後、任何`GET/SET`前先完整preflight：固定2個KEYS／9個ARGV，latest type只能none/string、index type只能none/set，TTL為正整數，code/market/channel非空，incoming須為JSON object，且`stockCode/market/tradingDate/updatedAt`與ARGV一致、`source='FUBON_INTRADAY',quoteStatus='LIVE',closed=false`。任一失敗穩定回`WRITE_FAILED`且零mutation，避免SET成功後才在SADD遇WRONGTYPE。通過才在同一次server-side operation解析current的`stockCode/market/source/closed/quoteStatus/tradingDate/updatedAt`。current missing可寫；malformed/identity不符fail closed；同日先套`VERIFIED_CLOSE(3) > LIVE(2) > PREVIOUS_CLOSE(1) > unknown(0)`，incoming LIVE不得降低現有順位，且順位提高或相等都仍要求strictly newer tuple。故current Fubon VERIFIED_CLOSE即使incoming LIVE時間較新也拒絕；一般Fubon LIVE只有strictly newer才寫，older/equal不改value/index TTL、不publish。
...
公開 raw quote 的 `LatestQuote` 仍是 19 欄 JSON，Redis key、TTL與 `price-update` channel 不變。enabled/no-config、adapter timeout、trial、wrong-date、malformed、older/equal等失敗只保留上一筆 cache；若完全沒有 cache，consumer 依既有 missing 語意處理，不拿 MIS/Yahoo/昨收冒充。**Requirement 108／Task 372 的對外包裝層覆寫：**9090 同名 exact GET 在 raw 19 欄後增加 `marketData`，並因 `start`／`end` 驗證可回 400；它不變更 raw record、raw 值、raw one miss 204 或 raw list 空 `[]`。**Requirement 109／114 的限定替代：**巢狀 `marketData.quoteDetail` 只讀獨立、已持久化的 `FUBON_BOOKS` primary／`YAHOO_TW` fallback Redis/DB snapshot，與 generic quote key 完全分離，不參與任何估值。**Requirement 110／Task 375 的相容 direct projection：**同一 9090 row 在既有 `marketData` 後追加相同 immutable snapshot 的 `quoteDetail`、`bidLevels`、`askLevels` 與 `dividendHistory`；它不改 raw 19 欄、不新增 I/O，且五檔只能供 read-only batch 累計深度驗證。Task 352 同次估值必須直接從 normalized batch 的成功 actual-trade quote 建值，不以 Redis 舊值繞過全批 no-write。
...
## Requirement 118 Design — 9090 public commodity batch

### Boundary and request flow

The new public surface is exactly GET /api/public/commodity-prices. It is a loopback/Tailscale private-network, anonymous read contract; it is not a browser page route, a tenant selector, or a vendor refresh endpoint. The complete live 9090 manifest becomes thirteen exact path/method pairs: twelve GET operations and the existing one POST crawler rescan. api-gateway Nginx owns an exact location only for this GET and proxies it to bff:8080. frontend continues to deny the same path. Tailscale Serve mounts the exact path to the same 127.0.0.1:9090 target; it must not create a root proxy, Funnel, wildcard, alternate listener, or a path for any business route.

The BFF inbound controller is a small publiccommodity component and delegates all work to PublicCommodityPriceService. It uses the existing qualified publicMarketDataBusinessClient, whose clone has no tenantHeaderFilter and therefore cannot carry X-User-* headers or Reactor caller identity. It makes one exact downstream GET to /api/market-data/commodity/live. No business, external-materials, Redis, PostgreSQL, Yahoo, broker, scheduler, refresh, export, cache-writer, or filesystem client is injected into this public BFF service. The existing business CommodityLiveQuoteService remains the only aggregation implementation: it reads Redis commodity spots and the PostgreSQL previous close under a read-only transaction. The BFF does not use the authenticated CommodityPriceBffController live method because that endpoint intentionally collapses upstream failure to an empty result for a page; the public contract must preserve upstream availability as typed 502/503/504.

### Public immutable DTO and mapping

The response is a Java record CommodityPriceBatchResponse(boolean marketOpen, CommodityPriceSlots quotes). CommodityPriceSlots is an immutable record with exactly three Jackson properties in this order: WTI, BRENT, GOLD. Every slot is required in JSON and nullable; the mapper always emits all three. Each non-null slot is an immutable CommodityPriceQuote with the fields commodityCode, unit, price, change, changePercent, sessionDate, quoteTime, polledAt, status, dayHigh, dayLow, and provider.

The mapper accepts only the fixed three keys from the downstream response and requires marketOpen to be a JSON boolean. A downstream null slot stays null; it is not synthesized, omitted, zero-filled, or converted into an HTTP error. A non-null downstream quote must have its matching fixed code, positive finite decimal price, nonblank provider, non-null ISO date/RFC3339 instants, and status strictly in LIVE, STALE, SETTLED before it can become a public DTO. WTI and BRENT receive unit USD_PER_BARREL; GOLD receives USD_PER_TROY_OUNCE. Change and changePercent are either both null or both finite numeric values. dayHigh and dayLow are independently null or positive finite decimals. This BFF validation is stricter than the persisted Redis adapter, whose nonblank/numeric checks do not authorize an unknown status or a zero/negative high/low to become public data. Any response that lacks a fixed key, has an unexpected code/key, has invalid required values, invalid enum/paired nullable/number invariants, or otherwise cannot be safely mapped is a BFF bad gateway failure rather than a partially invented payload.

marketOpen is copied from the business result and means only that the existing commodity producer session key is present. It does not assert freshness or availability for every code. Individual null quote slots mean the producer did not leave a valid persisted spot cache entry; they are a valid 200 response even when marketOpen is false. No value is calculated from a historical row merely to avoid null.

### Error, network, and contract rules

The service sets a bounded downstream timeout. A business non-2xx status, empty body, decode error, or invariant failure maps to sanitized 502; transport or connection failure maps to 503; timeout maps to 504. It returns no upstream URL, HTML, exception text, headers, raw payload, user data, account data, broker data, or token. The controller accepts no parameters and has no access to caller identity. SecurityConfig permits only this exact GET; all other methods remain protected at BFF and Nginx returns 405 for a known exact route with Allow: GET. Nginx and frontend exact matching make slash, child, and matrix variants 404.

### Request gate and deterministic problems

The controller receives the complete query MultiValueMap before calling the service. A normal request has no named query parameter; a bare trailing question mark is equivalent to no query. Named, empty-value, repeated, or multi-value parameters are rejected locally with 400, and no downstream subscription may be created. The same local gate rejects a GET that signals a body through positive Content-Length or any Transfer-Encoding; it does not consume or proxy that body. This makes the direct BFF route and the 9090/Tailscale route share identical behavior.

The downstream timeout is exactly five seconds. The scoped exception advice writes only application/problem+json ProblemDetail responses with type about:blank, instance /api/public/commodity-prices, and no extension member. Its fixed title/detail pairs are: 400 Invalid commodity price request／不支援 query parameter 或 request body; 502 Commodity prices downstream failure／商品報價暫時無法取得; 503 Commodity prices service unavailable／商品報價服務暫時無法連線; and 504 Commodity prices timeout／商品報價服務逾時. These describe BFF-generated failures. If Nginx itself cannot connect to BFF or times out, its existing text/html 502/504 remains a separately documented gateway response class and must not be represented as the BFF ProblemDetail.
...
日期／來源流程固定如下：HTTP先驗嚴格ISO有效日與既有<=7天界限 → gateway轉 `YYYYMMDD` → accounting lock中選定account與token → 只存取stock.filled_history並在共用budget/5秒timeout下呼叫 → auth-invalid最多一次完整重登入重試 → 把response、同次selected account及token保存為Python內部不可變結果。raw身份核對與HMAC只能使用此結果，不能呼叫結束後重新讀可變session狀態。原帳號和token不離開adapter；timeout/retry沿既有生命週期限制，不新增全域counter鍵。
...
class TradeReadRequest(BaseModel):
    ...
    def valid_iso_date(cls, value: str) -> str:
        ...
        date.fromisoformat(value)   # rejects invalid calendar dates → sanitized 400
        return value
```
...
**已核實官方契約。** [Python FilledHistory](https://www.fbs.com.tw/TradeAPI/docs/trading/library/python/trade/FilledHistory/) 明列身份、stock_no/buy_sell、成交價格數量與filled_no；SDK請求為YYYYMMDD、raw date範例為YYYY/MM/DD，後者須嚴格解析再輸出ISO。官方v2.1.1起單次最大30日，不改本系統既有差值<=7天限制。文件範例有請求／回應日期不一致的示意，不能因此放寬實際range validation。官方shape fixture可測完整正常路徑，不能冒稱已驗真人SDK。
...
3. raw `date` 嚴格有效 `YYYY/MM/DD`、round-trip相等，解析後落在 `[start_date, end_date]`（inclusive），normalized `filledDate` 為ISO。
...
**2026-08-30 節拍補正驗收：** `FubonTradeSyncScheduler` 兩個註冊與 JOBS 必同時更新；CronExpression 測有效交易日09:00/09:30/10:00/10:30/11:00/11:30/12:00/12:30/13:00/13:30/14:00共11次，排除08:30及14:30。沿用原feature/config/known-calendar/capacity gates，不再建立原Task385已落地的schema；另以fake SDK驗YYYYMMDD request、YYYY/MM/DD response→ISO DTO、錯格式/leap day/範圍邊界與同次selected account/token一致。既有手動ledger CRUD不改，不能當不可變供應商證據。
...
| 參數 | 型別與比對 | 無效值 |
|---|---|---|
| `connected` | 可省略；精確小寫 `true`／`false` | 空字串、數字、大小寫變體皆 400 |
...
- `el-table-column type="expand"`（比照既有 `TradingRadarView.vue` 展開列模式），展開內容顯示 `sdkReference`、`httpEndpoint`（有非空值即顯示，與 connected 無關）、完整 `requestSummary`／`responseSummary`。
...
[官方銀行](https://www.fbs.com.tw/TradeAPI/docs/trading/library/python/accountManagement/Balance/)、[交割款](https://www.fbs.com.tw/TradeAPI/docs/trading/library/python/accountManagement/QuerySettlement.txt)、[已實現損益](https://www.fbs.com.tw/TradeAPI/docs/trading/library/python/accountManagement/RealizedPnLDetail/) 皆為 Result success/data 信封。Python 同一accounting臨界區捕捉 selected account、response、token；必要raw branch/account非空且精確相同後，才用當次token HMAC-SHA256(branch:account)前24hex做fingerprint。缺欄不能以None==None或selected fallback通過。raw identity/token/原始message不可跨adapter或進log/DB。queryDate/observedAt是本地觀測，不得冒充來源date/revision；跨台北午夜拒绝。Java immutable records複驗日期/時間/fingerprint，使用各欄位適用的decimal parser。

### 銀行／交割的 transaction 邊界

```text
scheduler/manual → feature/config/owner preflight（NOT_SUPPORTED）
                 ...
                 → 獨立 writer bean（REQUIRES_NEW）
                    ...
                    → verify column precision → saveAndFlush → commit
                 → commit成功才計SUCCESS
```

不得在同service self-invoke 假裝切開 transaction、不得先查app_user/broker/bank/children、不得改成第一個snapshot-related DB action。無snapshot不新增、不改snapshotDate。commit時queryDate仍當日、observedAt不未來且不超過60秒；所有失敗保留舊child和aggregate。同銀行＋depositType有0列才insert、1列才update、>1列整批AMBIGUOUS_TARGET；新增必加入snapshot.getDeposits並設snapshot，同一managed集合供calculator讀，不能只save子列。其他銀行/存款/資產不動。
...
external-materials-service的FUBON_DIVIDEND_SYNC_ENABLED/config/known trading-day gate，Asia/Taipei `0 0 9 * * MON-FRI`及`0 30 13 * * MON-FRI`。沿用StockSourceQuery.collectTwRadarCodes：各owner最新台股holding∪台股stock_alert去0000；不改原17:00 DividendPersister全市場範圍/warmup。每輪radar empty則零外呼。

[官方dividends](https://www.fbs.com.tw/TradeAPI/docs/market-data/http-api/corporate-actions/dividends.txt) 只能收start_date/end_date，不能传symbol。external一次POST `/internal/market-data/dividends/read`傳本輪symbols/from/to，window固定queryDate−320天至+45天（<=366天），adapter只對官方送日期，一次市場batch後先嚴格取radar交集再輸出/存入。這個provider限制已明示，不假稱上游逐檔查詢，不逐檔重複全市場請求。

mapping驗symbol/exchange/date/dividendType；cashDividend才是現金，dividend參考價調整量不用。stockDividendShares為每千股配股數，只有核實既有stockDividend單位相容才轉，不假設全股票面額10。未核實股票部分null/具名STOCK_DIVIDEND_UNIT_UNVERIFIED，混合event可保留可信cash作PARTIAL，純無可映射金額不造event。含息/含權分別映射exDividendDate/exRightsDate，不互fallback；年取來源事件anchorDate年，發放日未知null，未來價格null不妨礙正股利。money按snapshot_event numeric(18,6)捨入後驗證，無法識別的同檔矛盾event整檔FAILED。

external只呼叫DividendSnapshotStore.record append既有snapshot/event/observation；provider=FUBON_SDK、sourceAvailableAt未知null、observedAt另記、scope實查window。固定PARTIAL/complete=false，空匹配也不是EMPTY_COMPLETE，source failure是FAILED。scopeTo+45只表達請求範圍，沒有future完整性保證；content_hash/event_key共用，不加新writer/投影。backend既有讀取觸發DividendCurrentStateProjectionService，既有CASE中FUBON_SDK屬ELSE50，官方權威/取消規則不改。不做capital_changes/減資。

external exact token POST `/internal/dividend/fubon-sync?dryRun=true|false`預設true、dryRun連attempt都不寫，controller委派service，回計數/outcome；API inventory httpEndpoint為adapter `/internal/market-data/dividends/read`，consumer補Java入口。以真DB驗重跑snapshot/event不重複、observation可追加及PARTIAL不能取消已知future event。

## Requirement 132／Task 397：可信實際成交與完整 quote 原子更新

此路徑只更新既有latest quote，不是book canonical的新來源。`price:{market}:{code}`已有完整JSON metadata，consumer不得僅局部改price/time。選官方Normal-mode **aggregates** 專屬stock WebSocket reference；依官方多連線流程在session lock內init_realtime後保留個股client，不覆寫TaiexIndexStream持有的reference。最多300symbols、額外1條連線，有界queue/backoff/stop；indices flag關閉仍能啟動股票，雙方reconnect/退訂互不干擾，auth generation變更重新完整subscribe。

external FUBON_STOCK_PUSH_ENABLED/config/MarketClock.isTwMarketOpenKnown明確true後，每30秒collectTwRadarCodes及renew desired symbols；Python POST `/internal/market-data/stock-push/subscriptions`只增量差集SDKsubscribe/unsubscribe aggregates/intradayOddLot=false，120秒lease逾期釋放；GET `/internal/market-data/stock-push/stream`為token-protected SSE。盤外/unknown取消，13:30後收盤由原官方close路徑負責。

來源核實依[官方aggregates](https://www.fbs.com.tw/TradeAPI/docs/market-data/websocket-api/market-data-channels/aggregates.txt)與[多連線](https://www.fbs.com.tw/TradeAPI/docs/market-data/making-connection.txt)。event=data/channel=aggregates，正確radar symbol、TWSE/TPEx與核實type；isTrial=true丟棄，缺該optional boolean不能單獨判壞，但lastTrade必需正price、正size、exact epoch microseconds。官方微秒例子與來源date對照測試固定unit，不能位數猜單位。sourceDate=來源日=當日，timestamp不未來且位於授權盤中，不能拿lastUpdated/receivedAt/lastTrial作成交時間。lastPrice包含試撮不可用；closePrice/time若有須與lastTrade一致。

SSE輸出FUBON_WS_AGGREGATES、sourceDate、tradeTimeMicros與同packet可信optional previousClose/OHLC/name；缺值null、不拼舊Redis/REST，volume單位未核實null，buy/sell quote留null，不傳完整五檔。external再驗radar及完整日期/價格，用同包昨收和成交算change（無昨收即null），建立PriceResult=LIVE/closed=false，不冒稱FUBON_INTRADAY或VERIFIED_CLOSE。

PriceCacheWriter窄入口委派既有writeTaiwanLive(result,true)/executeLatestWrite與原price-cache-monotonic-write.lua；整份JSON/date/time/status一次更新，不合併localHighLow假裝來源OHLC。same/older event拒絕、verified close不降級，只有WRITTEN可發布及沿用可信tick。不得直接HSET/JSON.SET、另複製Lua、写quoteDetail/bidLevels/askLevels、fubon_tw_live_quote_response或呼叫book PostgreSQL comparator。真Redis必驗metadata一致与時序保護，不只斷言price。

## Requirement 133／Task 398：既有 Redis 的來源技術快取與純讀介面

原「使用者另准新stock_technical_indicator表」明確作廢，本輪無SQL migration/column/index。已盤點TechnicalIndicatorService為history-based現算、無generic SQL/cache sink；既有alert KD歷史、行情quote及owner radar snapshot皆非合法承接字段。選擇在**既有Redis資料庫**新增獨立市場cache key，不挪用他者欄位。此為TTL可重建資料，不是SQL永續保存，也不改本地算法/雷達評分/API。

external唯一scheduler/service/writer/reader，FUBON_TECHNICAL_INDICATOR_SYNC_ENABLED/config/known trading-day gate，預設Asia/Taipei `0 40 13 * * MON-FRI`；scheduler/manual/dry-run都重驗台北時間>=13:40，較早回BEFORE_CLOSE且零SDK/Redis寫入。每檔外呼與提交前重驗同台北日，跨午夜停止，避免盤中值先占當日no-revision fence。只collectTwRadarCodes；controller不寫業務邏輯，無backend entity/JPA或external→business callback。每stock固定查queryDate−120天到queryDate，D日線KDJ(9,3,3)/MACD(12,26,9)/BB20，SDK三方法完整參數、有界timeout，共用保守60/min市場資料budget、429停止密集外呼，單輪<=30分鐘，未處理明示PARTIAL。

adapter POST `/internal/market-data/technical-indicators/read`傳symbol/from/to，官方三response都要echo正確identity/from/to/timeframe/params；BB period只接受exact integer或canonical integer字串。data每列嚴格date在window且不未來，最大來源日才候選，重複/矛盾日期整組SCHEMA_INVALID。KDJ k/d/j、MACD macdLine/signalLine、BB upper/middle/lower不能混名或造histogram；每組required值全有限才AVAILABLE。technical signed Decimal profile precision38/scale18允許negative/zero，BB upper>=middle>=lower，J/MACD不clamp，SDK finite float只保留其可提供precision。各組有sourceDate，沒sourceTimestamp就null；observedAt只作本地觀測，絕不把today/fetchedAt改名來源日。

### Redis schema 與原子更新

`fubon:technical:tw:{code}:D:v1`保存schemaVersion/symbol/台股/FUBON_SDK/固定parameters manifest和kdj/macd/bb records。每組包含status、sourceDate、nullable sourceTimestamp、parameters、完整payload、contentHash、首次observedAt、expiresAt；lastAttemptStatus/reason/observedAt独立保存，不能冒充新來源值。code與manifest嚴格校验，參數改版需獨立cache格式/身分，不在同key混算法。

external writer用專用小Lua/等價Redis原子transaction逐組compare-and-write，不改price/book Lua：无舊值合法可新增；sourceDate較新整組替换、較舊拒絕；同日同hash冪等、同日不同hash沒有來源revision則CONFLICT_NO_SOURCE_REVISION且保留舊組，不能用較晚fetchedAt決定新舊。單組失敗只更新attempt，不抹其他組、不將舊組標今日，不拼新K舊D。

有效組expiresAt固定sourceDate台北00:00+7天，已過期限不可新增；key TTL為尚有效組最晚期限（<=7天），由Redis server time計剩餘時間，純failure診斷<=15分鐘。等值重跑/失敗不延長既有組期限；reader各組分別判expiresAt，因此其他組延長key存活也不延長舊值。Redis eviction/restart是MISS、不可當0/補SQL；corrupt cache明示拒絕，不混不可信值。

### 讀回與驗證

external `FubonTechnicalIndicatorCacheRepository`只讀Redis、不呼SDK、不刷新TTL；token-protected GET `/internal/technical-indicators/fubon-cache?symbol=...`確定仍為radar代碼，回provider/params、各組date/value/status/lastAttempt/expiresAt。較最近應有交易日舊的sourceDate明示HISTORICAL，calendar unknown明示，不以系統today覆寫sourceDate。沒有任何既有UI/雷達consumer自動切換成富邦算法。

POST `/internal/technical-indicators/fubon-sync?dryRun=true|false`在external、exact token/預設true，dry-run零Redis/attempt寫入；回计數与SUCCESS/PARTIAL/FAILED/BEFORE_CLOSE；manual無收盤gate例外，全部不可用不得SUCCESS。API inventory三技術項目httpEndpoint统一adapter read，consumer記sync/readback。真Redis與fakeSDK必驗三組正常讀回、sourceDate獨立、失敗保留/標記、同日異值拒絕、新日替換、併發原子性、TTL與MISS、radar/Token邊界，不能只測disabled。
...
`ports.py`（或等價內層模組）只定義目前用得到的 Protocol：PortfolioReader、TradeReader、BankBalanceReader、SettlementReader、RealizedGainReader、QuoteReader、EtfHoldingsReader、DividendReader、TechnicalIndicatorReader，另設各自的 IndexStreamGateway／StockPushGateway 及僅供 readiness 的 RuntimeState。consumer 只持有自己的能力；不能建立一支涵蓋所有 SDK 方法的巨大介面。現有 `SdkGateway` 實作這些介面；只有 app 組裝 concrete gateway，業務模組不能 import concrete gateway、反射取得其私有 helper 或直接 import proprietary SDK。
...
Python indices與stock、Java indices與stock各自使用run物件保存generation、不可重設的cancel、worker、pending-open、connection/body、latest及subscription狀態。shared reference只指出current run。start／stop交換與ingestion admission有明確線性化點；stop失效當下的run、關自己的body/connection，不清除別人的run。舊callback／late-open／finally只能清自己的資源，不能再subscribe、開始ingest、清空新latest或把新run標為停止。

Python symbol驗證與connect不在ASGI event loop同步執行，同一run的啟動single-flight；connect5秒與cleanup2秒由共享bounded SDK executor處理。未結束thread仍記入容量且不可clear stop event復活，late connect不得subscribe。stock另保留scope/ACK generation、queue64、ACK5秒、最多300symbols、lease120秒、30秒reconciliation，index與stock不互退訂。Java stock subscription manager的shutdown是terminal；排隊refresh拿鎖後必再驗，shutdown後零subscriptions/start。一般clear空scope不等於shutdown。

Java SSE共用的只有純機械bounded UTF-8 reader：每line／frame各16KiB，含delimiter／comment／未知欄位，累積前限制；超限／非法UTF-8立即關自己的body，不呼ingestion。connect5秒、headers10秒，成功stream不設整段lifetime timeout；已cancel的open若晚完成仍須關閉其body。完整frame以既有event／id／JSON／來源微秒規則驗證才可dispatch。維持250/500/1000/2000/5000ms bounded reconnect及消毒錯誤，不新增broker或public入口。

stop前已admit的有限native或DB交易仍依既有完成／rollback規則處理，不能宣稱interrupt可強殺它；stop與new ingest admission同鎖或等效機制使stop完成後不會開始舊run的ingest。已開始的舊completion不得修改新run狀態。

### Task402：external 窄 ports 與中立值

以下 port 放在 `service/port/`、immutable value 放 `model/marketdata/`（可採等價內層路徑）；現有HTTP/JDBC/Redis adapter直接實作，不另疊無功能wrapper。表中的方法是必要能力上限，不整包複製大型client。

| port | 必要能力 | 現有外層實作 |
|---|---|---|
| FubonNormalizedQuotePort | fetch normalized batch | FubonNormalizedQuoteClient |
| LivePriceFetchPort | 台股batch、Yahoo台股actual fallback、既有single quote | PriceFetchClient |
...
| PriceProjectionPort | 既有write兩overload、writeTaiwanLive、writeTaiwanIndexLive、writeFubonStockPush、syncClosedFromDb | PriceCacheWriter |
...
| OrderBookRepository／OrderBookCachePort | persist/findRevision/findCanonical；find/writeStrictNewer | IntradayOrderBookSnapshotStore／QuoteDetailCache |
| YahooOrderBookSourcePort | probeTw、probeTwo | TwQuoteDetailFetchClient |
| FubonResponseRepository／FubonResponseCachePort | persist/find；find/writeStrictNewer | FubonLiveResponseStore／FubonLiveResponseCache |
| DividendEvidencePort | append record | DividendSnapshotStore |
| TaiwanHolidaySourcePort | getTwHolidays、getTwHolidaysKnown、peekTwHolidaysKnown | MarketDataFetchService |

PriceProjection各named entry維持其源別／副作用差別，不合成通用寫入器。deprecated FubonTwLiveQuoteProvider／ExistingTwLiveQuoteProvider不得重新接入production；必要相容僅限搬型別或獨立legacy fixture port，不污染正式port。

搬出PriceResult／TwQuoteBatchSummary、QuoteDetailResult／OrderBookLevel／YahooProbeOutcome、Fubon batch/envelope、StockSource的close/intraday結果、Taiex candidate/canonical/result、book/response store/cache結果、DividendEvent／DividendFetchResult／FetchStatus／PersistResult、DayQuote與writer outcome，保留原field shape／constructor與nullable。FubonMarketData不能再import DividendFetchClient內的record；port不得回傳concrete adapter nested DTO。supportedIdentity、revision驗證、book-validity等純函式放中立contract，不能讓core回呼concrete adapter static。外層其他market caller僅改型別，不改非富邦演算法或來源。

FubonTechnicalCache 既有 core value／port 目前仍反依賴 client.FubonMarketJson；本輪保留既有 FubonMarketDataPort／FubonTechnicalCachePort，將 Attempt／Group／Document／Read／Write 與 manifest 留中立層。JSON encode/decode、canonical-hash、persisted-format驗證改由 outer Redis codec 或真正中立的純 codec 承擔，不為純 codec 造介面。v1 Redis bytes、hash、逐組 CAS、7日到期及failure TTL完整相容，不更改既有資料。

### Task402：commit、有限 response 與 index repair

normalized quote production HTTP transport在subscription/read累積前限制2MiB，包含chunked/缺Content-Length；超限即cancel，禁止先ofString完成再算bytes。30秒總期限涵蓋headers與body，保留connect5秒、無redirect以及既有HTTP failure分類。strict envelope未成立零Fubon observation/DB/cache；沿現有radar範圍及transport-vs-DB-failure的不同fallback行為，不擴大來源。

可投影的store result須確定來自獨立已commit DB transaction。response store／book store／intraday quote writer採真正獨立transaction或有效NOT_SUPPORTED orchestrator加writer的邊界，不能因port易於組合就把Redis包入ambient transaction。每批原子性、source-aware comparator、canonical revision與Lua保持同一實作；commit失敗零Redis，raw候選不代替canonical。GETreader只讀cache candidate並查DBrevision，不repair、不外呼。

index採單一O(1) `ProjectionState(highestCommittedCanonical, attemptGeneration, needsRepair)`：DB APPLIED後先登記最高committed watermark及attempt，再做previous-close／Redis投影；任何DB failure不登記。completion只可CAS自身watermark/generation，成功後保留watermark而清needsRepair，防止舊失敗復活；較新canonical取代舊pending。previous-close或Redis例外保留needsRepair。exact-equal raw只有store返回同timestamp committed canonical且目前state需repair才可重投影，資料取canonical；older/raw contradiction不作來源。重啟無pending時equal不觸發repair，原兩分鐘poller純DB今日current修復不變。

### Task403：book identity 與 bridge 日期

BFF對每個raw row採納完整book前，驗exact stockCode與market，且原source/完整五層/排序/來源時間等條件全成立；錯配時nested及direct quoteDetail均用該raw row的safe unavailable、source=null、both sides空。不得只把book的code改成raw code或混源補值。

只有 external `FubonLiveResponseReadService.Quote.tradingDate` 與 BFF `FubonBridgeQuote.tradingDate` 改成LocalDate；對raw String使用嚴格ISO LocalDate parse後比較，失敗不覆蓋raw quote。投影回RawLatestQuote仍用ISO字串。raw 19欄、response_row opaque JSON、premiumDiscountPct與單份quote欄位不變，不順便遷移其他歷史DTO的日期或增加public schema。

### Task404：business internal HTTP 與富邦 local IO 邊界

MarketDataService只依InternalMarketDataPort及原repository。外層 `integration/marketdata/ExternalMaterialsMarketDataClient`負責固定internal base URL／URI／WebClient／codec；無URL字串constructor在core偷建adapter。共享五個record移至`model/marketdata/MarketDataResults`：DividendRateResult、EtfHolding、EtfHoldingsResult、DividendRow、DividendHistoryResult，保留field名／順序／nullable／日期String及compat constructor；PriceResult不遷移。所有public service方法名稱與參數維持，同module caller同步import，不承諾舊nested FQCN二進位相容。

| InternalMarketDataPort 方法 | 既有 internal HTTP | 總 deadline |
|---|---|---|
| quoteDetail(code,market) | GET /internal/quote-detail | 2秒 |
| dividendRate(code,market) | GET /internal/dividend-rate | 20秒（原無界） |
| etfHoldings(code,market) | GET /internal/etf-holdings | 20秒（原無界） |
| dividendHistory(code,market,years) | GET /internal/dividend-history | 20秒（原無界） |
| twHolidays(year) | GET /internal/tw-holidays | 20秒（原無界） |
| detectTwClosureToday() | 既有 POST /internal/tw-closure/detect | 20秒 |

最後一項有既有偵測副作用，不稱此port全為pure-read；不新增入口或重試，runtime驗收不觸發它。service保留同code/market完整book驗證、殖利率1h cache與stock-name fallback、當年holiday10m cache／失敗不毒化／非當年每次proxy、NYSE/LSE純計算。台股ETF所有success/failure/empty都只讀本地snapshot與純parser、零portcall；美股／其他既有來源不變。

另以secretless `FubonReadinessPort.readiness()` 回中立state/reason，既有FubonConfigState做lazy Files IO；只有outer HTTPclient/filter可使用含token的snapshot，core service/scheduler/ownership不持secret。`FubonCalendarPort.isTwTradingDayKnown(LocalDate)`保留Optional true/false/empty，外層delegator使用原MarketDataService；ETF純分類抽中立helper，不用新增IOport。StockMaster只分出真正使用的local-only name lookup與upsert能力，外層原service實作，保持庫存upsert在成功commit後才回補；不得把會vendor-call的resolveName接進成交。owner directory／固定email政策另由財務owner單元完成，不能用neutral DTO掩蓋Task399鎖後fresh-read。

FubonEtfHoldingsSyncService 的單純保存依 `FubonEtfHoldingsWritePort.save(FubonEtfHoldingsSnapshot)`，原 FubonEtfHoldingsWriter 直接實作並保持 REQUIRES_NEW／committed-on-return。庫存／銀行 writer 的實際 row-lock 依 `FubonSnapshotLockPort.lockLatestForFubonConfiguredOwner(Long)`，由原 AssetSnapshotMutationLock 實作，保留 MANDATORY、第一個 DB operation 與 Optional<AssetSnapshot>結果；不改其他 snapshot caller 的 lock 方法。純應用流程、計數器或計算不為形式再抽介面。
...
`FUBON_SYNC_OWNER_EMAIL` 是唯一新增設定，對應 `fubon.sync-owner-email`。部署值是本輪使用者指定的 email；Java不硬編私人值，文件範例用中性地址，不能將值放進log、錯誤、API目錄或報表DTO。正規化為 trim + lowercase(Locale.ROOT)，空／非法則 `SYNC_OWNER_NOT_CONFIGURED`。本地授權選擇不宣稱是券商官方的Google帳號對照。
...
兩支原adapter/read route及SDK方法不變，query_settlement固定`3d`。normalized reply最多2MiB、每批最多10,000行，在讀取中限制，不能先收完整字串才檢查；新report contract保留原嚴格raw帳號/日期/數值驗證，將「來源觀測有效」與「可投影到既有財務模型」分開。即使settlement coverageStatus仍UNVERIFIED／MISSING_SETTLEMENT_RANGE_CONTRACT，合法來源可存report；不得改成偽造VERIFIED。上述兩欄不進report DB，GET scope明訂SOURCE_REPORT_ONLY。

settlement每列sourceQueryDate為官方date，≤本地queryDate；有資料時settlementDate≥sourceQueryDate、currency=TWD，12個原始金額欄位都是exact integer，buySettlement≤0、sellSettlement≥0，兩者之和等於來源totalSettlementAmount。只存必要來源date/currency/兩方向金額；其他毛額／費稅／合計只驗形狀，不另存或拿來猜別種財務數據。所有optional欄位全None保留為NO_DATA_OBSERVED，sourceQueryDate仍必有；partial None失敗。當日非零、合法過去／未來日、同交割日多列與完全同值行都保留；不依未知銀行入帳時間丟棄，不合併或去重。空list只代表這次回應空，不表示沒有其他未交割。

realized每列sourceDate≤queryDate、股票代碼合法且非0000；Buy/Sell和五種官方orderType完整保留，不建立交易物件。qty為1..9,999,999,999，price正值canonical decimal precision≤20/scale≤10，profit/loss為非負exact integer且不可同時正值；只將互斥盈虧合成一份 `providerNetPnl=profit-loss` 保存，不重存三份同一事實。sourceDate不是tradeDate、rowIndex不是fillId，量價與日期相同仍可有多筆，完整保留multiset。currency來源未知在GET為null，報表不提供proceeds/investmentCost，也無報酬率或總資產公式。

queryDate/observedAt只是本地觀測。Task406的observedAt必須在取得SDK response的同一accounting capture臨界區固定為不可變UTC微秒時點；normalization、HTTP與writer不得重取完成時間替代。入口queryDate、capture與完成時間皆須同台北日，順序有序且不能跨午夜；鎖後today同日、observedAt≤now且age≤60秒。新報表observedAt必須 `Instant.getNano()%1000==0`，與Python微秒及PostgreSQL timestamptz無損相容；不默默round/truncate來改變sameAt比較。Java source日期用LocalDate、觀測用Instant、價格和金額用BigDecimal；傳輸JSON的decimal以stripTrailingZeros後toPlainString或等價serializer輸出字串，0固定`"0"`，不能用ToStringSerializer產生1E+3或將核心DTO降為String。

### Task406：三張最小 typed 表（規劃新增）

預定 changeset `v1.121.0-fubon-accounting-source-reports.sql`；已掃其他worktree到1.120.0，但建檔前仍須重新查版號是否碰撞。表名、欄位、constraint如下；不改原五類財務表、不新增原始capture／輪詢歷史副本。JPA/DAO entity只由business持有，BFF不能讀DB。

| 表 | 欄位與限制 |
|---|---|
| fubon_accounting_report | `id bigint GENERATED ... AS IDENTITY PRIMARY KEY`；`owner_user_id bigint NOT NULL REFERENCES app_user(id)`；`report_kind varchar(12) NOT NULL CHECK IN ('SETTLEMENT','REALIZED')`；`observed_at timestamptz NOT NULL`；`UNIQUE(owner_user_id,report_kind)` |
| fubon_settlement_report_row | `report_id bigint NOT NULL REFERENCES fubon_accounting_report(id) ON DELETE CASCADE`；`row_index int NOT NULL CHECK 1..10000`；`PRIMARY KEY(report_id,row_index)`；`source_query_date date NOT NULL`；`settlement_date date NULL`；`currency varchar(10) NULL`；`buy_settlement numeric(20,0) NULL`；`sell_settlement numeric(20,0) NULL` |
| fubon_realized_report_row | `report_id bigint NOT NULL`，同header FK/cascade；`row_index int NOT NULL CHECK 1..10000`，同複合PK；`source_date date NOT NULL`；`stock_code varchar(10) NOT NULL`；`buy_sell varchar(4) NOT NULL CHECK IN ('Buy','Sell')`；`order_type varchar(10) NOT NULL CHECK IN ('Stock','Margin','Short','DayTrade','SBL')`；`filled_qty bigint NOT NULL CHECK 1..9999999999`；`filled_price numeric(30,10) NOT NULL CHECK >0`；`provider_net_pnl numeric(20,0) NOT NULL` |

settlement row另有互斥CHECK：settlement_date/currency/buy/sell四欄全NULL，或四欄全非NULL且currency='TWD'、settlement_date≥source_query_date、buy≤0、sell≥0。writer在任何SQL前對整批重驗numeric與欄位相依，不讓PostgreSQL默默round；price即使DB型別有較寬整數容量，也不放寬來源precision≤20/scale≤10。兩種child只能寫入相符kind的header，writer以typed method固定kind，不接受caller自選kind。所有read/delete/lock都從owner/kind限定的header取得id，不讓任意reportId越權。

不存owner email/account/fingerprint、queryDate、rowCount或各類sum。queryDate由observedAt以Asia/Taipei衍生，rowCount由實際child算；settlement status與totalSettlementAmount由已存欄位在read service衍生，None不是零。child不重存owner，不加ledger/deposit/snapshot/fill FK。兩種report各自為最新成功來源觀測，不構成長期歷史交易帳。

### Task406：transaction、排序與晚完成

```text
既有scheduler/manual -> 原feature/global/config gate -> dedicated-owner preflight
 -> broker HTTP（NOT_SUPPORTED，同批explicit binding） -> 完整來源驗證
 -> dryRun=true：DRY_RUN，零writer／寫鎖
 -> saveSettlement 或 saveRealized 的獨立Spring writer（REQUIRES_NEW timeout30s）
    -> target app_user FOR UPDATE + fresh owner驗證
    -> owner/kind report header FOR UPDATE（不存在則在同owner鎖下插入）
    -> 時間／不可變batch／all numeric重驗
    -> 比較observedAt與canonical sorted multiset
    -> 必要時replace該report全部child與header -> flush -> commit
 -> commit後回報source-report結果；不呼snapshot/gain writer
```

每次只保存一種報表；兩個job互不造同次來源承諾，各有自己的observedAt。canonical排序用schema順序的來源欄位：日期自然順序、文字按固定順序、qty/decimal按數值、null固定在前，decimal忽略可等值的尾零；不含rowIndex或local名稱，保留重複數量。排序後rowIndex連續1..N，只供本份報表定位。

| incoming vs existing | 唯一允許結果 |
|---|---|
| 無header | 寫header與完整rows，SAVED |
| incoming observedAt較舊 | STALE_OBSERVATION，零變動 |
| 同時且canonical multiset不同 | OBSERVATION_CONFLICT，零變動 |
| 同時且完全相同 | NO_CHANGE，零變動 |
| 較新且canonical multiset相同 | 只更新observedAt，SAVED |
| 較新且不同 | 同transaction刪該report children、插入完整新rows、更新observedAt，SAVED |

空成功來源可替換為零child，只影響自己這份報表。不同scope／來源失敗／時間或decimal不合法不建立header，原報表完整保留；insert第N列失敗、flush失敗、真正commit失敗都rollback。序列化靠目標app_user/header行鎖與unique，不能靠process single-flight或fake。REQUIRES_NEW需要真正Spring proxy；外層ambient rollback不改已獨立commit的report，回傳SAVED前commit必已完成。

### Task406：既有manual結果的加法擴充

兩個既有manual endpoint/token/dryRun規則保持，並保留所有原欄位及原outcome enum值。另各加enum值 `SOURCE_REPORT_SAVED`、`SOURCE_REPORT_UNCHANGED`、`SOURCE_REPORT_REJECTED`、`ROLLED_BACK`；report writer中立結果映射如下。所有回應追加 `reportStatus`（typed enum）、`reportRowCount`（int）、`reportObservedAt`（nullable Instant），不暴露行明細、fingerprint或owner。

| 場景 | outcome / reportStatus / reportRowCount / reportObservedAt |
|---|---|
| 原gate/來源不合法 | 既有安全outcome / NOT_SAVED / 0 / null |
| 合法dryRun | DRY_RUN / NOT_SAVED / 0 / null |
| 首次或較新報表真commit | SOURCE_REPORT_SAVED / SAVED / committed N / committed observedAt |
| 同時同內容 | SOURCE_REPORT_UNCHANGED / UNCHANGED / existing N / existing observedAt |
| 較舊或等時衝突 | SOURCE_REPORT_REJECTED / REJECTED / 0 / null；reason分別STALE_OBSERVATION或OBSERVATION_CONFLICT |
| writer/flush/commit失敗 | ROLLED_BACK / NOT_SAVED / 0 / null；只給消毒reason |

settlement payableAmount/receivableAmount始終null；realized insertedCount/alreadyRepresentedCount/skippedNameUnresolvedCount始終0，既有rowCount可描述經驗證的來源行數，不能稱作已入帳。原SUCCESS不由report流程回傳，reportSaved counter只計真commit的SAVED；NO_CHANGE另計UNCHANGED。原process13個全域Python counters不因報表增加任意key，各Java獨立outcome counters隨enum明確更新。

### Task406：本人 GET、代看防護與一致快照

唯一新business路由是 `GET /api/brokers/fubon/accounting-reports`，無query selector；未知query或重複param以純parser拒絕400。controller只委派，既有verified caller context不改；read service在自己獨立 `readOnly=true, REQUIRES_NEW, isolation=REPEATABLE_READ` 交易中核對dedicated owner與caller。專用設定缺失/非法、owner不存在/停用、caller不同，一律403通用`FORBIDDEN`，不查報表。合法read只要求專用owner本人且ACTIVE，不要求其角色仍ADMIN或ADMIN_EMAIL仍同人，global/兩sync flags=false亦可讀既有資料。

BFF新增專頁service與 `GET /api/bff/fubon-accounting-reports`，在任何business report request前，由Spring Security OIDC principal使用既有 `BffUser.fromPrincipal(oidc).id()` 取actual id，再與Reactor context的既有TenantIdentity effective id精確比較。缺失／不相等為403，零business report request；不能拿client X-User-Id、query、cookie字面或configured-admin當actual id。B代看A、A代看B皆拒絕，A本人正常。controller只解析authentication/transport並委派頁面service guard，不修改全域TenantWebFilter／TenantIdentity或別頁代看能力；business的dedicated-owner gate仍是第二層。新增business report路徑的所有可達BFF forwarding必須靜態確認不會繞過此guard，不新增裸passthrough／公開manifest入口，驗證只用隔離授權fixture。

business在同一MVCC snapshot內查owner、兩個headers、兩種children、bulk local names並組immutable DTO，不從OSIV stale entity取得owner或讓lazy children跑到交易外，不取寫鎖或更新任何資料。名稱port只查本地StockMaster，最多10,000個唯一代碼，依每批最多1,000分段、禁止逐列N+1／vendor補字；查無名稱null，UI顯示代碼。GET不呼Python config/health、SDK、broker HTTP、scheduler、writer、cache refresh，syncDisabled只讀本地原有非秘密global及各自flag。

BFF專用internal adapter總deadline10秒、response在讀取中最多16MiB；不調高全域WebClient codec。transport/timeout/schema/超限回502通用`REPORT_UNAVAILABLE`，不回部分rows，不暴露URL/HTML/例外。正確403不被轉成502或ACCOUNT_PENDING；原未登入/OIDC gate沿全域security。本頁可設skipErrorToast，由view顯示通用403/502，不能改其他頁既有interceptor。

### Task406：固定只讀 wire 與畫面

下列兩個property固定存在；各自獨立observedAt/queryDate可null，所有list永不null。Java金額／價格欄位使用BigDecimal及canonical plain-string serializer，不以String作核心數值型別。DTO不含ownerId/email/account/fingerprint/報表DB id，也沒有交易日期、淨收款、成本或報酬率欄位。

```text
AccountingReportsResponse
  settlement: SettlementReport
    scope: SOURCE_REPORT_ONLY
    financialPosting: NOT_APPLIED
    syncDisabled: boolean                 # !globalEnabled || !settlementEnabled
    observedAt: Instant|null
    queryDate: LocalDate|null             # observedAt的Asia/Taipei日期
    rowCount: int                        # 含None佔位與重複行
    rows: SettlementReportRow[]
      rowIndex: int
      status: AVAILABLE|NO_DATA_OBSERVED
      sourceQueryDate: LocalDate
      settlementDate: LocalDate|null
      currency: TWD|null
      buySettlement: BigDecimal|null     # JSON canonical plain string
      sellSettlement: BigDecimal|null
      totalSettlementAmount: BigDecimal|null # 同列buy+sell，僅回應時計算
  realizedGain: RealizedGainReport
    scope: SOURCE_REPORT_ONLY
    financialPosting: NOT_APPLIED
    syncDisabled: boolean                 # !globalEnabled || !realizedEnabled
    observedAt: Instant|null
    queryDate: LocalDate|null
    rowCount: int
    rows: RealizedReportRow[]
      rowIndex: int
      sourceDate: LocalDate
      stockCode: string
      stockName: string|null              # 本地bulk projection
      buySell: Buy|Sell
      orderType: Stock|Margin|Short|DayTrade|SBL
      filledQty: long
      filledPrice: BigDecimal             # JSON canonical plain string
      providerNetPnl: BigDecimal          # JSON canonical plain string
      currency: null                     # 來源未提供，不推測
```

無header回observedAt/queryDate=null、rowCount=0、rows=[]，UI為「尚未同步」；有header且0row為「最近一次來源報表沒有資料」，不能混成零元。全None行要顯示來源query date和「來源未提供資料」，金額空白，不當0。來源報表是各API最新成功回應，可能包含不同sourceDate，不宣稱完整年度或當下即時全部帳務。

新頁 `/fubon-accounting-reports`（route name `FubonAccountingReports`、title `富邦帳務報表`）的導覽放現有資產／交易群組尾端；既有route/icon/relative order不變。僅兩個read-only表格與觀測時間、範圍／未自動入帳警語，所有日期/數值/status由server-ready DTO呈現，不前端重算總額或跨報表拼接。沒有sync/import/edit/delete/trade/export按鈕，reload只GET；loading/empty/error不同，request取消不toast，latest generation才更新狀態。前端依專案指示由Terra/high實作並作真browser驗證。

R121仍只用靜態FubonApiInfoService，不能注入報表repository/client；兩accounting項目在整條功能驗收後才connected=true，httpEndpoint仍是既有Python read route，consumer補本人報表GET/頁面與「來源報表、不自動入帳」。52/15/37是401–405尚無報表時基線；401–406整批交付最終52/17/35。原JOBS共64保持，兩job文字只改保存來源報表及四個原cron，不新增排程。

### Tasks405／406：驗證與部署界線

全部新測試均為規劃新增，不能引用不存在的測試宣稱已通過。真PostgreSQL驗兩owner、fresh-lock/OSIV、rollback、date/precision、multiset/duplicate/empty、microsecond/nanosecond boundary與late completion；兩連線控制write/read交錯，REPEATABLE_READ必保證兩種header與其children在同一snapshot完整一致，不以READ_COMMITTED多次查詢或mock替代。before/after摘要證明report同步不改五類原財務表。
...
與 UTC occurred_at。使用者要求 log 詳細清楚；`api_name` 必原樣保存，`message_header`、`stack_trace`
必完整保存非敏感診斷內容，不得 hash、truncate、字元替換或內容過濾。既有 credential、token、certificate、
raw account／identity 與 raw payload 的安全替換仍優先於此可觀測性需求。Python adapter 的 `redaction.py` 不可當作
Java service 的保護；BFF、business-services、external-materials-service 必各有 service-local、可測試的
`ApiErrorLogDiagnosticRenderer`（可命名等價，但三端契約相同）。它只讀自己 runtime 已持有的非空 secret 值做 exact
replacement，並清理 token/secret/password/authorization/bearer/api-key/certificate/account/identity 等 key 的 value；
以 Throwable class、每層 sanitized message、cause chain 與 StackTraceElement 組成完整 trace，不縮減其他 class、
message、cause chain 或 stack frame。不可直接 `printStackTrace`／序列化 Throwable，也不可把 request/response body、query
或 raw header 收入 renderer；不能以 ADMIN-only 讀取或 private-only 寫入取代 secret 邊界。

本功能不為了補強 log 額外收集 request/response body、query 或 raw HTTP headers，亦不新發起 broker
呼叫。已有 Throwable 時，producer 必保存經其 Java renderer 安全替換後的完整可用 class、message、cause chain 與 StackTraceElement。
...
`occurred_at` 是 producer 在判定一次失敗的邊界固定的 UTC `Instant`，並與 Throwable 組成不可變 capture envelope，
而非 retry/queue/final-response callback/DB insert 時重新取樣的時間；BFF private ingest 將該值原樣傳到 business
...
api_error_log
  ...
  source           VARCHAR(16) NOT NULL CHECK OPEN_API | FUBON_API
  operation_key    VARCHAR(160) not null
...
api_error_log_retention_guard trigger
  BEFORE UPDATE OR DELETE ON api_error_log
  UPDATE -> RAISE EXCEPTION for every row
  DELETE -> only permit OLD.occurred_at < transaction_timestamp() - interval '30 days'; otherwise RAISE EXCEPTION
~~~

catalog 只由 migration seed 上述完整固定清單，不設 settings endpoint、DataInitializer、HTTP write API
或 catalog delete/update 流程。v1.123 在 seed 完成後讓 catalog immutable trigger 拒絕任何 UPDATE/DELETE（包含
尚無 log 引用 row）；error log retention guard 永遠拒絕 UPDATE，且只允許資料庫當前時間已超過 30 天的 row DELETE。
未來真需改名只能新增另一個 key。error log 索引為 `(source, operation_key, occurred_at DESC, id DESC)`、
`(source, api_name, occurred_at DESC, id DESC)` 及 `(occurred_at DESC, id DESC)`。error log 在 30 天保留窗內
append-only，沒有 delete/update API 或手動清除流程；到期刪除只由下述 business retention scheduler 執行。migration 註冊後重產 `db/schema.sql`，
其 pg_dump 輸出才是後續 schema assertion 的權威。

business-services 是 table schema、catalog、backend insert、30 天 retention scheduler 和全部 read model 的 owner。
...
第二道 source/key/name 防線。business recorder 對 private ingest 傳入的 message/trace 也必再次以自己的 renderer
sanitize，不能假定 BFF 已處理。任何 recorder/DB failure 只寫 operation、HTTP status、error class 等最小訊息並吞掉，
不得回印 request/response/trace 輸入，亦不得改變被觀測 API 的回應、排程 outcome、retry 或領域 transaction。

`ApiErrorLogRetentionScheduler` 是不公開的 business maintenance job：每日 03:15 `Asia/Taipei`（設定
`api-error-log.retention-cron`，預設 `0 15 3 * * *`）呼叫單一 repository delete，條件必由 DB 使用
`occurred_at < current_timestamp - interval '30 days'` 計算，與 retention guard 同一時間基準。它不讀取 detail、
不走 BFF、不提供 run-now、也不呼叫 API／Fubon SDK；失敗僅寫最小診斷 log，讓下一日照常重試。
...
固定 `FUBON_API` catalog row 的原始 api name 與經 external 自己的 renderer 安全替換後的完整可用 message/trace；composite FK 驗證 name，無 SELECT/UPDATE/DELETE、
...
BFF 的 `PublicApiErrorCaptureWebFilter` 先做 exact route/method catalog match。每個可能處理 matched public
route 的 exception resolver（既有 public `*ExceptionAdvice` **及全域 `BusinessErrorAdvice`**）均接受
`ServerWebExchange`，並在捕獲當下以 `Clock` 固定 `Throwable + occurredAt` 的 immutable capture envelope 放入
exchange attribute；filter 在最終 response status 已確定後才判定：只有 5xx 才以 BFF renderer 建立一筆 ingest，所以既有
status/body/envelope 不改。renderer 不得讀取 `WebClientResponseException` raw response body。`BusinessErrorAdvice` 為維持原
public contract 可以把 body 原樣 relay 給同一 caller，但 logger 只能寫 status、exception class 與 sanitized renderer 輸出，
capture/ingest 也只能走 renderer，不能把 body 放入 console 或 DB。若 advice 把 Throwable 映射為 1xx–4xx，filter 必丟棄 attribute、zero row；這些是
...
時固定 occurredAt，並建立包含 HTTP status、operation 與產生位置的 boundary synthetic trace；它不對 1xx–4xx 或任一 2xx 建立 row。
...
error path 和 status fallback 重複寫入；catalog 外的 404/405 絕不進此流程。renderer 必完整保留非敏感
exception text，但須用 BFF local runtime secret exact replacement 與敏感 key-value replacement，不主動蒐集
request/response/header 資料。
...
external producer 保存經其 Java renderer 安全替換後的完整可用 Throwable 與既有 failure reason；已取得 non-2xx status 時 synthetic trace 明示
...
caught Throwable 轉換成無 cause 的 outcome，必在轉換前記錄經 Java renderer 安全替換後的 Throwable，或保留 cause 供 recorder 記錄，
不得只留下 reason enum。
...
occurred_at ASC, id ASC。operations 在 source=ALL 時固定使用 OPEN_API、FUBON_API、display_order、operation_key
的順序；單一 source 時使用 display_order、operation_key，避免相同 display_order 的不確定回傳。
...
測試以 fake/in-memory client 或 isolated PostgreSQL 覆蓋完整 catalog seed/key/name/order、composite FK 與 NULL source
拒絕、catalog immutable trigger 對未引用 row 的 UPDATE/DELETE、error row UPDATE 與 30 天內 DELETE 拒絕／到期 DELETE
允許、retention scheduler 的 DB cutoff／失敗不影響其他流程、apiName snapshot、非敏感
message/stacktrace 不截斷且 Java renderer 的 runtime-secret、key-value、`WebClientResponseException` raw-body sentinel
不進 DB/detail/list/console（包括 `BusinessErrorAdvice` log）、REQUIRES_NEW/no-impact、兩種排序、source/key
filter 與 ALL deterministic order、list/detail 分離、雙 ADMIN gate、private token 缺失/錯誤/重複、public matched 5xx
的 scoped/global advice raw Throwable capture 與 producer-boundary timestamp、204/captured-4xx/unknown 404 zero row、每個 Fubon
client failure 一次、pre-I/O zero row、stream invalid/EOF/valid-event reset state、external SQL 僅 parameterized
INSERT 與 Vue source/DOM lazy-detail／純文字 HTML sentinel 行為。Docker acceptance 先 recreate business-services 讓 Liquibase migration
