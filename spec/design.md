...
排程列表只更新既有一筆「已實現損益匯出」的描述，不增減 job：annotation 仍是同一個每分鐘 poll，時間點是 DB 動態設定，JOBS 總數不變（該段落落筆當下為 51 筆；**權威記載見本文件上方 `SchedulePublicBffController` 段落**，不要以此處的絕對數字為準——本需求並未增減 job）。
