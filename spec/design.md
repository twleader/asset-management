...
>
> ✅ **`db/schema.sql` 是可用的離線查證依據，由機械閘門保證同步（Requirement 102／Task 366）。** 它是 `db/init/01_dump.sql`（含真實個人財務資料，被 `.gitignore` 排除）「去除全部資料」後的可版控鏡像。過去它靠人工重產、長期落後（Task 245 當時僅 55 張 `CREATE TABLE`，Task 366 動工前為 74 張、運行中 DB 已是 85 張），**Task 366 起**改由 `scripts/tests/schema-sql-drift-test.sh` 逐位元比對「去除專案檔頭後的檔案內容 vs 此刻重跑 `pg_dump` 的輸出」，並由 `scripts/spec-check.sh` 的 **B10** 在每次 spec 機械檢查時執行。因此欄位型別、位數、nullable、預設值與索引定義都可以直接引用本檔。
>
...
>
> ⚠ **但「運行中 DB」不等於「main 的現況」。** 全機只有一套 `asset-*` 容器、數十個 worktree 並行推進，DB 可能已套用其他分支尚未 merge 的 changeset，鏡像會一併帶進來。凡是要斷言「main 上有沒有這張表／這一欄」，仍須以 `docker exec asset-postgres psql -U assets -d assets -c '\d <table>'` 加上比對 `SELECT id FROM databasechangelog ORDER BY orderexecuted DESC LIMIT 5` 與 main 的 `db.changelog-master.yaml` 尾端來複驗。**同理不要引用 `db/changelog/**` 描述現況**——那裡有永不執行的 changeset（下方 `v1.0.0` 的 `NUMERIC(20,4)` 即為前例）。
