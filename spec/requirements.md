...
- [ ] **「股票（即時）」增列 ETF 淨值與折溢價欄（Task 210）**：「當前即時資產」分頁的「股票（即時）」區塊，於末尾再增列 3 欄 `淨值／折溢價(%)／淨值時間`（完整欄序 0–20）。折溢價＝(市價 − 淨值) / 淨值，以百分比數值呈現（`1.2` 表示溢價 1.2%，負值為折價）。
...
- [ ] **ETF 淨值與折溢價每日入庫留存（Task 211）**：Redis 只保留最新一筆（TTL 96 小時）供匯出即時取用，**每日抓到的淨值與折溢價另須寫入 `etf_nav_history` 長期留存**，以便日後回看折溢價走勢（例如「這檔平常溢價 0.5%，現在 2% 是不是買貴了」）。
