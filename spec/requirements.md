...
- [ ] **測試與驗收。** 新增或更新 BFF controller test，斷言：清單總筆數為 52；歷史已驗收的 ETF 基線為 `connected=true` 9 筆／`connected=false` 43 筆；架構階段基線為15筆／37筆，交割／損益尚未串接；Task406 兩份來源報表完整驗收後是17筆／35筆，原 Task394/395 財務入帳仍未完成，各sdkReference/httpEndpoint須符合design的adapter allowlist（14條含stock subscriptions輔助控制路由；portfolio與indices各承接2項，technical read承接3項）。httpEndpoint只列adapter路徑，Java手動/sync/readback放consumer；不可把不同服務入口混用；每筆 `category` 屬於上述 7 類之一；每筆 `sdkReference` 非空白；每筆 `description` 皆含「純查詢」或「不影響券商端」字樣；清單中不存在任何 finding 1 條列出的下單/改單/刪單/批次/條件單/預約/匯撥類方法名稱（以字串比對逐一排除）；`futopt`／`futopt_accounting` 字樣不得出現在任何 `sdkReference`。`bash scripts/spec-check.sh`、BFF Maven test 與 frontend production build 皆須通過。實機驗收：以 Compose stack 重建並 recreate `bff` 與 `frontend`，確認未登入請求 `/api/bff/fubon-api` 仍受登入保護；登入後側欄「系統資訊」的「富邦證 API」頁可看到 52 筆資料、「已串接」欄位依實際驗收階段正確標示9筆（歷史ETF基線）、15筆（本階段五項完成後）或17筆（Task406 來源報表完整驗收後；原8筆僅為歷史基線）、可展開任一列看到請求／回應明細、依「已串接」與分類篩選皆可用，且畫面明確呈現「未串接」定位說明文字。
...
- [ ] **Task 401 保留 90 秒工作批次。** Java 仍最多 2 個 symbol worker，每 90 秒工作批次最多 20 個 symbol attempts，所有 symbol 起呼共用至少 3.1 秒 dispatch 間距；每 symbol 的 KDJ→MACD→BB 依序查詢，Python 三組共用 25 秒 aggregate deadline 與 process rolling 60/min historical budget，真 429 暫停尚未 dispatch 的查詢。排程整輪 30 分鐘、same-day／after-close／radar gates、取消與 PARTIAL outcome 不變。未處理不冒稱已全部取得；抽 Protocol 不把 method、symbol 或 retry 當成獨立 quota。
