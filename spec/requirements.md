...
- [ ] ~~**每日排程自動匯出（per-user，每日單一時間）**~~：**已由 Requirement 72 取代為多時間點**（比照 Requirement 69 對 Requirement 34 的取代）。原條文：使用者可在已實現損益頁「排程自動匯出」設定卡開啟每日排程，設定每日執行時間（時:分）與輸出資料夾，系統於該時間把該使用者的已實現損益匯出成 `.xlsx` 到指定目錄；每日固定一個時間。現況為「一份共用設定＋多個各自 guard 的每日時間點」，`realized_gain_export_schedule` 的 `run_hour`／`run_minute`／`last_run_date` 只留作舊 image rollback shadow，不再是排程來源。「不設交易日閘門」（已實現損益僅於有成交時異動，每日留存即可）仍然成立。
