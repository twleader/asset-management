...
### Requirement 77: 油價金價交易時段每分鐘即時報價，收盤後 5 分鐘取回收盤價校正
...
- [ ] **落地後補一次盤中實測並回填規格**：本需求的來源行為實測全部取自 2026-08-15（週六、Globex 休市）。實作落地後的**第一個盤中時段**（週日 18:00 ET 之後、即台北週一 06:00 之後任一時點）須實查一次 `range=1d&interval=1d` 的回應，把當下 `timestamp[0]` 與 `meta.regularMarketTime` 的實際日期歸屬記錄進 Task 337 的完成報告。這是**驗證項目而非實作前置**——上面的二分規則對兩種歸屬都成立，不必等這筆實測才能開工。
