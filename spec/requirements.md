...
- [ ] 股票持股表格的「股價」欄需依右上角基準日（所選快照的 snapshotDate）顯示對應股價：
  - 基準日 = 今日且該市場（台股／美股）盤中：顯示即時股價＋漲跌%，每 5 分鐘自動更新，且資料來源與「觀察股票」頁面一致（同一個 `StockPriceRepository` 快取）
  - 其他情形（基準日為過去日期，或當日市場已休市）：顯示該快照中保存的當日收盤價（`snapshotStockHolding.stockPrice`），不顯示漲跌%、不參與輪詢更新
- [ ] 切換基準日後，KPI 卡片、資產配置圓餅圖、銀行存款圖、持股圖與股票持股表格都需跟著基準日（所選快照）變動；不可被 5 分鐘的價格輪詢覆蓋回「最新快照」
...
- [ ] 股利歷史頁籤顯示最近 10 年股利：年度 / 每股現金股利 / 每股股票股利 / 除息日昨收價 / 除息日 / 發放日 / 填息天數
