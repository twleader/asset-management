...
### Requirement 123／Task 389–390：富邦「ETF 成分股持股明細查詢」排程串接，取代既有 MoneyDJ／Yahoo 爬蟲來源

**User Story：**作為使用本系統的投資人，我希望系統改用富邦官方 SDK 的唯讀查詢，在交易日 08:50、15:30 自動抓取「今日交易雷達」範圍內台股 ETF 的成分股持股明細並落地保存，取代現行不穩定的 MoneyDJ／Yahoo 爬蟲來源，讓我在股票分析對話框與資產總覽透視看到的 ETF 成分股資料更準確、更即時，且系統不會因為爬蟲網站改版而悄悄失效。

**背景（承接 Requirement 121／Task 386 的盤點結果）：** `spec/tasks/t386_fubon_api_documentation_view.md` 已盤點出富邦 SDK 具備 `marketdata.rest_client.stock.ownership.etf_holdings` 這支唯讀查詢能力（Fugle 行情 REST 的一部分，經 `fubon-broker-service` 容器內已驗證的 `raw_field` 呼叫模式即可存取，無需新登入/session 邏輯），但截至該任務為止**尚未串接**、且**回應欄位完全未經核實**（該 SDK 方法本身無 docstring，套件內也無任何文件說明回應形狀，只能透過真實帳號的一次即時呼叫才能得知確切欄位名稱）。目前系統已有的「ETF 成分股」功能（`external-materials-service` 的 `MarketDataFetchService.getEtfHoldings()`：台股走 MoneyDJ 網頁爬蟲、美股走 FinMind／Yahoo，12 小時記憶體快取、無資料庫持久化）供 `StockAnalysisDialog.vue`「ETF 成分股」圖表與 `DashboardBffController` 的資產總覽透視（look-through，Task 129）共用。本次決定：**台股 ETF 成分股改用富邦排程資料源取代 MoneyDJ 爬蟲；美股 ETF（Fubon 無覆蓋）維持既有 Yahoo／FinMind 爬蟲不變**。

**「今日交易雷達」範圍採用 Requirement 115 官方定義（跨全體使用者聯集），非單一 owner。** 權威實作目前在 `external-materials-service` 的 `StockSourceQuery.collectTwRadarCodes(Set<String> twCodes)`（`SELECT stock_holding.stock_code`（各 owner 最新一份快照）∪ `SELECT DISTINCT stock_alert.stock_code`，market 限台股，排除 `0000`）。本次新排程放在 `backend`（比照既有 `FubonInventorySyncScheduler`／`FubonTradeSyncScheduler` 所在的 `com.steven.assets.integration.fubon` package，因為 Fubon HTTP client／DB 寫入都在 backend），backend 本身就是 `stock_holding`／`asset_snapshot`／`stock_alert` 的權威擁有者（同一個 PostgreSQL 資料庫），故**在 backend 端以等價 SQL 重新實作雷達候選集合查詢**，不對 `external-materials-service` 發 HTTP 呼叫（避免新增不必要的服務間耦合）；ETF 判定沿用既有 `MarketDataService.isEtf(code, market)` 對台股採用的既定 heuristic（`code.startsWith("00")`）。**明確承認風險：這是與 `StockSourceQuery.collectTwRadarCodes` 邏輯等價但物理上獨立的第二份實作，兩處未來各自修改時有漂移風險**，可接受原因是本身就是既有已知模式（`TradingRadarService`／`StockSourceQuery` 本來就是兩份平行實作），且範圍窄（只加一個 `AND code LIKE '00%'` 過濾）。

**未知回應欄位的處理原則（比照 Requirement 120／Task 385 對 `FilledData` 欄位的既有做法，但更保守）：** `FilledData` 當時至少有官方 docstring 可核對；`ownership.etf_holdings` 完全沒有，唯一核實管道是在具備真實富邦憑證、`FUBON_ENABLED=true` 的環境對一支已知真實 ETF（如 `0050`）做一次真實呼叫並檢視原始回應。本次 spec 撰寫時的開發環境 `FUBON_ENABLED=false`，無法核實。因此本 Requirement 拆成兩個任務、明確切開「與欄位無關的基礎設施」和「依賴欄位核實的解析／串接」：

- **Task 389（本次可完整落地，無外部依賴）：** `fubon-broker-service` 新增唯讀 endpoint、backend 新增資料表／排程／唯讀 client，把富邦回應**原封不動**以 JSON 文字存入資料庫（不臆測、不重新命名任何欄位），排程本身、feature flag、交易日判斷、雷達範圍查詢、單一 flight 防重疊、`SchedulePublicBffController` 登錄皆在此任務完成並可獨立驗收（可直接下 SQL 查詢確認資料表有落地資料、排程確實在 08:50／15:30 觸發）。
- **Task 390（依賴真實環境核實欄位，前置任務為 Task 389）：** 把 Task 389 存下的原始 JSON 解析成既有 `MarketDataService.EtfHoldingsResult`／`EtfHolding` 形狀、切換 `getEtfHoldings()` 讀取路徑、刪除 `external-materials-service` 內已死的 MoneyDJ 爬蟲程式碼、同步更新 `spec/tasks/t386_fubon_api_documentation_view.md` 的已串接狀態與計數。**此任務的欄位解析函式必須先在具備真實富邦憑證的環境核實 `ownership.etf_holdings` 實際回應欄位名稱後才能定案**；核實不到時，解析函式必須明確 fail-soft（回傳 `success=false`／`message="尚未核實回傳欄位格式"`），不得臆測欄位名稱或以猜測值蒙混過關。

#### Acceptance Criteria — Task 389（抓取與落地基礎設施）

- [ ] **`fubon-broker-service` 新增唯讀 endpoint，原封不動保留原始回應。** 新增 `SdkGateway.read_etf_holdings(symbol: str)`（比照既有 `quote()` 的結構：使用 `_ensure_session()` 已快取的 `self._stock_client`，`raw_field(client, "ownership")` → `raw_field(ownership, "etf_holdings")` → `_run_bounded(lambda: method(symbol=symbol), ...)`；**不使用** `_accounting_lock`／`_wait_for_accounting_budget()`，因為 `marketdata.rest_client.stock` 這條線與帳務命名空間無關，比照既有 `quote()` 完全不觸碰帳務序列化鎖）。新增 token-protected `POST /internal/market-data/etf-holdings`，request body `{"codes": ["0050", ...]}`（1..50 檔，`extra="forbid"`, `strict=True`），逐檔呼叫並回傳 `{batchId, holdings: [{stockCode, status, reason, rawResponseJson}], counters}`；`rawResponseJson` 為該檔 Fubon 回應**序列化後的原始 JSON 字串**（不得重新命名、篩選或轉型任一欄位；僅需確認可被 JSON 序列化，序列化失敗算該檔查詢失敗）。既有三態語意（`DISABLED`／`MISCONFIGURED`／token 缺漏或錯誤）與既有 `/internal/market-data/tw-quotes` 完全一致。

- [ ] **backend 新增唯讀 client 方法與資料表，逐字保存原始回應。** `FubonBrokerClient` 新增 `readEtfHoldings(List<String> codes)`；`FubonHttpClient` 呼叫上述新路由，比照既有 `readTwQuotes` 的 gate／timeout／錯誤轉換模式。`FubonDtos` 新增 `EtfHoldingsBatchResponse(String batchId, List<EtfHoldingsItem> holdings, Map<String,Long> counters)`／`EtfHoldingsItem(String stockCode, String status, String reason, String rawResponseJson)`（`rawResponseJson` 為不可變 `String`，不得用 `Map`／`JsonNode` 代替，逐字保存 Python 端回傳的原始 JSON 文字）。新增 Liquibase changeset（版號銜接現有最新 `v1.119.0`，即 `v1.120.0-fubon-etf-holdings-snapshot.sql`；實際版號以建檔當下 `bash scripts/spec-check.sh` 核對過的最新版號為準，撞號時依既有編號避讓慣例調整）：新表 `fubon_etf_holdings_snapshot`（`etf_stock_code VARCHAR(20) NOT NULL`、`market VARCHAR(10) NOT NULL DEFAULT '台股'`、`fetched_at TIMESTAMP NOT NULL`、`success BOOLEAN NOT NULL`、`reason VARCHAR(50)`、`raw_response_json JSONB`、`updated_at TIMESTAMP NOT NULL DEFAULT now()`，`PRIMARY KEY (etf_stock_code)`——**同 key 覆寫（比照既有 `fubon_taiex_index_latest` 以 `PRIMARY KEY (index_code)` 不含日期欄位的最新一筆覆寫慣例；不是 `etf_nav_history`，該表 `UNIQUE (stock_code, market, nav_date)` 逐日留存歷史，語意相反），不保留歷史，只留「目前最新一次成功或失敗的抓取結果」**）。完成後依 `db/schema.sql` 檔頭「重新產生」段重產該檔，並以 `bash scripts/tests/schema-sql-drift-test.sh` 確認回 0。此表不透過任何 API／DTO 對外暴露，僅供 Task 390 讀取端使用。

- [ ] **新增排程，範圍為「今日交易雷達」中的台股 ETF，交易日 08:50、15:30 各執行一次。** 新增 `FubonEtfHoldingsSyncScheduler`：兩個獨立 `@Scheduled` 方法（`cron="0 50 8 * * MON-FRI"` 與 `cron="0 30 15 * * MON-FRI"`，`zone="Asia/Taipei"`），皆呼叫同一個 `FubonEtfHoldingsSyncService.syncScheduled()`；獨立 process-local `AtomicBoolean inFlight` single-flight（比照既有 `FubonTradeSyncScheduler`），與其他 Fubon 排程（庫存同步、成交同步、TAIEX 串流）各自獨立、互不阻塞。執行前依序：(1) 讀新增獨立 feature flag `FUBON_ETF_HOLDINGS_SYNC_ENABLED`（`.env`／`.env.example` 新增，預設 `false`）；為 `false` 時直接 no-op。(2) `FubonConfigState.snapshot().state()` 必須為 `READY`，否則 no-op。(3) 呼叫既有 `MarketDataService.isTwTradingDayKnown(today)`，只有明確 `Optional.of(true)` 才繼續，其餘一律 no-op（沿用既有 TWSE primary → 完整 DGPA provisional → unknown 的 authority 順序）。**本排程與 `FUBON_TW_LIVE_QUOTES_ENABLED` 不互斥**（與 `FUBON_TRADE_SYNC_ENABLED` 對 LIVE quote 的互斥規則不同）：`marketdata.rest_client.stock` 這條線的 `quote()`／新方法皆不佔用 `_accounting_lock`、且本排程一天只跑兩次、每次僅查當日雷達範圍內少量 ETF 代碼，與每 10 秒高頻的 LIVE 報價不構成資源衝突，只共用既有 `_run_bounded` 的 `_blocking_slots` 併發限流，理由需寫入 design.md，不得省略。

- [ ] **雷達範圍查詢：backend 端等價實作，僅過濾台股 ETF。** 新增查詢方法（可放在新的 `FubonEtfHoldingsSyncService` 內或獨立小型 repository helper），SQL 語意與 `external-materials-service` 的 `StockSourceQuery.collectTwRadarCodes` 完全等價：`SELECT stock_holding.stock_code FROM stock_holding WHERE market='台股' AND snapshot_id IN (SELECT DISTINCT ON (owner_user_id) id FROM asset_snapshot ORDER BY owner_user_id, snapshot_date DESC, id DESC)` 聯集 `SELECT DISTINCT stock_code FROM stock_alert WHERE market='台股'`，排除 `0000`；取得候選集合後，只保留 `MarketDataService.isEtf(code, "台股")` 為 `true` 的代碼（即 `code.startsWith("00")`）。候選集合為空時整批 no-op（不視為錯誤）。單輪最多查詢 50 檔（與 `POST /internal/market-data/etf-holdings` 的 request 上限一致），超過時**全部查詢，不得靜默截斷**——若 request 層有 50 檔上限，service 層須自行分批送出多個 batch request，逐批合併結果。

- [ ] **成功／失敗一律 upsert，不得因單檔失敗中止整批。** `FubonEtfHoldingsSyncService.syncScheduled()` 取得雷達 ETF 代碼後呼叫 `readEtfHoldings`，逐檔依 `status` upsert 進 `fubon_etf_holdings_snapshot`（成功寫 `success=true, raw_response_json=<原始回應>`；失敗寫 `success=false, reason=<失敗原因>, raw_response_json=NULL`），單檔失敗不影響其他檔的 upsert，也不中止本輪其餘代碼的查詢。`SchedulePublicBffController.JOBS` 新增一筆 `(BUSINESS, "券商庫存", "富邦 ETF 成分股持股同步", <白話說明含「唯讀查詢」「範圍為今日交易雷達台股 ETF」>, "交易日 08:50、15:30", "0 50 8 * * MON-FRI；0 30 15 * * MON-FRI", TPE)`，其測試斷言的總數同步由現行值加一（business 分類同步加一，跑測試取得當下真實基準值再改，不得假設寫死數字）。`docker-compose.yml` 的 `business-services` 服務 `environment` 區塊新增一行 `FUBON_ETF_HOLDINGS_SYNC_ENABLED: ${FUBON_ETF_HOLDINGS_SYNC_ENABLED:-false}`（緊鄰既有 `FUBON_TRADE_SYNC_ENABLED` passthrough）。

- [ ] **Task 389 不在本次範圍。** 不解析 `raw_response_json` 內容（欄位名稱未核實，不得臆測）；不修改 `MarketDataService.getEtfHoldings()` 既有行為；不刪除 `external-materials-service` 既有 MoneyDJ／Yahoo 爬蟲程式碼；不新增手動觸發端點；不做任何下單、改單、圈存或轉帳類 API 呼叫；不影響既有富邦庫存同步／成交同步／TW LIVE 報價／大盤指數串流的邏輯或 feature flag。

#### Acceptance Criteria — Task 390（欄位解析、讀取端切換、既有爬蟲清理；前置任務 Task 389）

- [ ] **解析函式必須先以真實環境核實欄位名稱，核實不到必須 fail-soft，不得臆測。** 新增一個獨立、可單元測試的解析函式（如 `FubonEtfHoldingsParser.parse(String rawResponseJson)`），把 `fubon_etf_holdings_snapshot.raw_response_json` 轉成 `List<MarketDataService.EtfHolding>`（`stockCode`／`stockName`／`weight` 必要；`shares` 若無對應欄位一律 `null`，既有前端 `StockAnalysisDialog.vue` 已對 `shares` 為 `null` 做 `—` 顯示，無需改動）。**實作前必須在具備真實富邦憑證、`FUBON_ENABLED=true` 的環境對至少一支已知台股 ETF（建議 `0050`）呼叫本任務新增的 `POST /internal/market-data/etf-holdings`，取得真實 `rawResponseJson` 並記錄實際欄位名稱**，解析函式的欄位對應以此次核實結果為準，並在函式註解寫明核實依據（時間、呼叫的 ETF 代碼、原始回應片段節錄）。**無法完成上述真實環境核實時，解析函式必須明確回傳「解析失敗、原因為欄位格式未核實」，不得以猜測欄位名稱的方式蒙混通過測試**——此為驗收阻斷條件，不是建議。

- [ ] **`MarketDataService.getEtfHoldings()` 依市場分流：台股讀本地新表，美股維持既有爬蟲不變。** 修改既有方法：`market=="台股"` 時，改讀 `fubon_etf_holdings_snapshot`（透過新增的 repository／JdbcTemplate 查詢），存在且 `success=true` 時用上述解析函式組出 `EtfHoldingsResult(stockCode, market, true, "Fubon", <fetched_at 格式化日期>, null, holdings)`；查無資料、`success=false`，或解析失敗時，回傳既有的「不可用」語意（`EtfHoldingsResult(stockCode, market, false, null, null, <對應訊息>, List.of())`），**不得回退呼叫 `external-materials-service` 的 MoneyDJ 爬蟲路徑**（該路徑本任務會移除）。`market=="美股"` 時完全不變，繼續呼叫既有 `priceServiceClient` → `external-materials-service` 的 `/internal/etf-holdings`（Yahoo／FinMind 路徑），沿用原本行為。`DashboardBffController` 的資產總覽透視（look-through，Task 129）呼叫的是同一支 `getEtfHoldings()`，行為隨之自動切換，不需額外改動 BFF 或前端。

- [ ] **刪除 `external-materials-service` 內已死的 MoneyDJ 爬蟲程式碼，保留美股 Yahoo／FinMind 路徑。** `MarketDataFetchService` 內原本 `fetchEtfHoldingsUncached()` 依市場分流呼叫 MoneyDJ（台股）或 Yahoo／FinMind（美股）；本任務後台股請求不再送達此方法（backend 已在上一條分流攔截），故移除 `getMoneyDjEtfHoldings()`／`parseMoneyDjHoldings()` 兩個方法與其專屬的 MoneyDJ HTTP 呼叫程式碼，`fetchEtfHoldingsUncached()` 精簡為只處理美股（Yahoo／FinMind）分支；`/internal/etf-holdings` endpoint 與 `getEtfHoldings()` 方法本身保留（仍供美股請求使用），僅刪除台股 MoneyDJ 專屬程式碼路徑；12 小時記憶體快取機制對美股請求維持不變。**不得整支刪除 `MarketDataFetchService` 或 `/internal/etf-holdings` endpoint**——美股 ETF 成分股查詢仍依賴它們。

- [ ] **同步更新「富邦證 API」盤點頁（Requirement 121／Task 386）的已串接狀態。** `bff/src/main/java/com/steven/assets/bff/fubonapi/FubonApiInfoBffController.java` 的 `APIS` 常數中，`sdkReference="marketdata.rest_client.stock.ownership.etf_holdings"` 那一筆 `connected` 改為 `true`，`httpEndpoint` 改為 `POST /internal/market-data/etf-holdings`，`consumer` 改為說明「business-services（富邦 ETF 成分股持股同步排程，交易日 08:50／15:30，範圍為今日交易雷達台股 ETF，需另啟用設定才會執行）」；對應調整 `requestSummary`／`responseSummary` 反映本任務實際實作的 request/response 形狀。`FubonApiInfoBffControllerTest` 的統計斷言同步更新：`connected==true` 筆數由 8 改為 9（新增 1 組 `(sdkReference, httpEndpoint)` tuple）、`connected==false` 由 44 改為 43，「行情查詢」分類的已串接筆數由 2 改為 3（分類總筆數 20 不變，52 筆總數不變）。`spec/design.md` 對應章節（Requirement 121 設計段的分類表與架構圖註解）與 `spec/requirements.md` Requirement 121 的 AC 文字同步更新這幾個數字，避免與既有 5 輪審查建立的「52／8／44」基準漂移卻沒人發現。

- [ ] **Task 390 不在本次範圍。** 不修改美股 ETF 成分股的資料來源或快取邏輯；不新增使用者可調整的排程時間或觸發按鈕；不影響既有富邦庫存同步／成交同步／TW LIVE 報價／大盤指數串流的邏輯或 feature flag；不做任何下單、改單、圈存或轉帳類 API 呼叫；若真實環境核實後發現 `ownership.etf_holdings` 回應形狀與既有 `EtfHolding(stockCode, stockName, weight, shares)` 四欄位無法對應（例如成分股清單改用完全不同的巢狀結構），允許在核實當下另行判斷是否需要調整 `EtfHolding` 形狀本身，但**不得**為了「先讓測試過」而虛構欄位。
