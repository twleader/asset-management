...
- [ ] **單一免登入唯讀端點**：BFF 新增 `GET /api/public/market-index?market=&range=`；Docker host 可由 `http://localhost:8080` 呼叫，同一 Compose network 內可由 `http://bff:8080` 呼叫，經 frontend nginx 亦可由 `http://localhost/api/public/market-index` 呼叫。此精確 GET 路徑不要求 OAuth session，且是 CLAUDE.md／structure.md 唯一具名、限縮的 Docker／自動化**非前端頁面**例外；前端 view 不得據此繞過 `/api/bff/{page-name}`。除它以外，既有 `/api/bff/gdp-twse/**` 查詢、回補、匯出與排程端點的授權規則全部不變。BFF 已有 host port 與 Docker network，故不得為此新增 service、port mapping 或第二套反向代理。
...
- [ ] **只開放精確 GET，不擴張寫入能力**：`SecurityConfig` 只對 `GET /api/public/market-index` 設 `permitAll`；POST/PUT/PATCH/DELETE 同路徑與 `/api/public/market-index/**` 其他子路徑不得被 wildcard 放行。安全測試與 Docker 驗證都必須逐項釘住同路徑 POST/PUT/PATCH/DELETE 及 descendant GET 在匿名情境回 401，不得只用相鄰 BFF endpoint 間接推論。端點不回傳 owner、帳號、匯出路徑或排程設定，也不提供回補、匯出、立即執行等寫入／檔案操作。
- [ ] **測試與實機驗證**：至少涵蓋——(a) 9 市場 × 8 期間共 72 組合法組合均通過 catalog/validation；(b) 未知 market/range 各回 400 `application/problem+json`，且 detail 含收到的非法值與完整合法 values；(c) 日線各期間筆數、所有陣列對齊及「先算 MA 再裁切」，並以獨立期望 map 釘住與既有 frontend 相同的 7 組交易日數；(d) `tradeValue` 的 Integer／Long／BigDecimal／浮點 Number／numeric String／null 精確轉換與非法值 typed exception，且無 double 二進位尾差；(e) malformed public 回 502 ProblemDetail、legacy `index-daily` 回 200 完整空 body；以真實 `MarketIndexChartService`＋可控 WebClient fixture 分別證明 transport／HTTP／decode fail-soft 仍回完整空 schema 200，並讓已 decode 的非法 `closePoint` 實際穿過 mapping／shaping，證明兩入口都以非 400／502 的 5xx 傳播，controller 外層 mock 不得取代這組反例；(f) 當日分時 metadata、ISO `LocalDate` JSON、legacy String shape、昨收漲跌、水平 MA 與全 null 成交量陣列；(g) 未登入精確 GET 回 200、相鄰 `/api/bff/gdp-twse/index-daily` 回 401、同路徑 POST/PUT/PATCH/DELETE 與 descendant GET 各回 401；(h) BFF 測試／package 通過；(i) 合併後從乾淨、已 `fetch` 且 `merge --ff-only origin/main` 的 main worktree，使用執行中 container 查得且驗證的 Compose project name 重建／recreate `bff`，以 bounded healthy wait 避免啟動競態；實際走 direct host、frontend nginx 與 Docker network 各呼叫日線／當日，且三條入口的代表 DAILY／INTRADAY 回應都必須 `labels` 非空、所有陣列等長，任一 sentinel 為空即停止並回報 runtime 無法證實，不得把 fail-soft 200 空 schema 當成功；72 組 fail-closed 迴圈另要求所有 DAILY 組合非空、回應 echo 的 market/range 正確且陣列等長，逐項驗證匿名負向路徑，並在結論前確認 Compose tag 與執行中 container image 均仍等於本輪 build image。
