...
> **Gateway network boundary 由 Requirement 66／Task 328 唯一管理。** 該任務已在獨立 non-root Nginx `api-gateway` 預留 exact `GET /api/assets/latest` → `bff:8080`，host 僅綁 `127.0.0.1:9090`，Tailscale Serve 只掛五條核准的 exact path（Requirement 71 另增第六條 `POST` exact path，實際 handler 集合為六條，見該 Requirement）。本文只實作 latest-assets 的 BFF／business 業務契約，不在 BFF 代理 quotes、不改 Compose host ports、不複製 Nginx／Tailscale 規則。
...
### Requirement 77: 今日股市分析改由本機規則引擎產生——LLM 成本歸零，且隨時可切回
...
### Requirement 78: 今日股市分析與資產配置建議的 Docker 外部唯讀 API（Nginx 9090 第七、八條路由）
...
- [ ] **以下具名文件位置須全部同步更正，不得只改其中一份**：
  - `CLAUDE.md`「BFF 與資料來源規範」第 1 節具名例外段落（現載「六條路由（五條唯讀 GET ＋ 一條寫入 POST）」與 BFF 匿名放行三條 GET 的列舉、「Frontend 對六路 exact／matrix 變體回 404」）與第 3 節標題及內文（現載「五條唯讀 GET ＋ 一條寫入 POST」、「Tailscale Serve 只以 path-scoped HTTPS `:9090` 掛相同六條 exact path」），須全部更正為八條（七條唯讀 GET ＋ 一條寫入 POST）並列出新增兩條路徑；Requirements 引用清單須併入 `77`／`78`，Tasks 清單須併入 `336`／`337`。
  ...
  - **Requirement 66／68／70 與 `spec/design.md` 內既有的「實際 handler 集合為六條／精確為六路」callout，在第七、八條落地後全部成為錯誤斷言，須逐一更正為八條／八路**：`spec/requirements.md:2625`、`:2660`、`:2705`、`:2716`，以及 `spec/design.md:6961`、`:6962`。Requirement 71 對同一類殘留是明文列管的（其 AC 有範圍聲明 ＋ 待修位置清單）；本 Requirement 不採「留待實作階段」的延後做法，改為**在本清單直接列出位置、於 Task 337 落地時一併修正**，避免再開一輪計數漂移。
...
### Requirement 79: 資產配置建議的三態引擎切換——完全本機、部分打 API、或維持現行全 LLM

**User Story:** 作為系統擁有者，我希望「資產配置建議」頁也能像「今日股市分析」一樣切換引擎：可以完全本機計算（零 API 成本），也可以只把「寫成人話」那段交給 LLM（大幅省錢但保留可讀性），或維持現行的完整 LLM ＋ 網路搜尋。

> **本 Requirement 與 Requirement 77 是兩件不同的事，不得混為一談。** Requirement 77 處理的是**每日無人值守、與寄送時段成正比**的固定成本；本頁是**使用者手動按才跑**（`PortfolioAdviceController:87` 是 `generate` 的唯一呼叫點，全樹無 `@Scheduled` 觸發），總成本取決於按幾次。因此本 Requirement 的動機不是「止血」，而是**給使用者一個可選的省錢檔位**，並保留「完整版」不動。
>
...
- [ ] **`model` 欄位記錄檔位與版本**：`portfolio_advice.model` 在 `local` 檔位寫 `local-allocation:v1`；`hybrid` 檔位寫 `hybrid-allocation:v1+<實際 Claude model id>`（例如 `hybrid-allocation:v1+claude-haiku-4-5`），使歷史列表可分辨每一筆是哪個檔位、以及 hybrid 用了哪顆模型。配置模板調整時須提升版本號（同 Requirement 77 對 `local-rule-engine:v1` 的理由）。
...
- [ ] **與 Requirement 78 的第八條相容**：`GET /api/public/portfolio-advice/latest` 只讀最新一筆，不因引擎檔位而改變行為；三檔位產出的列都經同一個 `PortfolioAdviceDto` 序列化。本 Requirement **不得**讓該公開端點觸發任何產生動作（Requirement 78 已明文禁止）。
