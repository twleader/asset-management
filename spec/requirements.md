...
**User Story:** 作為系統維運者，我希望 Docker 外部只需連到一個固定的 `9090` API 入口，就能查詢最新報價與股市大盤圖表資料；入口須以明確白名單隔離 `/internal/*` 與登入後 API，並透過 Tailscale 私網 HTTPS 提供遠端存取，避免 Google OAuth2、自簽憑證與公開 Internet port 的維護成本。

> **本 Requirement 的「所有 API」只指目前已正式核准給 Docker 外部工具使用的五個唯讀 GET operation：** `GET /api/quotes`、`GET /api/quotes/one`、`GET /api/public/market-index`、`GET /api/assets/latest`、`GET /api/public/exchange-rate/usd-twd`。它不代表把整個 `/api/**`、BFF、前端登入 API 或任何 `/internal/*` 全部公開。Task 328 以新的 Nginx `api-gateway` 收斂原本分散在 BFF `8080`、external-materials-service `8082` 與 frontend `80` 的 Docker 外部入口；三個上游服務仍保留各自的 container port，只撤除 host port mapping。前四支可經 Tailscale Serve 遠端存取；USD/TWD 僅限 host loopback，明確排除於 Serve mounts。`/api/assets/latest` 與 USD/TWD 的業務契約由各自獨立需求／任務實作，本 Requirement 只定義 gateway network boundary，不得在 Nginx 複製業務邏輯。
...
- [ ] **Nginx 採精確 allowlist，不得用 `/api/` wildcard proxy**：本機 `127.0.0.1:9090` 只允許下列五個 exact path，保留原始 query string、response body、status、content type，不做資料轉換、fallback、快取或重試：
  ...
  - `GET /api/public/exchange-rate/usd-twd` → `http://bff:8080$request_uri`（local-only，不進 Tailscale Serve）
...
- [ ] **遠端安全邊界為 path-scoped Tailscale Serve，不使用 root proxy、OAuth2 或自簽憑證**：主機安裝 Tailscale 並登入 tailnet 後，先讀取 `tailscale serve status --json`；只有空設定或 handler 集合精確等於本任務管理的四條 path 時才能 `tailscale serve reset`。若有任何其他 port/path/service handler，須列出後停止，不得靜默覆寫使用者既有 Serve 設定。清理後分別以 `tailscale serve --bg --https=9090 --set-path=<exact-path> http://127.0.0.1:9090/<同名 exact path>` 掛載 `/api/quotes`、`/api/quotes/one`、`/api/public/market-index`、`/api/assets/latest`；每條都必須 `--bg` 才會立即返回並持久化，target 必須包含同名 path 才能保留 upstream request path。實際 CLI 若不支援 `--set-path` 必須 fail closed、不建立 remote Serve；不得另猜 listener、改用 root proxy 或另開不同遠端 port。禁止把 `/` 或 `/api/` 掛到 `http://127.0.0.1:9090`；**不得掛載 `/api/public/exchange-rate/usd-twd`**，因此 tailnet 對該 path 必須 404。不得啟用 Tailscale Funnel、路由器 port-forward、公開 Internet listen、Nginx 自簽／私鑰或額外 OAuth proxy。TLS 憑證與裝置／使用者身份由 Tailscale 管理；多人 tailnet 應以 grants/ACL 只授權需要的 identity。若 Tailscale 未登入或 HTTPS 尚未啟用，Docker gateway 仍只能由本機 loopback 呼叫，不得退化為 `0.0.0.0:9090`。
- [ ] **Tailscale 設定不得把憑證或 auth key 寫入 repo**：提供可重複執行的主機端設定腳本，先檢查 `tailscale` CLI 與登入狀態，再讀 machine-readable Serve 現況確認所有權；其後於任何 reset 前依序完成五支本機 exact GET 的 endpoint-aware 健康檢查：quotes 必須是非空 array 並以首筆合法 `stockCode/market` 驗 quotes/one，market-index 必須有非空且等長的六個圖表陣列，assets/latest 與 USD/TWD 必須各自通過已核准的 status/content-type/payload 契約。quote list 無 sentinel 或任一支未健康都須明列原因並 fail closed，不得 reset/configure。因 preflight 可能耗時，`serve reset` 緊前必須再次讀 status、重驗所有權，並將解析後的 canonical JSON 與最初狀態比較；任何變動都停止，不得 reset。只有兩次狀態相同且五路全通過時，才可安全 reset 並設定四個 path-scoped background mounts。設定中途失敗的 cleanup 也須先重讀現況：只有空設定或本輪四條 handler 的安全子集合／精確集合、且所有 target 完全相符時才可 reset；若含任何其他 handler 或無法確認，須列出並留待人工處理，不得自動刪除。最後以 machine-readable status 證明沒有 `/`、`/api/` 或 USD/TWD handler。腳本不得接受、產生或保存 reusable auth key，不得修改 tailnet ACL，不得呼叫 Funnel。安裝／登入／啟用 Serve 需要 macOS 管理員核准或瀏覽器互動時須明確停止並由使用者完成。
...
- [ ] **API 本身不另要求呼叫者登入**：五條 API 都是具名唯讀資料；`/api/assets/latest` 固定由 BFF/business 的可信主要管理者設定決定 owner，不接受匿名 request 提供的 owner selector 或 `X-User-*`。前四支外層以 tailnet membership/grants、TLS、loopback bind 與 Nginx allowlist 保護；USD/TWD 以 loopback-only bind 加 exact allowlist 保護，絕不由 Tailscale Serve 掛載。因此 gateway 不新增 Google OAuth2 flow、API key 或 `X-Internal-Service-Token`。BFF 網頁原有 Google OAuth2 不變，Treasury 維運觸發端點原有 token／ADMIN 防護也不變且不經 gateway 暴露。
