...
**Task 351 現行年份契約（覆寫本 Requirement 早期「任意年度查詢／指定單一年度匯出」敘述；Requirement 55 的 JSON＋Excel 雙格式契約仍完整保留）：**
...
- [ ] **未知日曆不得猜成交易日或休市**：BFF 必須以各市場該年度假日 map 是否非空判定 `availability`；整個 holidays downstream 失敗時三市皆 `UNAVAILABLE`。特別是 TWSE 尚未公告明年或 external-materials 失敗時，`tw={}` 代表未知、不是「全年無假日」。external-materials 的 `fetchTwHolidaysFromTwse(year)` 必須改用 TWSE 官方可查歷年的 `https://www.twse.com.tw/holidaySchedule/holidaySchedule?response=json&date={year}`，並驗證 HTTP／`stat` 成功、頂層 `date` 與 ROC 年 `title` 均屬要求年份、每筆資料日期也屬要求年份；錯年、空資料或解析失敗一律回空，不得沿用目前只回當年度且無年份參數的 OpenAPI 猜去年。既有 `GET /internal/tw-holidays` 再改接 `MarketDataFetchService.getTwHolidaysKnown(year).orElse(Map.of())` 的同等 fail-closed 語意（不新增另一支 business API）：只有 TWSE 年度 base 非空且 operator closure calendar 已知，才可回非空 map；首次空結果不得毒化後續讀取，TWSE base 空但只有零星 closure 時仍須回空。`TradingCalendarView` 對 unavailable 市場的日格旗標設 `null`、不顯示交易圖示，並在日曆上方顯示「該市場年度日曆尚未取得」警示；不得沿用 `!emptyMap.containsKey(date)` 把所有平日畫成交易日。`dayClass` 也只有在 `tw/us/uk` 三值皆為明確 boolean 時才可判為「三市同交易」或「假日／休市」；任何 `null` 都必須維持 neutral／unknown，不得把未知畫成全市場休市。美／英雖為本地規則計算，若 BFF 實際回空也同樣 fail closed。
...
- [ ] **測試與年度邊界**：以 fixed clock／明確 anchor year 驗證三年查詢範圍、範圍外 400 且零 downstream、TWSE 歷年端點可解析去年／今年且錯年 payload 被拒絕、明年空 map 為 unavailable 且不得產假台股旗標／檔案、首次空後可重新 fetch 恢復、TWSE base 空但 closure-only 非空仍 unavailable、closure calendar unknown 仍 fail closed、非法子路徑 400 且零檔、今年＋明年的排序、平年／閏年天數、四個檔名、任一年度例外仍嘗試並回報另一年、手動與排程共用雙年度路徑、Drive 完整時四次／單年部分失敗時該年零次且另一年兩次，以及狀態保留年份與格式。前端測試必須驗證全 unknown／部分 unknown 均不產生「三市同交易」或「假日／休市」class、available → unavailable → cached available 與反向切換均讀正確逐年狀態、authority 失敗顯示年度與原因且沒有成功標記或磁碟／權限誤診、非台北瀏覽器時區下「今天」仍回伺服器年度，以及 Nginx exact 240 秒與 Axios 250 秒；`frontend/package.json` 的 `test` script 必須實際納入新增測試，不得只新增未執行檔。前端 build 後在實際 Docker 頁面確認年度選擇器只有三年、兩端月份按鈕會 disabled、去年與今年台股 authority 為 available、不可用市場警示、匯出對話框無可編輯年度且列出今年／明年。external-materials、backend、BFF、frontend 都須納入模組測試；Docker 必須 rebuild／recreate `external-materials-service`、`business-services`、`bff`、`frontend` 並在上游重建後重啟 BFF。無新增資料表或 Liquibase changeset。
...
- [ ] **各銀行完整交易時段每 2 秒 tick，交易日未知即 fail closed**：`ExchangeRatePoller` 新增全天 `@Scheduled(cron = "*/2 * * * * *", zone = "Asia/Taipei")` 的 USD/TWD 專用 job，方法內才依來源 session 決定是否外呼。台銀為本行營業日 `09:00:00–15:30:00` 與 `16:30:00–23:00:00`；兆豐為本行營業日 `09:00:00–15:30:00`，以及「本行營業日 `16:30:00` 至次一營業日 `08:00:00`（含中間例假日）」。session policy 以 `Asia/Taipei`、可注入 `Clock` 與 `MarketCalendar.isTwTradingDayKnown()`／等價 `Optional<Boolean>` 計算；既有 TWSE 年度休市表加 operator 維護的 `tw_market_closure` 是保守專案代理，不得宣稱銀行官方 authority。解析 session 所需的任一日期若 authority unavailable/unknown，所有來源都 fail closed；current date 已知休市時，不得開啟當日台銀、一般盤或新的兆豐夜盤，但不得中止由前一已知營業日 `16:30` 已開啟、持續至次一已知營業日 `08:00`（含中間週末／休市日）的兆豐夜盤。啟動時 DB/TWSE transient failure 由 FX known path 每年份 30 秒節流重試，恢復前仍 fail closed，且不得改變既有台股路徑對空結果的快取／退化。禁止 weekday fallback 或 production force-open bypass。
