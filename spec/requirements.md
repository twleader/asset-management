...
- [ ] ~~**每日排程自動匯出（per-user，每日單一時間）**~~：**已由 Requirement 72 取代為多時間點**（比照 Requirement 69 對 Requirement 34 的取代）。原條文：使用者可在已實現損益頁「排程自動匯出」設定卡開啟每日排程，設定每日執行時間（時:分）與輸出資料夾，系統於該時間把該使用者的已實現損益匯出成 `.xlsx` 到指定目錄；每日固定一個時間。Requirement 72 落地後為「一份共用設定＋多個各自 guard 的每日時間點」，`realized_gain_export_schedule` 的 `run_hour`／`run_minute`／`last_run_date` 只留作舊 image rollback shadow，不再是排程來源。「不設交易日閘門」（已實現損益僅於有成交時異動，每日留存即可）仍然成立。
...
- [ ] **排程執行機制與自癒（比照 Requirement 34／37）**：每分鐘 `@Scheduled` poll（`zone=Asia/Taipei`），以 `now >= 設定時分` ＋ `last_run_date` 當日 guard 判斷（非「分鐘精確相等」，避免排程執行緒被長工作卡住跨分鐘導致整日靜默漏跑）；服務重啟以 `ApplicationReadyEvent` 補跑當日已到點未執行者；`AtomicBoolean` 防重入；單一使用者失敗只記 `last_run_status`＋log、不影響其他使用者（成功或失敗都設當日 guard，避免整天每分鐘重試）。
  > ⚠ **Requirement 72 起部分修訂**：「設定時分」改為多個 child 時間點；當日 guard 改為逐 child 的 `realized_gain_export_schedule_time.last_run_date`（parent 的 `last_run_date` 只同步 rollback representative）；`AtomicBoolean` 改為 `tick` 與 `ApplicationReadyEvent` 共用同一個 `tryRunDueExports` 入口。每分鐘 poll、`now >=` 判斷、失敗只記狀態不影響他人、成功失敗都設 guard 等其餘語意不變。
...
### Requirement 72: 已實現損益每日匯出支援多個執行時間
...
>
> **編號說明：** Requirement 70／Task 327、Requirement 71／Task 329 與 Task 328 已先行使用；本需求使用 Requirement 72／Task 330。本需求**只改已實現損益一頁**，不得順手把交易日曆（R37）／油價金價（R41）／匯率（R42）等其餘**仍為單時間**的匯出頁一併改為多時間；也不得去動已各自有多時間／多列模型的 GDP-TWSE（R45，`index_export_schedule_time`）、交易雷達（R48，`trading_radar_export_time`）與交易紀錄（R49，Task 255 起每人多列排程）。
...
- [ ] **排程列表與文件同步**：此功能只改既有 `RealizedGainExportScheduleService` 每分鐘 poll 的資料驅動時間，不新增／不移除 `@Scheduled` annotation，故「排程列表」維持同一筆「已實現損益匯出」且總筆數（51 筆）與 annotation 計數不變；只把該筆描述更新為「每分鐘檢查已實現損益頁設定的多個每日時間」。Requirement 39／design 的單一 `run_hour/run_minute/last_run_date` 現況描述須就地註記由 Requirement 72 取代，避免後續照舊模型維護。
