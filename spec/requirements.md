...
- [ ] **濾除「中國＋他國國名」的個人刑案雜訊（Task 321）**：使用者於 2026-08-12 回報一則實際爬進 `news_headline` 的雜訊「扯！南韓養老院中國女看護 用腳踹輪椅老者致死」，並給出**三個並列判準**——「影響不了兩國外交關係，也沒有對社會造成重大衝擊，更不會影響財經、股市的新聞，要過濾」（三者皆不滿足才濾）。該則實測判定為 `KEEP:china-regime`。
    ...
    - **修法**：規則③把 `GEO_REGION` 這一個 disjunct 拆出並加守門——個人刑案（`PERSONAL_CRIME` ∧ ¬`PERSONAL_CRIME_ESCALATION`）不得單靠裸國名保留，須另有 `GEO_TRIGGER`，即**退回規則④對同一份 `GEO_REGION` 採用的標準**；命中者判新 label `DROP:china-personal-crime`。另三個 disjunct **完全不動**，既有 label `KEEP:china-regime`／`DROP:china-nonfinance` 的字串與適用情境不變。三層守門正對應使用者三判準：(c) 由規則①`FINANCE` 承接（在規則③之前）、(a) 由另三個 disjunct 加 `GEO_TRIGGER` 加 `PERSONAL_CRIME_STATE_ACTION`（遣返／脫北／難民／庇護／究責／譴責／召見／交涉／領事，共 9 詞）承接、(b) 由 `PERSONAL_CRIME_ESCALATION`（恐攻／恐怖攻擊／恐怖襲擊／恐怖組織／恐怖分子／恐怖份子／人質／挾持／無差別／隨機殺人／隨機砍人／屠殺／暴動／騷亂／戒嚴，共 15 詞）承接。
