...
- [ ] **`model` 欄位記錄檔位與版本**：`portfolio_advice.model` 在 `local` 檔位寫 `local-allocation:v1`；`hybrid` 檔位寫 `hybrid-allocation:v1+<實際 Claude model id>`（例如 `hybrid-allocation:v1+claude-haiku-4-5`），使歷史列表可分辨每一筆是哪個檔位、以及 hybrid 用了哪顆模型。配置模板調整時須提升版本號（同 Requirement 78 對 `local-rule-engine:v1` 的理由）。
