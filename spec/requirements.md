...
- [ ] 第三張卡（每日收盤日線圖）支援**市場切換**（一次顯示一個指數，沿用同一套「收盤＋週線 MA5／月線 MA20／季線 MA60／年線 MA240」五線版型與區間切換鈕；Task 285 前為四線）：頂部下拉選單可在「台股大盤 / 道瓊工業 / 標普 500 / 那斯達克綜合 / 費城半導體 / 英國富時 100 / 德國 DAX / 韓國 KOSPI / 日經 225」九者間切換；卡片標題、空狀態提示文字隨選取指數動態變化
  ...
  - BFF 將原 `.../twse-daily`、`.../refresh-twse-daily` **一般化**為 `GET /api/bff/gdp-twse/index-daily?market=&years=10` 與 `POST /api/bff/gdp-twse/refresh-index-daily?market=&years=10`（`market=TWSE` 走台股大盤、其餘走對應美股指數）；回傳格式（dates/closes/**ma5**(Task 285 新增)/ma20/ma60/ma240，**Task 288 起再加 volumes/turnovers/hasVolume**）與 MA 計算對兩市場完全相同，由同一支 BFF 服務（同義欄位同一來源），確保版面一致
...
- [ ] **9 個市場全部可查**：`market` 接受 `TWSE`、`DJI`、`SPX`、`IXIC`、`SOX`、`FTSE`、`DAX`、`KOSPI`、`N225`；省略時預設 `TWSE`，輸入先 `trim` 再轉大寫。回應須帶正規化後的 `market` 與中文 `marketLabel`，並以 `supportedMarkets: [{value,label}, ...]` 列出完整選項。未知代碼回 HTTP 400，不得把它當成海外指數查空資料後回 200。
...
- [ ] **收緊判準後不得放大外部請求量**：現行 `refreshAll(...)` 一次對 8 檔海外指數 ＋ SP500TR 各發一次 Yahoo `range=10y`。判準由 4 日容忍收緊為逐盤後，觸發頻率會明顯上升（凌晨重啟、來源延遲發布時的兩次補救檢查都可能觸發）。故自癒與補救檢查觸發時**只回補判定為落後的那些美股指數**，其餘指數維持既有節奏（仍由每日 07:00 全量回補涵蓋）；既有的指數間 500ms 禮貌間隔不得移除。
...
- [ ] **回補行為本身不變**：仍走既有 `MacroHistoryService.refreshUsIndexDaily(code)`（與前端「回補日線（10 年）」按鈕同一條路徑、Yahoo `range=10y` 一次抓整段、idempotent upsert），仍保留指數間的禮貌間隔與既有的含息報酬指數、TWSE 報酬指數增量收尾步驟。本 Requirement **不改**交易雷達的 stale 判定、不改任何規則參數、不改 `TradingRadarRuleEngine.RULE_VERSION`。
