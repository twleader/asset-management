...
- [ ] **來源是具名例外的單次、同時間點 Yahoo 台股 quote snapshot，不拼接不同時刻：**使用者第一次切入「行情五檔」或按該頁籤的「重新整理」時，前端呼叫 `GET /api/bff/stock-analysis/quote-detail?code=...&market=台股`。資料由 `external-materials-service` 在 request time 抓取 `https://tw.stock.yahoo.com/quote/{code}.TW` 的 server-rendered `quote.data`；同一份 snapshot 同時提供摘要、內外盤與 orderbook，禁止拿既有兩分鐘 Redis 的成交價再混 Yahoo 五檔，避免不同來源時間對不起來。這是 `CLAUDE.md` 與 `spec/steering/structure.md` 登記的「登入後行情五檔展示 snapshot」限縮例外，不是本系統權威即時價；不得供估值、損益、下單、警示、SSE、`/api/quotes*`、9090 公開 API 或其他 consumer 使用，也不得寫 Redis／DB或背景輪詢。切換走勢期間、指標或其他頁籤不得觸發此抓取；同一次開啟已成功載入後再切回沿用該 snapshot，只有重新整理才重抓。
...
- [ ] **不在本次範圍：**不做逐筆成交明細、券商分點、零股五檔、自動下單、WebSocket push 或背景輪詢；不把 Yahoo snapshot 寫入 Redis／DB；不擴充美股／英股 order book；不更動既有 `/api/quotes` 公開契約；不把內外盤比例解讀為買賣建議。
