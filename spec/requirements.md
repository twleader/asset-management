...
### Requirement 87／Task 348: 股票分析 popup 新增台股行情五檔頁籤
...
> **後續覆寫：**Requirement 108／Task 372 唯一放寬本需求「不進 `/api/quotes*`／9090 公開 API」的限制：同一份 typed 五檔 snapshot 可作為公開回應內 `marketData.quoteDetail` 的唯讀市場資料投影。此覆寫不改變本需求的其餘界線；該 snapshot 仍不得供估值、損益、下單、警示、SSE、Redis／DB 寫入或任何個人資產判斷使用。
...
- [ ] **來源是具名例外的單次、同時間點 Yahoo 台股 quote snapshot，不拼接不同時刻：**使用者第一次切入「行情五檔」或按該頁籤的「重新整理」時，前端呼叫 `GET /api/bff/stock-analysis/quote-detail?code=...&market=台股`。資料由 `external-materials-service` 在 request time 抓取 `https://tw.stock.yahoo.com/quote/{code}.TW` 的 server-rendered `quote.data`；同一份 snapshot 同時提供摘要、內外盤與 orderbook，禁止拿既有兩分鐘 Redis 的成交價再混 Yahoo 五檔，避免不同來源時間對不起來。這是 `CLAUDE.md` 與 `spec/steering/structure.md` 登記的「登入後行情五檔展示 snapshot」限縮例外，不是本系統權威即時價；不得供估值、損益、下單、警示、SSE 或其他 consumer 使用，也不得寫 Redis／DB或背景輪詢。Requirement 108／Task 372 是唯一巢狀公開覆寫：同一 typed snapshot 可僅作 `/api/quotes*` 回應的 `marketData.quoteDetail` 市場投影，不能讀／回傳任何個人資產資料，也不能拿來作任何決策或其他 consumer。切換走勢期間、指標或其他頁籤不得觸發此抓取；同一次開啟已成功載入後再切回沿用該 snapshot，只有重新整理才重抓。
...
- [ ] **不在本次範圍：**不做逐筆成交明細、券商分點、零股五檔、自動下單、WebSocket push 或背景輪詢；不把 Yahoo snapshot 寫入 Redis／DB；不擴充美股／英股 order book；不更動既有 `/api/quotes` 的 raw 19 欄 Redis 契約（Requirement 108／Task 372 僅例外允許其對外回應內的 `marketData.quoteDetail` 市場投影）；不把內外盤比例解讀為買賣建議。
...
- [ ] **唯讀資料邊界與來源責任。** top-level 19 欄仍以 `external-materials-service` 的 `PriceCacheReader` 原始 Redis 讀值為唯一來源，不得改用 display-price fallback。走勢圖只讀既有 history／indicator API，禁止觸發 `backfill-stock`。新增 container-only exact backend `GET /internal/public-market-data/dividends-readonly-result?code=&market=&years=`：保留既有 code／market 驗證、限制 `years=1..10`，只回 `DividendHistoryService.findFromDbReadOnly` 的完整 `DividendHistoryResult`；public BFF 固定傳 `years=10`。它刻意不用 `/api/market-data/**`，避免 `MarketDataBffRoutes` 將它 proxy 給已登入 frontend；不得新增 BFF route/controller，frontend Nginx 也不得 proxy `/internal/**`。既有 `/api/market-data/dividends-readonly` 的裸 `List<DividendRow>` 契約不得改動，避免破壞績效比較 consumer；新 endpoint 不進 Nginx／Tailscale allowlist、DB 無資料只回既有空 envelope，絕不 cold-sync／project／寫入。
  ...
  BFF 的 public quote aggregation 對 raw external 與上述 public-market business endpoints 都使用不帶 `TenantIdentity`／`X-User-*` 的專用 container-only WebClient，且只允許呼叫此需求列出的市場端點；不得以 configured admin 或任一使用者 context 補資料。這兩個 backend bridge 是精確的第二組 no-AdminGate 例外：不得把任一 exact GET 加入 `AdminGateInterceptor.addPathPatterns`，更不得以 `/internal/public-market-data/**` 建立寬鬆 bypass；它們只讓這個具名 BFF outbound client 在 asset-net 內取得無個人資料的市場結果，BFF 沒有對 browser 的 inbound route／controller／proxy 可轉送它們。ETF 成分股與 Yahoo 台股五檔可以做 request-time 市場讀取，但不得寫 Redis、DB、SSE、排程或觸發下單；五檔資料只作 `marketData.quoteDetail` 顯示，仍不得反向作估值、損益、交易、警示或個人資產決策。
- [ ] **部分資料盡量回、不可遮蔽有效報價。** 原始 quote 成功時，任一 `marketData` 子來源的 timeout、schema drift、unsupported market、空歷史或外部服務失敗，都只能讓該子區塊回前述 fixed typed unavailable／空資料與一般化 message，不能讓 19 欄有效 quote 消失、不能令同一清單的其他股票消失，也不得洩漏 upstream URL、HTML、stack trace 或例外文字。每筆 raw row 在開始 detail IO 前就要有完整的 default fallback `marketData`；global deadline 或取消時，尚未完成的 child／row 必須用該 fallback 合併回原 row，不得從 list 遺失。非台股與 `0000` 的五檔必須為 `supported=false, available=false` 且零 Yahoo 外呼。原始 price cache 本身 miss 仍只有 single endpoint 的 204；原始 list 讀取失敗的既有 gateway 失敗語意維持，不得捏造空的 200。
- [ ] **清單負載必須有界、可量測且維持順序。** `/api/quotes` 對每個 raw row 做相同市場投影時，最多 8 檔同時聚合，最終依 `PriceCacheReader.listAll` 原順序輸出；不得對 42+ 筆快取標的無上限 fan-out，也不得因一筆 Yahoo／ETF／歷史延遲阻塞或取消其他已可回覆的 row。每一列的 chart-series、intraday、quoteDetail、etfConstituents、dividends 必須並行啟動，無 retry；public defaults 固定為 raw quote 3 秒、chart-series 7 秒、quoteDetail 7 秒、ETF 5 秒、dividends 3 秒、intraday 2 秒，列級 deadline 7 秒。整支 request 以 controller 收到時起算 50 秒 global deadline（含 raw quote 讀取）；到時取消尚未完成的 enrichment、以預建 fallback 補齊所有 raw row 後立即 serialise。所有 deadline 值應可由受限 properties 覆寫供測試，但 production default 不得放寬；這保留至少 10 秒於 Nginx `proxy_read_timeout 60s` 的前後緣。測試必須以可控慢上游驗證 per-row child parallel、8 的並行上限、50 秒 deadline（可縮短設定值）、原始 row 不遺失與無 retry。
