...
> **後續覆寫（Requirement 108／Task 372）：**本條「不增加第二個新欄位」只鎖定 `PriceCacheReader.LatestQuote`、其 Redis payload 與 external raw source；它們仍固定為 19 欄，名稱、型別、nullable 語意、順序與讀取行為皆不變。Task 372 的**對外 host 回應包裝層**才可在這 19 欄之後增加唯一的 `marketData`；它不寫回 raw source，也不改任何 raw 值。該包裝層另因 `start`／`end` 驗證可回文件化的 400；single raw cache miss 的 204 與 raw list 空陣列語意不變。
...
> **後續覆寫（Requirement 108／Task 372）：**本條的「response shape 與 HTTP status 不變」是 raw Redis quote 讀取契約；目前 9090 host 端的兩條同名 exact GET 由 BFF 裝飾為 19 欄 raw quote 加唯一 `marketData`，並接受 `start`／`end` 的文件化 400。它仍是純讀：不觸發 quote producer、不寫 Redis、不 publish；raw one miss 204、raw list 空 `[]`、19 欄順序與原始值皆不變。
...
> **歷史公開投影註記：**Requirement 108／Task 372 當時放寬本需求「不進 `/api/quotes*`／9090 公開 API」的限制；現行 `marketData.quoteDetail` 的公開唯讀投影與 DB→Redis producer 例外均由 Requirement 109／Task 373 定義。這段不再限制五檔 producer 寫 PostgreSQL／Redis；snapshot 仍不得供估值、損益、下單、警示、SSE 或任何個人資產判斷使用。

**Task 348 歷史實作紀錄（非現行 Acceptance Criteria）：**以下 checklist 保留當時 UI 與 Yahoo 方案的追溯資料；其中任何 Yahoo、request-time、`YAHOO_TW`、不寫 DB/Redis、12 秒 timeout、HTML parser 或相應 Docker 驗收，皆不得用於目前實作。現行可執行資料契約是 Requirement 109／Task 373。
...
- [ ] **相容的兩支既有路徑，不新增第十條 9090 path。** `GET /api/quotes/one?code=&market=` 維持 `code`／`market` 必填、價格 Redis miss 維持 `204 No Content`；成功時既有 19 個 top-level `LatestQuote` property 的名稱、型別、nullable 語意與數值均不變，property 宣告順序與 raw record 一致，僅新增固定的 top-level `marketData` property。`GET /api/quotes?market=` 維持 `market` 選填、三市場／指定市場篩選、原本 Redis index 順序與空快取回 `[]`；回應仍是 array，且每一個原報價 row 在原有 19 欄之外新增同一個 `marketData` property。不得把原報價包進新 `quote` object、不得刪除或改名任何既有欄位、不得把清單改成 object。
