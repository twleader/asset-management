...
- [ ] **`RULE_VERSION` 與通知基準的條件式 V13 發布**：t274 不單獨升版。t274／t275／t309／t307 全部完成後，只有 t308 所有欲發布 production key 的 required-horizon holdout／walk-forward gate 全部通過，且另有一次明確 production 發布核准時，backend/DTO/frontend/tests 才一次升為**當時的下一個 production 版號**。**此處刻意不寫死版號字串**（Requirement 82／Task 341 起）：`RuleParameters.V13_VERSION = "TW_RULES_V13"` 是 **candidate 參數集**的標籤，與 production 的 `RULE_VERSION` 是**兩個獨立的命名空間**，`TW_RULES_V13` 永久保留給前者、production 不得竊用。production 已於 Task 341 因美股量價接線升至 `TW_RULES_V14`，故本條原記載的「升為 `TW_RULES_V13`」已不可實作（會讓發布版號低於當時的 production）。任一 key 為 `REJECTED`／`INSUFFICIENT`，或只有 `PROMOTED_CANDIDATE` 但尚未取得發布核准時，均維持**當下的 production 版號不動**。實際 V13 發布後 `TradingRadarNotificationService` 仍以 rule-version mismatch 視為未初始化；部署首輪只建新 baseline 不寄信，須實機驗證。

  **同步點的數量與位置一律現場 grep 取得，本文件不再記載固定數字**（Requirement 82／Task 341 起；實測基準：`TW_RULES_V12` 為 10 檔 19 處，早已不是本條原記載的「五處」）。**且字面值 grep 並不完整**——另有以常數比對耦合到版號的位置（`grep -ranE "V12_VERSION|v12Default" backend/src`，實測 25 處），`TradingRadarV13ActionPolicyTest` 與 `TreasuryYieldServiceTest` 這兩支升版後會紅的測試只會出現在後者。Task 298 的同步點記錄早已寫過同型教訓：「原 5 檔 7 處，全數處理，另補 2 處 grep 抓不到的方法名」。**若日後做跨版本全域字串替換，`backend/src/main/resources/db/changelog/` 的 `--comment` 絕不可改**（Liquibase checksum 含註解，改了會 `ValidationFailed` 並讓 business-services 進入 crash loop）。
...
### Requirement 82: 交易雷達美股大盤接上量價環境因子——消除「畫面顯示量比 0.79、風險提醒卻說資料不足」的自相矛盾
