...
- [ ] **`RULE_VERSION` 與通知基準的條件式 V13 發布**：t274 不單獨升版。t274／t275／t309／t307 全部完成後，只有 t308 所有欲發布 production key 的 required-horizon holdout／walk-forward gate 全部通過，且另有一次明確 production 發布核准時，backend/DTO/frontend/tests 才一次升為 `TW_RULES_V13`。任一 key 為 `REJECTED`／`INSUFFICIENT`，或只有 `PROMOTED_CANDIDATE` 但尚未取得發布核准時，均維持 `TW_RULES_V12`。實際 V13 發布後 `TradingRadarNotificationService` 仍以 rule-version mismatch 視為未初始化；部署首輪只建新 baseline 不寄信，須實機驗證。
