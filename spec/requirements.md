...
- [ ] **債券使用美債殖利率而非只看股票技術指標**：Requirement 58／Task 275 的 M3/Y5/Y10/Y30 歷史、as-of lag、控制 USD/TWD 的經驗利率敏感度與 BOND-only contribution 全部落地。freshness 只按權威美股交易日曆計 completed sessions，長假不以日曆天誤判 stale；任一要求日期的日曆狀態為 UNKNOWN 時整筆 rate evidence=`STALE/UNKNOWN_CALENDAR`，不得回退 weekday 猜測；`curveDate` 晚於 decision 當時 latest completed session 時須 `STALE/FUTURE_CURVE_DATE`，不得以 lag 0 冒充 fresh，且仍保留 batch/provider/manifest provenance。只有樣本數、符號、穩定度與 holdout 門檻通過的標的才啟用；其餘為 `null` 並揭露原因。利率與 FX 量測不同風險，不可互相取代。
