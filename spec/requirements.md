...
- [ ] **檔案粒度＝當日一檔、每次觸發全量重寫**：檔名 `alert_triggers_{ownerUserId}_{yyyyMMdd}.json`，內容為**該 owner 當日所有觸發**的陣列；同日多次觸發覆寫同一檔、跨日新檔（與既有九頁的檔名語意一致）。**不得用 append**：append 是 read-modify-write，同一檔可能被兩條路徑並發寫入而互相截斷，且中途失敗會留下半截 JSON；全量重寫天然冪等。檔名含 `ownerUserId` 的理由同 Requirement 51——多使用者共用同一個子路徑時不互相覆蓋，**不得以「實務上只有一人啟用」為由省略**。
- [ ] **「當日」一律以台北日期界定，不可用 `triggered_at`**（本需求最容易寫錯的一條）：`stock_alert_trigger` 有兩個時間欄，語意不同——`triggered_at` 存的是**市場牆鐘**（Requirement 53 明列的唯二例外之一：美股存紐約時間、英股存倫敦時間），`created_at` 存的是**台北牆鐘**。檔名日期與「當日內容」的篩選**都必須用 `created_at`**。若用 `triggered_at`：台北 2026-07-30 凌晨 01:00 觸發的美股警示，其 `triggered_at` 是 2026-07-29 13:00（紐約），會被歸進**前一天**的檔案——檔名日期與檔案實際產生日期分家，下游依日期取檔的程式會抓不到剛剛那一筆。JSON 內容中兩個時間欄**都要輸出**並標明時區語意，供下游自行判斷。
...
- [ ] **「立即匯出」按鈕（驗證用）**：提供 `POST /api/stock-alerts/export-setting/run-now`，以當前使用者身分把**當日已發生的觸發**重新產檔並（啟用時）上傳 Drive，回報本機落點與 Drive 落點。理由同 Requirement 51：Drive 設定對不對，使用者必須能在不等下一次觸發的情況下驗證。**當日尚無任何觸發時**須寫出一個 `triggers` 為空陣列的合法 JSON 並明白回報「當日尚無觸發」，不得回 404 或靜默不產檔——使用者按下按鈕的目的是驗證落點，空檔案同樣達成該目的。
...
- [ ] **測試**：（a）**當日邊界**——`created_at` 為台北當日、`triggered_at` 為紐約前一日的美股觸發，必須出現在台北當日的檔案裡（這是最容易寫反的一條）；（b）**owner 隔離**——A 的觸發不得出現在 B 的檔案，且 join 兩路徑（`alert_id` 與 `group_id`）都要涵蓋；（c）**群組觸發**的條件文案為 `buildGroupLabel` 的合併結果（`" 且 "` 串接）而非單條；（d）**未啟用**時完全不產檔；（e）**匯出失敗不影響** `stock_alert_trigger` 寫入與 email enqueue；（f）**Drive 合併去抖**三點缺一不可——(f1) 60 秒內連續 N 次觸發只呼叫一次 `copyTo`；(f2) **尾端補跑**：兩次觸發相隔 5 秒、之後不再有任何觸發，間隔到期後**必須**發生第二次 `copyTo`（這條是上面那個資料遺失寫法的唯一探針，缺了它壞實作也會全綠）；(f3) 被合併的那幾次 `gdrive_last_run_at` 與 `gdrive_last_status` **值保持不變**（不只是「不得寫成失敗」）；（g）rclone 一律以 `RcloneClient` 介面替身注入、不實際連網；（h）既有警示觸發行為（`last_triggered_*` 覆寫、email、24h cooldown）須有回歸測試確認未被改動。
