...
- [ ] **canonical DB 先於 Redis。** 新增 `stock_intraday_order_book` header 與 `stock_intraday_order_book_level` 五列子表，以 `(stock_code, market)` 保存一份最新 snapshot。header 的 stock code、market=`台股`、台北交易日、`source_updated_at`、`fetched_at`、source=`FUBON_BOOKS`、市場狀態及 actual price／昨收均為 `NOT NULL`；它保存同來源 OHLC／平均價／成交額／量。levels 固定保存 1..5 的 bid/ask price／lots，且每一側的 price/lot 必須同時為 null 或同時有效。schema 必有 composite FK、五檔 level、台股 market、價格正值／量非負、來源、台北日期與 paired side 等 CHECK，並同步重產 `db/schema.sql`。同一 transaction 以 `ON CONFLICT ... WHERE EXCLUDED.source_updated_at > stored.source_updated_at` 寫 header、替換五列並更新有效 stock name；equal／older 對 header、levels、name、fetched time 一律零 mutation。失敗時保留原 snapshot，且 Redis 零 mutation。

- [ ] **專用 Redis key 與單調修復。** DB 回傳 APPLIED 或既有 canonical row 後才可對 `price:quote-detail:{market}:{code}` 執行 dedicated atomic strict-newer 寫入，TTL 固定 24 小時。此 key 的 payload 必含可驗證的 source microsecond ordering token；同時間或較舊 snapshot 不可覆蓋、不可重設 TTL。DB 已接受而 Redis 失敗時，DB canonical row 必須保留，後續 round 只能使用該 canonical row 修復專用 key。任何五檔結果都不得寫 generic `price:{market}:{code}`、`price:index:*`、ticks、day-H/L 或 `price-update` publish。

- [ ] **查詢永遠 pure read。** `external-materials-service` 的既有 exact `GET /internal/quote-detail?code=&market=` 先讀 dedicated Redis，再以 PostgreSQL canonical snapshot fallback；不得在 request time 呼叫富邦、Yahoo、dispatcher、worker 或 writer，也不得修復快取。無 snapshot 時回既有 typed unavailable；`market != "台股"` 或 `code == "0000"` 在 external boundary 前回 `supported=false, available=false` 且零外呼。所有 supported-but-unavailable 結果（包括 cache/DB miss、business proxy transport/decode/5xx failure）固定 `source=null`、`marketStatus="UNKNOWN"`、`levels=[]` 與一般化訊息，絕不可暴露 `YAHOO_TW`。既有 business `GET /api/market-data/quote-detail`、登入 popup BFF 與 `PublicQuoteMarketDataService` 只 proxy 同一 pure-read result，保留完整 `QuoteDetailDto.Response` shape，將 production quote-detail timeout 收斂為 2 秒。回應 source 可用時固定為 `FUBON_BOOKS`；不得把 Yahoo 當 fallback。
...
**User Story：**作為呼叫既有 `GET /api/quotes` 與 `GET /api/quotes/one` 的批次市場資料 consumer，我希望不必理解前端畫面專用的巢狀結構，就能在每筆報價直接讀到同一份富邦最佳五檔、完整行情摘要及十年股利歷史（含年度小計）；資料仍必須是既有快取／資料庫的純讀快照，不得因 API 查詢而向富邦、Yahoo 或其他來源重新抓取。
...
- [ ] **直接行情與最佳五檔欄位。** 每個成功 row 必有 non-null 的 top-level `quoteDetail: QuoteDetail`，其值與同一 row 的 `marketData.quoteDetail` 完全相同，並保留現有的 `supported`／`available`、`source`、`message`、`sourceTime`、`fetchedAt`、`marketStatus`、成交／昨收／開高低／均價／漲跌／漲跌幅／總量／昨量／成交金額（億）／振幅／內外盤量與百分比／五檔小計／`levels`。每個成功 row 另必有 non-null top-level `bidLevels` 與 `askLevels`；每側都是最多五筆、順序固定由最佳價向外的 typed array，元素精確為 `{price: decimal, size: integer}`。只有 `quoteDetail.available=true` 且 `source="FUBON_BOOKS"` 時才可投影；從 `levels` 的買／賣各自取出正 price、正整數 lots 的 side，排除缺值或零量 level 後，買方必嚴格遞減、賣方必嚴格遞增、不得重複。任一側的投影若不符合上述順序，該側回 `[]`，不能輸出已知無效 book；不以 `buyPrice`／`sellPrice`、Yahoo、估算價或另一側資料補齊。五檔 unavailable、非台股、`0000`、source 非 `FUBON_BOOKS` 或 book 缺失時兩側都回 `[]`，但 typed `quoteDetail` 仍照既有 fallback 回傳；不得把 five-book failure 升級為 raw quote failure。
...
- [ ] **OpenAPI、測試與實機驗收。** `docs/openapi/docker-external-api.yaml`、9090 Swagger 摘要與 gateway OpenAPI assertion 必須明確列出新增 top-level fields、property order、required／nullable、`BookSideLevel`、`cashYieldPct`、`AnnualDividendSummary` 與所有 array items；不能用 loose object、任意 map 或從 schema 省略。BFF unit tests 必須覆蓋：(1) raw 19 欄和原 `marketData` prefix 保持、直接 `quoteDetail`／`dividendHistory` 與 nested 完全一致；(2) 完整 FUBON 五檔投影為正確 `{price,size}` 順序；(3) 非台股、source 非 FUBON、unavailable side 皆回空；缺值／零量 side 保留有序 partial array，亂序 side 回空，且不多打一個行情 request；(4) 多筆同年股利 event 的 event `cashYieldPct` 和年度加總／第一筆非 null 分母的畫面同口徑，及 empty／無分母 fallback；(5) list 順序與 raw one 204 不變、response 不含個人資產欄位。重建並 recreate `bff` 後，透過 `127.0.0.1:9090` 驗證一筆非 `0000` 台股單檔與 `market=台股` 清單皆有上述 top-level fields，`bidLevels`／`askLevels` 可供批次直接累計，且 9090 readback 的資料仍可追溯到既有 `FUBON_BOOKS` pure-read snapshot。
