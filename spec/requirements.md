...
- [ ] **上市／上櫃共用現行台股 2 分鐘 producer：**`PricePoller.scheduledTwIntradayUpdate` 維持 `@Scheduled(cron="0 0/2 9-13 * * MON-FRI", zone="Asia/Taipei")`，並繼續以 `MarketClock.isTwMarketOpen()` 當真實交易日與 09:00–13:30 守門。每輪以 `collectHeldStockCodes` 取「每位 owner 最新快照持股 ∪ stock_alert」的台股代碼，然後由這支排程在呼叫 `updatePrices` 前顯式 `tw.remove("0000")`；現行 collector 只在 `stock_alert` SQL 排除 `0000`，快照持股側沒有，故不得把排除誤寫成既有事實，也不得讓 `0000` 進個股 MIS。持股 schema 中上市與上櫃的 `market` 都是「台股」；`PriceFetchClient.getTwseRealTimePrice` 對每個代碼依序嘗試 `tse_{code}.tw` 與 `otc_{code}.tw`，故同一支排程必須同時覆蓋兩個掛牌所，不新增第二支 TPEX 個股排程。
...
- [ ] **只有真實成交價才寫 Redis：**上市或上櫃 MIS 回應只在 code 正確且 `z` 為可解析成交價時產生 `PriceResult`；`z='-'`、查無資料或 HTTP／解析失敗都回 `Optional.empty()`，不得以昨收 `y`、開盤 `o`、買價、賣價或中價充當最新成交價。有效結果繼續委派 `PriceCacheWriter.write(result,false)`，寫 `price:台股:{code}` 或 `price:美股:{code}` 及對應 index SET、TTL 24h，並 publish `price-update`；台股真實成交仍由 `IntradayTickStore` 累積。不建第二份 OTC Redis key、不直接寫 DB。
- [ ] **上櫃覆蓋必須有反回歸測試：**至少以可控 HTTP fixture 或抽出的 parser 證明 (a) `tse` 第一次查無時會繼續查 `otc`；(b) 合法 OTC 回應的 code/name/z/y/o/h/l/volume 能生成 `market="台股"` 的 `PriceResult`；(c) `PricePoller.scheduledTwIntradayUpdate` 取到該結果後呼叫既有 writer；(d) `z='-'` 零寫入且保留上輪 cache；(e) 台股與美股排程 reflection 契約皆為每 2 分鐘。不得為了寫測試而在正式碼新增測試專用 endpoint。
...
- [ ] **本任務自足新增 market-open＋provider timestamp原子寫入與一次性接管：**Python 不碰 Redis。Java provider把immutable observation連同`marketOpenAuthorized`交給唯一 `PriceCacheWriter` 新overload；該boolean只能在同輪`isTwMarketOpenKnown()`精確為`Optional.of(true)`時產生，任何legacy boolean clock、authority empty/throw都不得產生true。Redis Lua第一個守門就是該flag，不為true即回`MARKET_CLOSED`且對current missing亦不得SET／SADD／EXPIRE／PUBLISH。通過後才原子解析／比較current與incoming的`(tradingDate,updatedAt)`；current Fubon只有strictly newer才寫。一次性provider takeover另要求Fubon enabled、marketOpenAuthorized、本輪actual batch已驗，且incoming date較新或同日current精確為`source='TWSE',closed=false,quoteStatus='LIVE'`；其他來源、official close、non-LIVE或回退都拒絕。禁止JVM `GET→compare→SET`。既有provider路徑不擴張；契約與在途Task350相容但不依賴它。
