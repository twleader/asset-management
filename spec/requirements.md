...
### Requirement 123／Task 388–389：富邦「ETF 成分股持股明細查詢」排程串接，取代既有 MoneyDJ／Yahoo 爬蟲來源
...
- **Task 388（本次可完整落地，無外部依賴）：** `fubon-broker-service` 新增唯讀 endpoint、backend 新增資料表／排程／唯讀 client，把富邦回應**原封不動**以 JSON 文字存入資料庫（不臆測、不重新命名任何欄位），排程本身、feature flag、交易日判斷、雷達範圍查詢、單一 flight 防重疊、`SchedulePublicBffController` 登錄皆在此任務完成並可獨立驗收（可直接下 SQL 查詢確認資料表有落地資料、排程確實在 08:50／15:30 觸發）。
- **Task 389（依賴真實環境核實欄位，前置任務為 Task 388）：** 把 Task 388 存下的原始 JSON 解析成既有 `MarketDataService.EtfHoldingsResult`／`EtfHolding` 形狀、切換 `getEtfHoldings()` 讀取路徑、刪除 `external-materials-service` 內已死的 MoneyDJ 爬蟲程式碼、同步更新 `spec/tasks/t386_fubon_api_documentation_view.md` 的已串接狀態與計數。**此任務的欄位解析函式必須先在具備真實富邦憑證的環境核實 `ownership.etf_holdings` 實際回應欄位名稱後才能定案**；核實不到時，解析函式必須明確 fail-soft（回傳 `success=false`／`message="尚未核實回傳欄位格式"`），不得臆測欄位名稱或以猜測值蒙混過關。

#### Acceptance Criteria — Task 388（抓取與落地基礎設施）
...
- [ ] **Task 388 不在本次範圍。** 不解析 `raw_response_json` 內容（欄位名稱未核實，不得臆測）；不修改 `MarketDataService.getEtfHoldings()` 既有行為；不刪除 `external-materials-service` 既有 MoneyDJ／Yahoo 爬蟲程式碼；不新增手動觸發端點；不做任何下單、改單、圈存或轉帳類 API 呼叫；不影響既有富邦庫存同步／成交同步／TW LIVE 報價／大盤指數串流的邏輯或 feature flag。

#### Acceptance Criteria — Task 389（欄位解析、讀取端切換、既有爬蟲清理；前置任務 Task 388）
...
- [ ] **Task 389 不在本次範圍。** 不修改美股 ETF 成分股的資料來源或快取邏輯；不新增使用者可調整的排程時間或觸發按鈕；不影響既有富邦庫存同步／成交同步／TW LIVE 報價／大盤指數串流的邏輯或 feature flag；不做任何下單、改單、圈存或轉帳類 API 呼叫；若真實環境核實後發現 `ownership.etf_holdings` 回應形狀與既有 `EtfHolding(stockCode, stockName, weight, shares)` 四欄位無法對應（例如成分股清單改用完全不同的巢狀結構），允許在核實當下另行判斷是否需要調整 `EtfHolding` 形狀本身，但**不得**為了「先讓測試過」而虛構欄位。
