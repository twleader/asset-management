...
- [ ] **人工維護的靜態清單，涵蓋富邦官方 SDK（`fubon_neo` 2.2.9）已驗證存在的全部唯讀查詢方法，一律排除任何寫入／交易方法。** BFF 新增專屬本頁的 `FubonApiInfoBffController`，內建**人工維護**的靜態清單（不做任何動態掃描、反射、跨服務探索或對富邦/SDK 的 runtime 呼叫），逐一列出以下三個來源目前已驗證存在的唯讀查詢方法：(a) `sdk.accounting`（帳務查詢）；(b) `sdk.stock` 命名空間中**確認為查詢用途**的方法；(c) `sdk.marketdata.rest_client.stock`（Fugle 行情 REST）與 `sdk.marketdata.websocket_client.stock`（即時推播）。**`sdk.stock` 命名空間中任何下單／改單／刪單／批次下單／批次改價／批次改量／批次刪單／條件單下單／預約圈存／匯撥申請類方法（含但不限於 `place_order`、`cancel_order`、`modify_price`、`modify_quantity`、`batch_place_order`、`batch_cancel_order`、`batch_modify_price`、`batch_modify_quantity`、`cancel_condition_orders`、`single_condition`、`single_condition_day_trade`、`single_condition_stop`、`multi_condition`、`multi_condition_day_trade`、`multi_condition_stop`、`make_modify_price_obj`、`make_modify_quantity_obj`、`reserve_cash`、`reserve_stock`、`transfer_stock`、`time_slice_order`、`trail_profit`）一律禁止出現在本頁清單或程式碼常數中，不論是否標示為「未串接」——本頁的唯一目的是唯讀查詢能力盤點，絕不得成為交易功能的暗示或索引。`sdk.futopt`／`sdk.futopt_accounting`（期貨選擇權）命名空間整體不在本頁範圍內（本系統不支援期貨/選擇權資產類別）。** 每筆資料須含：`connected`（boolean，是否已被本系統任何服務實際呼叫）、`category`（連線狀態查詢／帳戶／庫存查詢／委託與交易資訊查詢／個股報價查詢／歷史成交查詢／行情查詢／即時推播其一）、`name`（中文名稱）、`sdkReference`（SDK 方法或頻道的完整可查證路徑，如 `sdk.accounting.bank_remain`、`marketdata.rest_client.stock.intraday.candles`）、`httpEndpoint`（本系統已串接時對應的 `method + path`；未串接時為空值或「－」，不得虛構路徑）、`description`（白話唯讀用途說明，須明確說明「純查詢」「不影響券商端」或同義字樣）、`consumer`（已串接時填實際呼叫端服務；未串接時填「－（尚未串接）」）、`requestSummary`（請求參數摘要）、`responseSummary`（回應內容摘要）。`requestSummary`／`responseSummary` 必須忠實反映可查證來源：已串接項目沿用各自經官方查證的 HTTP contract（ETF 依 Requirement123／Tasks389–390，本輪新增路徑以 Requirements128–133 與對應 task 修正為準）；`sdk.accounting`／`sdk.stock` 的未串接項目以 SDK 方法的 `__doc__` docstring 內容為準（docstring 為 `None` 者，`requestSummary`／`responseSummary` 一律據實寫「SDK 未提供可查證的參數／回傳說明（docstring 為空），僅能確認此方法存在於對應命名空間」，不得臆測或杜撰欄位）；`marketdata.rest_client.stock` 下各方法因原始碼為 thin proxy（僅轉呼叫 `symbol`／`market` 路徑參數與任意 `**params` query string，未在程式碼中定義欄位 schema），`requestSummary` 須明確寫出這個事實，`responseSummary` 只能以該查詢類別的通用資料型態概述（如「K 線資料，通常含開高低收與成交量」）並註明「實際完整欄位以官方回應為準，此 wrapper 未在程式碼中定義 schema」。清單總筆數與逐筆 `sdkReference`／`connected` 狀態須與下方設計章節列出的盤點結果完全一致（總計 52 筆：連線狀態查詢 2、帳戶／庫存查詢 7、委託與交易資訊查詢 18、個股報價查詢 2、歷史成交查詢 1、行情查詢 20、即時推播 2；歷史 Task386 基線為已串接 8 筆、未串接 44 筆；目前已驗收的 ETF 基線為 52/9/43；Task393、385 修復、396–398 五項完成並驗收後，本階段目標為 52/15/37；Task394/395 此時仍未串接，待兩項財務 writer 的來源證據核實並完成驗收後，原完整目標才是 52/17/35，不得預先標記為已完成——行情查詢的 `intraday.tickers` 雖無獨立對外 HTTP endpoint，但已被 `sdk_gateway.verify_taiex_index_symbol()` 在大盤指數即時串流啟動時實際呼叫以核對設定的指數代碼，依 `connected` 欄位定義（是否已被本系統任何服務實際呼叫）須標記為已串接；`ownership.etf_holdings` 已由 Requirement123／Tasks389–390 經 `POST /internal/market-data/etf-holdings` 串接）。程式內須以顯著註解提醒：日後 `fubon-broker-service` 新增、刪除或修改對外 endpoint，或富邦 SDK 版本升級改變命名空間方法時，必須同步核對並更新本清單。

- [ ] **新增唯讀 BFF 端點，維持既有登入保護邊界。** 新增精確 `GET /api/bff/fubon-api`，controller 直接回傳上述靜態清單，不得注入或呼叫任何 Repository、DB、Redis、WebClient、`fubon-broker-service`、business service 或其他外部系統，也不得在 runtime 對富邦 SDK 做任何 import 或呼叫——本頁資料完全來自程式碼常數，非即時查詢。此端點沿用既有 `anyExchange().authenticated()` 保護，不得新增 `permitAll`、不得掛上 9090 API gateway、Tailscale Serve 或 frontend nginx 9090 allowlist、不得新增 `docs/openapi/docker-external-api.yaml` 的 `paths` 項目。

- [ ] **前端頁面新增「已串接」欄位與可展開請求／回應明細，不改動既有項目。** router 沿用既有登入後 `/fubon-api`（name `FubonApi`、title `富邦證 API`）與 `FubonApiView.vue`；側欄「系統資訊」群組（`App.vue` 的 `mainMenuItems`）維持既有「富邦證 API」項目，緊接在「排程列表」「開放 API」之後，三者相對順序、path、存取控制皆不得改動。`frontend/src/api/index.js` 沿用既有 `fubonApi.get()` 呼叫 `GET /bff/fubon-api`。表格新增一欄「已串接」，以 `el-tag`（`connected=true` 顯示成功色「已串接」，`false` 顯示 info 色「未串接」）呈現，且提供依此欄位篩選（全部／已串接／未串接）；分類篩選改為對應新的 7 類。每一列使用 `el-table-column type="expand"`（比照既有「今日交易雷達」`TradingRadarView.vue` 的展開列模式）新增可展開明細，展開內容須顯示該筆的 `sdkReference`、`httpEndpoint`（若有）、完整 `requestSummary` 與 `responseSummary`，以易讀排版（如程式碼區塊或條列）呈現、不得截斷。頁面頂部維持既有 `el-alert` 唯讀提醒文字，並追加一句明確說明「未串接」的定位：「『未串接』代表富邦官方 SDK 具備此查詢能力，但本系統目前尚未整合呼叫；列在此處僅供資訊盤點，不代表可透過本系統呼叫，也絕不包含任何下單／改單／撤單等交易方法。」

- [ ] **不得提供任何可執行操作，重申唯讀鐵律。** 頁面不得提供「試打 API」「複製 curl」「輸入參數呼叫」等任何可觸發實際請求的互動元件，僅供閱讀；不得顯示或暗示任一支 API（含「未串接」項目）可用於下單、改單、撤單、圈存、轉帳或其他具金融副作用的操作。本次不修改 `fubon-broker-service` 任一既有 endpoint 的行為、不新增或暴露任何下單類 SDK 呼叫、不變更既有富邦庫存同步／成交同步／台股 LIVE 報價／大盤指數串流的邏輯或 feature flag，也不將本頁資料或其 BFF 端點掛上 9090 對外閘道。

- [ ] **測試與驗收。** 新增或更新 BFF controller test，斷言：清單總筆數為 52；目前已驗收的 ETF 基線為 `connected=true` 9 筆／`connected=false` 43 筆；本階段五項完成並驗收後為15筆／37筆，且Task394/395維持未串接；兩項財務writer來源核實及功能驗收完成後的原完整目標才是17筆／35筆，各sdkReference/httpEndpoint須符合design的adapter allowlist（14條含stock subscriptions輔助控制路由；portfolio與indices各承接2項，technical read承接3項）。httpEndpoint只列adapter路徑，Java手動/sync/readback放consumer；不可把不同服務入口混用；每筆 `category` 屬於上述 7 類之一；每筆 `sdkReference` 非空白；每筆 `description` 皆含「純查詢」或「不影響券商端」字樣；清單中不存在任何 finding 1 條列出的下單/改單/刪單/批次/條件單/預約/匯撥類方法名稱（以字串比對逐一排除）；`futopt`／`futopt_accounting` 字樣不得出現在任何 `sdkReference`。`bash scripts/spec-check.sh`、BFF Maven test 與 frontend production build 皆須通過。實機驗收：以 Compose stack 重建並 recreate `bff` 與 `frontend`，確認未登入請求 `/api/bff/fubon-api` 仍受登入保護；登入後側欄「系統資訊」的「富邦證 API」頁可看到 52 筆資料、「已串接」欄位依實際驗收階段正確標示9筆（目前ETF基線）、15筆（本階段五項完成後）或17筆（含394/395的原完整目標；原8筆僅為歷史基線）、可展開任一列看到請求／回應明細、依「已串接」與分類篩選皆可用，且畫面明確呈現「未串接」定位說明文字。
...
- [ ] **核實狀態與分段實作。** Task394財務同步仍未完成；本階段只放行安全adapter/token/解析/預檢/no-write，所有缺範圍證據的合法回應明示coverageStatus=UNVERIFIED、reason=MISSING_SETTLEMENT_RANGE_CONTRACT。沒有已獲准的VERIFIED產生算法；財務writer與SUCCESS/清零驗收留待官方範圍及完整去識別對帳證據經新一輪spec審查後，不能由fake或設定值放行。API inventory維持connected=false；原使用者要求仍保留。
...
- [ ] **完整性與在途判定不能猜。** 只将 `settlementDate > queryDate` 視為未來交割候選，過去不計；同日非零無法核實已入銀行時整批 AMBIGUOUS_SETTLEMENT，不靠時鐘猜已扣款。重複/矛盾交割日期整批拒绝。完整兩列覆寫前須核實來源覆蓋待寫的完整未來交割區間；Python/Go官方僅說3d/三日，不足以證明所有未交割；精確缺口為起訖與日種類、產品範圍、未產製/空值語意及完整未來交割覆蓋，現階段為SETTLEMENT_SCOPE_UNVERIFIED/no-write。裸空 details 或單一 None 佔位不允許清零；完整且明確零值來源可正常寫零。
- [ ] **既有在途語意與原子快照。** orchestrator NOT_SUPPORTED 的 HTTP/preflight 完成後，獨立 REQUIRES_NEW writer 第一 DB operation 是共用 AssetSnapshotMutationLock；鎖後 recheck owner/active fubon broker/台北富邦銀行與類型。目標 `currency=TRANSIT_TWD`、買股待付款 amount 非正、賣股待收款 amount 非負，0/1/多列明確處理；不改一般 TWD 存款假裝在途。加總後 HALF_UP scale2 並驗 numeric(20,2)，新增同步 managed collection，兩列與 SnapshotAggregateCalculator、saveAndFlush 同交易；保留其他資產。跨日/觀測超過 60 秒、溢位、重複或來源缺證據均全批 no-write。
- [ ] **入口與證據。** business `POST /internal/brokers/fubon/settlement-sync?dryRun=true|false` exact token保護、預設true；dry-run 檢查所有來源可用條件但不進 writer。API inventory httpEndpoint 為 adapter `/internal/settlement/read`，manual 放 consumer。本階段測官方None/零/負值、日期與coverage缺證據時零writer呼叫及零DB寫鎖；成功/歸零、真PostgreSQL首鎖/aggregate/rollback留待充分證據及writer設計重新獲審後。未核實來源不可因保持flag=false被寫成完成。
...
- [ ] **核實狀態與分段實作。** [富邦官方Go已實現損益說明](https://www.fbs.com.tw/TradeAPI/docs/trading/library/go/accountManagement/RealizedPnLDetail/)已核實每成交一列、profit/loss不共存、損益含手續費與交易稅；Python仍未提供逐列fillId、net proceeds、取得成本或date→filledDate契約。Task395財務同步尚未完成，本階段只放行adapter/解析/token/預檢/no-write，writer與正常財務新增待來源證據及獨立審查，connected=false。
- [ ] **唯讀與固定時段。** business-services 獨立 FUBON_REALIZED_GAIN_SYNC_ENABLED，同 Requirement129 四個每日 cron，不加交易日 gate；adapter `POST /internal/realized-gains/read` 只呼叫 selected account 的 realized_gains_and_loses。官方 Result/mandatory branch/account/order_type/date 皆驗，僅 Stock/Sell。raw identity 留 Python，HMAC fingerprint 才能跨界；profit/loss 非負含零、price 正、qty exact integer；sourceDate 不晚於 queryDate。
...
- [ ] **DB 序列化保護整批。** HTTP/preflight 在 transaction 外；独立 writer 以 repository `LOCK TABLE realized_gain IN SHARE ROW EXCLUSIVE MODE` 作 gain 查詢／新增前的第一個 DB 動作，有界 timeout，跨 JVM 與一般 CRUD 的寫入互斥。鎖後重新核對完整 scope、證據與所有候選，零 existing 且完整可靠才新增；一 existing 完全一致保留；任何歧義零新增。不存在的 unique 不能用 catch 模擬；saveAll/flush 失敗整批 rollback、commit 後才記 SUCCESS。禁止新增 schema/provenance side table，禁止修改既有 gain 或 ledger。
- [ ] **手動／測試誠實。** business `POST /internal/brokers/fubon/realized-gain-sync?dryRun=true|false` 預設true、獨立 exact token filter；僅計數/outcome/reason。API inventory httpEndpoint 為 `/internal/realized-gains/read`，consumer 才放 Java 路徑。本階段測未核實來源零writer呼叫／零寫鎖，含手動改過FUBON_SYNC ledger仍不得當可信證據；取得來源與身分mapping後才測PostgreSQL跨實例table lock、scope隔離、precision後重跑、正常新增及rollback。真人SDK樣本未驗證須明示，不能以自造fixture宣稱真成本。
...
- [ ] 六個新job與t385節拍必同時更新實作、JOBS註冊與測試；394/395的job描述須標來源契約待核實、目前只查詢/no-write。以先合入ETF session為集成基線：58 jobs（business24/external34）加本輪3+3後為64（27/37）；API inventory52/9/43在其他五項完成後為52/15/37（新增bank/dividends/stock-push/三項technical共6），交割/損益仍未串接；原全部完成目標才是52/17/35。實作時以實際已合入清單核對，不能覆蓋ETF項目或用尚缺ETF的舊計數通過。
...
- [ ] **分段授權不等於session完成。** 本輪獨立審查可分別判定Task393、385修復、396–398正常功能，以及394/395安全adapter/preflight/no-write切片是否可實作；394/395財務writer明確未放行，不能記為整體spec已無阻礙。其精確來源證據清單在兩任務檔「目前核實狀態與實作切片」。五項已驗收後仍須向使用者回報剩餘兩项；兩項未完成時不合併第二個session、不刪除兩個原Claude branches。
