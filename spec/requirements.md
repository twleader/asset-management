...
> **本 Requirement 的「所有 API」只指目前已正式核准給 Docker 外部工具使用的五個唯讀 GET operation：** `GET /api/quotes`、`GET /api/quotes/one`、`GET /api/public/market-index`、`GET /api/assets/latest`、`GET /api/public/exchange-rate/usd-twd`。它不代表把整個 `/api/**`、BFF、前端登入 API 或任何 `/internal/*` 全部公開。Task 328 以新的 Nginx `api-gateway` 收斂原本分散在 BFF `8080`、external-materials-service `8082` 與 frontend `80` 的 Docker 外部入口；三個上游服務仍保留各自的 container port，只撤除 host port mapping。五支都可經 path-scoped Tailscale Serve 遠端存取；USD/TWD 是公開匯率資訊，與另外四支採相同的精確路徑邊界，不再限 host loopback。`/api/assets/latest` 與 USD/TWD 的業務契約由各自獨立需求／任務實作，本 Requirement 只定義 gateway network boundary，不得在 Nginx 複製業務邏輯。

**Acceptance Criteria:**

- [ ] **純唯讀，不新增任何寫入或抓取副作用**：⚠ **此條由 Requirement 71 新增的第六條 `POST /api/public/crawler-data/rescan` 局部推翻**——該路由刻意有外部抓取副作用（觸發 NewsPoller 重新搜尋），是本 Requirement 「純唯讀」原則的唯一具名例外，經 30 秒全域 Redis 冷卻節流，詳見 Requirement 71。本條對其餘五條唯讀路由的約束不變。報價端點只讀既有 Redis 快取（`price:{market}:{code}`、`price:index:{market}`，以及 Task 349 起用來補充官方折溢價的 `price:etfnav:{market}:{code}`），不觸發外部 API 抓取、不寫入 Redis、不寫入資料庫、不發送 `price-update` pub/sub 訊息。與既有 `PricePoller`／`PriceCacheWriter`／`ClosePersister` 的寫入路徑完全無關、不共用任何寫入程式碼路徑。
- [ ] **兩種查詢操作**：
...
- [ ] **Nginx 採精確 allowlist，不得用 `/api/` wildcard proxy**：本機 `127.0.0.1:9090` 只允許下列五個 exact path（Requirement 71 另增第六條 `POST` exact path，不計入本條列舉，見該 Requirement），保留原始 query string、response body、status、content type，不做資料轉換、fallback、快取或重試：
...
- [ ] **前端 port 80 不得形成第二個匿名入口**：`frontend/nginx.conf` 必須在既有 generic `location /api/` 之前，以 exact location 對上述五條 path 回 `404`（Requirement 71 另增第六條 exact deny location，見該 Requirement），並以 anchored deny regex 封鎖受保護路徑**每一個 segment**的 matrix-parameter 變體（例如 `/api/public/market-index;x=1`、`/api/public;x=1/market-index`、`/api/assets;x=1/latest`），避免 WebFlux 去除任一 segment 的 matrix parameter 後仍命中 controller；其餘 SPA、OAuth2 與已登入 `/api/bff/**` 行為不變。此防線與 BFF 的精確 GET `permitAll` 是不同層次：BFF permitAll 讓 api-gateway 能取資料，frontend block 則確保 Docker 外部 API 只從 9090 出入。
- [ ] **遠端安全邊界為 path-scoped Tailscale Serve，不使用 root proxy、OAuth2 或自簽憑證**（Requirement 71 落地後，本條與下方多處「五條／五路」須併入該 Requirement 新增的第六條、以及 Requirement 79 新增的第七、八條一併理解，實際 handler 集合為八條，見該二 Requirement）：主機安裝 Tailscale 並登入 tailnet 後，先讀取 `tailscale serve status --json`；只有空設定或 handler 集合精確等於本任務管理的五條 path 時才能 `tailscale serve reset`。若有任何其他 port/path/service handler，須列出後停止，不得靜默覆寫使用者既有 Serve 設定。清理後分別以 `tailscale serve --bg --https=9090 --set-path=<exact-path> http://127.0.0.1:9090/<同名 exact path>` 掛載 `/api/quotes`、`/api/quotes/one`、`/api/public/market-index`、`/api/assets/latest`、`/api/public/exchange-rate/usd-twd`；每條都必須 `--bg` 才會立即返回並持久化，target 必須包含同名 path 才能保留 upstream request path。實際 CLI 若不支援 `--set-path` 必須 fail closed、不建立 remote Serve；不得另猜 listener、改用 root proxy 或另開不同遠端 port。禁止把 `/` 或 `/api/` 掛到 `http://127.0.0.1:9090`，也不得加入五條之外的 handler。不得啟用 Tailscale Funnel、路由器 port-forward、公開 Internet listen、Nginx 自簽／私鑰或額外 OAuth proxy。TLS 憑證與裝置／使用者身份由 Tailscale 管理；多人 tailnet 應以 grants/ACL 只授權需要的 identity。若 Tailscale 未登入或 HTTPS 尚未啟用，Docker gateway 仍只能由本機 loopback 呼叫，不得退化為 `0.0.0.0:9090`。
- [ ] **Tailscale 設定不得把憑證或 auth key 寫入 repo**：提供可重複執行的主機端設定腳本，先檢查 `tailscale` CLI 與登入狀態，再讀 machine-readable Serve 現況確認所有權；其後於任何 reset 前依序完成五支本機 exact GET 的 endpoint-aware 健康檢查：quotes 必須固定為 200 `application/json` 非空 array，並以首筆合法 `stockCode/market` 驗 quotes/one 固定為 200 `application/json`；market-index 必須固定為 200 `application/json`，且有非空、等長的六個圖表陣列；assets/latest 與 USD/TWD 必須各自通過已核准的 status/content-type/payload 契約。quote list 無 sentinel 或任一支未健康都須明列原因並 fail closed，不得 reset/configure。因 preflight 可能耗時，`serve reset` 緊前必須再次讀 status、重驗所有權，並將解析後的 canonical JSON 與最初狀態比較；任何變動都停止，不得 reset。只有兩次狀態相同且五路全通過時，才可安全 reset 並設定五個 path-scoped background mounts。設定中途失敗的 cleanup 也須先重讀現況：只有空設定或本輪五條 handler 的安全子集合／精確集合、且所有 target 完全相符時才可 reset；若含任何其他 handler 或無法確認，須列出並留待人工處理，不得自動刪除。最後以 machine-readable status 證明 handler 精確等於五條核准 path，且沒有 `/`、`/api/` 或額外 handler。腳本不得接受、產生或保存 reusable auth key，不得修改 tailnet ACL，不得呼叫 Funnel。安裝／登入／啟用 Serve 需要 macOS 管理員核准或瀏覽器互動時須明確停止並由使用者完成。
- [ ] **五路 preflight 一律先驗 status/header 再驗 payload，且有 reset 前回歸**：每個 endpoint 都須分開保存 response headers 與 body，先要求 HTTP status 精確為 `200`、`Content-Type` media type 為 `application/json`（允許參數）後才解析 body；quotes list、quotes/one、market-index 不得只靠 JSON body 形狀推定 content type。自動化 shell regression 至少模擬這三路各自回 `200 text/plain`，逐案證明腳本失敗且完全未呼叫 `tailscale serve reset`，並以正常案例證明只設定五條 handler；既有兩次 status/canonical ownership、partial cleanup 與 `${status}`／`${quote_one_status}` 行為不得退化。
...
- [ ] **API 本身不另要求呼叫者登入**：五條 API 都是具名唯讀資料；`/api/assets/latest` 固定由 BFF/business 的可信主要管理者設定決定 owner，不接受匿名 request 提供的 owner selector 或 `X-User-*`。五支外層都以 tailnet membership/grants、TLS、loopback bind、path-scoped Serve 與 Nginx exact allowlist 保護；USD/TWD 是公開匯率資訊，採相同遠端邊界。因此 gateway 不新增 Google OAuth2 flow、API key 或 `X-Internal-Service-Token`。BFF 網頁原有 Google OAuth2 不變，Treasury 維運觸發端點原有 token／ADMIN 防護也不變且不經 gateway 暴露。
...
- [ ] **測試與實機驗證**：保留既有 quote 測試——(a) 有快取時列出全部與依 market 篩選；(b) 全市場皆無快取時列出回空陣列；(c) 單一命中與 cache miss；(d) 缺必填參數回錯誤；(e) 讀取邏輯零寫入互動。另須驗證——(f) `nginx -t` 與 Compose config 通過；(g) 本機 9090 五條 GET 各實際命中正確 upstream 且 query/status/body 未被改寫；尚未落在本輪 checkout 的 assets/USD-TWD endpoint 須先以 stub upstream 契約測試證明 exact proxy，再於各 endpoint 落地後補真實 payload runtime 驗證，不得把當下 BFF 的 401/404 當成功；(h) 五條 path 的 POST/PUT/PATCH/DELETE 均為 405；(i) `/internal/health`、`/internal/refresh`、`/api/bff/gdp-twse/index-daily`、`/actuator/health`、descendant 與未知 path 均為 404；(j) frontend port 80 對五條 path 均 404；(k) host `8080`／`8082` 無 listener，`9090` 只綁 `127.0.0.1`；(l) container image provenance 等於本輪 build image，重建／recreate 後 gateway 與所有 upstream healthy；(m) Tailscale 已登入且 Serve 啟用時，以 tailnet HTTPS `:9090` 做五路正向驗證：quotes 必須分離擷取 status/body，固定為 200 `application/json` array；若非空則以首筆 `stockCode`／`market` 呼叫 quotes/one，另行固定驗 200 `application/json` 後才驗 payload，空 array 時明列 `NO_QUOTE_SENTINEL`、不得把未測單筆冒充通過；market-index 必須分離擷取 status/body，固定為 200 `application/json`，且至少一個代表查詢的 `labels` 非空、`labels/closes/ma5/ma20/ma60/ma240` 等長；assets/latest 須為 200 `application/json`、body 非空、`valuationPolicy=TARGET_SESSION_WITH_EXPLICIT_FALLBACK`、`snapshot.id` 與 `liveAssets.snapshotId` 為相同整數；USD/TWD 須為 200 `application/json`，固定 `pair=USD/TWD`、`baseCurrency=USD`、`quoteCurrency=TWD`、`refreshIntervalSeconds=2`、`timezone=Asia/Taipei`，`spot` 為非空 object、`history` 為非空 array 且 `count == history.length`。任何 `201` 或其他非預期 2xx 都不得以 `curl --fail` 冒充成功。另驗 `/`、`/api/`、未知 path 與五條之外的 handler 皆 404；`tailscale serve status --json` 必須精確含五條核准 handler，不得含 root、`/api/` 或額外 handler。未登入或未啟用 Serve 時須誠實回報待使用者授權，不得宣稱遠端 HTTPS 已驗證。
...
> **Gateway network boundary 由 Requirement 66／Task 328 唯一管理。** 該任務已在獨立 non-root Nginx `api-gateway` 預留 exact `GET /api/assets/latest` → `bff:8080`，host 僅綁 `127.0.0.1:9090`，Tailscale Serve 只掛五條核准的 exact path（Requirement 71 另增第六條 `POST` exact path、Requirement 79 另增第七、八條唯讀 `GET` exact path，實際 handler 集合為八條，見該二 Requirement）。本文只實作 latest-assets 的 BFF／business 業務契約，不在 BFF 代理 quotes、不改 Compose host ports、不複製 Nginx／Tailscale 規則。
...
- [ ] **沿用五路 Nginx／Tailscale 邊界，不建立第二入口**（Requirement 71 落地後新增第六條 `POST` exact path、Requirement 79 落地後另增第七、八條唯讀 `GET` exact path，本條「五路」須併入該二 Requirement 一併理解，實際 handler 集合為八條，見該二 Requirement）：Compose 的 gateway 唯一 host mapping 維持 `127.0.0.1:9090:9090`，BFF 與 external-materials-service 不發布 host port。`api-gateway` 對本 exact GET 保留 query/body/status/content type，非 GET 回 405，trailing slash／descendant／其他 path 回 404；frontend port 80 對五路 exact 與各 segment 的 matrix-parameter 變體回 404。Tailscale Serve 以 path-scoped HTTPS `:9090` 掛五條同名 exact path，包含 `/api/public/exchange-rate/usd-twd`；禁止 root `/`、`/api/` wildcard、上述五條核准精確路徑之外的 handler、Funnel、公網 listener、自簽憑證或額外 OAuth。不得以可偽造的 Tailscale header 決定本機／遠端權限，也不得修改 gateway／Compose／Serve 設定來建立旁路；既有 Serve script 可因五路安全契約修正而收緊，但不得擴張路徑或 listener。
