...
- [ ] KPI 卡片列共 5 張（資產總計 / 存款總計 / 股票現值 / **債券** / 預估年配息），改用 flex 平均分配寬度。**第 4 張「信託基金」改為「債券」**：因信託基金非獨立資產類別、且需讓債券 ETF 正確歸入債券，債券卡顯示 `bondValue`（債券型：債券 ETF ＋ 債券型基金；與「現金/債券/股票」圓餅圖 tab **同一 business-service 來源** `GET /api/snapshots/history`，前端依 `id` 對應選中快照的 history row），採快照凍結逐筆 `currentValue`、不套盤中 live（同 Requirement 25「巨觀資產配置毋須 intraday 精度」）；`sub` 顯示「佔比 X.X%」（分類值無逐筆成本口徑，不顯示損益）。**「股票現值」卡維持原樣**（`totalStockValue` 全股票即時值、盤中 live、`sub` 顯示損益並依正負上色）。**注意（刻意的取捨）：債券 ETF 同時計入「股票現值」（含全部股票部位）與「債券」，兩卡為不同視角、刻意重疊，故此列五卡不再嚴格加總 == `totalAssets`**（有別於原「存款 + 股票現值 + 信託基金 == totalAssets」；嚴格分割維度請看「現金/債券/股票」圓餅圖 tab）
- [ ] 「資產配置分佈」面板提供 tab 切換：
