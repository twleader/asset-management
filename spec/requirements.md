...
- [ ] **測試與驗收。** 新增或更新 BFF controller test，斷言：清單總筆數為 52；`connected=true` 恰為 9 筆且 `sdkReference`／`httpEndpoint` 與 7 支 HTTP endpoint（`portfolio/read` 對應 2 筆 `sdkReference`、`taiex-index/stream` 對應 2 筆 `sdkReference`：`indices` 頻道與 `intraday.tickers` 的內部驗證呼叫、`etf-holdings` 對應 1 筆 `sdkReference`——Requirement 123／Task 390 新增的第 7 支 endpoint）完全對應；`connected=false` 恰為 43 筆；每筆 `category` 屬於上述 7 類之一；每筆 `sdkReference` 非空白；每筆 `description` 皆含「純查詢」或「不影響券商端」字樣；清單中不存在任何 finding 1 條列出的下單/改單/刪單/批次/條件單/預約/匯撥類方法名稱（以字串比對逐一排除）；`futopt`／`futopt_accounting` 字樣不得出現在任何 `sdkReference`。`bash scripts/spec-check.sh`、BFF Maven test 與 frontend production build 皆須通過。實機驗收：以 Compose stack 重建並 recreate `bff` 與 `frontend`，確認未登入請求 `/api/bff/fubon-api` 仍受登入保護；登入後側欄「系統資訊」的「富邦證 API」頁可看到 52 筆資料、「已串接」欄位正確標示 9 筆已串接、可展開任一列看到請求／回應明細、依「已串接」與分類篩選皆可用，且畫面明確呈現「未串接」定位說明文字。
