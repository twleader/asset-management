...
- [ ] **文件與計數同步。** 現行所有描述「八條／八路／七 GET＋一 POST」的 current-state 文件、安裝說明、Tailscale 腳本／測試訊息與 `run-stack` Codex skill 改為「九條／九路／八 GET＋一 POST」，並列出新路徑；任務索引加入 Task 346／347並保留 Task 349，current range 為 `344–347、349`，Task 348 由在途 worktree 保留。Requirements 實際為 87 個，Requirement 87 由在途 worktree 保留，最新編號為 88。歷史段落若明確標示「當時五／六／八條快照」可保留，但任何聲稱是**現在**的數字不得過期。
