...
- [ ] **`RULE_VERSION` 升版 `TW_RULES_V15` → `TW_RULES_V16`**：`score`／`action` 對多數標的實質變動（實測 35 檔全部受影響，最大單檔一周軌 ±3.00 分），完全符合本專案既有升版判準「使用者可觀察行為有實質變化」（〈三軌持有期與真正的週K／日K 棒〉一節的升版判準段），三個不升版先例（Task 249 輸出完全相同／Task 281 純揭露／Task 336 顯示值捨入修正）**一個都不適用**。須同步更新的 hardcode 共七處，逐一列出以免遺漏：`TradingRadarRuleEngine.java:49`（常數本體與其 Javadoc 新增 V16 沿革段落）、`TradingRadarDto.java:9`、`TradingRadarService.java:752` 註解、`frontend/src/views/TradingRadarView.vue:28`（fallback 值）／`:1125`（`ref` 初始值）／`:804`（免責文字改述為「`TW_RULES_V15` 與 `TW_RULES_V16` 為不同規則版本，兩者的分數不可直接比較」）、`docs/openapi/docker-external-api.yaml:579`（`ruleVersion: TW_RULES_V15` 的 synthetic example——`t356_radar_three_horizon_weekly_k.md:240` 已記載此處是全樹唯一不屬 Java／Vue／測試的版號 hardcode，且**既有 OpenAPI 測試只驗 example 存不存在、不驗內容，不會有任何紅燈提醒**）。既有測試中的版號斷言亦須同步：`TradingRadarRuleEngineTest.java:533`、`BacktestServiceTest.java:806/808/821/837`、`TreasuryYieldServiceTest.java:178/189`、`TradingRadarControllerCurrentTest.java:27`、`TradingRadarV13ActionPolicyTest.java:112` 註解。
