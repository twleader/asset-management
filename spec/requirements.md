...
- [ ] **Nginx 採精確 allowlist，不得用 `/api/` wildcard proxy**：本機 `127.0.0.1:9090` 只允許下列五個 exact path，保留原始 query string、response body、status、content type，不做資料轉換、fallback、快取或重試：
...
> **Gateway network boundary 由 Requirement 66／Task 328 唯一管理。** 該任務已在獨立 non-root Nginx `api-gateway` 預留 exact `GET /api/assets/latest` → `bff:8080`，host 僅綁 `127.0.0.1:9090`，Tailscale Serve 只掛五條核准的 exact path。本文只實作 latest-assets 的 BFF／business 業務契約，不在 BFF 代理 quotes、不改 Compose host ports、不複製 Nginx／Tailscale 規則。
