...
- [ ] **純唯讀，不新增任何寫入或抓取副作用**：⚠ **此條由 Requirement 71 新增的第六條 `POST /api/public/crawler-data/rescan` 局部推翻**——該路由刻意有外部抓取副作用（觸發 NewsPoller 重新搜尋），是本 Requirement 「純唯讀」原則的唯一具名例外，經 30 秒全域 Redis 冷卻節流，詳見 Requirement 71。本條對其餘五條唯讀路由的約束不變。新端點只讀既有 Redis 快取（`price:{market}:{code}` 與 `price:index:{market}`，見 design.md「Redis key schema」），不觸發外部 API 抓取、不寫入 Redis、不寫入資料庫、不發送 `price-update` pub/sub 訊息。與既有 `PricePoller`／`PriceCacheWriter`／`ClosePersister` 的寫入路徑完全無關、不共用任何寫入程式碼路徑。
- [ ] **兩種查詢操作**：
