...
| 366 | 重產 `db/schema.sql` 稽核基準線（74→85 張表，另補 7 張既有表的欄位／索引漂移），並以 `spec-check.sh` B10 呼叫新測試 `scripts/tests/schema-sql-drift-test.sh` 建立機械防漂移閘門（含跨 worktree 的嚴重度分流）；同步統一 spec-auditor／arch-auditor 對該檔互相矛盾的指引（Requirement 102；避讓當下 Task 364／365 與 Requirement 100／101 尚在別的 worktree 在途，其後皆已 landed） | [tasks/t366_schema_sql_drift_guard.md](tasks/t366_schema_sql_drift_guard.md) |
