...
| 元件 | 技術 | Port | 對外 | 角色 |
|------|------|------|------|------|
| **Frontend** | Vue 3 + Vite 5 + Element Plus + Nginx | 80 | ✅ | SPA；十四條 external-only API 明確回 404 |
| **API Gateway** | Nginx Alpine（non-root） | 9090 | ✅（僅 `127.0.0.1`） | Docker 外部十四條 exact route（十三 GET ＋ 一 POST）唯一入口；每條須有完整 OpenAPI 3 契約 |
...
Tailscale ──► HTTPS :9090（僅十四條 exact path）──┘
...
- **Docker 外部 API 只經 9090 exact allowlist。** BFF 與 external service 不發布 host port；host 只綁
  loopback `127.0.0.1:9090`，current allowlist 精確為十三支 GET：`/api/quotes`、`/api/quotes/one`、
  ...
  `/api/public/transactions`、`/api/public/trading-calendar`、`/api/public/commodity-prices`、`/api/public/srpp/daily-context`，以及唯一一支 POST `/api/public/crawler-data/rescan`。
  Tailscale Serve 只掛相同十四條 method/path，不使用 root／`/api/` proxy、Funnel 或公網 listener。
  每一支掛載到 9090 的 API 都必須在同一 task/commit 具備完整 OpenAPI 3 operation，gateway 與
  `docs/openapi/docker-external-api.yaml` 的 method/path 集合必須以 contract test 雙向精確相等；
  Swagger UI／YAML 本身不掛到 9090。
