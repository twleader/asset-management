...
2. **前端頁面 BFF 路徑前綴：** `/api/bff/{page-name}/...`。Gateway route 將 `/api/bff/{page}/**` rewrite 為 `/api/{resource}/**`。
   - **具名、限縮例外（Requirements 67／68／70／71／78／79／86／108–113／118／163／165；Tasks 317／325／327／329／337／338／347／372／373／375–378／383／452–454／456）**：BFF 只對十四條 manifest 的 exact path/method 開放：十三條 GET `GET /api/quotes`、`GET /api/quotes/one`、`GET /api/public/market-index`、`GET /api/assets/latest`、`GET /api/public/exchange-rate/usd-twd`、`GET /api/public/market-analysis/today`、`GET /api/public/portfolio-advice/latest`、`GET /api/public/trading-radar/today`、`GET /api/public/trading-radar/stock`、`GET /api/public/transactions`、`GET /api/public/trading-calendar`、`GET /api/public/commodity-prices`、`GET /api/public/srpp/daily-context`，以及唯一 POST `/api/public/crawler-data/rescan`。`portfolio-advice/latest`、`assets/latest`、兩支交易雷達及交易紀錄是 owner-scoped：不帶 `email` 時走 configured-admin bootstrap、顯式 tenant header 和 cleared caller identity；Requirement 140 起這五支可另帶 `email` query 參數改依該帳號查詢（只驗 `isActive()`，不要求為 configured-admin），細節見 `spec/requirements.md`／`spec/design.md` Requirement 140；`GET /api/public/srpp/daily-context`（Requirement 163）沿用同一 selector，但空白 email 回 400，且兩種 lookup 都有 5 秒逾時；雷達 `today` 是第一頁 typed list，`stock` 只回當次 current-read exact `(stockCode,market)` full detail，交易紀錄只讀保存的 ledger。交易日曆與 commodity-prices 是 global no-tenant market-data read：commodity batch 只經 qualified no-tenant business client 一次讀既有已持久化 WTI／BRENT／GOLD live 聚合，固定 slots 且無 refresh、vendor I/O 或寫入。Quotes 由 BFF no-tenant container clients 讀 external-materials 原始 19 欄，再從明列 business readonly market endpoints 加入公開市場 `marketData`（Requirement 165：`/one` 在 raw 204 時，19 欄改由同一 no-tenant business client 的唯讀 `/api/market-data/history/stock` 組成 30 日內最近收盤的 `CLOSE_FALLBACK`，不寫任何快取、不呼叫 Fubon bridge），並可由同一 immutable result 追加 `quoteDetail`、`bidLevels`、`askLevels`、`dividendHistory`；不得讀或回傳個人持股／成本／交易／快照。禁止任何 wildcard／descendant、同路徑其他 method與前端 view 援引；Controller 仍只委派 service，BFF 不直查 DB 或外部行情。每個 9090 exact path/method 都必須在 `docs/openapi/docker-external-api.yaml` 有同 commit 的完整 OpenAPI 3 operation，gateway 與文件 path/method 集合必須機械相等；YAML 每次變更都必須重產本專案 Swagger Markdown 並覆寫 `/Users/steven/Project/SRPP/docs/9090 Port API Swagger.md`，且對 API、input、output 和所有 class attribute 提供具體說明。USD/TWD 的每 2 秒外部抓取與 live Redis producer只屬於 `external-materials-service`，business/BFF 僅唯讀 cache/DB 與聚合。
...
`127.0.0.1:9090:9090`。它轉送十四條 exact route：十三條唯讀 GET（quotes 兩條到 BFF，BFF 再讀 external service 原始快取；
 market-index、assets/latest、USD/TWD、market-analysis/today、portfolio-advice/latest、交易雷達 `today` 列表／`stock` 明細、configured-admin 交易紀錄、global 交易日曆、global commodity batch（既有 persisted WTI／BRENT／GOLD 聚合）與 SRPP 共用計算結果（Requirement 163）亦到 BFF；
後八條由 Requirements 79／86／111–113／118／163、Tasks 338／347／376–378／383／452–454 新增；assets/latest、portfolio advice、兩支雷達及交易紀錄不帶 `email` 時解析 configured admin（Requirement 140 起可另帶 `email` 選擇該帳號），日曆不選 tenant）＋ 一條寫入 POST
...
Tailscale Serve 只掛相同十四條 exact path，包含公開 USD/TWD 匯率與第六條寫入路由；禁止
root／`/api/` proxy、Funnel、自簽憑證與另加 OAuth。十四條 path/method 必須與版本控管的完整
...
```
spec/
├── requirements.md       # 159 個 Requirements（最新編號為 166，136–140 間為並行 worktree 保留跳號）
