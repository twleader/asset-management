...
2. **前端頁面 BFF 路徑前綴：** `/api/bff/{page-name}/...`。Gateway route 將 `/api/bff/{page}/**` rewrite 為 `/api/{resource}/**`。
   - **具名、限縮例外（Requirements 67／68／70／71／78／79／86／108–113；Tasks 317／325／327／329／337／338／347／372／373／375–378）**：BFF 只對十二條 manifest 的 exact path/method 開放：十一條 GET `GET /api/quotes`、`GET /api/quotes/one`、`GET /api/public/market-index`、`GET /api/assets/latest`、`GET /api/public/exchange-rate/usd-twd`、`GET /api/public/market-analysis/today`、`GET /api/public/portfolio-advice/latest`、`GET /api/public/trading-radar/today`、`GET /api/public/trading-radar/stock`、`GET /api/public/transactions`、`GET /api/public/trading-calendar`，以及唯一 POST `/api/public/crawler-data/rescan`。`portfolio-advice/latest`、兩支交易雷達及交易紀錄是 owner-scoped：固定走 configured-admin bootstrap、顯式 tenant header 和 cleared caller identity；雷達 `today` 是第一頁 typed list，`stock` 只回當次 current-read exact `(stockCode,market)` full detail，交易紀錄只讀保存的 ledger。交易日曆是 global no-tenant market-data read，不得 bootstrap owner。Quotes 由 BFF no-tenant container clients 讀 external-materials 原始 19 欄，再從明列 business readonly market endpoints 加入公開市場 `marketData`，並可由同一 immutable result 追加 `quoteDetail`、`bidLevels`、`askLevels`、`dividendHistory`；不得讀或回傳個人持股／成本／交易／快照。禁止任何 wildcard／descendant、同路徑其他 method與前端 view 援引；Controller 仍只委派 service，BFF 不直查 DB 或外部行情。每個 9090 exact path/method 都必須在 `docs/openapi/docker-external-api.yaml` 有同 commit 的完整 OpenAPI 3 operation，gateway 與文件 path/method 集合必須機械相等；YAML 每次變更都必須重產本專案 Swagger Markdown 並覆寫 `/Users/steven/Project/SRPP/docs/9090 Port API Swagger.md`，且對 API、input、output 和所有 class attribute 提供具體說明。USD/TWD 的每 2 秒外部抓取與 live Redis producer只屬於 `external-materials-service`，business/BFF 僅唯讀 cache/DB 與聚合。
   - **具名、限縮的行情五檔資料來源例外（Requirement 87／Task 348；Requirement 108／Task 372；Requirement 109／Task 373；Requirement 110／Task 375）**：登入後共用 `StockAnalysisDialog`、`/api/quotes*` 的 `marketData.quoteDetail` 與同一 response 的 direct `quoteDetail`／`bidLevels`／`askLevels` 都只讀同一個 typed `FUBON_BOOKS` snapshot。它只能由 `TwLiveQuoteDispatcher` 已授權十秒台股 round 的同一富邦 batch 建立，必須先寫 PostgreSQL canonical row 再寫 dedicated Redis `price:quote-detail:{market}:{code}`；query path 是 Redis → DB pure read，禁止 request-time Yahoo／富邦／dispatcher／writer 外呼，也不得寫 generic quote、index、tick、day-H/L 或 SSE。它不是權威即時價，不能供估值、損益、下單、警示或 SSE 使用；direct batch projection 只提供同一份市場五檔供累計深度判讀，無個人資產資料。business 只能 proxy／fail-soft；該頁籤與 direct batch projection 以外的即時價仍只走 Redis、收盤價仍只走 `stock_price_history`。
...
`127.0.0.1:9090:9090`。它轉送十二條 exact route：十一條唯讀 GET（quotes 兩條到 BFF，BFF 再讀 external service 原始快取；
 market-index、assets/latest、USD/TWD、market-analysis/today、portfolio-advice/latest、交易雷達 `today` 列表／`stock` 明細、configured-admin 交易紀錄、global 交易日曆亦到 BFF；
後六條由 Requirements 79／86／111–113、Tasks 338／347／376–378 新增；portfolio advice、兩支雷達及交易紀錄固定解析 configured admin，日曆不選 tenant）＋ 一條寫入 POST
...
Tailscale Serve 只掛相同十二條 exact path，包含公開 USD/TWD 匯率與第六條寫入路由；禁止
root／`/api/` proxy、Funnel、自簽憑證與另加 OAuth。十二條 path/method 必須與版本控管的完整
...
```
spec/
├── requirements.md       # 112 個 Requirements（最新編號為 113）
├── design.md             # 架構圖、ERD、Service 職責、Sequence
├── tasks.md              # 索引（Task 1–228、264–267、269–292、297–309、311–342、344–378）＋尚未歸檔的 201 起區段
├── tasks/                # 任務檔
│   ├── README.md         # 自足任務檔規範
