...
**對應 Requirements:** Requirement 128（歷史交付；現行 target 規則由 Requirement 152／Task 434 定義）
**前置任務:** 無；重用既有 Fubon adapter、configured admin、共用快照鎖與聚合計算器。
**Liquibase changeset:** 無；不得新增表、欄位、索引或變更 `db/schema.sql`。

> **現行契約覆蓋聲明（2026-09-15）：** 此歷史交付曾在 target 缺席時建立並投影 `證券戶`；Requirement 152／Task 434 現行規則改為只覆寫既有的台北富邦銀行／證券戶／TWD。不存在該唯一 TWD 證券戶時必須 fail closed，不得新增、轉移、轉幣或改寫活存／任何其他存款列。以下已勾選內容與完成報告僅保留當時實作證據，不可作為後續實作指示。
