...
**對應 Requirements:** Requirement 128（歷史交付；原本的台北富邦銀行證券戶投影已不再是現行契約）
**前置任務:** 無；重用既有 Fubon adapter、configured admin、共用快照鎖與聚合計算器。
**Liquibase changeset:** 無；不得新增表、欄位、索引或變更 `db/schema.sql`。

> **現行契約覆蓋聲明（2026-09-13）：** 此歷史交付將 `bank_remain.balance` 投影到 `證券戶`；使用者已明確改定為只覆寫既有的台北富邦銀行／台幣活存。不存在該唯一 TWD 活存時必須 fail closed，不得新增 `證券戶` 或任何存款列。以下已勾選內容與完成報告僅保留當時實作證據，不可作為後續實作指示。
