...
- [ ] **356.1b-2** `swingReasons` 必須比照既有兩軌加入還原權息揭露句。現行 `TradingRadarService` 對 `reasons` 與 `shortReasons` **各加一句**（「MA／KD、兩日確認與規則漲跌已使用還原權息／分割價…」「MA／KD、擴充指標與相對量已使用同一份還原權息／分割序列。」），不補則同一頁三軌中兩軌有揭露、一軌沒有。
...
- [ ] **356.2b-2** **分桶規則必須與 BFF 既有的 `ChartSeriesAligner.weekly(...)` 逐字一致**（同為 `WeekFields.ISO` 的 `weekBasedYear` ＋ `weekOfWeekBasedYear`，`open` 取該週最早交易日、`high`／`low` 取極值、`close` 取最晚交易日、`weekEndDate` 取該週最晚交易日）。理由：同一檔股票在「股票分析走勢圖的週K」與「交易雷達的週K」如果連**哪幾天算同一週、哪一天是週收盤**都不一樣，使用者無從解釋，這違反本專案「同義欄位必須同源」的規範。價基與進行中週的差異是刻意且已記載的（見背景段），**分桶規則的差異則不可接受**。
  ...
  >
  > **`ChartSeriesAligner.weekly` 另有兩條納入條件，本任務刻意不照抄，必須明文列為第四、第五處差異：**
  > 1. **`requestedStart` 起始週丟棄**——BFF 在 `start` 不是週一時把 `start` 所屬那一整週丟掉（避免圖表首週被截斷成半根）。business 這一側沒有 `requestedStart` 概念，**不適用**。
  > 2. **invalid candle 整週丟棄**——BFF 只要該週任一日的 candle 不合法（O/H/L/C 任一為 null 或非正、`H < max(O,C)`、`L > min(O,C)`、`H < L`）就丟掉整週；本任務採 356.2c 的**逐欄** null ＋ `close` 缺值才丟整根，再由 356.3c-2 把 `high`／`low` 為 null 的週排除在**指標序列**之外。
  >
  > 兩者因此仍保證**週界完全相同**（哪幾天算同一週、哪一天是週收盤），只在「壞資料週是否呈現」上不同。**這一點必須在雙邊測試中明確斷言，而不是留白**：356.2 的測試案例除了「一般週、短週、ISO 跨年、單日成週」，還要加上 `ChartSeriesAlignerTest` 既有的「中段 invalid／壞週」一組，並明訂兩邊的預期結果**刻意不同**——否則這個分歧不會被任何測試抓到。
...
- [ ] **356.3d** **最少樣本 60 根完成週**：完成週不足 60 根時整組週K 指標為 `null`（不是部分算、不是以短序列硬算）。理由寫進具名常數 Javadoc：週MACD 的 OSC 要到第 34 根完成週才有第一個值（EMA26 以 26 根 SMA 作 seed → 第 26 根才有 DIF；signal 以 9 根 DIF 的 SMA 作 seed → 第 34 根才有 MACD），60 根留了 26 根的 EMA 收斂餘裕。**此數字為判斷性取值，無回測依據**，不得於任何文案宣稱它能提高準確度。
  ...
  > **這個 60 會有兩份宣告，這是刻意的。** 理由是**循環參照**，不是 Spring 依賴——`TradingRadarRuleEngine` 本身也標了 `@Component`，而且 `MIN_COMPLETED_WEEKS` 是 `public static final int`（compile-time constant），引用它不會產生任何 Spring 依賴。真正的障礙是 `RadarInputAssembler` 已經依賴引擎的型別（`new TradingRadarRuleEngine.CandleInput(...)`），反向 import 會讓兩個類互相依賴。故引擎自留一份常數，兩份必須以測試釘住相等（值改一邊而沒改另一邊即紅燈）。
...
- [ ] **356.4a** `TradingRadarService.buildStock` 的 `priceHistoryRepo.findRecentN(code, market, 250)` 改為 **500**（as-built 以具名常數 `TradingRadarService.SERIES_FETCH_ROWS` 承載，三處共用；`BacktestService.WINDOW` 另有一份同值宣告，其 Javadoc 自述與之一致——**該不變式目前只寫在 Javadoc、沒有測試釘住**，比照 `MIN_COMPLETED_WEEKS` 那組補一條相等斷言）（≈100 個 ISO 週，扣掉 provenance／未來列剔除後仍足以取得 60 根完成週）。台股大盤 `twseRepo.findTopNByOrderByTradingDateDesc(241)` 與美股大盤 `usIndexDailyHistoryRepo.findTopNByIndexCodeOrderByTradingDateDesc(IXIC_CODE, 241)` 同步改為 `500`。
...
- [ ] **356.4d-3** `assemble(...)` 既有的 `completedRowCount` 參數**仍以日K 契約的完成列數傳入**（`indicatorSeriesRows.size()`，上限 241），不得因為 `combined` 改成長序列就順手改傳長序列長度。改了雖不影響 `confirm`（只讀 `0..period`）與 `returnStdDev60Ratio`（只讀 `0..60`），但 `completedCloses` 會由 ≤241 變成 ~499，直接撞上 356.4g 的逐位不變斷言。
...
- [ ] **356.4e** **兩個已查證會被擴窗改掉的既有輸出，必須明確釘回日K 契約視窗**（這兩條是實作時最容易漏、且漏了完全靜默的地方）：
  ...
  | `volumeRatio` | **沒有列數上限**：迴圈由最新完成日的下一列往舊掃，終止條件是「湊滿 `VOLUME_LOOKBACK`(20) 筆**正**成交量」**或掃完整份 list**，呼叫端傳的是整份 `adjustedRows` | 前段正成交量不足 20 筆的序列（`volume` 大量為 null 的舊列、指數映射列——`BacktestService` 的 TAIEX 映射就完全沒帶 volume）現況回 `null`，擴窗後會湊滿而回出值 → 量價因子由缺值變有值 → 分數改變 | 一併改吃 `adjustedRows.subList(0, min(size, dailyContractRows + (liveAdded ? 1 : 0)))`。356.4g 的測試資料須刻意包含一筆「前 21 列含大量 null volume、但更舊的列有正成交量」的序列 |
...
- [ ] **356.4f** 其餘既有輸出已各自有界，**不得**在本次順手改動其視窗：`indicators`／`week52High`／`week52Low` 由 `indicatorRows = min(size, liveAdded ? 241 : 240)` 界定、`completedCloses` 由 `completedRowCount` 界定、`kdBandWidthPercent` 取前 9 列、`previousAdjustedClose`／`completedChangePercent` 取固定索引。
...
- [ ] **356.4g** 必須有回歸測試以**固定資料**斷言擴窗前後**日K 路徑的中間值**——`indicators`、`completedCloses`、`week52High/Low`、`kdBandWidthPercent`、三個 `Confirmation`、`ma60BiasPercent`／`ma60BiasPercentile`、`volumeRatio`、`ruleChangePercent`、`distributionAdjusted`——**逐位不變**。

  > **⚠ 這條測試的對象只有日K 中間值，不包含最終 `score`／`action`／`reasons`／`risks`。** 擴窗正是週K 因子生效的前提：250 列 ≈ 48–50 個 ISO 週 < 356.3d 的 60 根門檻 → 四組週K 因子全缺值並重分配權重；500 列 ≈ 100 週 → 四組生效（權重合計 `0.10`／`0.15`／`0.18`）。兩臂的加權平均**必然不同**，`reasons` 另外還會多出三則「最新價位於週MA5／週MA10／週MA20 之上／之下」。把最終輸出寫進逐位不變的斷言，等於要求一條**不可能綠燈**的測試。最終輸出的逐位不變**只在兩臂都把 `weekly` 強制為 `null` 時**成立——那一組要另外寫，並再加一條**反向**測試：`weekly` 由缺值變可用時 `score` 確實改變（否則「新因子有沒有接上」根本沒有被驗證）。測試資料必須**刻意包含**一筆除息日落在第 241 列之外的事件，否則 356.4e 第二列的防護不會被覆蓋到。**不得**以「應該不會變」作為結論。
...
- [ ] **356.5b** 三個分量，每個各自 clamp 在 `[-1,1]`，**取可用值的平均**（缺值不計入平均、不以 0 冒充）：
  - **收盤區間位置**：`closePosition = (close - low) / (high - low)`，貢獻 `= clamp((closePosition - 0.5) * 2)`。收在上緣為正（買方主導），下緣為負。全幅非正（`high - low <= 0`，含無波動、漲跌停鎖死與倒置髒列）時此分量缺值。
  - **實體方向**：`sign(close - open)`，即 `+1`／`-1`／`0`。四個輸入任一缺值時缺值；**全幅非正（`high - low <= 0`）時亦缺值**。

    因此實體方向的純函數簽章為 `bodyDirection(open, high, low, close)`（不是只吃 `open`／`close`）；DTO 的 `dailyCandle.bodyDirection`／`weeklyIndicators.bodyDirection` 也一併為 `null`、畫面顯示 `—`。

    > **這是本任務唯一一處「缺值判準不只看該分量自己的輸入」，理由要寫進 Javadoc。** 正常來源資料下 `high >= max(open, close)`、`low <= min(open, close)`，全幅為零就代表這根 K 棒是一個沒有價格區間的點，不是十字線。若只判「缺 `open`」，`sign(close - open)` 會回 `0`，整個因子得到一個**看似有效的中性讀數**（`0` 會進 `Accumulator` 的 `sumW`），使用者與後續稽核都會讀成「十字線＝多空平衡」，那是完全不同的狀態。台股漲跌停鎖死、極低流動性標的整日只有一筆成交，都會走到這一條。
    >
    > **但不得把那組不等式寫成「資料保證」——本專案沒有這個保證。** `db/schema.sql` 的 `stock_price_history` 對 `open_price`／`high_price`／`low_price` **三欄皆 nullable 且沒有任何 CHECK**（唯一的 CHECK 是 `ck_sph_close_price_positive`）；唯一的寫入端 `StockSourceQuery.upsertHistory`／`upsertVerifiedHistory` 也只擋非正收盤，對 O/H/L 不做關係檢查。BFF 之所以有 `ChartSeriesAligner.validCandle`（會擋 `H < max(O,C)`、`L > min(O,C)`、`H < L`），正是因為髒列真的存在；`TechnicalIndicatorService` 更有「`high`／`low` 缺值時 fallback 用 `close`」的既有慣例，會自己產生 `high == low` 的退化 K 棒。**因此判準必須寫成「全幅非正」而不是「相等」**：`high < low` 的倒置髒列若只判相等，會走到 `sign(close - open)` 回 `±1`，讓一根確定無效的 K 棒拿到滿貢獻，且完全靜默。

  - [ ] **356.5b-2** **三支純函數必須共用同一條全幅判準。** 第一階段已落地的 `closePosition` 與 `lowerShadowRatio` 用的是 `BigDecimal range = high.subtract(low); if (range.signum() <= 0) return null;`；`bodyDirection` 必須用**同一段條件**（`compareTo` 語意，**不得用 `equals`**——還原後 BigDecimal 的 scale 會由 4 變 8，`equals` 會判不相等）。三支的全幅判準抽成同一個私有 helper 或同一行條件，不得各寫一份。
  - [ ] **356.5b-3** **同步更新第一階段已落地的程式與測試。** `TradingRadarRuleEngine.bodyDirection(BigDecimal open, BigDecimal close)` 由兩參數改為四參數；其 Javadoc 中「缺 `open` 或 `close` 時缺值」與把 `0` 稱作「十字線」的敘述一併更新。`TradingRadarCandleComponentsTest` 全部呼叫點同步改簽章，其中 `highEqualsLowMakesRangeComponentsUnavailable` 的 `bodyDirection` 斷言由 `isEqualByComparingTo("0")` 改為 `isNull()`（含該行註解），`dojiHasZeroBodyDirection` 必須改以 **`high > low`** 的十字線構造（現況是 `bodyDirection(bd(12), bd(12))`，完全沒有 high／low，直接補成 `high = low = 12` 會踩進新的 null 分支而得到相反的期望）。另須新增一筆 `high < low` 的倒置髒列案例，斷言三個分量全缺。
  - **下影線比例**：`lowerShadow = (min(open, close) - low) / (high - low)`，貢獻 `= clamp((lowerShadow - 0.25) * 4)`。長下影線代表低檔有承接，為正貢獻；`0.25` 為中性點（四分之一全幅），是判斷性取值、無回測依據，須於 Javadoc 明寫。缺 `open`／`high`／`low`／`close`，或全幅非正（`high - low <= 0`）時缺值。

  綜合上述三條：**全幅非正（含 `high == low` 與 `high < low`）時整個 `DAILY_CANDLE` 因子必為 `null`**（三個分量同時缺值），這與「驗證」段的斷言一致。
- [ ] **356.5c** **三個分量抽成 `public static` 純函數**（`RadarInputAssembler.volumeRatio`／`bandWidthPercent` 雖然也是抽出來供測試直接呼叫，但它們是 `@Component` 上的 **instance** 方法，不要照抄成 instance；這裡要的是無狀態 static），引擎的因子計算與 DTO 的 `DailyCandle`／`WeeklyIndicators` 映射共用同一支，不得各寫一份。** 注意兩邊要的東西不同：DTO 要的是**未經 clamp／未經線性轉換的原值**（`closePosition ∈ [0,1]`、`bodyDirection ∈ {-1,0,1} 或 null`、`lowerShadowRatio ∈ [0,1]`），引擎要的是由原值再轉出的 contribution。純函數回原值，轉換留在引擎。
- [ ] **356.5d** 三個分量全部缺值時整個 `DAILY_CANDLE` 因子為 `null`，權重由 `Accumulator` 重分配，**並在 `risks` 加一則揭露**。缺值原因有兩種，文案必須分開、不得互相冒充：全幅非正時為「最新完成日 K 棒沒有價格區間（漲跌停鎖死或整日單一成交價），日K 棒因子不採計，缺值權重已重分配。」；`open`／`high`／`low` 缺漏（歷史列常見，三欄皆 nullable）時為「最新完成日 K 棒的 OHLC 資料缺漏，日K 棒因子不採計，缺值權重已重分配。」**不得沉默**——理由與 356.6e 對週K 的要求相同，而且這條在台股是每天都可能命中的實際路徑，不是理論案例。
...
- [ ] **356.6c `WEEKLY_BIAS`（週線乖離）**：`clamp(-bias10 / 15)` 與 `clamp(-bias20 / 20)` 取可用值平均。負乖離（現價在週均線下方）為正貢獻＝低接位置，正乖離為負貢獻＝追價成本。分母：`bias10` 由日K 的 `10` 放寬為 `15`（週MA10 ≈ 一季尺度，乖離量級大於日MA10）；`bias20` **維持 `20` 不變**（週MA20 ≈ 半年尺度，實測乖離量級與日MA20 相近，刻意不放寬——這不是筆誤）。兩者皆為判斷性取值、無回測依據，須於 Javadoc 明寫。
- [ ] **356.6d `WEEKLY_CANDLE_VOLUME`（週K 棒與量能）**：三個分量取可用值平均——(i) 週收盤區間位置，公式同 356.5b 第一項但用週K 棒；(ii) 週實體方向 `sign(weeklyClose - weeklyOpen)`，**缺值判準同 356.5b／356.5b-2**（四個輸入任一缺值，或週全幅非正 `weeklyHigh - weeklyLow <= 0` 時缺值；後者代表整週沒有任何價格區間，不是十字線）；(iii) 週量價確認，沿用日K 的四象限：`changePercent > 0 && volumeRatio >= 1.2 => +1`；`changePercent < 0 && volumeRatio >= 1.2 => -1`；`changePercent > 0 && volumeRatio <= 0.7 => -0.4`；`changePercent < 0 && volumeRatio <= 0.7 => +0.4`；其餘 `0`。
...
- [ ] **356.9b** `TradingRadarEvidenceGate.GatedActions` 擴為三軌。既有形狀為 `(mediumAction, shortAction, reasons, candidateMediumAction, candidateShortAction)`，**新增 `swingAction`（緊接 `shortAction`）與 `candidateSwingAction`（緊接 `candidateShortAction`）**——命名必須與既有兩軌對稱，不得寫成 `rawSwingAction`。`apply(...)` 對三軌各自套用既有的 buy 降級與 risk-evidence 判定；既有兩軌的 gate 行為不得改變。
  ...
  > **swing 軌的結構性 gate 採「與 SHORT 同形」**：confidence gate ＋ `ASSET_SPECIFIC`，**不**套用 MEDIUM 的「個股必須同時具備 VALUATION 與 FINANCIAL 證據才開買進閘門」。理由：基本面資料是季度更新，一個月尺度的建議不該因為財報季尚未更新就整軌關閉；SWING 的估值與財務仍以 group 權重各 `.10` 進入 `swingConfidence`，只是不作為硬性結構條件。**此選擇為判斷性取值、無回測依據**，須於 `swingOpen()` 的 Javadoc 明寫。
...
- [ ] **356.10a-3** **大盤週K 聚合的回傳值必須有顯式 null 防護。** `buildMarket`／`buildUsMarket` 的整段是被 `try/catch` 包住的，聚合回傳 null 造成的 NPE 會被 catch 靜默吞成 `incompleteMarket()`——**不報錯，卻讓整組大盤變 `DATA_INCOMPLETE`，連帶關掉全部個股的買進閘門**。這與既有 `usContext == null` 三元防護同一理由，須加顯式防護並於 Javadoc 寫明。
...
- [ ] **356.11a** `TradingRadarDto` 新增兩個 record：
  ...
  - `WeeklyIndicators` 的 `volume` 與其餘指標**同進退**：完成週不足 60 根時整組（含 `volume`）皆為 `null`。理由：整組指標缺值時單獨露出一個成交量，會讓畫面看起來像「部分可用」，與 356.3d「不是部分算」的立意相反。
...
- [ ] **356.12a** `TradingRadarExportService` 個股決策工作表新增「1周~1月 動作」「1周~1月 分數」兩欄（緊接既有短期欄之後），以及週K 摘要欄（`週結束日`、`週MA10`、`週K`、`週D`、`週OSC`、`週量比`、`週漲跌%`）與日K 棒欄（`日K收盤位置`、`日K實體`、`日K下影線比`）。**另補上 1周~1月 軌的四個證據欄**（`1周~1月證據信心`、`1周~1月下檔風險`、`1周~1月風險覆蓋`、`1周~1月候選動作`），位置比照既有的短期／中期兩套；否則匯出會變成三軌分數卻只有兩軌證據。**既有表頭字串「短中期分歧」改為「持有期分歧」**（它現在描述的是三軌）。**欄位位置**：swing 的動作／分數緊接短期欄之後、swing 四個證據欄比照既有短期／中期兩套的位置；週K 7 欄與日K 棒 3 欄則**附加在整張表真正最末**（既有即時折溢價兩欄之後），沿用 Task 320 的既有慣例，把既有欄索引的位移降到最小（既有測試中硬編碼的欄索引仍須同步更新），並**同步更新既有測試 `TradingRadarDualFormatTest` 中對該表頭字串的斷言**。

  > **`TradingRadarExportService` 的 `headers`／`formats`／每列 cell 是三份必須同步的平行清單，只改表頭一定爆。** 程式內既有註解已寫明「headers／formats／rows 三者長度與順序必須一致——`ExportDoc.Table` 只在 **runtime** 才擲長度不符」，且 `formats` 是逐欄帶索引註解的 `ExportDoc.Format` 清單。本次一次新增 2（swing 動作／分數）＋7（週K）＋3（日K 棒）＋4（swing 證據）＝ **16 欄**，三份清單都要照同一順序插入。驗證段須有一條「表頭長度 ＝ formats 長度 ＝ 每列 cell 數」的斷言，Excel 與 JSON 各驗一次。大盤總覽工作表在**真正最末**新增與個股決策同名同序的週K 七欄（`週結束日`、`週MA10`、`週K`、`週D`、`週OSC`、`週量比`、`週漲跌%`），取自 `market`／`usMarket` 節點；週線**方向**由既有的「支持訊號」／「風險提醒」兩欄的文字承載（356.10c），**不另立「週線方向」欄**。Excel 與 JSON 走同一份 `ExportDoc`；舊快照對應欄位留白。
...
- [ ] **356.12c** `frontend/src/views/TradingRadarView.vue`：
  ...
  - **收合列在既有「KD」欄之後新增一欄「週K」**（使用者 2026-08-22 追加）：顯示 `週K/D` 與週漲跌幅兩行（例：`K 58.1 / D 54.3`、`+1.2%`），資料取自 `weeklyIndicators.k`／`d`／`changePercent`，缺值顯示 `—`。**只加這一欄**——收合列已有 15 欄並需橫向滾動，日K 棒與週K 的完整數值仍只在展開列。週漲跌沿用既有漲紅跌綠配色。`weeklyIndicators` 為 `null`（舊快照或完成週不足 60 根）時整欄顯示 `—`，不得顯示 0。
...
- [ ] **356.13a-2b** **為了讓 356.13 的驗收條目可斷言，下列成員由 `private` 放寬為 package-private，值與行為一律不動**：`BacktestService` 的 `WINDOW`／`WARMUP`／`DAILY_CONTRACT_ROWS`／`WEEKLY_WARMUP_ROWS`，以及 `TradingRadarService.bestScore`。替代方案（測試端複製字面值 241／500、或反射讀 private）都更差——前者正是 `BacktestService` 既有註解點名要避免的「斷言與實作各自漂移」。放寬後**不得**有任何 production 類跨界取用，須以測試釘住。
- [ ] **356.13a-2c** **放大 `WARMUP` 會讓既有回測測試的 fixture 一次紅十幾條，且失敗訊息看起來像「回測邏輯壞了」。** `BacktestServiceTest` 中寫死列數（270／280／330）的 fixture 一律改以 `WARMUP_ROWS + n` 表示，使可評估訊號日數與改動前**逐一相同**（30／40／90）；區間索引（如 250／260）同步改為 `WARMUP_ROWS + 10/20`，區間長度與期望值不變。**不得**為了讓測試變綠而調整期望值。
...
- [ ] **356.14b** 以 `rg -n` 掃描 active code 中所有「短期／中期」「雙軌」「兩軌」「`TW_RULES_V14`」「週線 MA5」等敘述，凡實際行為已改者一併修正 Javadoc、行內註解與畫面文案。**歷史任務檔、歷史版本敘述與 Liquibase changeset 不做批次取代。** 掃描範圍必須包含**執行期組出來的字串**（`positionOf`／`describeHeat` 等傳入的 label、`TradingRadarExportService` 的表頭常數、`TradingRadarView.vue` 的 tag 文案），不只是 Javadoc 與行內註解——本次已知的**四處**是：`positionOf(..., "週線", ...)`（見 356.6a）、匯出表頭「短中期分歧」（見 356.12a），以及兩則會直接顯示在展開列與 Excel 的 risks／reasons 字串——`TradingRadarEvidenceGate` 的「底層外幣債券幣別資料不完整，**兩軌**買進／加碼／試單閘門關閉…」與 `TradingRadarRuleEngine` 的「基本面多項惡化，**兩軌**的買進／加碼／試單閘門關閉…」，兩者在 V15 之後都是錯誤陳述，一律改為「三軌」。掃描指令至少要跑 `rg -n '兩軌|雙軌|短中' backend/src/main bff/src/main frontend/src`。
...
- **擴窗回歸（356.4g）**：以固定資料斷言 `findRecentN` 由 250 改 500 後，**日K 路徑的全部中間值**逐位不變（**不含**最終 `score`／`action`／`reasons`／`risks`——那些本來就會因為週K 因子生效而改變）。測試資料**必須刻意包含一筆除息日落在日K 契約視窗之外的事件**，用以覆蓋 `distributionAdjusted` 不得翻面；另須有一組專測 `ma60BiasPercentile` 的案例，**`liveAdded=true` 與 `liveAdded=false` 兩種都要驗，且兩者觀測數都必須是 182**（不是 441、也不是 181）。**全部價格比對一律用 `compareTo` / `isEqualByComparingTo`，不得用 `assertEquals` / `equals`**——BigDecimal scale 會由 4 變 8（見 356.4e 的說明段），用 `equals` 會紅燈在一個不是缺陷的地方。另兩組：(i) 兩臂都把 `weekly` 強制為 `null` 時最終輸出逐位不變；(ii) **反向**斷言 `weekly` 由缺值變可用時 `score` 確實改變。
- **`volumeRatio` 不被擴窗改值**：測試資料含「前 21 列大量 `volume` 為 null、更舊的列才有正成交量」的序列，斷言擴窗後仍回 `null`（該函式沒有列數上限，只以 20 筆正樣本為界，是第三個會被擴窗靜默改值的地方）。
...
- **執行期文案無「兩軌」殘留**：以四處具名字串正面斷言——`positionOf` 的 MA5 label 為「日線 MA5」、匯出表頭為「持有期分歧」、`TradingRadarEvidenceGate` 與 `TradingRadarRuleEngine` 兩則閘門關閉 risks 句改為「三軌」。**判準是「執行期字串」（`positionOf` label、匯出表頭常數、`risks`／`reasons` 字面量、Vue tag 文案），不是整份原始碼零命中**——描述歷史版本（「V14（歷史）：…雙軌權重架構」）、兩軌相容入口、以及與持有期無關的「短中長期債」都刻意保留，照「零命中」做會逼人去改一批本來正確的 Javadoc。
- **週分桶與 BFF 一致**：`WeeklyBarAggregator` 與 `ChartSeriesAlignerTest` 共用的週界案例（一般週、短週、ISO 跨年、單日成週）產生**相同的週界與 `weekEndDate`**；「中段 invalid／壞週」案例則明確斷言兩邊**刻意不同**（BFF 整週丟棄、business 逐欄 null）。
...
- **日K 棒因子**：全幅非正（`high == low` 漲跌停鎖死、以及 `high < low` 的倒置髒列）→ 三分量全缺、因子 null 且 `risks` 有揭露句；缺 `open` 而全幅為正 → 只剩收盤位置分量；**十字線（`high > low` 且 `close == open`）實體方向為 `0`——`high == low` 不是十字線，實體方向缺值**；長下影線為正貢獻、長上影線為負貢獻。
...
- **swing 證據鏈不是 medium 的複本**：構造一組 short 與 medium 明顯不同的證據，斷言 `swingEvidenceConfidence` 與 `EvidenceGroup.swingCoverage` **不恆等於** medium 對應值；並斷言 horizon 分派已無 default 落 medium 的路徑。

  > **`swingDownsideRisk`／`swingRiskCoverage` 不在此斷言之列，因為它們在現行風險模型下必然等於 medium，這是既有限制而非本任務的缺陷。** `TradingRadarEvidenceConfidenceResolver.risk(Inputs, LocalDate, Horizon)` 的 `horizon` 參數目前**只**用於 `dividendReason` 的文字；數值端的 `eventRisk` 同時看 5 日與 20 日兩個計數、與 horizon 無關——因此**改版前 `shortRisk` 與 `mediumRisk` 的 `downsideRisk` 就已經恆等**。要讓三軌的下檔風險數值真正分岔，需要新的 horizon-specific 風險語意與其校準依據，本任務**沒有**提供，也不得為了讓測試好看而臨時發明。此限制須寫進 `risk()` 的 Javadoc；swing 的 `dividend_event` 仍須帶自己那份顯式視窗字串，且 `evidence.risk(SWING)` 必須回自己那個物件（不是 medium 的參照）。
...
- `dailyCandle` 的 `open/high/low/close` 與該檔最新完成日 K 一致；`closePosition ∈ [0,1]`、`bodyDirection ∈ {-1,0,1} 或 null`、`lowerShadowRatio ∈ [0,1]`。
...
完成日期：2026-08-22。`RULE_VERSION` = `TW_RULES_V15`。

### 測試與建置

| 項目 | 結果 |
|---|---|
| `mvn -f backend/pom.xml test` | **1343 passed / 0 failures / 0 errors**（本任務前為 1220） |
| `mvn -f bff/pom.xml test` | 166 passed / 0 failures |
| `npm run build`（frontend） | exit 0，2406 modules |
| `scripts/spec-check.sh` | BLOCK 0 / CHECK 0 |
| arch-auditor | 0 critical / 0 major / 1 minor（已補 `structure.md` §3.2 具名例外之四） |

### 實資料驗收（`GET /api/trading-radar`，容器內帶 `X-User-*`）

- `ruleVersion=TW_RULES_V15`；35 檔中 33 檔三軌皆有分數，**25 檔三軌分數互異**（證明不是同一分數切三刀）。
- `dailyCandle` 35 檔全有；週K 指標可用 33 檔。例：2330 `completedWeeks=105`、週MA10 `2375.0`、週K `58.1`、週OSC `-21.9`、週量比 `0.55`；日K 棒 `closePosition=1.0`、`bodyDirection=1`、`lowerShadowRatio=0.222`。
- `weekEndDate=2026-08-14`——**進行中週（2026-08-17～21）正確被排除**，符合 356.2d。
- 「OHLC 資料缺漏」「週K 完成週不足」在 35 檔中 **0 列**。
- 台股與美股大盤各有自己的 `weeklyIndicators`（`ma10` 分別為 44682.4 / 25910.84），非同一份複製。
- 瀏覽器實機確認三軌欄並列且無擠壓（2330：一周 66／1周~1月 69／1月~6月 73），「持有期分歧」欄正常。

### 三軌回測（`POST /internal/backtest/rules`，horizons `5/10/20/60/120`，51 標的，2016-08-22～2026-08-21）

`diff` 為對同標的同期間基準的平均報酬差額（正＝優於基準）。

| 述詞 | n(h=5) | diff@5 | diff@10 | diff@20 | diff@60 | diff@120 |
|---|---:|---:|---:|---:|---:|---:|
| `SHORT_BUY` | 145 | +0.09 | +0.97 | +0.27 | +2.71 | +6.00 |
| `SWING_BUY` | 241 | +0.28 | +0.79 | +0.30 | +0.55 | +2.12 |
| `MEDIUM_BUY` | 567 | +0.12 | +0.51 | +0.11 | **−0.07** | **−0.60** |
| `SHORT_PROFIT_TAKING` | 133 | +0.25 | +1.02 | +1.92 | +3.82 | +9.24 |
| `SWING_PROFIT_TAKING` | 133 | +0.25 | +1.02 | +1.92 | +3.82 | +9.24 |
| `MEDIUM_PROFIT_TAKING` | 133 | +0.25 | +1.02 | +1.92 | +3.82 | +9.24 |
| `SHORT_EXTREME_OVERSOLD_PROTECTED` | 1417 | +0.88 | +1.12 | +2.16 | +1.18 | +0.84 |
| `SWING_EXTREME_OVERSOLD_PROTECTED` | 1790 | +0.80 | +1.16 | +2.02 | +1.46 | +0.58 |
| `MEDIUM_EXTREME_OVERSOLD_PROTECTED` | 1757 | +0.90 | +1.26 | +2.18 | +1.45 | +0.77 |

**三項如實揭露（356.13d：結果不好也不得隱藏、不得事後調參）：**

1. **獲利了結在三軌完全同值，且每個 horizon 都優於基準。** 同值是因為 `profitTakingConfirmed` 對三軌施加同一個覆寫，三個述詞命中的是同一批列——swing 版本不提供任何獨立資訊。更重要的是方向：訊號成立後的前瞻報酬**高於**基準 `+0.25 ~ +9.24`，代表「高檔多證據轉弱就減碼」在這段歷史上是**虧錢的**，與 t273 推翻「KD 死叉單獨減碼」是同一個方向的證據。本任務**未據此調整**門檻或權重（那需要先改規格再重審）。
2. **`MEDIUM_BUY` 在自己的主場 horizon 反而略輸基準**（60 日 −0.07、120 日 −0.60），而 `SHORT_BUY` 與新增的 `SWING_BUY` 在五個 horizon 全部優於基準。
3. **`SWING_BUY` 的樣本數（241）介於 short（145）與 medium（567）之間，diff 全為正**，是三軌中唯一「每個 horizon 都贏基準且樣本充足」的買進述詞。

### 356.13a-3：暖機由 240 放大到 305 的樣本影響

`WARMUP = max(FULL_WINDOW 240, (MIN_COMPLETED_WEEKS 60 + 1) × 5 = 305) = 305`。

- 51 檔中 **3 檔** 的可評估訊號日低於 `MIN_SAMPLES = 30`：`009826`（rows 15，evaluated 0）、`009816`（rows 131，evaluated 0）、`009804`（rows 333，evaluated 28）。
- 其餘 48 檔可評估訊號日中位數 **2099**、最大 2134，不受影響。
- 三檔皆為上市未滿兩年的新標的，非既有主要持股；其數字一律標記 `sampleInsufficient`，不得用於決策。

### 與本計畫的偏差

1. **編號避讓**：分支進行中 `origin/main` landed 了 Requirement 92／Task 355（股票分析價格圖日K／週K），本任務避讓為 **Requirement 93／Task 356**。
2. **兩處判斷性決定**（規格未預先涵蓋，已於 Javadoc 標為判斷性取值、無回測依據）：swing 的結構性 evidence gate 採「與 SHORT 同形」；`WEEKLY_OSC_FULL_SCALE_PCT = 2.0` 與日K 的 `0.5` 並存。
3. **`swingDownsideRisk`／`swingRiskCoverage` 恆等於 medium**：`risk(Inputs, LocalDate, Horizon)` 的 horizon 目前只影響文字、不影響數值，改版前 short 與 medium 即已恆等。此為既有限制，未在本任務發明新的 horizon-specific 風險語意。
4. **實作過程中查出並修正的四個缺陷**（都不在原計畫預期內）：
   - production 的 `StockInput` 只傳 25 個引數而命中相容建構式 → `dailyCandle`／`weekly` 恆為 `null`，**五個新因子在正式路徑上完全沒有生效**；同一陷阱在 `BacktestService` 另有兩處。**把修正還原回缺陷版本後，1334 個既有測試全部綠燈**——新因子對分數的實際影響零覆蓋，已補 5 條會在缺陷版紅燈的測試。
   - `swingReasons` 缺少還原權息揭露句，且程式註解宣稱了一個不存在的機制。
   - 日K 棒的兩種缺值原因會互相冒充，使 OHLC 缺漏被誤報成「漲跌停鎖死」。
   - 回測完全沒有 swing 軌述詞，該軌零稽核覆蓋（已補 `SWING_BUY`／`SWING_PROFIT_TAKING`／`SWING_EXTREME_OVERSOLD_PROTECTED`）。
5. **未量測**：雷達建置延遲在擴窗前後的對照未取得數字（需要在同一台機器上以相同資料跑前後兩版，本次未做）。實機回應時間主觀上無明顯變化，但**這不構成量測證據**。
