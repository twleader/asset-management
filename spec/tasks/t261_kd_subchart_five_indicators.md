...
6. **子圖高度與 x 軸標籤（使用者回饋後調整）**：子圖 90px 過矮，三條 KD 線擠成一團看不出交叉——這才是「線圖很亂」的真因（不是指標太多）。改為 `grid[1].height: 155`、上下 pane 約 58:42；KD legend 那一列上下各留 16px（原本只有 6~8px，緊貼上圖底軸）。另 x 軸標籤原本旋轉 30 度且連續出現多個「2025-08」（ECharts 預設每隔 N 格顯示一個，而 N 個交易日常落在同月），改為**只在月份切換那一格顯示**、依可視範圍抽稀至最多 10 個、取消旋轉。
7. **legend 改為兩組（使用者回饋後調整）**：原本十項全放頂端一列，實機過密。改為 ECharts `legend` 陣列——上組（股價／月線／季線／年線／成本均價，`top: 8`）與下組（K9／D9／J9／K3D2／RSV，`bottom: 156`＝KD 子圖正上方、兩張圖中間），`grid[0].bottom` 190 → 200 讓出該列，`itemGap` 回到 30。
