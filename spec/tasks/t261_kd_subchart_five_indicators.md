...
6. **子圖高度與 x 軸標籤（使用者回饋後調整；時序上晚於第 7 點，最終座標以此為準）**：子圖 90px 過矮，三條 KD 線擠成一團看不出交叉——這才是「線圖很亂」的真因（不是指標太多）。改為 `grid[1].height: 155`、上下 pane 約 58:42；KD legend 那一列上下各留 16px（原本只有 6~8px，緊貼上圖底軸）。另 x 軸標籤原本旋轉 30 度且連續出現多個「2025-08」（ECharts 預設每隔 N 格顯示一個，而 N 個交易日常落在同月），改為**只在月份切換那一格顯示**、依可視範圍抽稀至最多 10 個、取消旋轉。
7. **legend 改為兩組（使用者回饋後調整；發生在第 6 點之前）**：原本十項全放頂端一列，實機過密。改為 ECharts `legend` 陣列——上組（股價／月線／季線／年線／成本均價，`top: 8`）與下組（K9／D9／J9／K3D2／RSV，置於 KD 子圖正上方、兩張圖中間），並讓出該列所需空間，`itemGap` 回到 30。
   ⚠️ 此點當時的座標（legend `bottom: 156`、`grid[0].bottom` 190 → 200）**已被第 6 點的加高取代**，勿照抄。**現行座標以程式碼為準**：`legend[1].bottom: 231`、`grid[0] { top: 72, bottom: 283 }`、`grid[1] { height: 155, bottom: 60 }`。
8. **MA 累加方向（架構查證後修正）**：新增的 `maAt()` 原本以「舊→新」累加，而既有 `simpleMa()`／`taiexSimpleMa()` 是「新→舊」。double 加法不可結合，末位差經 `setScale(2, HALF_UP)` 會在 `x.xx5` 邊界翻面——實測 MA20 約 **1.7%** 的日子會與 `computeAll()` 差 0.01，走勢圖 legend 的「月線MA20」就會跟觀察清單表格對不上，正好牴觸本任務的核心宣稱。已把 `maAt()` 迴圈方向改為新→舊（**不動 `simpleMa`**，動它會改到 `computeAll()` 的對外數值），並新增「40 組隨機漫步價格掃邊界」的迴歸測試釘住此順序。
