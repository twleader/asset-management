...
**前置任務:** 無
**後續任務:** t274（波動度正規化門檻）、t275（債券 ETF 的利率因子）、t276（既有技術指標與量能納入評分）、t277（短線／中長線雙軌建議）——**四者**的門檻、因子形式與權重都必須引用本任務的輸出
...
- [ ] 273.1 新增 `BacktestService`，`@Service`、`@Transactional(readOnly = true)`。輸入為「標的清單（可選，預設全部台股）」與「日期區間（可選，預設全部）」，輸出為結構化統計結果。
  ...
  **資料來源**（只讀，不寫）：
  ...
  >
  > 故本任務必須：(a) 輸出**逐標的、逐 horizon 標記「ETF 折溢價欄位可得的天數」**；(b) 述詞 1 的 `OVERBOUGHT` 與述詞 6 的 `buyGate` 對 **ETF 標的**額外標記「本次量測未含折溢價否決」；(c) **禁止**把 ETF 的 `buyGate` 統計直接當成 production 行為的證據供 t274／**t275**／t276／t277 引用。**t275 尤其要注意**——它的評分對象 100% 是 ETF。
...
- [ ] 273.4b t274／t275／t276／t277 的 header 都寫著「門檻／因子須以 t273 的量測結果決定」，但 273.4 的 8 條述詞是**寫死的**，只能回答「現行門檻表現如何」，**回答不了「改成 X 會如何」**。本任務須額外提供：

  - [ ] **273.4b.1 門檻掃描**：對下列可調量接受**候選值清單**，每個候選值各跑一次完整統計並輸出可比較的表：
    - 季線乖離的 `σ` 倍數（t274 需要，先行量測顯示搜尋範圍在 9σ–15σ）
    - `σ` 的絕對下限（t274 需要）
    - KD 過熱／超賣門檻（現行 `80`／`85`／`20`／`15`）
    - `volumeRatio` 的爆量界線（t276 需要）
  ...
  - [ ] **273.4b.3 輸入／輸出契約須寫死在本檔**：`POST /internal/backtest/rules` 的 request body schema（`codes`／`from`／`to`／`horizons`／`thresholds`／`predicates`）與 CSV 欄位清單須明文定義。**下游四份任務要據此撰寫自己的驗收指令**——t276 的驗收指令已經硬編了 `{"codes":["0050"]}`，若 schema 與之不符，那條指令會靜默失效。
...
- [ ] 273.5 每條述詞在 `+5`／`+20`／`+60`／`+240` 個**交易日**（非日曆日，即序列的索引位移）後各計算一次前瞻報酬。
  ...
  - 四個 horizon **一律全部輸出**，不得只輸出其中一部分。`+5` 對應使用者的短線需求（原話：「5 個交易日後可獲利 5%，這也是不錯的建議」），`+60`／`+240` 對應「持有期數周至兩年」。t276／t277 要靠「同一條規則在不同 horizon 的表現差異」決定該規則屬於哪一軌。
...
- [ ] 273.8.2 **不得因回測結果自動改動任何門檻。** 本任務只產生數據。門檻的調整一律留給 t274／t276／t277，且須引用本框架的輸出作為依據。**當回測結果與使用者已表明的風險偏好衝突時，一律先呈現數據供使用者裁決，不得逕行改動**——`KD_OVERHEAT_K` 即為既有先例（量測結果與直覺相反，最終仍依使用者偏好保留該門檻，並在 Javadoc 標記為「非實證」）。
