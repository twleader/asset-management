...
### Merge 進 main 時的補充（另一 session 執行，2026-08-04）

本任務分支（`claude/daily-volume-chart-9a77fd`）完成時 main 已先落地 t287（大盤排程支援多時段多指數），故 merge 進 main 時另外處理：
- 6 個檔案衝突（`db.changelog-master.yaml` 的 `v1.88.0` 雙重取號——兩支分支各自預留同號但 changeset id 本身不同、皆已各自在 DB 執行成功，故兩個 include 都保留，非本任務重新取號；`GdpTwseBffController.java` 的 `getIndexDaily`／`movingAverage` 採本分支版本（已含 MA5 相容）；`GdpTwseView.vue` 的 `cardTitle`／`legend.data` 採本分支版本，並補上原本被兩邊都遺漏的 `legendData` 初始陣列缺少「週線 (MA5)」；三份 spec 文件為純文字並列）。
- Merge 後跑 `mvn test` 額外發現 bff `SchedulePublicBffControllerTest` 失敗一項：main 既有的「大盤指數匯出」JOBS 描述（t287 帶入，與本任務無關）誤寫成「命中後**各**產出 JSON 與 Excel 兩份」，未含測試要求的「同時產出」字樣，一併修正措辭（不影響行為）。
- `GdpTwseView.vue` 的排程卡「上次執行」顯示：使用者回報 t287 遺留的顯示錯誤（後端已把 `lastRunAt`/`lastRunStatus` 搬到 `times[]` 逐列，前端頂層綁定仍讀舊欄位，永遠顯示空白），順手修成逐列顯示；與本任務的成交量子圖無關，記於此因是同一次 merge 一併處理。
