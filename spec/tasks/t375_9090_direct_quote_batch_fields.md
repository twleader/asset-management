...
- [ ] 375.3 新增 typed `BookSideLevel(BigDecimal price, Long size)`，top-level `bidLevels`、`askLevels` 永遠是 non-null array、每側最多五筆。僅當 direct `quoteDetail.available=true` 且 `source` 精確為 `FUBON_BOOKS` 才從它的 `levels` 投影：各 side 獨立取正 price、正 lots、level order 由近到遠的項目；買方輸出須嚴格價格遞減、賣方輸出須嚴格價格遞增、不得重複。遇到某 side 缺值／零量可形成較短的有效 partial side；若投影後違反 side 順序，該 side 固定為 `[]`。source 非 FUBON、unsupported、unavailable、`0000`、非台股或無 book 時兩側都 `[]`。絕不可用 `buyPrice`／`sellPrice`、估算價、Yahoo 或另一側偽裝／補齊五檔。
...
- [ ] 375.7 在 `PublicQuoteMarketDataServiceTest` 或等價 BFF tests 驗證：完整 raw 19 個欄位與原 `marketData` prefix；direct `quoteDetail`／`dividendHistory` 對 nested counterpart 的值一致；完整 FUBON levels 變成正確 ordered `{price,size}`；非台股、non-FUBON、unavailable side 回空，缺值／零量 side 保留有序 partial array，亂序 side 回空，且沒有多一個行情 request；多筆同年 dividend event 的 row yield、年度 sum／第一筆非 null 分母的畫面同口徑、empty／無分母；list order、one cache miss 204、不含 configured-admin／持股／帳戶／成本／交易／快照欄位。
