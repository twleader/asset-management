...
- [x] 247.1.1 `docker-compose.yml` **第 91 行**（business-services）與**第 158 行**（external-materials-service）
      兩處掛載，由 `- ${HOME}/.config/rclone/rclone.conf:/etc/rclone/rclone.conf:ro`
      改為 `- ${HOME}/.config/rclone:/etc/rclone:ro`。
- [x] 247.1.2 **`RCLONE_CONFIG` 環境變數的值不變**（business 第 62 行、ext 第 142 行，均為
...
- [x] 247.1.3 兩處掛載旁的註解須更新，寫明三件事：（a）掛的是目錄不是單檔，因為 `rclone config` 原子替換會讓
...
- [x] 247.1.4 **已知副作用須在註解記載**：該目錄下其他檔案會一併以唯讀進入容器。實測 host 上
...
- [x] 247.2.1 **新建 `GdriveSelfCheck`**（`backend/src/main/java/com/steven/assets/service/`，名稱可自訂）
      ...
      **自檢邏輯全部放這裡，`ProcessRcloneClient` 的既有程式碼不動**（除 247.2.6 指定的那一句字串）。
      ...
      ```
- [x] 247.2.2 **啟動自檢掛在新的薄元件 `GdriveSelfCheckStarter`**，以
...
- [x] 247.2.3 **前置條件：先查「全庫是否有任一列啟用 Drive」，否則整個啟動自檢跳過。**
...
- [x] 247.2.4 **整段自檢（含前置查詢）包在單一 catch-all 內，任何例外只寫 WARN 後 return。**
...
- [x] 247.2.5 **L1：來源 config 實際讀得到內容。**
...
- [x] 247.2.6 **`ProcessRcloneClient.initConfig()` 既有的 `log.error`（`:137`）維持不變。**
      本任務不改它。自檢**新增**的訊息一律 WARN（「功能不可用」而非「系統故障」，且本機輸出完全正常）；
      同一次失敗會同時出現既有的一則 ERROR 與自檢的一則 WARN，這是可接受的——前者是初始化事實，
      後者才帶修法。
- [x] 247.2.6.1 **唯一授權改動 `ProcessRcloneClient` 的地方：速率限制分支的使用者可見字串**
      ...
      **純字串、無邏輯變更。**
- [x] 247.2.7 **L2：解析 `[<remote>]` section 的 token JSON，檢查 `refresh_token` 存在且非空。**
      ...
      **不得只寫裸的 `rclone config reconnect <remote>:`**——實測那樣會跳過同意畫面而再次拿不到
      ...
      沒讓你按『繼續／允許』，就是沒拿到」。
- [x] 247.2.8 **L3：實跑一次唯讀探測 `rclone lsd <remote>:`。**
...
- [x] 247.3.2 **時序理由必須理解，不可簡化掉這一項**：R50／R51 部署 recreate 當下九張表的 `gdrive_enabled`
...
- [x] 247.3.4 **結果一律不得寫入 `gdrive_last_run_at`／`gdrive_last_status`。**
...
- [x] 247.3.5 **結果改走當次回應。** 九頁的設定儲存 response DTO 各加一個**非持久化**的警告欄位
...
- [x] 247.3.6 **自檢失敗不得讓儲存回非 2xx。** 使用者必須能先把設定存起來再去修授權，否則唯一的修正入口
...
- [x] 247.4.1 ext 端同樣**新建獨立自檢元件**（不塞進 `ProcessGdriveUploader`），實作 L1／L2／L3，
...
- [x] 247.4.2 **ext 端的 L3 需要先給 `GdriveUploader` 一個唯讀探測方法——這是對既有安全不變量的明示修改。**
...
- [x] 247.4.3 **ext 端的自檢元件需注入 `ObjectMapper`**（Spring Boot 自動配置已提供）——
...
- [x] 247.4.4 **ext 端的前置條件查的是 `crawler_export_setting`**（全域一列、無 owner）：
...
- [x] 247.4.5 **ext 端不需要「啟用當下自檢」，但 business 端的爬蟲頁需要**：爬蟲設定的 `PUT` 在 business
...
- [x] 247.4.6 **兩支各自實作，不抽跨服務共用 module。** `backend` 與 `external-materials-service` 是兩個獨立
...
- [x] 247.5.1 **不實作 config 熱重載。** 兩支 client 都在啟動時複製一份到 `/tmp` 後執行期不重讀來源；
...
- [x] 247.5.2 **不新增 API 端點、不新增 UI、不做健康檢查頁、不新增 DB 欄位。**
      啟用時的結果**只走當次回應的 `gdriveSelfCheckWarning`**（不入庫、不碰 `gdrive_last_run_at`／
      `gdrive_last_status`，見 247.3.4），啟動時的只進 `docker logs`。
- [x] 247.5.3 **不改任何上傳行為、不改 schema、不新增 Liquibase changeset。**
- [x] 247.5.4 **不新增 `@Scheduled`**（自檢掛 `ApplicationReadyEvent` 與 `PUT` 路徑），故「公開資訊 → 排程列表」
      （`bff/src/main/java/com/steven/assets/bff/schedulelist/SchedulePublicBffController.java` 的 `JOBS`，`:53`）
      **不需新增或修改任何項目**。
- [x] 247.5.5 **不得把 token 內容寫進 log。** L2 只解析 token JSON 的**鍵名**判斷 `refresh_token` 是否存在，
      log 只寫存在與否；**絕不**輸出 `access_token`／`refresh_token` 的值。L1 亦不得為了診斷而 dump config
      檔內容——該檔含 `[gdrive-crypt]` 的 crypt 解密密碼。

### 247.6 測試

- [x] 247.6.1 **L2 的 token 解析**（backend，新增一支自檢測試類，命名由實作者決定）須涵蓋**五種**輸入：
...
- [x] 247.6.2 **前置 DB 查詢擲例外時自檢靜默結束**：以替身讓 `JdbcTemplate` 擲 `BadSqlGrammarException`，
      驗證不擲出、不影響啟動、只寫一則 WARN。
- [x] 247.6.3 **全庫無任何列啟用時整個啟動自檢跳過**：以介面替身驗證**零次** rclone 呼叫。
      這是 Requirement 50／51「既有部署不要求 rclone remote 存在」的直接回歸。
- [x] 247.6.4 **啟用當下自檢失敗仍回 2xx、設定已存入、且狀態兩欄未被改動**：驗證儲存不因自檢失敗而變成
...
- [x] 247.6.5 **兩個啟用掛載點各需一組「false→true 觸發、true→true 不觸發」測試**：
      （a）八頁走 `resolveUpdate` 的那一處，（b）`CrawlerExportPathService.update()` 那一處。
      **(b) 不可省略**——它是唯一不走 `resolveUpdate` 的頁面，也是實測中最早被打開的一列。
- [x] 247.6.6 **(b) 完全不呼叫 rclone**：以介面替身驗證**零次**呼叫。這是「(b) 只做 L1＋L2、不做 L3」的
      機械保證，也順帶擋掉「儲存因探測而變慢」的回歸。
- [x] 247.6.7 **L1 失敗時只 warn**：來源檔讀取失敗時自檢不擲例外、服務仍可啟動。
- [x] 247.6.8 **自檢一律不實際連網**——(a) 的 L3 走介面替身。backend 既有的
...
- [x] 247.6.9 ext 端對應的 L2 解析測試同 247.6.1（該服務有自己的測試樹
...
### 實際改了哪些檔（42 changed ＋ 7 新增）

**新增（7）**
- `backend/.../service/GdriveSelfCheck.java` — 葉節點自檢元件（L1／L2／L3 ＋ 八張表 EXISTS 前置查詢）
- `backend/.../service/GdriveSelfCheckStarter.java` — `@EventListener(ApplicationReadyEvent.class)` ＋ daemon 執行緒
- `backend/src/test/.../GdriveSelfCheckTest.java`（14 項）
- `external-materials-service/.../service/GdriveSelfCheck.java` — ext 端同一套（各自實作，不抽跨服務 module）
- `external-materials-service/src/test/.../GdriveSelfCheckTest.java`（12 項）
- `external-materials-service/src/test/.../CrawlerExportPathQueryTest.java`（3 項）
- `frontend/src/utils/gdriveSelfCheck.js` — 九頁共用的警告顯示 helper

**修改（重點）**
- `docker-compose.yml` — 兩處掛載改 `${HOME}/.config/rclone:/etc/rclone:ro`（RCLONE_CONFIG 值不變）
- `backend/.../GdriveOutputSupport.java` — `resolveUpdate` 於 false→true 時呼叫 `checkLocal`
- `backend/.../CrawlerExportPathService.java` — 爬蟲頁另掛一次（該頁不走 `resolveUpdate`）
- 八支匯出 service ＋ 九支 DTO — 帶回 `gdriveSelfCheckWarning`
- `backend/.../ProcessRcloneClient.java` — 只改速率限制分支的使用者可見字串（247.2.6.1）
- `external-materials-service/.../GdriveUploader.java`／`ProcessGdriveUploader.java` — 新增唯讀探測 ＋ javadoc 安全不變量同步
- `external-materials-service/.../CrawlerExportPathQuery.java` — 新增 EXISTS 查詢
- `frontend/src/views/` 九支 view

### 驗證輸出（全部實跑）

```
backend  : Tests run: 239, Failures: 0, Errors: 0  → BUILD SUCCESS
ext      : Tests run: 105, Failures: 0, Errors: 0  → BUILD SUCCESS
bff      : compile OK
frontend : vite build OK（2397 modules）
build --no-cache ＋ up -d --force-recreate → 兩服務皆 healthy、零 BeanCurrentlyInCreationException
```

**自檢會不會叫（正反兩面都驗）**
- 正常組態：business／ext 皆**完全靜默**（三層通過）
- `GDRIVE_OUTPUT_REMOTE=NoSuchRemote` 重啟：兩服務各出 L2＋L3 兩則 WARN，執行緒名為
  `drive-self-check`／`drive-selfcheck`（證實走獨立 daemon 執行緒、未阻塞啟動）

**啟用當下自檢（(b) 路徑）端到端**
```
PUT /api/export-schedule/settings  gdriveEnabled false→true  → HTTP 200
response: "gdriveSelfCheckWarning": null
          "gdriveLastRunAt": "2026-07-28 20:42:24"        ← 未被污染
          "gdriveLastStatus": "成功：GDriveOutput:…（211992 bytes）"  ← 未被污染
```
**回歸**：`run-now` 實跑上傳成功（211992 bytes 落 Drive），九項 `gdrive_last_status` 維持成功。

### 與原計畫的偏差

1. **L1 失敗時「跑 L2、跳過 L3」**（原 247.2.5 未明訂三層先後）。跑 L2 是因為「L1 失敗 ⇒ configReady 必為
   false」不成立（configReady 啟動時判定一次，L1 檢查的是當下）；跳過 L3 是因為來源設定已壞時，
   打網路的成敗無法區分「設定問題」與「授權問題」，只會讓 log 出現兩個看似無關的錯誤。
   ext 端以 `uploader.isAvailable()` 達成同一效果。
2. **ext 的 L3 逾時沿用該服務既有的 45 秒**，非 design.md 所寫的 20 秒——ext 端沒有「互動式列目錄」的
   20 秒常數，硬加一個新常數只為對齊文件並不划算。啟動自檢走 daemon 執行緒，逾時長短不影響啟動。
3. **`configBroken()`／`RECONNECT_HINT` 同時列出兩個服務名**。原稿只寫 `business-services`，但該訊息會經
   (b) 回到爬蟲頁，而該頁的實際上傳者是 ext；兩容器掛同一份 host 設定，來源壞掉本來就兩邊都要重建。
4. **`reconnectHint` 補上「重新授權後仍須 recreate」**。L2 解析的是啟動時的 `/tmp` 副本，不補這句，
   使用者照著修完會看到警告一字不變。

### 審查後修正（實作完成後由三個獨立 lens 審出，皆已修）

- **critical**：ext 的 `GdriveSelfCheck` 有兩個建構子卻都沒有 `@Autowired` → Spring 無法選擇 →
  **external-materials-service 起不來**。105 個測試全過是因為本專案測試皆為純 Mockito、不載入 Spring context，
  這類 wiring 錯誤**只有實際啟動容器才攔得到**。已補 `@Autowired` 並以 recreate 驗證兩服務皆 healthy。
- **major**：ext 在 `configReady=false` 時靜默 return，漏報最關鍵的那句（該容器整個生命週期都不會同步）。
- **major**：啟用當下的 L2 讀啟動快照，修法訊息未提「須 recreate」。
- **major**：`configBroken()` 服務名寫死 business。
- **minor**：backend L2 缺 `isObject()` 防護（`readTree("123")` 是合法 JSON，會把格式壞掉誤報成缺 refresh_token）；
  backend L1 只攔 `IOException`（unchecked 例外會讓儲存變 500，違反 247.3.6）；
  「內容為空」措辭沿用了 ENOENT 才成立的後果；`docker-compose.yml` ext 區塊殘留改目錄前的復原步驟；
  spec 247.5.2 與 design.md 殘句與 247.3.4 矛盾（已一併修正）。
