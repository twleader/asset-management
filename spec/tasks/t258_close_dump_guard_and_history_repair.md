...
本任務在 `spec/requirements.md` 的 Requirement 7 新增了兩條 Markdown checkbox 形式的 AC。撰寫當下 `origin/main`（`9835e4a7`）的 `requirements.md` 仍是 Markdown checkbox 格式，故照該格式寫入；但分支 `claude/spec-gherkin-bdd-format-2f210e` 的工作區已把該檔全數轉成 Gherkin（`zh-TW` 方言），merge 時會撞號。該分支的 `HANDOFF-gherkin.md` 自述前一輪曾因「讀取範圍短於覆蓋範圍」靜默蓋掉 14 行硬約束，故此處預先給出等價場景，reconcile 時直接採用：

```gherkin
  場景: 收盤 dump 只能寫本次目標交易日的收盤前後所取得的值
    `dumpRedisToDb` 原本只要 Redis 有 `price` 就寫進當日列，不檢查 payload 的 `tradingDate`
    也不檢查該值有多舊。代號集合來自 `price:index:{market}`，其中含開機 `warmCacheOnStartup`
    以 `collectAllStockCodes` 寫入、之後再也不被盤中 cron 刷新的「曾持有／曾觀察」標的。
    更嚴重的是 `PriceCacheWriter.syncClosedFromDb` 會從 DB 最近收盤寫回 Redis，
    使錯誤值透過 Redis 洗一圈回到 DB、每個交易日自我延續一列。
    實測 2026 年已累積台股 122 列、美股 3 列、英股 3 列假收盤。

    假設 Redis `price:{market}:{code}` 的 payload 存在且含 `price`
    當 `ClosePersister.dumpRedisToDb` 準備把它寫進 `stock_price_history` 當日列
    那麼 payload 的 `tradingDate` 必須等於本次 dump 的目標交易日
    而且 payload 的 `updatedAt` 距本次執行時刻不得超過 12 分鐘
    而且 `updatedAt` 一律以台北牆鐘解讀（`PriceCacheWriter` 三處寫入皆綁 `MarketClock.TW_ZONE`）
    而且 守不過即跳過該檔不寫任何列，並記錄被跳過的檔數與代號
    但是 不得改用昨收、開盤價或中價回填
    但是 不得以 payload 的 `closed` 欄位當守門條件（13:32 取的合法值其 `closed` 為 `false`）

  場景: 既寫錯的收盤歷史必須能被權威來源覆寫修復
    `HistoricalBackfillService.backfillTwStock` 有兩道獨立阻擋使它修不了既有列：
    起點一律 `maxDate.plusDays(1)`（碰不到區間中段）、for-loop 內 `existsHistory` 命中即 `continue`。
    `StockSourceQuery.upsertHistory` 本身是真 upsert，但那兩道守門讓 backfill 從不對既有日期呼叫它。

    假設 `stock_price_history` 中某市場某區間存在錯誤的收盤列
    當 呼叫 `POST /internal/repair/history?market=&from=&to=`
    那麼 以既有的 `fetchTwHistoricalRange`／`fetchUsHistoricalRange`／`fetchUkHistoricalRange` 重抓權威日線
    而且 一律 `upsertHistory` 覆寫既有列，不檢查 `existsHistory`
    而且 跳過該市場當日 bar（今日列獨佔給 `ClosePersister`）
    而且 重跑同一區間結果不變（冪等）
    但是 來源未回傳某日 bar 時該日既有列保留不動，不刪除、不猜值、不以鄰日內插
```
