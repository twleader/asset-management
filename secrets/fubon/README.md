...
本系統只可用券商 API 讀取帳務、庫存、已成交紀錄或行情；**絕不得**透過 API 或 SDK 建立、送出、買入、賣出、改單、撤單、重送或以其他方式改變委託、部位或資金狀態。此禁令同樣適用於 internal token、排程、腳本及任何未來新增的券商整合，`dryRun`、角色或 feature flag 均不可成為例外。

部署的 API key 必須具可驗證的唯讀權限；若券商無法提供這種權限，必須維持 `FUBON_ENABLED=false`，不可只依賴程式約定。使用者若採納系統建議，須自行在券商官方介面完成交易，不得由本服務代為執行。
