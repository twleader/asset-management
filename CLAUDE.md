...
- **具名、限縮的行情五檔資料來源例外（Requirement 87／Task 348；Requirement 108／Task 372；Requirement 109／Task 373；Requirement 110／Task 375；Requirement 114／Task 379）**：登入後跨頁共用 `StockAnalysisDialog`、公開 `marketData.quoteDetail` 與同一 9090 row 的 direct `quoteDetail`／`bidLevels`／`askLevels` 都只能讀 `external-materials-service` 的同一個 typed persisted snapshot。snapshot 僅由 Requirement 106 已授權的台股十秒 LIVE round 建立：有效完整富邦 book 是 `FUBON_BOOKS` primary；僅同輪 selected code 缺有效完整 Fubon book 時可由 bounded、8-code fair-cursor Yahoo worker 建立完整 `YAHOO_TW` fallback。PostgreSQL 以 atomic comparator/revision 成為 canonical；Redis 只接受較新 DB revision。query path 將 Redis 當 candidate，必以 PostgreSQL revision 驗證後才回 cache，否則回 DB canonical，整段都是 pure read、禁止 request-time 向 Yahoo、富邦、dispatcher、worker 或 writer 外呼。這份資料**不是本系統的權威即時價**，不得供估值、損益、下單、警示或 SSE 使用；business 只 proxy／fail-soft。9090 的公開投影固定保留 `marketData.quoteDetail`，並可用同一 immutable snapshot 追加 direct `quoteDetail`、`bidLevels`／`askLevels` 供 read-only batch 做累計深度驗證；這不授權下單、不讀 configured-admin、任何使用者／帳戶／券商／持股／成本／交易／快照／投資組合資料。專用 key 不得取代既有 Redis 即時價或 `stock_price_history` 收盤價，也不得寫 generic price index、ticks、day-H/L 或 `price-update`。
- **Requirement 114／Task 379 current-state rule:** The FUBON-only wording in the preceding order-book exception is replaced for the existing ten-second producer and the source eligibility of 9090 direct quote projections. On each existing 10-second Fubon batch, a selected Taiwan code whose Fubon response lacks a valid complete five-level book may be fetched by an independent bounded Yahoo worker and stored as `YAHOO_TW`; a valid Fubon book remains `FUBON_BOOKS` primary. Both sources must be complete self-contained snapshots, not a field-by-field merge; only either source may be exposed on an available nested or direct response. PostgreSQL alone applies the source-time comparator atomically (same source strictly newer; Fubon over Yahoo; Yahoo over Fubon only with a strictly newer source time) and assigns a strictly increasing `canonical_revision`; Redis stores only the DB-returned revision and Lua rejects a delayed equal/older revision. Readers validate cache revision against PostgreSQL before returning it, preventing a cache write that temporarily lags a committed DB revision from being served. Yahoo uses an 8-code fair cursor under the four-probe/two-second/nine-second budget, and typed probes: only HTTP 404 may advance `.TW` to `.TWO`; any valid matching 2xx snapshot is found regardless of `TAI`/`TWO`, while all other failures stop the code for that round. The public / internal read path stays pure read with zero request-time vendor I/O; unavailable responses retain null source. This exception never authorizes business/BFF external HTTP, generic price mutation, valuation, order placement, alerts, SSE, or personal-data access.
- **Requirement 115／Task 380 Taiwan LIVE radar rule:** The external-materials ten-second dispatcher may query **only** the current dispatcher input intersected with per-owner latest Taiwan holdings plus Taiwan alerts, excluding `0000`; this effective radar set is the sole Fubon, MIS, Yahoo and book-worker candidate set. Empty/error radar collection fails closed to zero Taiwan live-market-data external request, not to the prior generic MIS→Yahoo universe. Every well-formed normalized Fubon response row is DB-canonical then mirrored to dedicated Redis, including typed failure rows, complete process-global counters and optional incomplete order-book slots; it is never an SDK raw payload, generic price/tick/SSE mutation, owner data, inventory data or broker write. The existing public `GET /api/quotes/one` may pure-read only single-quote-safe response state through a container-internal Redis/DB bridge: a stored SUCCESS row fills the existing one-copy `DetailedLatestQuote` quote fields only when source/trading-date/normalized timestamp exactly match the raw canonical quote, while only distinct Fubon metadata/failure/raw-returned-book fields are appended and `premiumDiscountPct` remains raw NAV data. Adapter-wide counters are never bridge/9090/no-tenant output. It must never add a synonymous nested quote or duplicate price/OHLC/source/status fields, call Fubon/Yahoo/dispatcher at request time, or change the `GET /api/quotes` list response/read shape.
- **Requirement 116／Task 381 Fubon TAIEX stream rule:** The proprietary Fubon `indices` WebSocket is held only by `fubon-broker-service`; it exposes one exact token-protected internal SSE response `GET /internal/market-data/taiex-index/stream`, while `external-materials-service` actively opens that GET and is the only DB/Redis writer. It is effective only when `FUBON_ENABLED=true` **and** `FUBON_TAIEX_INDEX_STREAM_ENABLED=true` with one valid configured symbol and ready shared-token config. When either flag is false or the symbol is invalid, Python opens no SDK WebSocket and Java neither reads the token nor opens/retries SSE GET. Once those non-secret gates pass, Java may lazily check the token file only to establish readiness; missing/empty token yields sanitized misconfiguration with zero SDK／SSE GET/retry／DB／Redis work. The adapter never POSTs/reverse-calls Spring, writes DB/Redis, exposes SDK raw/secret/account data, or imports/calls orders. The configured Taiwan index identity comes from `intraday/tickers?type=INDEX&exchange=TWSE`, never a hard-coded document sample. The consumer accepts only its exact `TWSE`/`INDEX` event with canonical decimal precision ≤20 and scale ≤10, and uses provider epoch microseconds as the sole strictly-newer authority. Its independent DB transaction must commit `fubon_taiex_index_latest` before it projects the returned canonical row through strict-newer `price:台股:0000`. Raw stale/equal input cannot refresh Redis, publish, tick, day-H/L or overwrite a verified close/daily history; the sole recovery exception is an equal-time **reread committed canonical DB row** when Redis is missing/older, which can advance only through the same Lua fence and never uses raw payload. The existing Yahoo two-minute index poll remains fallback-only and first reads today’s Fubon current row to repair cache without a Yahoo call. No 9090/BFF/gateway/public endpoint is added.
- **Requirement 117／Task 382 radar decision-clarity rule:** Evidence-gate diagnostics are adverse safety information, never support evidence. `Evidence.reasons()` is an existing untagged compatibility list and must not route gate output; a deterministic local accessor maps only a same-horizon coverage/freshness failure and its 5-session SHORT or 20-session SWING/MEDIUM dividend evidence to that horizon. `GatedActions` must retain non-null horizon-local medium/short/swing diagnostics; an individual gate failure appears only in that horizon's risks, while an explicitly all-horizon failure appears only in every supplied track. Its existing `reasons()` accessor is the medium→short→swing stable-distinct audit-only `actionGateReasons`, not a list to copy into every horizon. Production, offline baseline and V13 candidate consumers all use their matching local list. `TW_RULES_V18` reflects this observable reason/risk classification change while scores, rule weights/thresholds, candidate/final actions and `EVIDENCE_GATE_V1` stay unchanged; notification baselines rebuild without sending a first-transition email. The frontend may keep a non-persistent user-selected holding-period focus but must retain all three tracks and never synthesize a recommendation, recompute a signal, write data, expose a new API or make a profitability claim. Completed-K copy must use `dailyCandle.asOfDate`; top-level quote `asOfDate` may be intraday and cannot be called a completed K. OpenAPI is 1.9.0 with action-gate reasons labelled as adverse three-track audit metadata.
...
- **具名、限縮例外（Requirements 66–68／70／71／77／78／79／86／108–113；Tasks 317、325、327、328、329、336、337、338、347、372、373、375–378）**：Docker 外部 HTTP 只能從 non-root Nginx `api-gateway` 的 loopback `127.0.0.1:9090` 十二條 exact path/method（十一條唯讀 GET ＋唯一寫入 POST `/api/public/crawler-data/rescan`，見 Requirement 71）進入；BFF 與 external-materials-service 都不映射 host port。BFF 僅放行 exact `GET /api/quotes`、`GET /api/quotes/one`、`GET /api/public/market-index`、`GET /api/assets/latest`、`GET /api/public/exchange-rate/usd-twd`、`GET /api/public/market-analysis/today`、`GET /api/public/portfolio-advice/latest`、`GET /api/public/trading-radar/today`、`GET /api/public/trading-radar/stock`、`GET /api/public/transactions`、`GET /api/public/trading-calendar` 與上述 POST；禁止 `/api/**`／descendant wildcard 與同路徑其他 method。quotes 只用 no-tenant container client 讀 external-materials 原始 Redis 19 欄，再用無身份 business 唯讀 market endpoints 聚合 `marketData`；同一 immutable child 可直接投影 batch-ready `quoteDetail`、`bidLevels`、`askLevels`、`dividendHistory`，不得新增 I/O、改用 generic buy/sell 或繞過富邦 pure-read snapshot，且不得查個人資料。交易雷達與交易紀錄都固定由 configured-admin bootstrap 選 owner、清除 caller Reactor identity 並顯式傳遞 tenant headers；雷達 `today` 只回首頁 typed list、`stock` 只回該 request current-read 的 exact `(stockCode, market)` detail，交易紀錄只回已保存 ledger，三者都不得寫 snapshot／Redis／DB、觸發券商或交易。交易日曆是 global no-tenant read，使用無身份 container client，僅回市場日曆／狀態且不得做 refresh、匯出或寫入。`/api/public/crawler-data/rescan` 是唯一有外部抓取副作用的例外，經 business 端 30 秒全域 Redis 冷卻節流。Frontend 對十二路 exact／matrix 變體回 404，view 不得援引此例外。Controller 仍只委派 service，BFF 不直查 DB／外部行情。USD/TWD 的台銀／兆豐／Yahoo 外部抓取、交易時段判定與每 2 秒 Redis producer 只能位於 `external-materials-service`；business/BFF 只做唯讀 cache/DB 與聚合。
...
**3. Docker 外部 API 一律經 Nginx 9090 gateway（十一條唯讀 GET ＋ 一條寫入 POST）**
...
- Host 只綁 `127.0.0.1:9090`，精確放行 `GET /api/quotes`、`/api/quotes/one`、
  ...
  owner 走 configured-admin bootstrap），以及第九條交易雷達列表 `GET /api/public/trading-radar/today`
  和第十條指定股票明細 `GET /api/public/trading-radar/stock`（Requirement 111 / Task 376；owner 同樣走 configured-admin，且不得寫入匯出 snapshot）、第十一條 configured-admin ledger `GET /api/public/transactions`（Requirement 112 / Task 377；只讀已保存交易紀錄）、第十二條 global 日曆 `GET /api/public/trading-calendar`（Requirement 113 / Task 378；無 tenant、僅市場資料）。
- `bff` 與 `external-materials-service` 不發布 host port；`frontend:80` 對上述十二條回 `404`，
...
- Tailscale Serve 只以 path-scoped HTTPS `:9090` 掛相同十二條 exact path，包含 USD/TWD 公開匯率
...
| 文件 | 說明 |
|------|------|
| `spec/requirements.md` | User Stories + Acceptance Criteria（116 個 Requirements；最新編號為 117） |
| `spec/design.md` | 架構圖、ERD、API 端點、關鍵業務邏輯 |
| `spec/tasks.md` | 任務索引（Task 1–228、264–267、269–292、297–309、311–342、344–382）＋尚未歸檔的 Task 201 起區段；Task 229–263、268、293–296 以各自 `spec/tasks/tNNN_*.md` 為準 |
