...
/** Writes the latest quote through the single atomic Task 350 Redis primitive. */
@Slf4j
@Component
...
public class PriceCacheWriter {
    ...
    private CacheWriteOutcome executeLatestWrite(
        ...
        payload.put("priceChange", changeOrNull(result.price(), result.previousClose(), result.change()));
    ...
    private static boolean timed(PriceResult result) {
        return result.tradingDate() != null && result.freshnessInstant() != null;
    }
    ...
    private static BigDecimal changeOrNull(BigDecimal price, BigDecimal previous, BigDecimal fallback) {
