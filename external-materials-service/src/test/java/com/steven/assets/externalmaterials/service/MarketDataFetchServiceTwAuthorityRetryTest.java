...
class MarketDataFetchServiceTwAuthorityRetryTest {
    ...
    @Test
    void dgpaProvisionalDrivesSharedKnownCalendarWhileDualSourceFailureStaysUnknown() {
        Map<String, String> completeDgpaYear = Map.of("2027-01-01", "DGPA holiday");
        DgpaCalendarAuthority dgpa = mock(DgpaCalendarAuthority.class);
        when(dgpa.fetchHolidays(2027)).thenReturn(Optional.of(completeDgpaYear));
        MutableClock clock = new MutableClock();
        SequencedService provisional = new SequencedService(
                knownClosures(), dgpa, clock, Map.of());
        MarketCalendar knownCalendar = new MarketCalendar(provisional, clock);

        assertThat(knownCalendar.isTwTradingDayKnown(LocalDate.of(2027, 1, 4))).contains(true);
        assertThat(knownCalendar.isTwTradingDayKnown(LocalDate.of(2027, 1, 1))).contains(false);
        verify(dgpa).fetchHolidays(2027);

        DgpaCalendarAuthority unavailableDgpa = mock(DgpaCalendarAuthority.class);
        when(unavailableDgpa.fetchHolidays(2027)).thenReturn(Optional.empty());
        SequencedService unavailable = new SequencedService(
                knownClosures(), unavailableDgpa, new MutableClock(), Map.of());

        assertThat(new MarketCalendar(unavailable)
                .isTwTradingDayKnown(LocalDate.of(2027, 1, 4))).isEmpty();
    }
